...
KAppearanceOptions::KAppearanceOptions(KConfig *config, QString group, QWidget *parent, const char *)
    : KCModule(parent, "kcmkonqhtml"), m_pConfig(config), m_groupname(group), fSize(10), fMinSize(HTML_DEFAULT_MIN_FONT_SIZE)

{
    setQuickHelp(
        i18n("<h1>Konqueror Fonts</h1>On this page, you can configure "
    ...
    QWhatsThis::add(m_MedSize, i18n("This is the relative font size Konqueror uses "
    ...
    connect(m_pFonts[0], SIGNAL(activated(const QString &)), SLOT(slotStandardFont(const QString &)));
    connect(m_pFonts[0], SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pFonts[0]->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotStandardFont(const QString &)));
    connect(m_pFonts[0], SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pFonts[1], SIGNAL(activated(const QString &)), SLOT(slotFixedFont(const QString &)));
    connect(m_pFonts[1], SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pFonts[1]->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotFixedFont(const QString &)));
    connect(m_pFonts[1], SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pFonts[2], SIGNAL(activated(const QString &)), SLOT(slotSerifFont(const QString &)));
    connect(m_pFonts[2], SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pFonts[2]->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotSerifFont(const QString &)));
    connect(m_pFonts[2], SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pFonts[3], SIGNAL(activated(const QString &)), SLOT(slotSansSerifFont(const QString &)));
    connect(m_pFonts[3], SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pFonts[3]->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotSansSerifFont(const QString &)));
    connect(m_pFonts[3], SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pFonts[4], SIGNAL(activated(const QString &)), SLOT(slotCursiveFont(const QString &)));
    connect(m_pFonts[4], SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pFonts[4]->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotCursiveFont(const QString &)));
    connect(m_pFonts[4], SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pFonts[5], SIGNAL(activated(const QString &)), SLOT(slotFantasyFont(const QString &)));
    connect(m_pFonts[5], SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pFonts[5]->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotFantasyFont(const QString &)));
    connect(m_pFonts[5], SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pFontSizeAdjust, SIGNAL(valueChanged(int)), SLOT(slotFontSizeAdjust(int)));
    connect(m_pFontSizeAdjust, SIGNAL(valueChanged(int)), SLOT(changed()));
    ...
    wtstr = i18n(
        "Select the default encoding to be used; normally, you will be fine with 'Use language encoding' "
        "and should not have to change this.");
    QWhatsThis::add(label, wtstr);
    QWhatsThis::add(m_pEncoding, wtstr);

    connect(m_pEncoding, SIGNAL(activated(const QString &)), SLOT(slotEncoding(const QString &)));
    connect(m_pEncoding, SIGNAL(activated(const QString &)), SLOT(changed()));

    ++r;
    lay->setRowStretch(r, 8);
