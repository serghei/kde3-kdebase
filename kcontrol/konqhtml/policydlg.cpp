...
PolicyDialog::PolicyDialog(Policies *policies, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, QString::null, Ok | Cancel, Ok, true), policies(policies)
{
    ...
    connect(le_domain, SIGNAL(textChanged(const QString &)), SLOT(slotTextChanged(const QString &)));
...
void PolicyDialog::accept()
{
    ...
    FeatureEnabledPolicy pol = (FeatureEnabledPolicy)cb_feature_policy->currentItem();
