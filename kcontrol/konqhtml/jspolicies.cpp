...
JSPolicies::JSPolicies(KConfig *config, const QString &group, bool global, const QString &domain)
    : Policies(config, group, global, domain, "javascript.", "EnableJavaScript")
{
}

JSPolicies::JSPolicies() : Policies(0, QString::null, false, QString::null, QString::null, QString::null)
{
}
...
void JSPolicies::load()
{
    ...
    window_open = config->readUnsignedNumEntry(key, is_global ? KHTMLSettings::KJSWindowOpenSmart : INHERIT_POLICY);

    key = prefix + "WindowResizePolicy";
    window_resize = config->readUnsignedNumEntry(key, is_global ? KHTMLSettings::KJSWindowResizeAllow : INHERIT_POLICY);

    key = prefix + "WindowMovePolicy";
    window_move = config->readUnsignedNumEntry(key, is_global ? KHTMLSettings::KJSWindowMoveAllow : INHERIT_POLICY);

    key = prefix + "WindowFocusPolicy";
    window_focus = config->readUnsignedNumEntry(key, is_global ? KHTMLSettings::KJSWindowFocusAllow : INHERIT_POLICY);

    key = prefix + "WindowStatusPolicy";
    window_status = config->readUnsignedNumEntry(key, is_global ? KHTMLSettings::KJSWindowStatusAllow : INHERIT_POLICY);
}
...
JSPoliciesFrame::JSPoliciesFrame(JSPolicies *policies, const QString &title, QWidget *parent)
    : QGroupBox(title, parent, "jspoliciesframe"), policies(policies)
{
    ...
    wtstr = i18n(
        "If you disable this, Konqueror will stop "
    ...
    wtstr = i18n(
        "Some websites set the focus to their browser window on their "
    ...
    wtstr = i18n(
        "Some websites change the status bar text by setting "
...
{
    QRadioButton *button;
    button = static_cast< QRadioButton * >(js_popup->find(policies->window_open));
    if(button != 0)
        button->setChecked(true);
    button = static_cast< QRadioButton * >(js_resize->find(policies->window_resize));
    if(button != 0)
        button->setChecked(true);
    button = static_cast< QRadioButton * >(js_move->find(policies->window_move));
    if(button != 0)
        button->setChecked(true);
    button = static_cast< QRadioButton * >(js_focus->find(policies->window_focus));
    if(button != 0)
        button->setChecked(true);
    button = static_cast< QRadioButton * >(js_statusbar->find(policies->window_status));
    if(button != 0)
        button->setChecked(true);
}
