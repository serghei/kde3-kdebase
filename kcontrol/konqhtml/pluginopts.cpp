...
PluginPolicies::PluginPolicies(KConfig *config, const QString &group, bool global, const QString &domain)
...
KPluginOptions::KPluginOptions(KConfig *config, QString group, QWidget *parent, const char *)
    : KCModule(parent, "kcmkonqhtml"), m_pConfig(config), m_groupname(group), m_nspluginscan(0), global_policies(config, group, true)
{
     ...
     *************************************************************************/
    QPushButton *domainSpecPB = new QPushButton(i18n("Domain-Specific Settin&gs"), globalGB);
    domainSpecPB->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    connect(domainSpecPB, SIGNAL(clicked()), SLOT(slotShowDomainDlg()));

    domainSpecificDlg = new KDialogBase(KDialogBase::Swallow, i18n("Domain-Specific Policies"), KDialogBase::Close, KDialogBase::Close, this,
                                        "domainSpecificDlg", true);
    ...
    QString wtstr = i18n(
        "This box contains the domains and hosts you have set "
...
void KPluginOptions::scan()
{
    ...
    if(m_changed)
    {
        int ret = KMessageBox::warningYesNoCancel(this, i18n("Do you want to apply your changes "
    ...
    if(!scanExe)
    {
        ...
        KMessageBox::sorry(this, i18n("The nspluginscan executable cannot be found. "
    ...
    *m_nspluginscan << scanExe << "--verbose";
    ...
    connect(m_nspluginscan, SIGNAL(readReady(KProcIO *)), this, SLOT(progress(KProcIO *)));
    connect(m_nspluginscan, SIGNAL(processExited(KProcess *)), this, SLOT(scanDone()));
...
void KPluginOptions::dirInit()
{
    ...
    connect(m_widget->dirEdit, SIGNAL(textChanged(const QString &)), SLOT(dirEdited(const QString &)));

    connect(m_widget->dirList, SIGNAL(executed(QListBoxItem *)), SLOT(dirSelect(QListBoxItem *)));

    connect(m_widget->dirList, SIGNAL(selectionChanged(QListBoxItem *)), SLOT(dirSelect(QListBoxItem *)));
}
...
void KPluginOptions::dirLoad(KConfig *config, bool useDefault)
{
    ...
    { // keep sync with kdebase/nsplugins
...
PluginDomainDialog::PluginDomainDialog(QWidget *parent) : QWidget(parent, "PluginDomainDialog")
{
...
PluginDomainListView::PluginDomainListView(KConfig *config, const QString &group, KPluginOptions *options, QWidget *parent, const char *name)
    : DomainListView(config, i18n("Doma&in-Specific"), parent, name), group(group), options(options)
{
}
...
void PluginDomainListView::setupPolicyDlg(PushButton trigger, PolicyDialog &pDlg, Policies *pol)
{
    QString caption;
    switch(trigger)
    {
