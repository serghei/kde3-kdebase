...
KJavaScriptOptions::KJavaScriptOptions(KConfig *config, QString group, QWidget *parent, const char *name)
    ...
    , _removeECMADomainSettings(false)
{
    ...
    js_policies_frame = new JSPoliciesFrame(&js_global_policies, i18n("Global JavaScript Policies"), this);
...
JSDomainListView::JSDomainListView(KConfig *config, const QString &group, KJavaScriptOptions *options, QWidget *parent, const char *name)
    : DomainListView(config, i18n("Do&main-Specific"), parent, name), group(group), options(options)
{
}
...
void JSDomainListView::updateDomainListLegacy(const QStringList &domainConfig)
{
    ...
    for(QStringList::ConstIterator it = domainConfig.begin(); it != domainConfig.end(); ++it)
    {
        ...
        {
            QListViewItem *index = new QListViewItem(domainSpecificLV, domain, i18n(KHTMLSettings::adviceToStr(javaScriptAdvice)));
...
void JSDomainListView::setupPolicyDlg(PushButton trigger, PolicyDialog &pDlg, Policies *pol)
{
    ...
    pDlg.setFeatureEnabledWhatsThis(
        i18n("Select a JavaScript policy for "
