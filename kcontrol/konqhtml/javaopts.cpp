...
JavaPolicies::JavaPolicies(KConfig *config, const QString &group, bool global, const QString &domain)
    : Policies(config, group, global, domain, "java.", "EnableJava")
{
}

JavaPolicies::JavaPolicies() : Policies(0, QString::null, false, QString::null, QString::null, QString::null)
{
}
...
KJavaOptions::KJavaOptions(KConfig *config, QString group, QWidget *parent, const char *name)
    ...
    , _removeJavaDomainSettings(false)
{
    ...
    QLabel *pathLA = new QLabel(i18n("&Path to Java executable, or 'java':"), pathHB);
...
JavaDomainListView::JavaDomainListView(KConfig *config, const QString &group, KJavaOptions *options, QWidget *parent, const char *name)
...
void JavaDomainListView::updateDomainListLegacy(const QStringList &domainConfig)
{
    ...
    pol.defaults();
    for(QStringList::ConstIterator it = domainConfig.begin(); it != domainConfig.end(); ++it)
    {
        QString domain;
        ...
        {
            QListViewItem *index = new QListViewItem(domainSpecificLV, domain, i18n(KHTMLSettings::adviceToStr(javaAdvice)));
...
void JavaDomainListView::setupPolicyDlg(PushButton trigger, PolicyDialog &pDlg, Policies *pol)
{
    QString caption;
    switch(trigger)
    {
        case AddButton:
            caption = i18n("New Java Policy");
            pol->setFeatureEnabled(!options->enableJavaGloballyCB->isChecked());
            break;
        case ChangeButton:
            caption = i18n("Change Java Policy");
            break;
