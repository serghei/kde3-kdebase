...
DomainListView::DomainListView(KConfig *config, const QString &title, QWidget *parent, const char *name)
...
void DomainListView::addPressed()
{
    ...
    if(pDlg.exec())
    {
        QListViewItem *index = new QListViewItem(domainSpecificLV, pDlg.domain(), pDlg.featureEnabledPolicyText());
...
void DomainListView::initialize(const QStringList &domainList)
{
    ...
    for(QStringList::ConstIterator it = domainList.begin(); it != domainList.end(); ++it)
    {
        QString domain = *it;
        Policies *pol = createPolicies();
        ...
        if(pol->isFeatureEnabledPolicyInherited())
            ...
            policy = i18n("Reject");
        QListViewItem *index = new QListViewItem(domainSpecificLV, domain, policy);
...
void DomainListView::setupPolicyDlg(PushButton /*trigger*/, PolicyDialog & /*pDlg*/, Policies * /*copy*/)
{
    // do nothing
}
