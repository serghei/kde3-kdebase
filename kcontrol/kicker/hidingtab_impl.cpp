...
HidingTab::HidingTab(QWidget *parent, const char *name) : HidingTabBase(parent, name), m_panelInfo(0)
{
    // connections
    ...
    connect(KickerConfig::the(), SIGNAL(positionPanelChanged(int)), SLOT(switchPanel(int)));
    connect(m_panelList, SIGNAL(activated(int)), KickerConfig::the(), SIGNAL(hidingPanelChanged(int)));
}
...
void HidingTab::storeInfo()
{
    ...
    m_panelInfo->_unhideLocation = m_backgroundRaise->isChecked() ? triggerComboToConfig(m_backgroundPos->currentItem()) : 0;
}
...
{
    if(position == PositionTab::PosTop || position == PositionTab::PosBottom)
    {
...
{
    m_backgroundPos->setEnabled((m_automatic->isChecked() || m_background->isChecked()) && m_backgroundRaise->isChecked());
}
