...
AppletTab::AppletTab(QWidget *parent, const char *name) : AppletTabBase(parent, name)
{

    connect(level_group, SIGNAL(clicked(int)), SLOT(level_changed(int)));

    connect(lb_trusted, SIGNAL(selectionChanged(QListViewItem *)), SLOT(trusted_selection_changed(QListViewItem *)));

    connect(pb_add, SIGNAL(clicked()), SLOT(add_clicked()));
    connect(pb_remove, SIGNAL(clicked()), SLOT(remove_clicked()));

    connect(lb_available, SIGNAL(selectionChanged(QListViewItem *)), SLOT(available_selection_changed(QListViewItem *)));
