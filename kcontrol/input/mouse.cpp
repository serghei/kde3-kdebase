...
MouseConfig::MouseConfig(QWidget *parent, const char *name) : KCModule(parent, name)
{


    setQuickHelp(
        i18n("<h1>Mouse</h1> This module allows you to choose various"
    ...
    wtstr = i18n(
        "The default behavior in KDE is to select and activate"
    ...
    wtstr = i18n(
        "If you check this option, pausing the mouse pointer"
    ...
    wtstr = i18n(
        "If you have checked the option to automatically select"
    ...
    QBoxLayout *lay = new QVBoxLayout(tab2, KDialog::marginHint(), KDialog::spacingHint());
    ...
    wtstr = i18n(
        "The threshold is the smallest distance that the"
    ...
    wtstr = i18n(
        "The double click interval is the maximal time"
    ...
    wtstr = i18n(
        "If you click with the mouse (e.g. in a multi-line"
    ...
    wtstr = i18n(
        "If you click with the mouse and begin to move the"
    ...
    wtstr = i18n(
        "If you use the wheel of a mouse, this value determines the number of lines to scroll for each wheel movement. Note that if this number "
        "exceeds the number of visible lines, it will be ignored and the wheel movement will be handled as a page up/down movement.");
...

    {
        ...
        QBoxLayout *vbox = new QVBoxLayout(mouse, KDialog::marginHint(), KDialog::spacingHint());
...
// It only works if libusb is available.
    ...
    struct device_table
    {
        ...
        QString Name;
        int flags;
    } device_table[] = {{VENDOR_LOGITECH, 0xC00E, "M-BJ58", "Wheel Mouse Optical", HAS_RES},
    ...
    for(bus = usb_busses; bus; bus = bus->next)
    {
        for(dev = bus->devices; dev; dev = dev->next)
        {
            for(int n = 0; device_table[n].idVendor; n++)
                if((device_table[n].idVendor == dev->descriptor.idVendor) && (device_table[n].idProduct == dev->descriptor.idProduct))
                {
    ...
    KAboutData *about = new KAboutData("kcmmouse", I18N_NOOP("Mouse"), 0, 0, KAboutData::License_GPL, I18N_NOOP("(c) 1997 - 2005 Mouse developers"));
...
void MouseConfig::load(bool useDefaults)
{
    ...
    int time_to_max = ac.readNumEntry("MKTimeToMax", (5000 + interval / 2) / interval);
    time_to_max = ac.readNumEntry("MK-TimeToMax", time_to_max * interval);
...
void MouseConfig::save()
{
    ...
    ac.writeEntry("MKTimeToMax", (mk_time_to_max->value() + interval / 2) / interval);
    ac.writeEntry("MK-MaxSpeed", mk_max_speed->value());
    ac.writeEntry("MKMaxSpeed", (mk_max_speed->value() * interval + 500) / 1000);
...
void MouseSettings::load(KConfig *config)
{
    ...
    double accel;
    XGetPointerControl(kapp->getDisplay(), &accel_num, &accel_den, &threshold);
...
{
    XChangePointerControl(kapp->getDisplay(), true, true, int(qRound(accelRate * 10)), 10, thresholdMove);
