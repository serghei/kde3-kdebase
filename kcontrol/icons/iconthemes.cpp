...
IconThemesConfig::IconThemesConfig(QWidget *parent, const char *name) : KCModule(parent, name)
{
    ...
    connect(m_iconThemes, SIGNAL(selectionChanged(QListViewItem *)), SLOT(themeSelected(QListViewItem *)));

    QPushButton *installButton = new QPushButton(i18n("Install New Theme..."), this, "InstallNewTheme");
    connect(installButton, SIGNAL(clicked()), SLOT(installNewTheme()));
    m_removeButton = new QPushButton(i18n("Remove Theme"), this, "RemoveTheme");
...
QListViewItem *IconThemesConfig::iconThemeItem(const QString &name)
{
    ...
    for(item = m_iconThemes->firstChild(); item; item = item->nextSibling())
        if(m_themeNames[item->text(0)] == name)
            return item;
...
void IconThemesConfig::loadThemes()
{
    ...
    for(it = themelist.begin(); it != themelist.end(); ++it)
    {
        ...
        m_iconThemes->insertItem(new QListViewItem(m_iconThemes, name, icontheme.description()));
...
{
    KURL themeURL = KURLRequesterDlg::getURL(QString::null, this, i18n("Drag or Type Theme URL"));
...
bool IconThemesConfig::installThemes(const QStringList &themes, const QString &archiveName)
{
    ...
    KProgressDialog progressDiag(this, "themeinstallprogress", i18n("Installing icon themes"), QString::null, true);
    ...
    {
        progressDiag.setLabel(i18n("<qt>Installing <strong>%1</strong> theme</qt>").arg(*it));
        kapp->processEvents();

        if(progressDiag.wasCancelled())
            break;

        currentTheme = dynamic_cast< KArchiveDirectory * >(const_cast< KArchiveEntry * >(rootDir->entry(*it)));
...
QStringList IconThemesConfig::findThemeDirs(const QString &archiveName)
{
    ...
    for(QStringList::Iterator it = entries.begin(); it != entries.end(); ++it)
    {
        ...
        if(possibleDir->isDirectory())
        {
            ...
            if(subDir && (subDir->entry("index.theme") != NULL || subDir->entry("index.desktop") != NULL))
...
void IconThemesConfig::updateRemoveButton()
{
    ...
    if(selected)
    {
        ...
        if(m_themeNames[selected->text(0)] == KIconTheme::current() || m_themeNames[selected->text(0)] == KIconTheme::defaultThemeName())
            enabled = false;
    }
