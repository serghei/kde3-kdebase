...
static bool isValidShortURL(const QString &cmd, bool verbose = false)
{
    ...
    {
        if(verbose)
            kdDebug() << "KShortURIFilter::isValidShortURL: " << cmd << " matches FQDN_PATTERN" << endl;
...
// bug 133687
    ...
    {
        if(verbose)
            kdDebug() << "KShortURIFilter::isValidShortURL: " << cmd << " matches IPv4_PATTERN" << endl;
        return true;
    }
    ...
    {
        if(verbose)
            kdDebug() << "KShortURIFilter::isValidShortURL: " << cmd << " matches IPv6_PATTERN" << endl;
        return true;
    }

    if(verbose)
        kdDebug() << "KShortURIFilter::isValidShortURL: '" << cmd << "' is not a short URL." << endl;
...
KShortURIFilter::KShortURIFilter(QObject *parent, const char *name, const QStringList & /*args*/)
    : KURIFilterPlugin(parent, name ? name : "kshorturifilter", 1.0), DCOPObject("KShortURIFilterIface")
{
    configure();
}
...
bool KShortURIFilter::filterURI(KURIFilterData &data) const
{
    ...
    if(!isMalformed && (url.protocol().length() == 4) && (url.protocol() != QString::fromLatin1("http")) && (url.protocol()[0] == 'h')
       && (url.protocol()[1] == url.protocol()[2]) && (url.protocol()[3] == 'p'))
    {
    ...
    if(cmd[0] == '#' || cmd.find(man_proto, 0, true) == 0 || cmd.find(info_proto, 0, true) == 0)
    {
        if(cmd.left(2) == QFL1("##"))
    ...
    else
    {
        if(url.isLocalFile())
        {
            ...
            if((url.hasRef() || !url.query().isEmpty())                         // avoid the calling exists() when not needed
               && QFile::exists(url.path()) && !url.path().endsWith(QFL1("/"))) // /tmp/?foo is a namefilter, not a query
            {
                path = url.path();
                ref = url.ref();
                query = url.query();
...
K_EXPORT_COMPONENT_FACTORY(libkshorturifilter, KGenericFactory< KShortURIFilter >("kcmkurifilt"))
