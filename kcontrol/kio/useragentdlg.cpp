...
void UserAgentDlg::save()
{
    ...
    if(!deleteList.isEmpty())
    {
        ...
        for(QStringList::Iterator it = deleteList.begin(); it != deleteList.end(); ++it)
        {
            cfg.setGroup(*it);
    ...
    while(item != 0)
    {
        if(item->text(0) == site)
        {
            QString msg = i18n(
                              ...
                              "</qt>")
                              .arg(site);
            int res = KMessageBox::warningContinueCancel(this, msg, i18n("Duplicate Identification"), i18n("Replace"));
