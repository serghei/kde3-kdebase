...
KManualProxyDlg::KManualProxyDlg(QWidget *parent, const char *name) : KProxyDialogBase(parent, name, true, i18n("Manual Proxy Configuration"))
{
    mDlg = new ManualProxyDlgUI(this);
    setMainWidget(mDlg);

    mDlg->pbCopyDown->setPixmap(BarIcon("down", KIcon::SizeSmall));
    QSizePolicy sizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed, mDlg->pbCopyDown->sizePolicy().hasHeightForWidth());
...
void KManualProxyDlg::setProxyData(const KProxyData &data)
{
    ...
    bool useSameProxy =
        (!mDlg->leHttp->text().isEmpty() && data.proxyList["http"] == data.proxyList["https"] && data.proxyList["http"] == data.proxyList["ftp"]);
...
bool KManualProxyDlg::validate()
{
    ...
    if(count == 0)
    {
        showErrorMsg(i18n("Invalid Proxy Setting"), i18n("One or more of the specified proxy settings are "
...
bool KManualProxyDlg::handleDuplicate(const QString &site)
{
    ...
    while(item != 0)
    {
        if(item->text().findRev(site) != -1 && item != mDlg->lbExceptions->selectedItem())
        {
            QString msg = i18n(
                "You entered a duplicate address. "
                "Please try again.");
            QString details = i18n(
                                  "<qt><center><b>%1</b></center> "
                                  "is already in the list.</qt>")
                                  .arg(site);
...
{
    QString result;
    if(getException(result, i18n("Change Exception"), mDlg->lbExceptions->currentText()) && !handleDuplicate(result))
...
QString KManualProxyDlg::urlFromInput(const KLineEdit *edit, const QSpinBox *spin) const
{
    if(!edit)
        return QString::null;
...
void KManualProxyDlg::showErrorMsg(const QString &caption, const QString &message)
{
    QString cap(caption);
    QString msg(message);
    ...
    QString details = i18n(
        "<qt>Make sure none of the addresses or URLs you "
...
bool KManualProxyDlg::getException(QString &result, const QString &caption, const QString &value)
{
    QString label;
    ...
    if(mDlg->cbReverseProxy->isChecked())
        label = i18n(
            "Enter the URL or address that should use the above proxy "
            "settings:");
    else
        label = i18n(
    ...
    QString whatsThis = i18n(
        "<qt>Enter a valid address or url.<p>"
    ...
    bool ok;
    result = KInputDialog::text(caption, label, value, &ok, 0, 0, 0, QString::null, whatsThis);
