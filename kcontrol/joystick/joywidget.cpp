...
JoyWidget::JoyWidget(QWidget *parent, const char *name) : QWidget(parent, name), idle(0), joydev(0)
{
    ...
    // create area to show an icon + message if no joystick was detected
    {
        ...
        icon->setPixmap(KGlobal::iconLoader()->loadIcon("messagebox_warning", KIcon::NoGroup, KIcon::SizeMedium, KIcon::DefaultState, 0, true));
...
void JoyWidget::init()
{
    ...
    {
        messageBox->show();
        message->setText(QString("<qt><b>%1</b></qt>")
...
void JoyWidget::deviceChanged(const QString &dev)
{
    ...
    {
        KMessageBox::sorry(this, i18n("The given device name is invalid (does not contain /dev).\n"
...
void JoyWidget::showDeviceProps(JoyDevice *joy)
{
    ...
    buttonTbl->setLeftMargin(QMAX(buttonTbl->verticalHeader()->width(), axesTbl->verticalHeader()->width()));
...
void JoyWidget::calibrateDevice()
{
    ...
    if(KMessageBox::messageBox(this, KMessageBox::Information, i18n("<qt>Calibration is about to check the precision.<br><br>"
                               ...
                               i18n("Calibration"), KStdGuiItem::ok(), KStdGuiItem::cancel())
       != KMessageBox::Ok)
        return;
...
void JoyWidget::resetCalibration()
{
    ...
    {
        KMessageBox::information(this, i18n("Restored all calibration values for joystick device %1.").arg(joydev->device()),
