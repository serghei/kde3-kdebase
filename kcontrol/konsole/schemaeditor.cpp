...
SchemaEditor::SchemaEditor(QWidget *parent, const char *name) : SchemaDialog(parent, name)
{
    schMod = false;
    loaded = false;
    schemaLoaded = false;
...
void SchemaEditor::previewLoaded(bool l)
{
    ...
    {
        QWMatrix mat;
        pix = spix->xForm(mat.scale(180.0 / spix->QPixmap::width(), 100.0 / spix->QPixmap::height()));
...
void SchemaEditor::removeCurrent()
{
    ...
    {
        int code = KMessageBox::warningContinueCancel(this, i18n("You are trying to remove a system schema. Are you sure?"),
                                                      i18n("Removing System Schema"), KGuiItem(i18n("&Delete"), "editdelete"));
    ...
    if(!QFile::remove(base))
        KMessageBox::error(this, i18n("Cannot remove the schema.\nMaybe it is a system schema.\n"), i18n("Error Removing Schema"));
...
void SchemaEditor::saveCurrent()
{
    ...
    else
    {
        ...
        bool ok;
        fullpath = KInputDialog::getText(i18n("Save Schema"), i18n("File name:"), fullpath, &ok, this);
        if(!ok)
            return;
    }
    ...
    if(f.open(IO_WriteOnly))
    {
        ...
        if(transparencyCheck->isChecked())
        {
            ...
            QString tra;
            tra.sprintf("transparency %1.2f %3d %3d %3d", 1.0 * (100 - shadeSlide->value()) / 100, c.red(), c.green(), c.blue());
            t << tra << endl;
        }
        ...
        if(!backgndLine->text().isEmpty())
        {
            ...
            QString image;
            image.sprintf("image %s %s", (const char *)smode.latin1(), (const char *)backgndLine->text().utf8());
            t << image << endl;
        }
        t << endl;
        ...
        for(int i = 0; i < 20; i++)
        {
            QString scol;
            if(type[i] == 0)
                scol.sprintf("color %2d %3d %3d %3d %2d %1d # %s", i, color[i].red(), color[i].green(), color[i].blue(), transparent[i], bold[i],
...
{
    int result = KMessageBox::questionYesNo(this, i18n("The schema has been modified.\n"
                                                       "Do you want to save the changes?"),
                                            i18n("Schema Modified"), KStdGuiItem::save(), KStdGuiItem::discard());
...
void SchemaEditor::readSchema(int num)
{
    ...
    QString fPath = locate("data", "konsole/" + ((SchemaListBoxText *)schemaList->item(num))->filename());

    if(fPath.isNull())
        fPath = locate("data", ((SchemaListBoxText *)schemaList->item(num))->filename());

    if(fPath.isNull())
    {
        KMessageBox::error(this, i18n("Cannot find the schema."), i18n("Error Loading Schema"));
    ...
    if(!sysin)
    {
        KMessageBox::error(this, i18n("Cannot load the schema."), i18n("Error Loading Schema"));
        loadAllSchema();
        return;
    }
