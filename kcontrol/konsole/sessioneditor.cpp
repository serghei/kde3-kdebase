...
SessionEditor::SessionEditor(QWidget *parent, const char *name) : SessionDialog(parent, name)
{
    sesMod = false;
    oldSession = -1;
    loaded = false;
...
{
    int result = KMessageBox::questionYesNo(this, i18n("The session has been modified.\n"
                                                       "Do you want to save the changes?"),
                                            i18n("Session Modified"), KStdGuiItem::save(), KStdGuiItem::discard());
...
void SessionEditor::saveCurrent()
{
    ...
    if(!exec.isEmpty()) // If Execute field is empty, default shell is used.
    {
        ...
        {
            int result =
                KMessageBox::warningContinueCancel(this, i18n("The Execute entry is not a valid command.\n"
                                                              "You can still save this session, but it will not show up in Konsole's Session list."),
                                                   i18n("Invalid Execute Entry"), KStdGuiItem::save());
    ...
    else
    {
        ...
        bool ok;
        fullpath = KInputDialog::getText(i18n("Save Session"), i18n("File name:"), fullpath, &ok, this);
        if(!ok)
            return;
    }
...
void SessionEditor::removeCurrent()
{
    ...
    {
        int code = KMessageBox::warningContinueCancel(this, i18n("You are trying to remove a system session. Are you sure?"),
                                                      i18n("Removing System Session"), KGuiItem(i18n("&Delete"), "editdelete"));
    ...
    {
        KMessageBox::error(this, i18n("Cannot remove the session.\nMaybe it is a system session.\n"), i18n("Error Removing Session"));
        return;
    }
    removeButton->setEnabled(sessionList->count() > 1);
    loadAllSession();
