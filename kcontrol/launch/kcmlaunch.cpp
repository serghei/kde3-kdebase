...
LaunchConfig::LaunchConfig(QWidget *parent, const char *name, const QStringList &) : KCModule(LaunchFactory::instance(), parent, name)
{
    QVBoxLayout *Form1Layout = new QVBoxLayout(this, 0, KDialog::spacingHint());

    setQuickHelp(
        i18n("<h1>Launch Feedback</h1>"
    ...
    QWhatsThis::add(GroupBox1, i18n("<h1>Busy Cursor</h1>\n"
    ...
    connect(cb_busyCursor, SIGNAL(activated(int)), SLOT(slotBusyCursor(int)));
    ...
    connect(sb_cursorTimeout, SIGNAL(valueChanged(int)), SLOT(checkChanged()));
    ...
    connect(cb_taskbarButton, SIGNAL(toggled(bool)), SLOT(slotTaskbarButton(bool)));
    ...
    connect(sb_taskbarTimeout, SIGNAL(valueChanged(int)), SLOT(checkChanged()));
...
void LaunchConfig::slotTaskbarButton(bool b)
{
...
void LaunchConfig::load(bool useDefaults)
{
    ...
    bool busyCursor = c.readBoolEntry("BusyCursor", Default & BusyCursor);

    bool taskbarButton = c.readBoolEntry("TaskbarButton", Default & TaskbarButton);
...
void LaunchConfig::checkChanged()
{
    ...
    bool savedBusyCursor = c.readBoolEntry("BusyCursor", Default & BusyCursor);

    bool savedTaskbarButton = c.readBoolEntry("TaskbarButton", Default & TaskbarButton);
    ...
    emit changed(savedBusyCursor != newBusyCursor || savedTaskbarButton != newTaskbarButton || savedCursorTimeout != newCursorTimeout
                 || savedTaskbarTimeout != newTaskbarTimeout || savedBusyBlinking != newBusyBlinking || savedBusyBouncing != newBusyBouncing);
}
