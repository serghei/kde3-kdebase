...
ExtendedIntNumInput::ExtendedIntNumInput(QWidget *parent, const char *name) : KIntNumInput(parent, name)
{
}
...
void ExtendedIntNumInput::setRange(int min, int max, int step, bool slider)
{
    ...
    if(slider)
    {
        disconnect(m_slider, SIGNAL(valueChanged(int)), m_spin, SLOT(setValue(int)));
        disconnect(m_spin, SIGNAL(valueChanged(int)), this, SLOT(spinValueChanged(int)));

        this->min = min;
        this->max = max;
        sliderMax = (int)floor(0.5 + 2 * (log(max) - log(min)) / (log(max) - log(max - 1)));
        ...
        connect(m_spin, SIGNAL(valueChanged(int)), this, SLOT(slotSpinValueChanged(int)));
    }
}
...
KAccessConfig::KAccessConfig(QWidget *parent, const char *) : KCModule(parent, "kcmaccess")
{

    KAboutData *about = new KAboutData(I18N_NOOP("kaccess"), I18N_NOOP("KDE Accessibility Tool"), 0, 0, KAboutData::License_GPL,
    ...
    QVBoxLayout *vbox = new QVBoxLayout(bell, KDialogBase::marginHint(), KDialogBase::spacingHint());

    QGroupBox *grp = new QGroupBox(i18n("Audible Bell"), bell);
    grp->setColumnLayout(0, Qt::Horizontal);
    vbox->addWidget(grp);

    QVBoxLayout *vvbox = new QVBoxLayout(grp->layout(), KDialogBase::spacingHint());
    ...
    QWhatsThis::add(customBell,
                    i18n("Check this option if you want to use a customized bell, playing"
    ...
    QWhatsThis::add(gestureConfirmation, i18n("If this option is checked, KDE will show a confirmation dialog whenever a keyboard accessibility "
...
{
    KAboutData about(I18N_NOOP("kaccess"), I18N_NOOP("KDE Accessibility Tool"), 0);
