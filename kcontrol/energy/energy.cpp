...
KEnergy::KEnergy(QWidget *parent, const char *name) : KCModule(parent, name)
{
    m_bChanged = false;
    m_bEnabled = false;
    ...
    setQuickHelp(
        i18n("<h1>Display Power Control</h1> If your display supports"
...
void KEnergy::slotChangeStandby(int value)
{
    ...
    if(m_bMaintainSanity)
    {
        ...
        if((m_Suspend > 0 && m_Standby > m_Suspend) || (m_SuspendDesired && m_Standby >= m_SuspendDesired))
            m_pSuspendSlider->setValue(m_Standby);
        if((m_Off > 0 && m_Standby > m_Off) || (m_OffDesired && m_Standby >= m_OffDesired))
...
void KEnergy::slotChangeSuspend(int value)
{
    ...
    if(m_bMaintainSanity)
    {
        ...
        if((m_Off > 0 && m_Suspend > m_Off) || (m_OffDesired && m_Suspend >= m_OffDesired))
