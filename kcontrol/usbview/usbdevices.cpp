...
void USBDevice::parseLine(QString line)
{
    if(line.startsWith("T:"))
        sscanf(line.local8Bit().data(), "T:  Bus=%2d Lev=%2d Prnt=%2d Port=%d Cnt=%2d Dev#=%3d Spd=%3f MxCh=%2d", &_bus, &_level, &_parent, &_port,
               &_count, &_device, &_speed, &_channels);
    ...
    {
        sscanf(line.local8Bit().data(), "B:  Alloc=%3d/%3d us (%2d%%), #Int=%3d, #Iso=%3d", &_bwUsed, &_bwTotal, &_bwPercent, &_bwIntr, &_bwIso);
        _hasBW = true;
    }
    else if(line.startsWith("D:"))
    {
        char buffer[11];
        sscanf(line.local8Bit().data(), "D:  Ver=%x.%x Cls=%x(%10s) Sub=%x Prot=%x MxPS=%d #Cfgs=%d", &_verMajor, &_verMinor, &_class, buffer, &_sub,
               &_prot, &_maxPacketSize, &_configs);
        _className = buffer;
    }
    else if(line.startsWith("P:"))
        sscanf(line.local8Bit().data(), "P:  Vendor=%x ProdID=%x Rev=%x.%x", &_vendorID, &_prodID, &_revMajor, &_revMinor);
}
...
QString USBDevice::dump()
{
...
#ifndef Q_OS_FREEBSD
    r += i18n("<tr><td><i>USB Version</i></td><td>%1.%2</td></tr>").arg(_verMajor, 0, 16).arg(QString::number(_verMinor, 16).prepend('0').right(2));
#endif
    r += "<tr><td></td></tr>";
    ...
    r += i18n("<tr><td><i>Revision</i></td><td>%1.%2</td></tr>").arg(_revMajor, 0, 16).arg(QString::number(_revMinor, 16).prepend('0').right(2));
    r += "<tr><td></td></tr>";
...
bool USBDevice::parse(QString fname)
{
    ...
    if(showErrorMessage && error)
    {
        ...
        KMessageBox::error(
            0, i18n("Could not open one or more USB controller. Make sure, you have read access to all USB controllers that should be listed here."));
    }
