...
KRandRSystemTray::KRandRSystemTray(QWidget *parent, const char *name)
    : KSystemTray(parent, name), m_popupUp(false), m_help(new KHelpMenu(this, KGlobal::instance()->aboutData(), false, actionCollection()))
{
...
void KRandRSystemTray::contextMenuAboutToShow(KPopupMenu *menu)
{
    ...
    KAction *actPrefs =
        new KAction(i18n("Configure Display..."), SmallIconSet("configure"), KShortcut(), this, SLOT(slotPrefs()), actionCollection());
    actPrefs->plug(menu);
...
void KRandRSystemTray::configChanged()
{
    ...
    if(!first)
        KRandrPassivePopup::message(i18n("Screen configuration has changed"), currentScreen()->changedMessage(), SmallIcon("window_fullscreen"), this,
...
void KRandRSystemTray::populateMenu(KPopupMenu *menu)
{
    ...
    for(int j = 0; j < numSizes; j++)
    {
        ...
        lastIndex = menu->insertItem(
            i18n("%1 x %2").arg(currentScreen()->pixelSize(highestIndex).width()).arg(currentScreen()->pixelSize(highestIndex).height()));
