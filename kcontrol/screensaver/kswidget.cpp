...
KSWidget::KSWidget(QWidget *parent, const char *name, int f) : QXEmbed(parent, name, f), colormap(None)
{
...
#undef B
    ...
    Window w = XCreateWindow(x11Display(), parentWidget() ? parentWidget()->winId() : RootWindow(x11Display(), x11Screen()), x(), y(), width(),
                             height(), 0, x11Depth(), InputOutput, visual, flags, &attrs);
    create(w);
#endif
}
