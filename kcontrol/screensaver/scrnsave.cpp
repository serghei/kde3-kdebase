...
const uint widgetEventMask = // X event mask
    (uint)(ExposureMask | PropertyChangeMask | StructureNotifyMask);
...
KScreenSaver::KScreenSaver(QWidget *parent, const char *name, const QStringList &) : KCModule(KSSFactory::instance(), parent, name)
{
    mSetupProc = 0;
    mPreviewProc = 0;
    mTestWin = 0;
    ...
    // Add non-KDE path
    KGlobal::dirs()->addResourceType("scrsav", KGlobal::dirs()->kde_default("apps") + "apps/ScreenSavers/");

    setQuickHelp(
        i18n("<h1>Screen Saver</h1> This module allows you to enable and"
    ...
    KGlobal::dirs()->addResourceType("scrsav", KGlobal::dirs()->kde_default("apps") + relPath);

    readSettings(false);

    mSetupProc = new KProcess;
    connect(mSetupProc, SIGNAL(processExited(KProcess *)), this, SLOT(slotSetupDone(KProcess *)));

    mPreviewProc = new KProcess;
    connect(mPreviewProc, SIGNAL(processExited(KProcess *)), this, SLOT(slotPreviewExited(KProcess *)));

    QBoxLayout *topLayout = new QHBoxLayout(this, 0, KDialog::spacingHint());

    // left column
    QVBoxLayout *leftColumnLayout = new QVBoxLayout(topLayout, KDialog::spacingHint());
    QBoxLayout *vLayout = new QVBoxLayout(leftColumnLayout, KDialog::spacingHint());
    ...
    QBoxLayout *groupLayout = new QVBoxLayout(mSaverGroup->layout(), KDialog::spacingHint());
    ...
    groupLayout = new QVBoxLayout(mSettingsGroup->layout(), KDialog::spacingHint());

    mEnabledCheckBox = new QCheckBox(i18n("Start a&utomatically"), mSettingsGroup);
    mEnabledCheckBox->setChecked(mEnabled);
    QWhatsThis::add(mEnabledCheckBox, i18n("Automatically start the screen saver after a period of inactivity."));
    ...
    connect(mWaitEdit, SIGNAL(valueChanged(int)), this, SLOT(slotTimeoutChanged(int)));
    ...
    mLockCheckBox = new QCheckBox(i18n("&Require password to stop"), mSettingsGroup);
    mLockCheckBox->setEnabled(mEnabled);
    mLockCheckBox->setChecked(mLock);
    connect(mLockCheckBox, SIGNAL(toggled(bool)), this, SLOT(slotLock(bool)));
    groupLayout->addWidget(mLockCheckBox);
    QWhatsThis::add(mLockCheckBox, i18n("Prevent potential unauthorized use by requiring a password"
    ...
    QWhatsThis::add(mLockLbl, i18n("The amount of time, after the screen saver has started, to ask for the unlock password."));
    ...
    if(mWaitLockEdit->sizeHint().width() < mWaitEdit->sizeHint().width())
    {
    ...
    connect(mWaitLockEdit, SIGNAL(valueChanged(int)), this, SLOT(slotLockTimeoutChanged(int)));
    ...
    // right column
    QBoxLayout *rightColumnLayout = new QVBoxLayout(topLayout, KDialog::spacingHint());
    ...
    QPushButton *advancedBt = new QPushButton(i18n("Advanced &Options"), this, "advancedBtn");
    advancedBt->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
    connect(advancedBt, SIGNAL(clicked()), this, SLOT(slotAdvanced()));
    ...
    KAboutData *about = new KAboutData(I18N_NOOP("kcmscreensaver"), I18N_NOOP("KDE Screen Saver Control Module"), 0, 0, KAboutData::License_GPL,
...
void KScreenSaver::resizeEvent(QResizeEvent *)
{
    ...
    if(mMonitor)
    {
        mMonitor->setGeometry((mMonitorLabel->width() - 200) / 2 + 23, (mMonitorLabel->height() - 186) / 2 + 14, 151, 115);
    }
}
...
void KScreenSaver::findSavers()
{
    if(!mNumLoaded)
    {
        mSaverFileList = KGlobal::dirs()->findAllResources("scrsav", "*.desktop", false, true);
    ...
    for(int i = 0; i < 5 && (unsigned)mNumLoaded < mSaverFileList.count(); i++, mNumLoaded++)
    {
    ...
    if((unsigned)mNumLoaded == mSaverFileList.count())
    {
        ...
        connect(mSaverListView, SIGNAL(currentChanged(QListViewItem *)), this, SLOT(slotScreenSaver(QListViewItem *)));
...
void KScreenSaver::slotTest()
{
    ...
    if(!path.isEmpty())
    {
        ...
        if(!mTestWin)
        {
            ...
            mTestWin->setGeometry(0, 0, kapp->desktop()->width(), kapp->desktop()->height());
        }
