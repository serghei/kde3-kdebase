...
KSmartcardConfig::KSmartcardConfig(QWidget *parent, const char *name) : KCModule(parent, name), DCOPObject(name)
{
    ...
    KAboutData *about = new KAboutData(I18N_NOOP("kcmsmartcard"), I18N_NOOP("KDE Smartcard Control Module"), 0, 0, KAboutData::License_GPL,
    ...
    if(_ok)
    {
        ...
        _popUpKardChooser->insertItem(i18n("Change Module..."), this, SLOT(slotLaunchChooser()));
        // The config backend
        ...
        connect(base->_readerHostsListView, SIGNAL(rightButtonPressed(QListViewItem *, const QPoint &, int)), this,
                SLOT(slotShowPopup(QListViewItem *, const QPoint &, int)));


        if(!connectDCOPSignal("", "", "signalReaderListChanged(QStringList)", "loadReadersTab(QStringList)", FALSE))

            kdDebug() << "Error connecting to DCOP server" << endl;


        if(!connectDCOPSignal("", "", "signalCardStateChanged(QString,bool,QString)", "updateReadersState (QString,bool,QString) ", FALSE))
...
void KSmartcardConfig::updateReadersState(QString readerName, bool isCardPresent, QString atr)
{
...
void KSmartcardConfig::loadReadersTab(QStringList lr)
{
    ...
    if(!config->readBoolEntry("Enable Support", false))
    {
        ...
        //  New view items
        KListViewItem *temp;
        kapp->dcopClient()->call("kded", "kded", "unloadModule(QCString)", data, rettype, retval);
    ...
    for(QStringList::Iterator _slot = lr.begin(); _slot != lr.end(); ++_slot)
    {
        ...
        kapp->dcopClient()->call("kded", "kardsvc", "getCardATR(QString)", dataATR, rettype, retval);
...
void KSmartcardConfig::getSupportingModule(KListViewItem *ant, QString &cardATR) const
{
    ...
    if(!modName.isNull())
    {
        ...
        KListViewItem *hil = new KListViewItem(ant, i18n("Managed by: "), type, subType, subSubType);
    ...
    else
    {
        ...
        KListViewItem *hil = new KListViewItem(ant, i18n("No module managing this card"));
        hil->setSelectable(FALSE);
    }
}
void KSmartcardConfig::load()
{
    ...
    void KSmartcardConfig::load(bool useDefaults)
    {
        ...
        kapp->dcopClient()->call("kded", "kardsvc", "getSlotList ()", data, rettype, retval);
        QStringList readers;
        readers.clear();
    ...
    void KSmartcardConfig::save()
    {
        if(_ok)
        {
            ...
            if(base->enableSupport->isChecked())
            {
                ...
                kapp->dcopClient()->call("kded", "kded", "loadModule(QCString)", data, rettype, retval);
                config->sync();

                kapp->dcopClient()->call("kded", "kardsvc", "reconfigure()", data, rettype, retval);
            }
            else
            {
    ...
    {
        return i18n(
            "<h1>smartcard</h1> This module allows you to configure KDE support"
