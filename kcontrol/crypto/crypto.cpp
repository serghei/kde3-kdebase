...
CipherItem::CipherItem(QListView *view, const QString &cipher, int bits, int maxBits, KCryptoConfig *module)
    : QCheckListItem(view, QString(), CheckBox)
{
    m_cipher = cipher;
    m_bits = bits;
    m_module = module;
...
OtherCertItem::OtherCertItem(QListView *view, const QString &sub, const QString &md5, bool perm, int policy, QDateTime exp, KCryptoConfig *module)
    : QListViewItem(view, QString()), _sub(sub), _md5(md5), _exp(exp), _perm(perm), _policy(policy)
...
YourCertItem::YourCertItem(QListView *view, QString pkcs, QString pass, QString name, KCryptoConfig *module) : QListViewItem(view, QString())
...
KCryptoConfig::KCryptoConfig(QWidget *parent, const char *name, const QStringList &) : KCModule(KryptoFactory::instance(), parent, name)
{
    QGridLayout *grid;
    ...
    setQuickHelp(
        i18n("<h1>Crypto</h1> This module allows you to configure SSL for"
             " use with most KDE3 applications, as well as manage your personal"
    ...
    KAboutData *about = new KAboutData(I18N_NOOP("kcmcrypto"), I18N_NOOP("KDE3 Crypto Control Module"), 0, 0, KAboutData::License_GPL,
    ...
    grid = new QGridLayout(tabSSL, 7, 2, KDialog::marginHint(), KDialog::spacingHint());
    ...
    whatstr = i18n(
        "TLS is the newest revision of the SSL protocol."
    ...
    whatstr = i18n(
        "SSL v2 is the second revision of the SSL protocol."
    ...
    whatstr = i18n(
        "SSL v3 is the third revision of the SSL protocol."
        " It is most common to enable v2 and v3.");
    ...
    whatstr = i18n(
        "Select the ciphers you wish to enable when using the"
    ...
    connect(mUseSSLv2, SIGNAL(toggled(bool)), SSLv2Box, SLOT(setEnabled(bool)));
#else
    ...
    connect(mUseSSLv3, SIGNAL(toggled(bool)), SSLv3Box, SLOT(setEnabled(bool)));
    ...
    QString whatStr =
        i18n("<qt>Use these preconfigurations to more easily configure the SSL encryption settings. You can choose among the following modes: <ul>");
    ...
    whatstr = i18n(
        "If selected, you will be notified when entering an SSL"
        " enabled site");
    ...
    whatstr = i18n(
        "If selected, you will be notified when leaving an SSL"
        " based site.");
    ...
    whatstr = i18n(
        "If selected, you will be notified before sending"
...
///////////////////////////////////////////////////////////////////////////
    ...
    QGridLayout *grid2 = new QGridLayout(egdframe, 2, 2, KDialog::marginHint(), KDialog::spacingHint());
    ...
    whatstr = i18n(
        "This list box shows which certificates of yours KDE3"
  ...
  grid->addMultiCellWidget(ocbg, 14, 14, 0, 5);
    ...
    whatstr = i18n(
        "This list box shows which site and person certificates KDE3"
        " knows about. You can easily manage them from here.");
    ...
    whatstr = i18n(
        "This button allows you to export the selected certificate"
    ...
    whatstr = i18n(
        "This button removes the selected certificate"
    ...
    whatstr = i18n(
        "This button tests the selected certificate"
        " for validity.");
    ...
    whatstr = i18n(
        "This list box shows which certificate authorities KDE3"
        " knows about. You can easily manage them from here.");
...
void KCryptoConfig::load(bool useDefaults)
{
  ...
  mWarnRevoked->setChecked(config->readBoolEntry("WarnRevoked", true));
    ...
    while(item)
    {
        item->setOn(config->readBoolEntry(item->configName(), item->bits() >= 56));
    ...
    {
        if((*i).isEmpty() || *i == "<default>" || *i == "General")
            continue;
        policies->setGroup(*i);
        KSSLCertificate *cert = KSSLCertificate::fromString(policies->readEntry("Certificate", QString()).local8Bit());
        if(cert)
        {
            new OtherCertItem(otherSSLBox, cert->getSubject(), *i, policies->readBoolEntry("Permanent", true), policies->readNumEntry("Policy", 3),
    ...
    for(QStringList::Iterator i = groups.begin(); i != groups.end(); ++i)
    {
        if((*i).isEmpty() || *i == "<default>")
            continue;
        pcerts->setGroup(*i);
        YourCertItem *j = new YourCertItem(yourSSLBox, pcerts->readEntry("PKCS12Base64"), pcerts->readEntry("Password"), *i, this);
    ...
    for(QStringList::Iterator i = groups.begin(); i != groups.end(); ++i)
    {
        ...
        HostAuthItem *j = new HostAuthItem(hostAuthList, KResolver::domainToAscii(*i), authcfg->readEntry("certificate"), this);
        j->setAction(aa);
        j->setOriginalName(*i);
    }
    ...
    for(QStringList::Iterator i = groups.begin(); i != groups.end(); ++i)
    {
        if((*i).isEmpty() || *i == "<default>")
            continue;
        if(!sigcfg.hasGroup(*i))
            continue;
        sigcfg.setGroup(*i);
        if(!sigcfg.hasKey("x509"))
            continue;
        new CAItem(caList, (*i), sigcfg.readEntry("x509"), sigcfg.readBoolEntry("site", false), sigcfg.readBoolEntry("email", false),
                   sigcfg.readBoolEntry("code", false), this);
    }
...
{
#ifdef HAVE_SSL
    if(!mUseTLS->isChecked() && !mUseSSLv2->isChecked() && !mUseSSLv3->isChecked())
  ...
  config->writeEntry("WarnRevoked", mWarnRevoked->isChecked());
    ...
    while(item)
    {
        if(item->isOn())
        {
            ...
            config->writeEntry(item->configName(), false);
    ...
    for(OtherCertItem *x = static_cast< OtherCertItem * >(otherSSLBox->firstChild()); x; x = static_cast< OtherCertItem * >(x->nextSibling()))
    {
        KSSLX509Map cert(x->configName());
        QString thisCN = cert.getValue("CN");
    ...
    // Go through the non-deleted ones and save them
    for(YourCertItem *x = static_cast< YourCertItem * >(yourSSLBox->firstChild()); x; x = static_cast< YourCertItem * >(x->nextSibling()))
    {
    ...
    // Go through the non-deleted ones and save them
    for(CAItem *x = static_cast< CAItem * >(caList->firstChild()); x; x = static_cast< CAItem * >(x->nextSibling()))
    {
        if(!x->modified && !x->isNew)
            continue;
        if(x->isNew)
        {
            x->isNew = false;
            _signers->addCA(x->getCert(), x->getSite(), x->getEmail(), x->getCode());
        }
        else
        {
            _signers->setUse(x->configName(), x->getSite(), x->getEmail(), x->getCode());
        }
        x->modified = false;
        doGen = true;
    }
    ...
    if(defCertBox->currentItem() == 0)
        ...
        config->writeEntry("DefaultCert", defCertBox->currentText());
    ...
    for(HostAuthItem *x = static_cast< HostAuthItem * >(hostAuthList->firstChild()); x; x = static_cast< HostAuthItem * >(x->nextSibling()))
    {
...
QString KCryptoConfig::handbookSection() const
{
...
#ifdef HAVE_SSL
    CipherItem *item;
    for(item = static_cast< CipherItem * >(SSLv2Box->firstChild()); item; item = static_cast< CipherItem * >(item->nextSibling()))
    {
        item->setOn(item->bits() >= 56 && item->bits() <= 128);
    }

    for(item = static_cast< CipherItem * >(SSLv3Box->firstChild()); item; item = static_cast< CipherItem * >(item->nextSibling()))
    {
        item->setOn(item->bits() >= 56 && item->bits() <= 128);
    }
...
void KCryptoConfig::slotExportCert()
{
    ...
    if(x)
    {
        ...
        if(cert)
        {
            ...
            KMessageBox::sorry(this, i18n("Could not open the certificate."), i18n("SSL"));
    }
}
...
void KCryptoConfig::slotRemoveCert()
{
    ...
    if(x)
    {
        ...
        configChanged();
        if(next)
            otherSSLBox->setSelected(next, true);
    }
}
...
void KCryptoConfig::slotVerifyCert()
{
    ...
    */
    ...
    {
        KMessageBox::detailedError(this, i18n("This certificate has failed the tests and should be considered invalid."),
                                   KSSLCertificate::verifyText(v), i18n("SSL"));
    }
...
TryImportPassAgain:
    ...
    if(!cert)
    {
        rc = KMessageBox::warningYesNo(this, i18n("The certificate file could not be loaded. Try a different password?"), i18n("SSL"), i18n("Try"),
                                       i18n("Do Not Try"));
        if(rc == KMessageBox::Yes)
            goto TryImportPassAgain;
        return;
    }
    ...
    for(YourCertItem *i = static_cast< YourCertItem * >(yourSSLBox->firstChild()); i; i = static_cast< YourCertItem * >(i->nextSibling()))
    {
        if(i->configName() == name)
        {
            rc = KMessageBox::warningContinueCancel(
                this, i18n("A certificate with that name already exists. Are you sure that you wish to replace it?"), i18n("SSL"), i18n("Replace"));
    ...
    new YourCertItem(yourSSLBox, cert->toString(),
...
void KCryptoConfig::slotCAImport()
{
    ...
    if(certLookup && KOSSL::self()->X509_LOOKUP_load_file(certLookup, certFile.local8Bit(), X509_FILETYPE_PEM))
    {
        ...
        for(int i = 0; i < KOSSL::self()->OPENSSL_sk_num(certStore_objs); i++)
        {
            ...
            if(!x || !x->x509V3Extensions().certTypeCA())
            {
                if(x)
                {
                    ...
                    KMessageBox::error(this, emsg, i18n("SSL"));
                    delete x;
                }
                continue;
            }
            ...
            // search for dups
            for(CAItem *m = static_cast< CAItem * >(caList->firstChild()); m; m = static_cast< CAItem * >(m->nextSibling()))
            {
    ...
    { // try to load it manually as a single X.509 DER encoded
        ...
        if(certStore)
        {
            KOSSL::self()->OPENSSL_sk_free(certStore);
            certStore = NULL;
        }
        ...
        if(!x)
        {
            KMessageBox::sorry(this, i18n("The certificate file could not be loaded."), i18n("SSL"));
            return;
        }

        if(!x->x509V3Extensions().certTypeCA())
        {
            KMessageBox::sorry(this, i18n("This is not a signer certificate."), i18n("SSL"));
            return;
        }

        name = x->getSubject();

        for(CAItem *i = static_cast< CAItem * >(caList->firstChild()); i; i = static_cast< CAItem * >(i->nextSibling()))
        {
            if(i->configName() == name)
            {
                ...
                if(*x == *y)
                {
                    KMessageBox::error(this, i18n("You already have this signer certificate installed."), i18n("SSL"));
                    delete x;
                    delete y;
                    return;
                }
...
{
    if(KMessageBox::questionYesNo(this, i18n("Do you want to make this certificate available to KMail as well?"), QString(), i18n("Make Available"),
                                  i18n("Do Not Make Available"))
       == KMessageBox::Yes)
    {
        KProcess proc;
        proc << "kleopatra";
...
void KCryptoConfig::slotCARestore()
{
    ...
    int rc = KMessageBox::warningContinueCancel(this, i18n("This will revert your certificate signers database to the KDE3 default.\nThis operation "
                                                           "cannot be undone.\nAre you sure you wish to continue?"),
                                                i18n("SSL"), i18n("Revert"));
    if(rc == KMessageBox::Cancel)
    {
        return;
    }
    ...
    for(QStringList::Iterator i = groups.begin(); i != groups.end(); ++i)
    {
        ...
        if(!sigcfg.hasKey("x509"))
            continue;
        new CAItem(caList, (*i), sigcfg.readEntry("x509", QString()), sigcfg.readBoolEntry("site", false), sigcfg.readBoolEntry("email", false),
                   sigcfg.readBoolEntry("code", false), this);
    }
...
{
    HostAuthItem *j = new HostAuthItem(hostAuthList, QString(), QString(), this);
...
void KCryptoConfig::slotRemoveHostAuth()
{
    ...
    if(x)
    {
        ...
        configChanged();
        if(next)
            hostAuthList->setSelected(next, true);
    }
}
...
void KCryptoConfig::slotAuthCombo()
{
    ...
    if(x)
    {
        if(hostCertBox->currentItem() == 0)
            ...
            x->setCertName(hostCertBox->currentText());
        configChanged();
    }
}
...
void KCryptoConfig::slotTestOSSL()
{
    ...
    {
        KMessageBox::detailedSorry(this, i18n("Failed to load OpenSSL."), i18n("libssl was not found or successfully loaded."), i18n("OpenSSL"));
        return;
    }

    if(!KOSSL::self()->hasLibCrypto())
    {
        KMessageBox::detailedSorry(this, i18n("Failed to load OpenSSL."), i18n("libcrypto was not found or successfully loaded."), i18n("OpenSSL"));
        return;
    }
...
bool KCryptoConfig::loadCiphers()
{
    ...
    if(ctx)
        KOSSL::self()->SSL_CTX_free(ctx);
    if(ssl)
        KOSSL::self()->SSL_free(ssl);
#endif
...
void KCryptoConfig::setAuthCertLists()
{
    ...
    if(!noneDef && defCertBox->currentItem() == 0)
        configChanged();
    ...
    if(!noneHost && hostCertBox->currentItem() == 0)
        configChanged();
