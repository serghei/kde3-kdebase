...
StatisticsView::StatisticsView(QWidget *parent, KConfig *config, const char *name)
    : QWidget(parent, name), configFile(config), dataList(0), connectionsCount(0), filesCount(0), calcCount(0)
{
...
void StatisticsView::calculate()
{
    ...
    if((expandedInfoCb->isChecked()) || (expandedUserCb->isChecked()))
    {
        ...
        while(item != 0)
        {
            if(connCount)
            {
                if((QString(item->text(1)).contains(i18n("CONNECTION OPENED"))) && (QString(item->text(2)).contains(rService))
                   && (QString(item->text(3)).contains(rHost)))
                {
            ...
            {
                if((QString(item->text(1)).contains(i18n("FILE OPENED"))) && (QString(item->text(2)).contains(rService))
                   && (QString(item->text(3)).contains(rHost)))
                {
                    if(expandedInfoCb->isChecked())
                        item2 = item->text(2);
