...
WidgetCanvas::WidgetCanvas(QWidget *parent, const char *name) : QWidget(parent, name), shadeSortColumn(true)
{
...
void WidgetCanvas::resetTitlebarPixmaps(const QColor &actMed, const QColor &disMed)
{
    if(close_pix)
        delete close_pix;
    if(maximize_pix)
    ...
    pact.begin(close_pix);
    pdis.begin(dis_close_pix);
    ...
    close_pix->setMask(bitmap);
    dis_close_pix->setMask(bitmap);

    minimize_pix = new QPixmap(16, 16);
    dis_minimize_pix = new QPixmap(16, 16);
    pact.begin(minimize_pix);
    pdis.begin(dis_minimize_pix);
    ...
    minimize_pix->setMask(bitmap);
    dis_minimize_pix->setMask(bitmap);

    maximize_pix = new QPixmap(16, 16);
    dis_maximize_pix = new QPixmap(16, 16);
    pact.begin(maximize_pix);
    pdis.begin(dis_maximize_pix);
    ...
    maximize_pix->setMask(bitmap);
    dis_maximize_pix->setMask(bitmap);

    normalize_pix = new QPixmap(16, 16);
    dis_normalize_pix = new QPixmap(16, 16);
    pact.begin(normalize_pix);
    pdis.begin(dis_normalize_pix);
    ...
    normalize_pix->setMask(bitmap);
    dis_normalize_pix->setMask(bitmap);

    menu_pix = new QPixmap(16, 16);
    dis_menu_pix = new QPixmap(16, 16);
    pact.begin(menu_pix);
    pdis.begin(dis_menu_pix);
    ...
    menu_pix->setMask(bitmap);
    dis_menu_pix->setMask(bitmap);

    pinup_pix = new QPixmap(16, 16);
    dis_pinup_pix = new QPixmap(16, 16);
    pact.begin(pinup_pix);
    pdis.begin(dis_pinup_pix);
    ...
    pinup_pix->setMask(bitmap);
    dis_pinup_pix->setMask(bitmap);

    pindown_pix = new QPixmap(16, 16);
    dis_pindown_pix = new QPixmap(16, 16);
    pact.begin(pindown_pix);
    pdis.begin(dis_pindown_pix);
    ...
    pindown_pix->setMask(bitmap);
    dis_pindown_pix->setMask(bitmap);
}
...
void WidgetCanvas::drawSampleWidgets()
{
    ...
    QColorGroup cg(txt, back, back.light(highlightVal), back.dark(lowlightVal), back.dark(120), txt, window);
    ...
    qDrawWinPanel(&paint, 15, 5, width() - 48, height(), cg, FALSE, &brush);
    ...
    KPixmapEffect::gradient(pmTitle, iaTitle, iaBlend, KPixmapEffect::HorizontalGradient);
    ...
    int spot = 0;
    hotspots[spot++] = HotSpot(QRect(65, 25 - 14, textLen, 14), CSM_Inactive_title_text);

    hotspots[spot++] = HotSpot(QRect(60, 10, (width() - 160) / 2, 20), CSM_Inactive_title_bar);

    hotspots[spot++] = HotSpot(QRect(60 + (width() - 160) / 2, 10, (width() - 160) / 2, 20), CSM_Inactive_title_blend);

    hotspots[spot++] = HotSpot(QRect(20, 12, 40, 20), CSM_Inactive_title_button);
    hotspots[spot++] = HotSpot(QRect(tmp, 12, 60, 20), CSM_Inactive_title_button);


    // Active window

    qDrawWinPanel(&paint, 20, 25 + 5, width() - 40, height(), cg, FALSE, &brush);

    paint.setBrush(aTitle);
    paint.setPen(aTitle);
    ...
    KPixmapEffect::gradient(pmTitle, aTitle, aBlend, KPixmapEffect::HorizontalGradient);
    ...
    hotspots[spot++] = HotSpot(QRect(75, 50 - 14, textLen, 14), CSM_Active_title_text);
    hotspots[spot++] = HotSpot(QRect(65, 35, (width() - 152) / 2, 20), CSM_Active_title_bar);
    hotspots[spot++] = HotSpot(QRect(65 + (width() - 152) / 2, 35, (width() - 152) / 2, 20), CSM_Active_title_blend);

    hotspots[spot++] = HotSpot(QRect(25, 35, 40, 20), CSM_Active_title_button);
    hotspots[spot++] = HotSpot(QRect(tmp, 35, 60, 20), CSM_Active_title_button);

    // Menu bar

    // qDrawShadePanel ( &paint, 25, 55, width()-52, 28, cg, FALSE, 2, &brush);
    kapp->style().drawPrimitive(QStyle::PE_PanelMenuBar, &paint, QRect(QPoint(25, 55), QSize(width() - 52, 28)), cg);
    ...
    kapp->style().drawPrimitive(QStyle::PE_Panel, &paint, QRect(30, 59, textLen + 10, 21), cg);
    paint.drawText(35, 74, file);

    hotspots[spot++] = HotSpot(QRect(35, 62, textLen, 14), CSM_Text);
    hotspots[spot++] = HotSpot(QRect(27, 57, 33, 21), CSM_Background);
    ...
    qDrawShadePanel(&paint, 25, 80 + 5 - 4, width() - 7 - 45 - 2, height(), cg, TRUE, 2, &brush);
    ...
    hotspots[spot++] = HotSpot(QRect(140, 113 - 20, textLen, 14), CSM_Standard_text);
    ...
    hotspots[spot++] = HotSpot(QRect(140, 121, textLen, 14), CSM_Select_text);
    hotspots[spot++] = HotSpot(QRect(120, 115, textLen + 40, 32), CSM_Select_background); // select bg
    ...
    hotspots[spot++] = HotSpot(QRect(column2 + 18, 113 - 20, textLen, 17), CSM_Link);
    ...
    hotspots[spot++] = HotSpot(QRect(column3, 113 - 20, textLen, 17), CSM_Followed_Link);
    ...
    hotspots[spot++] =
        HotSpot(QRect(xpos + 16, ypos + ((28 - paint.fontMetrics().height()) / 2), textLen, paint.fontMetrics().height()), CSM_Button_text);
    hotspots[spot++] = HotSpot(QRect(xpos, ypos, textLen + 32, 28), CSM_Button_background);
    ...
    kapp->style().drawPrimitive(QStyle::PE_ButtonCommand, &paint, QRect(xpos, ypos, textLen + 32, 28), cg2,
                                QStyle::Style_Enabled | QStyle::Style_Raised);
    paint.setPen(buttonTxt);
    paint.drawText(xpos, ypos, textLen + 32, 28, AlignCenter, i18n("Push Button"));
    ...
    hotspots[spot++] = HotSpot(QRect(56, 83, textLenNew, 14), CSM_Text);

    paint.setFont(menuFont);
    textLenOpen = paint.fontMetrics().width(i18n("Menu item", "Open"));

    hotspots[spot++] = HotSpot(QRect(56, 105, textLenOpen, 14), CSM_Text);
    ...
    hotspots[spot++] = HotSpot(QRect(28, 78, 88, 77), CSM_Background);

    hotspots[spot++] = HotSpot(QRect(25, 80 + 5 - 4, width() - 7 - 45 - 2 - 16, height()), CSM_Standard_background);


    // Valance

    qDrawWinPanel(&paint, 0, 0, width(), height(), parentWidget()->colorGroup(), TRUE, 0);

    // Stop the painting

    hotspots[spot++] = HotSpot(QRect(0, 0, width(), height()), CSM_Background); // ?
