...
void ComponentChooser::slotServiceSelected(QListBoxItem *it)
{
    ...
    if(somethingChanged)
    {
        if(KMessageBox::questionYesNo(this, i18n("<qt>You changed the default component of your choice. Do you want to save that change now?</qt>"),
    ...
    if(cfgType.isEmpty() || (cfgType == "component"))
    {
        if(!(configWidget && configWidget->qt_cast("CfgComponent")))
        {
            ...
            cfgcomp->ChooserDocu->setText(
                i18n("Choose from the list below which component should be used by default for the %1 service.").arg(it->text()));
            newConfigWidget = cfgcomp;
        }
        else
        {
            static_cast< CfgComponent * >(configWidget)
                ->ChooserDocu->setText(
                    i18n("Choose from the list below which component should be used by default for the %1 service.").arg(it->text()));
        }
    }
...
void ComponentChooser::load()
{
    if(configWidget)
    {
        CfgPlugin *plugin = static_cast< CfgPlugin * >(configWidget->qt_cast("CfgPlugin"));
        if(plugin)
        {
