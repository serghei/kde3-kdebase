...
KDMConvenienceWidget::KDMConvenienceWidget(QWidget *parent, const char *name) : QWidget(parent, name)
{
    QString wtstr;
    ...
    wtstr = i18n(
        "The delay (in seconds) before the automatic login kicks in. "
    ...
    connect(npuserlv, SIGNAL(clicked(QListViewItem *)), SLOT(slotChanged()));
    ...
    connect(userlb, SIGNAL(activated(const QString &)), SLOT(slotSetAutoUser(const QString &)));
    connect(puserlb, SIGNAL(textChanged(const QString &)), SLOT(slotSetPreselUser(const QString &)));
    connect(npuserlv, SIGNAL(clicked(QListViewItem *)), SLOT(slotUpdateNoPassUser(QListViewItem *)));
}
...
void KDMConvenienceWidget::save()
{
    ...
    config->writeEntry("PreselectUser", npRadio->isChecked() ? "None" : ppRadio->isChecked() ? "Previous" : "Default");
...
void KDMConvenienceWidget::slotAddUsers(const QMap< QString, int > &users)
{
    ...
    for(it = users.begin(); it != users.end(); ++it)
    {
        ...
        if(it.data() != 0)
            (new QCheckListItem(npuserlv, it.key(), QCheckListItem::CheckBox))->setOn(noPassUsers.find(it.key()) != noPassUsers.end());
    }
    ...
    for(it = users.begin(); it != users.end(); ++it)
    {
        if(it.data() > 0)
        {
            if(it.key() != autoUser && userlb->listBox())
                delete userlb->listBox()->findItem(it.key(), ExactMatch | CaseSensitive);
            if(it.key() != preselUser && puserlb->listBox())
                delete puserlb->listBox()->findItem(it.key(), ExactMatch | CaseSensitive);
        }
        if(it.data() != 0)
