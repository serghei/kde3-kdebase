...
{
    disconnect(logobutton, SIGNAL(clicked()), this, SLOT(slotLogoButtonClicked()));
...
{
    QStringList langlist = KGlobal::dirs()->findAllResources("locale", QString::fromLatin1("*/entry.desktop"));
    langlist.sort();
...
void KDMAppearanceWidget::loadColorSchemes(KBackedComboBox *combo)
{
    ...
    for(QStringList::ConstIterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        QString str;
        if(!(str = config.readEntry("Name")).isEmpty() || !(str = config.readEntry("name")).isEmpty())
        {
...
{
    // XXX: Global + local schemes
    QStringList list = KGlobal::dirs()->findAllResources("data", "kstyle/themes/*.themerc", false, true);
    for(QStringList::ConstIterator it = list.begin(); it != list.end(); ++it)
    {
        KSimpleConfig config(*it, true);
...
{
    QString flogo = logo.isEmpty() ? locate("data", QString::fromLatin1("kdm/pics/kdelogo.png")) : logo;
    QImage p(flogo);
    if(p.isNull())
        return false;
...
void KDMAppearanceWidget::slotLogoButtonClicked()
{
    ...
    KFileDialog dialogue(locate("data", QString::fromLatin1("kdm/pics/")), KImageIO::pattern(KImageIO::Reading), this, 0, true);
...
void KDMAppearanceWidget::iconLoaderDropEvent(QDropEvent *e)
{
    ...
    if(url)
    {
        ...
        if(!url->isLocalFile())
        {
            pixurl.setPath(KGlobal::dirs()->resourceDirs("data").last() + "kdm/pics/" + url->fileName());
...
void KDMAppearanceWidget::save()
{
    ...
    config->writeEntry("LogoArea", noneRadio->isChecked() ? "None" : logoRadio->isChecked() ? "Logo" : "Clock");
...
{
    return i18n(
        "<h1>KDM - Appearance</h1> Here you can configure the basic appearance"
