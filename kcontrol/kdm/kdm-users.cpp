...
KDMUsersWidget::KDMUsersWidget(QWidget *parent, const char *name) : QWidget(parent, name)
{
#ifdef __linux__
    struct stat st;
    ...
    QWhatsThis::add(minGroup,
                    i18n("Users with a UID (numerical user identification) outside this range will not be listed by KDM and this setup dialog."
    ...
    connect(optinlv, SIGNAL(clicked(QListViewItem *)), SLOT(slotUpdateOptIn(QListViewItem *)));
    connect(optinlv, SIGNAL(clicked(QListViewItem *)), SLOT(slotChanged()));
    optoutlv = new KListView(this);
    optoutlv->addColumn(i18n("Hidden Users"));
    optoutlv->setResizeMode(QListView::LastColumn);
    QWhatsThis::add(optoutlv, i18n("KDM will show all non-checked non-system users. Entries denoted with '@' are user groups. Checking a group is "
                                   "like checking all users in that group."));
    wstack->addWidget(optoutlv);
    connect(optoutlv, SIGNAL(clicked(QListViewItem *)), SLOT(slotUpdateOptOut(QListViewItem *)));
    connect(optoutlv, SIGNAL(clicked(QListViewItem *)), SLOT(slotChanged()));
    ...
    connect(usercombo, SIGNAL(activated(int)), SLOT(slotUserSelected()));
    ...
    connect(userbutton, SIGNAL(clicked()), SLOT(slotUserButtonClicked()));
    QToolTip::add(userbutton, i18n("Click or drop an image here"));
    QWhatsThis::add(
        userbutton,
        i18n("Here you can see the image assigned to the user selected in the combo box above. Click on the image button to select from a list"
    ...
    connect(rstuserbutton, SIGNAL(clicked()), SLOT(slotUnsetUserPix()));
...
{
    KFileDialog dlg(m_notFirst ? QString::null : KGlobal::dirs()->resourceDirs("data").last() + "kdm/pics/users",
                    KImageIO::pattern(KImageIO::Reading), this, 0, true);
...
void KDMUsersWidget::save()
{
    ...
    config->writeEntry("ShowUsers", cbinverted->isChecked() ? "NotHidden" : "Selected");
    ...
    config->writeEntry("FaceSource", rbadmonly->isChecked() ? "AdminOnly" : rbprefadm->isChecked() ? "PreferAdmin" : rbprefusr->isChecked()
                                                                                                                         ? "PreferUser"
                                                                                                                         : "UserOnly");
}
...
void KDMUsersWidget::slotAddUsers(const QMap< QString, int > &users)
{
    ...
    for(it = users.begin(); it != users.end(); ++it)
    {
        ...
        (new QCheckListItem(optinlv, *name, QCheckListItem::CheckBox))->setOn(selectedUsers.find(*name) != selectedUsers.end());
        (new QCheckListItem(optoutlv, *name, QCheckListItem::CheckBox))->setOn(hiddenUsers.find(*name) != hiddenUsers.end());
        if((*name)[0] != '@')
