...
public:
    ...
    {
        for(QValueList< StringPair >::ConstIterator it = list.begin(); it != list.end(); ++it)
            if((*it).storeName == c)
                return (*it);
...
QString KLocaleConfigTime::userToStore(const QValueList< StringPair > &list, const QString &userFormat) const
{
    QString result;
    ...
    {
        bool bFound = false;
        for(QValueList< StringPair >::ConstIterator it = list.begin(); it != list.end() && !bFound; ++it)
        {
...
QString KLocaleConfigTime::storeToUser(const QValueList< StringPair > &list, const QString &storeFormat) const
{
    QString result;
...
KLocaleConfigTime::KLocaleConfigTime(KLocale *_locale, QWidget *parent, const char *name) : QWidget(parent, name), m_locale(_locale)
{
    // Time
    QGridLayout *lay = new QGridLayout(this, 7, 2, KDialog::marginHint(), KDialog::spacingHint());
    lay->setAutoAdd(TRUE);

    m_labCalendarSystem = new QLabel(this, I18N_NOOP("Calendar system:"));
    m_comboCalendarSystem = new QComboBox(false, this);
    connect(m_comboCalendarSystem, SIGNAL(activated(int)), this, SLOT(slotCalendarSystemChanged(int)));
    ...
    connect(m_comboTimeFmt, SIGNAL(textChanged(const QString &)), this, SLOT(slotTimeFmtChanged(const QString &)));

    m_labDateFmt = new QLabel(this, I18N_NOOP("Date format:"));
    m_comboDateFmt = new QComboBox(true, this);
    connect(m_comboDateFmt, SIGNAL(textChanged(const QString &)), this, SLOT(slotDateFmtChanged(const QString &)));

    m_labDateFmtShort = new QLabel(this, I18N_NOOP("Short date format:"));
    m_comboDateFmtShort = new QComboBox(true, this);
    connect(m_comboDateFmtShort, SIGNAL(textChanged(const QString &)), this, SLOT(slotDateFmtShortChanged(const QString &)));

    m_labWeekStartDay = new QLabel(this, I18N_NOOP("First day of the week:"));
    m_comboWeekStartDay = new QComboBox(false, this);
    connect(m_comboWeekStartDay, SIGNAL(activated(int)), this, SLOT(slotWeekStartDayChanged(int)));

    updateWeekDayNames();

    m_chDateMonthNamePossessive = new QCheckBox(this, I18N_NOOP("Use declined form of month name"));
    connect(m_chDateMonthNamePossessive, SIGNAL(clicked()), SLOT(slotDateMonthNamePossChanged()));
...
void KLocaleConfigTime::save()
{
    ...
    if(m_locale->nounDeclension())
    {
        ...
        if(b != m_locale->dateMonthNamePossessive())
            config->writeEntry("DateMonthNamePossessive", m_locale->dateMonthNamePossessive(), true, true);
    }
...
void KLocaleConfigTime::slotLocaleChanged()
{
    ...
    CalendarVector::iterator it = qFind(calendars.begin(), calendars.end(), calendarType);
    if(it != calendars.end())
    ...
    kdDebug(173) << storeToUser(timeMap(), m_locale->timeFormat()) << endl;
    kdDebug(173) << userToStore(timeMap(), QString::fromLatin1("HH:MM:SS AMPM test")) << endl;
}
...
void KLocaleConfigTime::slotTranslate()
{
    ...
    m_comboCalendarSystem->changeItem(m_locale->translate("Calendar System Gregorian", "Gregorian"), 0);
    m_comboCalendarSystem->changeItem(m_locale->translate("Calendar System Hijri", "Hijri"), 1);
    m_comboCalendarSystem->changeItem(m_locale->translate("Calendar System Hebrew", "Hebrew"), 2);
    m_comboCalendarSystem->changeItem(m_locale->translate("Calendar System Jalali", "Jalali"), 3);
    ...
    str = m_locale->translate(
              ...
              "dates. The sequences below will be replaced:</p>")
          + datecodes;
