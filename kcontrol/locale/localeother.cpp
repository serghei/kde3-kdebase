...
KLocaleConfigOther::KLocaleConfigOther(KLocale *locale, QWidget *parent, const char *name) : QWidget(parent, name), m_locale(locale)
{
    // Other
    QGridLayout *lay = new QGridLayout(this, 3, 2, KDialog::marginHint(), KDialog::spacingHint());
    ...
    connect(m_combPageSize, SIGNAL(activated(int)), SLOT(slotPageSizeChanged(int)));
    ...
    connect(m_combMeasureSystem, SIGNAL(activated(int)), SLOT(slotMeasureSystemChanged(int)));
...
void KLocaleConfigOther::save()
{
    ...
    if(i != m_locale->pageSize())
        config->writeEntry("PageSize", m_locale->pageSize(), true, true);
    ...
    if(i != m_locale->measureSystem())
        config->writeEntry("MeasureSystem", m_locale->measureSystem(), true, true);
...
{
    m_combMeasureSystem->changeItem(m_locale->translate("The Metric System", "Metric"), 0);
    m_combMeasureSystem->changeItem(m_locale->translate("The Imperial System", "Imperial"), 1);
