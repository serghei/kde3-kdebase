...
KLocaleConfig::KLocaleConfig(KLocale *locale, QWidget *parent, const char *name) : QWidget(parent, name), m_locale(locale)
{
    QGridLayout *lay = new QGridLayout(this, 3, 3, KDialog::marginHint(), KDialog::spacingHint());
    ...
    connect(m_comboCountry, SIGNAL(activated(const QString &)), this, SLOT(changedCountry(const QString &)));

    m_labLang = new QLabel(this, I18N_NOOP("Languages:"));
    m_labLang->setAlignment(AlignTop);

    m_languages = new QListBox(this);
    connect(m_languages, SIGNAL(selectionChanged()), SLOT(slotCheckButtons()));
    ...
    connect(m_addLanguage, SIGNAL(activated(const QString &)), SLOT(slotAddLanguage(const QString &)));
    ...
    connect(m_removeLanguage, SIGNAL(clicked()), SLOT(slotRemoveLanguage()));
    connect(m_upButton, SIGNAL(clicked()), SLOT(slotLanguageUp()));
    connect(m_downButton, SIGNAL(clicked()), SLOT(slotLanguageDown()));
...
void KLocaleConfig::loadLanguageList()
{
    ...
    {
        QString str = locate("locale", QString::fromLatin1("%1/entry.desktop").arg(*it));
        if(!str.isNull())
            prilang << str;
    }

    // add all languages to the list
    QStringList alllang = KGlobal::dirs()->findAllResources("locale", QString::fromLatin1("*/entry.desktop"), false, true);
    ...
    for(QStringList::ConstIterator it = langlist.begin(); it != langlist.end(); ++it)
    {
        if((*it).isNull())
        {
            ...
            m_addLanguage->insertSubmenu(m_locale->translate("Other"), submenu, QString::null, -1);
            menu_index = -2; // first entries should _not_ be sorted
            continue;
        }
        KSimpleConfig entry(*it);
        entry.setGroup("KCM Locale");
        QString name = entry.readEntry("Name", m_locale->translate("without name"));
...
void KLocaleConfig::loadCountryList()
{
    ...
    QStringList regionlist = KGlobal::dirs()->findAllResources("locale", sub + QString::fromLatin1("*.desktop"), false, true);
    ...
    // add all languages to the list
    QStringList countrylist = KGlobal::dirs()->findAllResources("locale", sub + QString::fromLatin1("*/entry.desktop"), false, true);
    ...
    for(QStringList::ConstIterator it = countrylist.begin(); it != countrylist.end(); ++it)
    {
        ...
        QString flag(locate("locale", QString::fromLatin1("l10n/%1/flag.png").arg(tag)));
...
void KLocaleConfig::readLocale(const QString &path, QString &name, const QString &sub) const
{
    ...
    // read the name
    QString filepath = QString::fromLatin1("%1%2/entry.desktop").arg(sub).arg(path);
...
void KLocaleConfig::save()
{
    ...
    if(m_locale->languageList().isEmpty())
        ...
        config->writeEntry("Language", m_locale->languageList(), ':', true, true);
...
void KLocaleConfig::slotCheckButtons()
{
    ...
    m_downButton->setEnabled(m_languages->currentItem() != -1 && m_languages->currentItem() < (signed)(m_languages->count() - 1));
}
...
void KLocaleConfig::slotTranslate()
{
    ...
    QToolTip::add(m_comboCountry, m_locale->translate("This is where you live. KDE will use the defaults for "
                                                      "this country or region."));
    QToolTip::add(m_addLanguage, m_locale->translate("This will add a language to the list. If the language is already "
                                                     "in the list, the old one will be moved instead."));

    QToolTip::add(m_removeLanguage, m_locale->translate("This will remove the highlighted language from the list."));

    QToolTip::add(m_languages, m_locale->translate("KDE programs will be displayed in the first available language in "
...
{
    QString fileName = locate("locale", QString::fromLatin1("l10n/%1/entry.desktop").arg(m_locale->country()));
