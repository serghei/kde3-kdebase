...
KTheme::KTheme(QWidget *parent, const QString &xmlFile) : m_parent(parent)
{
    QFile file(xmlFile);
...
KTheme::KTheme(QWidget *parent, bool create) : m_parent(parent)
{
    if(create)
    {
...
void KTheme::apply()
{
    ...
    for(uint i = 0; i <= desktopList.count(); i++)
    {
        ...
        if(!desktopElem.isNull())
        {
            ...
            desktopConf.writeEntry("ReverseBlending", static_cast< bool >(getProperty(desktopElem, "blending", "reverse").toUInt()));
            desktopConf.writeEntry("Pattern", getProperty(desktopElem, "pattern", "name"));
            desktopConf.writeEntry("Wallpaper", unprocessFilePath("desktop", getProperty(desktopElem, "wallpaper", "url")));
    ...
    if(!panelElem.isNull())
    {
        ...
        if(!kickerBgUrl.isEmpty())
        {
            ...
            kickerConf.writeEntry("ColorizeBackground", static_cast< bool >(getProperty(panelElem, "background", "colorize").toUInt()));
        }
        kickerConf.writeEntry("Transparent", static_cast< bool >(getProperty(panelElem, "transparent", "value").toUInt()));
...
QString KTheme::getProperty(QDomElement parent, const QString &tag, const QString &attr) const
{
    ...
    {
        kdWarning() << QString("No such property found: %1->%2->%3").arg(parent.tagName()).arg(tag).arg(attr) << endl;
        return QString::null;
    }
}

void KTheme::createIconElems(const QString &group, const QString &object, QDomElement parent, KConfig *cfg)
{
    cfg->setGroup(group);
...
void KTheme::createColorElem(const QString &name, const QString &object, QDomElement parent, KConfig *cfg)
{
...
void KTheme::createSoundList(const QStringList &events, const QString &object, QDomElement parent, KConfig *cfg)
{
