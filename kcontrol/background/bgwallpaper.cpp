...
BGMultiWallpaperList::BGMultiWallpaperList(QWidget *parent, const char *name) : QListBox(parent, name)
{
    setAcceptDrops(true);
...
BGMultiWallpaperDialog::BGMultiWallpaperDialog(KBackgroundSettings *settings, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, i18n("Setup Slide Show"), Ok | Cancel, Ok, true), m_pSettings(settings)
{
...
void BGMultiWallpaperDialog::slotAdd()
{
    ...
    KFileDialog fileDialog(KGlobal::dirs()->findDirs("wallpaper", "").first(), mimeTypes.join(" "), this, 0L, true);

    fileDialog.setCaption(i18n("Select Image"));
    KFile::Mode mode = static_cast< KFile::Mode >(KFile::Files | KFile::Directory | KFile::ExistingOnly | KFile::LocalOnly);
