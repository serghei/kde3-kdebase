...
BGDialog::BGDialog(QWidget *parent, KConfig *_config, bool _multidesktop) : BGDialog_UI(parent, "BGDialog")
{
    ...
    {
        // desktop
        connect(m_comboDesktop, SIGNAL(activated(int)), SLOT(slotSelectDesk(int)));
    }
    if(m_numScreens > 1)
    {
        connect(m_comboScreen, SIGNAL(activated(int)), SLOT(slotSelectScreen(int)));
    }
    ...
    connect(m_buttonGroupBackground, SIGNAL(clicked(int)), SLOT(slotWallpaperTypeChanged(int)));
    connect(m_urlWallpaperBox, SIGNAL(activated(int)), SLOT(slotWallpaper(int)));
    connect(m_urlWallpaperButton, SIGNAL(clicked()), SLOT(slotWallpaperSelection()));
    connect(m_comboWallpaperPos, SIGNAL(activated(int)), SLOT(slotWallpaperPos(int)));
    connect(m_buttonSetupWallpapers, SIGNAL(clicked()), SLOT(slotSetupMulti()));

    // set up the background colour stuff
    connect(m_colorPrimary, SIGNAL(changed(const QColor &)), SLOT(slotPrimaryColor(const QColor &)));
    connect(m_colorSecondary, SIGNAL(changed(const QColor &)), SLOT(slotSecondaryColor(const QColor &)));
    connect(m_comboPattern, SIGNAL(activated(int)), SLOT(slotPattern(int)));
    ...
    // advanced options
    connect(m_buttonAdvanced, SIGNAL(clicked()), SLOT(slotAdvanced()));

    connect(m_buttonGetNew, SIGNAL(clicked()), SLOT(slotGetNewStuff()));
...
{
    return i18n(
        "<h1>Background</h1> This module allows you to control the"
...
void BGDialog::loadWallpaperFilesList()
{
    ...
    m_wallpaper.clear();
    int i = 0;
    for(QMap< QString, QPair< QString, QString > >::Iterator it = papers.begin(); it != papers.end(); ++it)
    {
...
{
    KFileDialog dlg(QString::null, QString::null, this, "file dialog", true);
    ...
    QString uri;
    for(QMap< QString, int >::ConstIterator it = m_wallpaper.begin(); it != m_wallpaper.end(); ++it)
    {
        if(it.data() == j)
        {
            uri = it.key();
...
void BGDialog::updateUI()
{
    ...
    if(r->backgroundMode() == KBackgroundSettings::Program && wallpaperMode == KBackgroundSettings::NoWallpaper)
        groupBox3->setEnabled(false);
    else
        groupBox3->setEnabled(true);

    if((multiMode == KBackgroundSettings::NoMultiRandom) || (multiMode == KBackgroundSettings::NoMulti))
    {
        // No wallpaper
        if(wallpaperMode == KBackgroundSettings::NoWallpaper)
        {
            ...
            m_buttonGroupBackground->setButton(m_buttonGroupBackground->id(m_radioNoPicture));
        }
        ...
        else
        {
            ...
            m_buttonGroupBackground->setButton(m_buttonGroupBackground->id(m_radioPicture));
        }
    }
    ...
    else
    {
        ...
        m_comboWallpaperPos->setEnabled(true);
        m_lblWallpaperPos->setEnabled(true);
        m_buttonGroupBackground->setButton(m_buttonGroupBackground->id(m_radioSlideShow));
    }
