...
bool list_devices(QListView *lBox, char *criteria)
{
    ...
    for(i = 0; i < info.num; i++)
    {
        ...
        lastitem =
            new QListViewItem(lBox, lastitem, QString(cudv[i].name), cudv_status, QString(cudv[i].location), QString(cudv_desc ? cudv_desc : "N/A"));
...
static const char chip_name[NUM_CHIPS][16] = {"P2SC",         "Power",         "Power2",  "Power3",   "Power3-II", "PowerPC",
...
struct model _models[] = {{"02", "7015-930", "25", Power},
...
bool GetInfo_CPU(QListView *lBox)
{
    ...
    lastitem = new QListViewItem(lBox, lastitem, QString("OS"),
                                 QString(info.sysname) + QString(" ") + QString(info.version) + QString(".") + QString(info.release));
