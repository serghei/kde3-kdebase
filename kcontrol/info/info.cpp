...
static QListViewItem *XServer_fill_screen_info(QListViewItem *lBox, QListViewItem *last, Display *dpy, int scr, int default_scr)
{
    ...
    item = new QListViewItem(lBox, last, i18n("Screen # %1").arg((int)scr, -1), (scr == default_scr) ? i18n("(Default Screen)") : QString::null);
    item->setExpandable(true);
    if(scr == default_scr)
        item->setOpen(true);
    last = new QListViewItem(item, i18n("Dimensions"), i18n("%1 x %2 Pixel (%3 x %4 mm)")
                                                           .arg((int)DisplayWidth(dpy, scr))
                                                           .arg((int)DisplayHeight(dpy, scr))
                                                           .arg((int)DisplayWidthMM(dpy, scr))
                                                           .arg((int)DisplayHeightMM(dpy, scr)));

    last = new QListViewItem(item, last, i18n("Resolution"), i18n("%1 x %2 dpi").arg((int)(xres + 0.5)).arg((int)(yres + 0.5)));
    ...
    last = new QListViewItem(item, last, i18n("Root Window ID"), HexStr((unsigned long)RootWindow(dpy, scr), HEXDIGITS));
    last = new QListViewItem(item, last, i18n("Depth of Root Window"), (DisplayPlanes(dpy, scr) == 1)
    ...
    last = new QListViewItem(item, last, i18n("Number of Colormaps"),
                             i18n("minimum %1, maximum %2").arg((int)MinCmapsOfScreen(s)).arg((int)MaxCmapsOfScreen(s)));
    last = new QListViewItem(item, last, i18n("Default Colormap"), Value((int)DefaultColormap(dpy, scr)));
    last = new QListViewItem(item, last, i18n("Default Number of Colormap Cells"), Value((int)DisplayCells(dpy, scr)));
    last = new QListViewItem(item, last, i18n("Preallocated Pixels"), i18n("Black %1, White %2")
    ...
    last = new QListViewItem(item, last, i18n("Options"),
                             i18n("backing-store: %1, save-unders: %2")
                                 .arg((DoesBackingStore(s) == NotUseful) ? NO : ((DoesBackingStore(s) == Always) ? YES : i18n("When mapped")))
    ...
    last = new QListViewItem(item, last, i18n("Largest Cursor"),
                             (width == 65535 && height == 65535) ? i18n("unlimited") : QString::fromLatin1("%1 x %2").arg(width).arg(height));

    last = new QListViewItem(item, last, i18n("Current Input Event Mask"), HexStr((unsigned long)EventMaskOfScreen(s), HEXDIGITS));
    item = last;
    struct _event_table *etp;
    for(etp = event_table; etp->name; etp++)
    {
        if(EventMaskOfScreen(s) & etp->value)
            item = new QListViewItem(last, item, i18n("Event = %1").arg(HexStr(etp->value, HEXDIGITS)), etp->name);
    }
...
static bool GetInfo_XServer_Generic(QListView *lBox)
{
    ...
    last = new QListViewItem(next, i18n("Name of the Display"), DisplayString(dpy));

    last = new QListViewItem(next, last, i18n("Vendor String"), QString::fromLatin1(ServerVendor(dpy)));
    last = new QListViewItem(next, last, i18n("Vendor Release Number"), Value((int)VendorRelease(dpy)));

    last = new QListViewItem(next, last, i18n("Version Number"),
                             QString::fromLatin1("%1.%2").arg((int)ProtocolVersion(dpy)).arg((int)ProtocolRevision(dpy)));
    ...
    if(pmf)
    {
        ...
        for(i = 0; i < n; i++)
        {
            item = new QListViewItem(last, item, i18n("Pixmap Format #%1").arg(i + 1), i18n("%1 BPP, Depth: %2, Scanline padding: %3")
    ...
    last = new QListViewItem(next, last, i18n("Maximum Request Size"), ByteString(req_size * 4));
    last = new QListViewItem(next, last, i18n("Motion Buffer Size"), ByteString(XDisplayMotionBufferSize(dpy)));

    last = item = new QListViewItem(next, last, i18n("Bitmap"));
    last->setExpandable(true);
    item = new QListViewItem(last, item, i18n("Unit"), Value(BitmapUnit(dpy)));
    item = new QListViewItem(last, item, i18n("Order"), Order(BitmapBitOrder(dpy)));
    item = new QListViewItem(last, item, i18n("Padding"), Value(BitmapPad(dpy)));

    last = new QListViewItem(next, last, i18n("Image Byte Order"), Order(ImageByteOrder(dpy)));
...
void KInfoListWidget::load()
{
    ...
    ErrorString = i18n("No information available about %1.").arg(title) + QString::fromLatin1("\n\n") + DEFAULT_ERRORSTRING;
...
KInfoListWidget::KInfoListWidget(const QString &_title, QWidget *parent, const char *name, bool _getlistbox(QListView *lbox))
    : KCModule(parent, name), title(_title)
{
