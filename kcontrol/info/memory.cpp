...
KMemoryWidget::KMemoryWidget(QWidget *parent, const char *name) : KCModule(parent, name)
{

    KAboutData *about = new KAboutData(I18N_NOOP("kcminfo"), I18N_NOOP("KDE Panel Memory Information Control Module"), 0, 0, KAboutData::License_GPL,
    ...
    ram_colors_initialized = swap_colors_initialized = all_colors_initialized = false;
    ...
    for(i = MEM_RAM_AND_HDD; i < MEM_LAST; i++)
    {
        ...
        switch(i)
        {
            case MEM_RAM_AND_HDD:
                ...
                hint = i18n(
                    "This graph gives you an overview of the "
            ...
            case MEM_RAM:
                ...
                hint = i18n(
                    "This graph gives you an overview of "
            ...
            case MEM_HDD:
                ...
                hint = i18n(
                    "The swap space is the <b>virtual memory</b> "
...
bool KMemoryWidget::Display_Graph(int widgetindex, int count, t_memsize total, t_memsize *used, QColor *color, QString *text)
{
    ...
    while(count--)
    {
        ...
        if(localheight > 0)
        {
            ...
            {
                paint.drawText(0, startline - localheight, width, localheight, AlignCenter | WordBreak, QString("%1 %2%").arg(*text).arg(percent));
            }
        }
...
void KMemoryWidget::update_Values()
{
    ...
    for(i = TOTAL_MEM; i < MEM_LAST_ENTRY; i++)
    {
        ...
        if(Memory_Info[i] == NO_MEMORY_INFO)
            ...
            label->setText(i18n("%1 bytes =").arg(KGlobal::locale()->formatNumber(Memory_Info[i], 0)));
    }
    ...
    for(i = TOTAL_MEM; i < MEM_LAST_ENTRY; i++)
    {
        ...
        label->setText((Memory_Info[i] != NO_MEMORY_INFO) ? formatted_unit(Memory_Info[i]) : Not_Available_Text);
    }
    ...
    ok1 = Display_Graph(MEM_RAM, 4, Memory_Info[TOTAL_MEM], used, ram_colors, ram_text);
    ...
    Display_Graph(MEM_HDD, 2, Memory_Info[SWAP_MEM], used, swap_colors, swap_text);

    /* RAM + SWAP usage: */
    if(Memory_Info[SWAP_MEM] == NO_MEMORY_INFO || Memory_Info[FREESWAP_MEM] == NO_MEMORY_INFO)
    ...
    Display_Graph(MEM_RAM_AND_HDD, 3, ok1 ? Memory_Info[TOTAL_MEM] + Memory_Info[SWAP_MEM] : NO_MEMORY_INFO, used, all_colors, all_text);
}
