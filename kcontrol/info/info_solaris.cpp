...
bool GetInfo_Partitions(QListView *lBox)
{
    ...
    while(getmntent(mnttab, &mnt) == 0)
    {
        ...
        new QListViewItem(lBox, mnt.mnt_special, mnt.mnt_mountp, mnt.mnt_fstype, total, avail, QString(timetxt), mnt.mnt_mntopts);
    }
    fclose(mnttab);
...
QListViewItem *mktree(QListViewItem *top, const char *path)
{
    ...
    QListViewItem *parent, *previous, *result;
    char *str = strdup(path), *token;
    ...
    while(token != NULL)
    {
        ...
        if(result == NULL)
        {
             ...
             */
            result = new QListViewItem(parent, previous, token);
        }
        else
        {
            /*
...
int dump_minor_node(di_node_t node, di_minor_t minor, void *arg)
{
    ...
    item = new QListViewItem((QListViewItem *)arg, di_minor_name(minor));
    item->setExpandable(true);
    item->setOpen(false);
    new QListViewItem(item, i18n("Spectype:"), (di_minor_spectype(minor) == S_IFCHR) ? i18n("character special") : i18n("block special"));
    type = di_minor_nodetype(minor);
    new QListViewItem(item, i18n("Nodetype:"), (type == NULL) ? "NULL" : type);
...
int dump_node(di_node_t node, void *arg)
{
    ...
    QListViewItem *top = (QListViewItem *)arg, *parent, *previous;
    char *path;
    char *drivername;
    char *names;
    QString compatnames;
    ...
    previous = new QListViewItem(parent, i18n("Driver Name:"), (drivername == NULL) ? i18n("(driver not attached)") : drivername);
    previous = new QListViewItem(parent, previous, i18n("Binding Name:"), di_binding_name(node));
    ...
    previous = new QListViewItem(parent, previous, i18n("Compatible Names:"), compatnames);

    previous = new QListViewItem(parent, previous, i18n("Physical Path:"), QString(path));
    ...
    if((prop = di_prop_next(node, DI_PROP_NIL)) != DI_PROP_NIL)
    {
        ...
        do
        {
            ...
            tmp->setOpen(false);
            prev = new QListViewItem(tmp, i18n("Type:"), prop_type_str(prop));
            new QListViewItem(tmp, prev, i18n("Value:"), propvalue(prop));
