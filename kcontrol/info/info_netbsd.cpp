...
bool GetInfo_CPU(QListView *lBox)
{
    static hw_info_mib_list_t hw_info_mib_list[] = {
        {1, HW_MODEL, "Model"},         {1, HW_MACHINE, "Machine"},   {1, HW_MACHINE_ARCH, "Architecture"},
        {0, HW_NCPU, "Number of CPUs"}, {0, HW_PAGESIZE, "Pagesize"}, {0, 0, 0}};
...
// on system
static bool GetDmesgInfo(QListView *lBox, const char *filter, void func(QListView *, QString s))
{
    ...
    while(!(s = t->readLine().local8Bit()).isEmpty())
    {
        ...
        if(!filter || (filter[0] == '^' && s.find(&filter[1]) == 0) || (filter[0] != '^' && s.contains(filter)))
        {
            if(func)
                func(lBox, s);
            else
...
void AddIRQLine(QListView *lBox, QString s)
{
    int pos, irqnum;
    char numstr[3];
...
bool GetInfo_Partitions(QListView *lbox)
{
    ...
    for(; num--; ++mnt)
    {
        ...
        vv[1] = QString::fromLatin1("%1 (%2%%)").arg(KIO::convertSize(big[1])).arg(mnt->f_blocks ? mnt->f_bavail * 100 / mnt->f_blocks : 0);

        // FIXME: these two are large enough to punctuate
        vv[2] = QString::number(mnt->f_files);
        vv[3] = QString::fromLatin1("%1 (%2%%) ").arg(mnt->f_ffree).arg(mnt->f_files ? mnt->f_ffree * 100 / mnt->f_files : 0);

        vv[4] = QString::null;
#define MNTF(x)                                                                                                                                      \
    if(mnt->f_flags & MNT_##x)                                                                                                                       \
        vv[4] += QString::fromLatin1(#x " ");
        MNTF(ASYNC)
        MNTF(DEFEXPORTED)
        MNTF(EXKERB)
        MNTF(EXNORESPORT)
...
#undef MNTF
        ...
        new QListViewItem(lbox,
                          ...
                          mnt->f_mntfromname, mnt->f_mntonname, mnt->f_fstypename, vv[0], vv[1], vv[2], vv[3], vv[4]);
    }
