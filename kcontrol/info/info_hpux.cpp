...
static bool Find_in_LOOKUPTABLE(QListView *lBox, char *machine)
{
    ...
    while(Entry->Name[0])
    {
        ...
        {
            olditem = new QListViewItem(lBox, olditem, i18n("PA-RISC Processor"), QString(PA_NAME[Entry->parisc_name]));
            olditem = new QListViewItem(lBox, olditem, i18n("PA-RISC Revision"), QString("PA-RISC ") + QString(PA_REVISION[Entry->parisc_rev]));
            return true;
        }
        else
            ++Entry; // next Entry !
...
bool GetInfo_CPU(QListView *lBox)
{
    ...
    olditem =
        new QListViewItem(lBox, olditem, i18n("Machine Identification Number"), strlen(info.__idnumber) ? QString(info.__idnumber) : i18n("(none)"));

    if(psd.psd_proc_cnt <= 0)
        psd.psd_proc_cnt = 1; // Minimum one CPU !

    olditem = new QListViewItem(lBox, olditem, i18n("Number of Active Processors"), Value(psd.psd_proc_cnt));

    pstat_getprocessor(&pro, sizeof(pro), 1, 0);
    olditem = new QListViewItem(lBox, olditem, i18n("CPU Clock"), Value(pro.psp_iticksperclktick / 10000) + " " + i18n("MHz"));
    ...
    switch(sysconf(_SC_CPU_VERSION))
    {
        ...
        case CPU_PA_RISC2_0:
            ...
            {
                case 64:
                    str2 = "PA-RISC 2.0w (64 bit)";
                    break;
                case 32:
                    str2 = "PA-RISC 2.0n (32 bit)";
                    break;
                default:
                    str2 = "PA-RISC 2.0";
                    break;
    ...
    for(i = PS_PA83_FPU; i <= PS_PA89_FPU; ++i)
    {
        ...
        {
            str = QString((i == PS_PA83_FPU) ? "PS_PA83_FPU" : "PS_PA89_FPU") + QString(" (")
                  + QString(((1 << (i - 1)) & pro.psp_coprocessor.psc_enabled) ? i18n("enabled") : i18n("disabled")) + QString(")");
...
{
    return (GetInfo_ReadfromFile(lBox, INFO_PCI) + GetInfo_ReadfromFile(lBox, INFO_PCI_EISA));
}
...
#define NUMCOLS 5
    ...
    while((fstab_ent = getfsent()) != NULL)
    {
        ...
        {
            new QListViewItem(lbox, QString(fstab_ent->fs_spec), QString(fstab_ent->fs_file) + QString("  "),
                              (svfs.f_basetype[0] ? QString(svfs.f_basetype) : i18n("n/a")), Value((total + 512) / 1024, 6) + MB,
...
static const char formatNames[6][15] = {"ADFUnknown", "ADFMuLaw", "ADFALaw", "ADFLin16", "ADFLin8", "ADFLin8Offset"};
...
bool GetInfo_Sound(QListView *lBox)
{
    ...
    olditem =
        new QListViewItem(lBox, olditem, i18n("Alib Version"), Value(audio->alib_major_version) + QString(".") + Value(audio->alib_minor_version));

    olditem = new QListViewItem(lBox, olditem, i18n("Protocol Revision"),
                                Value(audio->proto_major_version) + QString(".") + Value(audio->proto_minor_version));

    olditem = new QListViewItem(lBox, olditem, i18n("Vendor Number"), Value(audio->vnumber));

    olditem = new QListViewItem(lBox, olditem, i18n("Release"), Value(audio->release));

    olditem = new QListViewItem(lBox, olditem, i18n("Byte Order"),
                                QString((audio->byte_order == ALSBFirst)
                                            ? i18n("ALSBFirst (LSB)")
                                            : ((audio->byte_order == AMSBFirst) ? i18n("AMSBFirst (MSB)") : i18n("Invalid Byteorder."))));

    olditem =
        new QListViewItem(lBox, olditem, i18n("Bit Order"),
                          QString((audio->sound_bit_order == ALeastSignificant)
                                      ? i18n("ALeastSignificant (LSB)")
                                      : ((audio->sound_bit_order == AMostSignificant) ? i18n("AMostSignificant (MSB)") : i18n("Invalid Bitorder."))));
    ...
    new QListViewItem(olditem, i18n("Input Gain Limits"), Value(audio->max_input_gain));
    new QListViewItem(olditem, i18n("Output Gain Limits"), Value(audio->min_output_gain) + QString(" ") + Value(audio->max_output_gain));
    new QListViewItem(olditem, i18n("Monitor Gain Limits"), Value(audio->min_monitor_gain) + QString(" ") + Value(audio->max_monitor_gain));
    new QListViewItem(olditem, i18n("Gain Restricted"), Value(audio->gm_gain_restricted));
    olditem->setOpen(true);


    olditem = new QListViewItem(lBox, olditem, i18n("Lock"), Value(audio->lock));
    olditem = new QListViewItem(lBox, olditem, i18n("Queue Length"), Value(audio->qlen));
    olditem = new QListViewItem(lBox, olditem, i18n("Block Size"), Value(audio->block_size));
    olditem = new QListViewItem(lBox, olditem, i18n("Stream Port (decimal)"), Value(audio->stream_port));
    olditem = new QListViewItem(lBox, olditem, i18n("Ev Buffer Size"), Value(audio->ev_buf_size));
    olditem = new QListViewItem(lBox, olditem, i18n("Ext Number"), Value(audio->ext_number));
