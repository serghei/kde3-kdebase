...
static struct
{
    QString module, pci, vendor, device, subvendor, rev;
} dri_info;
        ...
        if(rx.search(line) > 0)
        {
               ...
               && num >= 7)
            {
                for(int i = 2; i <= 6; i++)
                {
                    ...
                    {
                        case 2:
                            dri_info.vendor = line;
                            break;
                        case 3:
                            dri_info.device = line;
                            break;
                        case 4:
                            dri_info.subvendor = line;
                            break;
                        case 6:
                            dri_info.rev = line;
                            break;
...
{
    static int attribs[] = {GLX_RGBA, GLX_RED_SIZE,         1, GLX_GREEN_SIZE,     1,   GLX_BLUE_SIZE,        1, GLX_DEPTH_SIZE,
                            1,        GLX_STENCIL_SIZE,     1, GLX_ACCUM_RED_SIZE, 1,   GLX_ACCUM_GREEN_SIZE, 1, GLX_ACCUM_BLUE_SIZE,
                            1,        GLX_ACCUM_ALPHA_SIZE, 1, GLX_DOUBLEBUFFER,   None};
    XVisualInfo *visinfo;
...
static void print_extension_list(const char *ext, QListViewItem *l1)
{
    int i, j;
    ...
    while(1)
    {
        if(ext[j] == ' ' || ext[j] == 0)
        {
            ...
            if(!l2)
                l2 = new QListViewItem(l1, qext.mid(i, len));
            else
                l2 = new QListViewItem(l1, l2, qext.mid(i, len));
            i = j;
            if(ext[j] == 0)
            {
                break;
...
static void print_limits(QListViewItem *l1, const char *glExtensions, bool GetProcAddress)
{
    /*  TODO
         GL_SAMPLE_BUFFERS
         GL_SAMPLES
...
#define KCMGL_SIZE(x) (sizeof(x) / sizeof(x[0]))
    ...
    const struct token_name various_limits[] = {{1, GL_MAX_LIGHTS, i18n("Max. number of light sources")},
    ...
    const struct token_name texture_limits[] = {{1, GL_MAX_TEXTURE_SIZE, i18n("Max. texture size")},
    ...
    const struct token_name float_limits[] = {{2 | KCMGL_FLOAT, GL_ALIASED_POINT_SIZE_RANGE, "ALIASED_POINT_SIZE_RANGE"},
    ...
    const struct token_name stack_depth[] = {{1, GL_MAX_MODELVIEW_STACK_DEPTH, "MAX_MODELVIEW_STACK_DEPTH"},
    ...
    const struct token_name arb_fp[] = {{1, GL_MAX_TEXTURE_COORDS_ARB, "MAX_TEXTURE_COORDS"},
    ...
    const struct token_name arb_vp[] = {{1 | KCMGL_PROG, GL_MAX_PROGRAM_ENV_PARAMETERS_ARB, "MAX_PROGRAM_ENV_PARAMETERS"},
                                        {1 | KCMGL_PROG, GL_MAX_PROGRAM_LOCAL_PARAMETERS_ARB, "MAX_PROGRAM_LOCAL_PARAMETERS"},
    ...
    const struct token_name arb_vs[] = {{1, GL_MAX_VERTEX_ATTRIBS_ARB, "MAX_VERTEX_ATTRIBS"},
    ...
    const struct token_name arb_fs[] = {{1, GL_MAX_FRAGMENT_UNIFORM_COMPONENTS_ARB, "MAX_FRAGMENT_UNIFORM_COMPONENTS"},
                                        {1, GL_MAX_TEXTURE_IMAGE_UNITS_ARB, "MAX_TEXTURE_IMAGE_UNITS"},
                                        {1, GL_MAX_TEXTURE_COORDS_ARB, "MAX_TEXTURE_COORDS"},
                                        {0, 0, 0}};
#endif

    const struct token_name frame_buffer_props[] = {{2, GL_MAX_VIEWPORT_DIMS, i18n("Max. viewport dimensions")},
    ...
    for(uint i = 0; i < KCMGL_SIZE(groups); i++)
    {
        ...
        if(l2)
            l2 = new QListViewItem(l1, l2, groups[i].descr);
        else
            l2 = new QListViewItem(l1, groups[i].descr);
        l3 = NULL;
        ...
        for(cur_token = groups[i].group; cur_token->type; cur_token++)
        {
            ...
            if(glGetError() == GL_NONE)
            {
                ...
                if(l3)
                    l3 = new QListViewItem(l2, l3, cur_token->name, s);
                else
                    l3 = new QListViewItem(l2, cur_token->name, s);
            }
        }
    }
}
...
static QListViewItem *print_screen_info(QListViewItem *l1, QListViewItem *after)
{
    ...
    if(after)
        l1 = new QListViewItem(l1, after, IsDirect ? i18n("Direct Rendering") : i18n("Indirect Rendering"));
    else
        l1 = new QListViewItem(l1, IsDirect ? i18n("Direct Rendering") : i18n("Indirect Rendering"));
    if(IsDirect)
        if(get_dri_device())
        {
            ...
            l2 = new QListViewItem(l1, l2, i18n("3D Accelerator"), i18n("unknown"));
    if(l2)
        l2 = new QListViewItem(l1, l2, i18n("Driver"));
    else
        l2 = new QListViewItem(l1, i18n("Driver"));
    l2->setOpen(true);
