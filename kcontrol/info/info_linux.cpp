...
bool GetInfo_ReadfromFile(QListView *lbox, const char *FileName, const QChar &splitChar, QListViewItem *lastitem = 0, QListViewItem **newlastitem = 0)
{
    bool added = false;
    QFile file(FileName);
...
bool GetInfo_PCI(QListView *lBox)
{
    ...
    if((num = GetInfo_ReadfromPipe(lBox, "lspci -v", true)) || (num = GetInfo_ReadfromPipe(lBox, "/sbin/lspci -v", true))
       || (num = GetInfo_ReadfromPipe(lBox, "/usr/sbin/lspci -v", true)) || (num = GetInfo_ReadfromPipe(lBox, "/usr/local/sbin/lspci -v", true)))
        return num;
    ...
    for(i = 1; i < 256; i++)
    {
        ...
        switch((int)rq.block_major)
        {
            ...
            case 3:
                first_letter = 'a';
            set_ide_name:
                devname = QString("/dev/hd%1%2").arg(QChar(first_letter + minor / 64)).arg(minor & 63);
                break;
            case 22:
                first_letter = 'c';
                goto set_ide_name;
            ...
            case 8:
                first_letter = 'a';
            set_scsi_name:
                devname = QString("/dev/sd%1%2").arg(QChar(first_letter + minor / 16)).arg(minor & 15);
                break;
            case 65:
                first_letter = 'q';
                goto set_scsi_name;
            ...
            case 108:
            case 109:
                devname = QString("/dev/cciss/c%1d%2").arg((int)rq.block_major - 104).arg(minor & 15);
                break;
            ...
            case 78:
            case 79:
                devname = QString("/dev/ida/c%1d%2").arg((int)rq.block_major - 72).arg(minor & 15);
                break;

            default:
                devname = QString("%1/%2").arg((int)rq.block_major).arg(minor);
        }

        /* TODO: get device size */
        QString size = "";

        new QListViewItem(lbox, devname, QString(new_raw_devs ? "/dev/raw/raw%1" : "/dev/raw%1").arg(i), "raw", size, " ", "");
    }
    close(f);
}
#else
...
/* loop through all partitions... */
...
#endif
    {
        ...
        if(found_in_List && statfs(FS_FILE, &sfs) == 0)
        {
            ...
            avail = (getuid() ? sfs.f_bavail : sfs.f_bfree) * ((LONG_TYPE)sfs.f_bsize);
        };
        /*
        ...
        if(total)
            new QListViewItem(lbox, QString(FS_NAME) + "  ", QString(FS_FILE) + "  ", QString(FS_TYPE) + "  ",
                              Value((int)(((total / 1024) + 512) / 1024), 6) + MB, Value((int)(((avail / 1024) + 512) / 1024), 6) + MB, mountopts);
        else
            new QListViewItem(lbox, QString(FS_NAME), QString(FS_FILE), QString(FS_TYPE), " ", " ", mountopts);
    }
