...
DesktopBehaviorModule::DesktopBehaviorModule(KConfig *config, QWidget *parent, const char *) : KCModule(parent, "kcmkonq")
{
...
public:
    DesktopBehaviorPreviewItem(DesktopBehavior *rootOpts, QListView *parent, const KService::Ptr &plugin, bool on)
        : QCheckListItem(parent, plugin->name(), CheckBox), m_rootOpts(rootOpts)
    {
        m_pluginName = plugin->desktopEntryName();
        setOn(on);
    }
    DesktopBehaviorPreviewItem(DesktopBehavior *rootOpts, QListView *parent, bool on)
        : QCheckListItem(parent, i18n("Sound Files"), CheckBox), m_rootOpts(rootOpts)
    {
        m_pluginName = "audio/";
        setOn(on);
    }
...
public:
    DesktopBehaviorMediaItem(DesktopBehavior *rootOpts, QListView *parent, const QString name, const QString mimetype, bool on)
        : QCheckListItem(parent, name, CheckBox), m_rootOpts(rootOpts), m_mimeType(mimetype)
    {
        setOn(on);
    }
...
DesktopBehavior::DesktopBehavior(KConfig *config, QWidget *parent, const char *) : DesktopBehaviorBase(parent, "kcmkonq"), g_pConfig(config)
{
    ...
    strButtonTxt1 = i18n(
        "You can choose what happens when"
    ...
    QString wtstr = strButtonTxt1 + i18n(
...
void DesktopBehavior::fillMediaListView()
{
    ...
    QString excludedMedia =
        g_pConfig->readEntry("exclude", "media/hdd_mounted,media/hdd_unmounted,media/floppy_unmounted,media/cdrom_unmounted,media/floppy5_unmounted");
...
void DesktopBehavior::saveMediaListView()
{
    ...
    QStringList exclude;
    for(DesktopBehaviorMediaItem *it = static_cast< DesktopBehaviorMediaItem * >(mediaListView->firstChild()); it;
        it = static_cast< DesktopBehaviorMediaItem * >(it->nextSibling()))
    {
        if(!it->isOn())
            exclude << it->mimeType();
    }
...
void DesktopBehavior::load(bool useDefaults)
{
    ...
    for(int c = 0; c < choiceCount; c++)
        if(s == s_choices[c])
        {
            middleComboBox->setCurrentItem(c);
            break;
        }
    ...
    for(int c = 0; c < choiceCount; c++)
        if(s == s_choices[c])
        {
            rightComboBox->setCurrentItem(c);
            break;
        }
...
void DesktopBehavior::save()
{
    ...
    for(DesktopBehaviorPreviewItem *item = static_cast< DesktopBehaviorPreviewItem * >(previewListView->firstChild()); item;
