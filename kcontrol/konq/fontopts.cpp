...
#define LASTCOLUMN 2
    QGridLayout *lay = new QGridLayout(this, LASTLINE + 1, LASTCOLUMN + 1, 0, KDialog::spacingHint());
    ...
    row++;
    connect(m_pStandard, SIGNAL(activated(const QString &)), SLOT(slotStandardFont(const QString &)));
    connect(m_pStandard, SIGNAL(activated(const QString &)), SLOT(changed()));
    connect(m_pStandard, SIGNAL(textChanged(const QString &)), SLOT(slotStandardFont(const QString &)));
    connect(m_pStandard, SIGNAL(textChanged(const QString &)), SLOT(changed()));
    ...
    connect(m_pSize, SIGNAL(valueChanged(int)), this, SLOT(slotFontSize(int)));
    row += 2;
...
#define COLOR_BUTTON_COL 1
    ...
    connect(m_pNormalText, SIGNAL(changed(const QColor &)), SLOT(slotNormalTextColorChanged(const QColor &)));
    ...
    */
    ...
    if(m_bDesktop)
    {
        ...
        connect(m_cbTextBackground, SIGNAL(clicked()), SLOT(slotTextBackgroundClicked()));
        ...
        connect(m_pTextBackground, SIGNAL(changed(const QColor &)), SLOT(slotTextBackgroundColorChanged(const QColor &)));
    ...
    else
    {
        ...
        connect(m_pNbLines, SIGNAL(valueChanged(int)), this, SLOT(changed()));
        connect(m_pNbLines, SIGNAL(valueChanged(int)), SLOT(slotPNbLinesChanged(int)));

        QString thwt = i18n(
            "This is the maximum number of lines that can be"
        ...
        connect(m_pNbWidth, SIGNAL(valueChanged(int)), this, SLOT(changed()));
        connect(m_pNbWidth, SIGNAL(valueChanged(int)), SLOT(slotPNbWidthChanged(int)));

        thwt = i18n(
            "This is the maximum width for the icon text when konqueror "
