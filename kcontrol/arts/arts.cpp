...
static bool startArts()
{
    ...
    if(startServer)
        kapp->kdeinitExec(startRealtime ? "artswrapper" : "artsd", QStringList::split(" ", args));
    return startServer;
}
...
void KArtsModule::initAudioIOList()
{
    ...
    *artsd << "-A";
    ...
    connect(artsd, SIGNAL(processExited(KProcess *)), this, SLOT(slotArtsdExited(KProcess *)));
    connect(artsd, SIGNAL(receivedStderr(KProcess *, char *, int)), this, SLOT(slotProcessArtsdOutput(KProcess *, char *, int)));
...
KArtsModule::KArtsModule(QWidget *parent, const char *name) : KCModule(parent, name), configChanged(false)
{
    setButtons(Default | Apply);

    setQuickHelp(
        i18n("<h1>Sound System</h1> Here you can configure aRts, KDE's sound server."
    ...
    QString deviceHint = i18n(
        "Normally, the sound server defaults to using the device called <b>/dev/dsp</b> for sound output. That should work in most cases. On some "
        "systems where devfs is used, however, you may need to use <b>/dev/sound/dsp</b> instead. Other alternatives are things like "
        "<b>/dev/dsp0</b> or <b>/dev/dsp1</b>, if you have a soundcard that supports multiple outputs, or you have multiple soundcards.");

    QString rateHint = i18n(
        "Normally, the sound server defaults to using a sampling rate of 44100 Hz (CD quality), which is supported on almost any hardware. If you "
        "are using certain <b>Yamaha soundcards</b>, you might need to configure this to 48000 Hz here, if you are using <b>old SoundBlaster "
        "cards</b>, like SoundBlaster Pro, you might need to change this to 22050 Hz. All other values are possible, too, and may make sense in "
        "certain contexts (i.e. professional studio equipment).");

    QString optionsHint = i18n(
        "This configuration module is intended to cover almost every aspect of the aRts sound server that you can configure. However, there are some "
        "things which may not be available here, so you can add <b>command line options</b> here which will be passed directly to <b>artsd</b>. The "
        "command line options will override the choices made in the GUI. To see the possible choices, open a Konsole window, and type <b>artsd "
        "-h</b>.");
    ...
    connect(hardware->midiMapper, SIGNAL(textChanged(const QString &)), this, SLOT(slotChanged()));

    KAboutData *about = new KAboutData(I18N_NOOP("kcmarts"), I18N_NOOP("The Sound Server Control Module"), 0, 0, KAboutData::License_GPL,
...
void KArtsModule::load(bool useDefaults)
{
    ...
    startRealtime->setChecked(config->readBoolEntry("StartRealtime", true) && realtimeIsPossible());
...
void KArtsModule::saveParams(void)
{
    ...
    config->writeEntry("Arguments", createArgs(networkTransparent->isChecked(), fullDuplex->isChecked(), fragmentCount, fragmentSize, dev, rate, bits,
...
int KArtsModule::userSavedChanges()
{
    ...
    QString question = i18n(
        "The settings have changed since the last time "
...
void KArtsModule::calculateLatency()
{
    ...
    if(general->latencySlider->value() < 490)
    {
        ...
        general->latencyLabel->setText(i18n("%1 milliseconds (%2 fragments with %3 bytes)").arg(latencyInMs).arg(fragmentCount).arg(fragmentSize));
    }
    else
    {
        fragmentCount = 128;
...
bool KArtsModule::realtimeIsPossible()
{
    ...
    if(!checked)
    {
        ...
        *checkProcess << "check";
        ...
        connect(checkProcess, SIGNAL(processExited(KProcess *)), this, SLOT(slotArtsdExited(KProcess *)));
...
void KArtsModule::restartServer()
{
    ...
    if(starting)
    {
        ...
        KStartArtsProgressDialog dlg(this, "start_arts_progress", restarting ? i18n("Restarting Sound System") : i18n("Starting Sound System"),
...
QString KArtsModule::createArgs(bool netTrans, bool duplex, int fragmentCount, int fragmentSize, const QString &deviceName, int rate, int bits,
                                const QString &audioIO, const QString &addOptions, bool autoSuspend, int suspendTime)
{
    QString args;
...
KStartArtsProgressDialog::KStartArtsProgressDialog(KArtsModule *parent, const char *name, const QString &caption, const QString &text)
...
void KStartArtsProgressDialog::slotProgress()
{
...
void KStartArtsProgressDialog::slotFinished()
{
