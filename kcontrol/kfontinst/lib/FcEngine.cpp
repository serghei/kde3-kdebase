...
#ifdef HAVE_XFT
static bool drawChar(QPixmap &pix, XftDraw *xftDraw, XftFont *xftFont, XftColor *xftCol, const QString &text, int pos, int &x, int &y, int w, int h,
                     int fSize, int offset)
{
    XGlyphInfo extents;
...
static bool drawString(QPixmap &pix, XftDraw *xftDraw, XftFont *xftFont, XftColor *xftCol, const QString &text, int x, int &y, int h, int offset)
{
    XGlyphInfo extents;
...
static bool drawGlyph(QPixmap &pix, XftDraw *xftDraw, XftFont *xftFont, XftColor *xftCol, FT_UInt i, int &x, int &y, int &w, int &h, int fSize,
                      int offset)
{
    XGlyphInfo extents;
...
CFcEngine::CFcEngine() : itsIndex(-1), itsIndexCount(1)
{
}
...
bool CFcEngine::draw(const KURL &url, int w, int h, QPixmap &pix, int faceNo, bool thumb)
{
    ...
    if((url == itsLastUrl && faceNo == itsIndex) || parseUrl(url, faceNo))
    {
        ...
        int offset = thumb ? h <= 32 ? 2 : 3 : 4, x = offset, y = offset;
        ...
        if(itsSizes.size())
        {
            ...
            XftColorAllocValue(pix.x11Display(), DefaultVisual(pix.x11Display(), pix.x11Screen()), DefaultColormap(pix.x11Display(), pix.x11Screen()),
                               &xrenderCol, &xftCol);

            XftDraw *xftDraw = XftDrawCreate(pix.x11Display(), (Pixmap)(pix.handle()), (Visual *)(pix.x11Visual()), pix.x11Colormap());
            ...
            if(xftDraw)
            {
                ...
                if(thumb)
                {
                    ...
                    int fSize = h <= 32 ? h - (offset * 2)                  // 1 line of chars...
                                        : h <= 64 ? (h - (offset * 3)) / 2  // 2 lines...
                ...
                else
                {
                    ...
                    bool lc = true, uc = true, punc = true;
                                        ...
                                        for(int i = 1; i < face->num_glyphs && y < w && !stop; ++i)
                                        {
                                            ...
                                            else
                                            {
                                                if(x + extents.width < w)
                                                    XftDrawGlyphs(xftDraw, &xftCol, xftFont, x, y + extents.y, (const FT_UInt *)&i, 1);
                                                if(extents.width > 0)
...
QString CFcEngine::getPreviewString()
{
    ...
    return str.isEmpty() ? i18n("A sentence that uses all of the letters of the alphabet", "The quick brown fox jumps over the lazy dog") : str;
}
...
QString CFcEngine::createName(FcPattern *pat, int faceNo)
{
    ...
    QString name(getFcString(pat, FC_FAMILY, faceNo)), str;
    int intVal;
    bool comma = false;
...
bool CFcEngine::parseUrl(const KURL &url, int faceNo, bool all)
{
    ...
    if(KFI_KIO_FONTS_PROTOCOL == url.protocol())
    {
        ...
        {
            KIO::UDSEntry::Iterator it(udsEntry.begin()), end(udsEntry.end());
...
void CFcEngine::parseName(const QString &name, int faceNo, bool all)
{
    ...
    if(all)
    {
        ...
        FcPattern *pat = FcPatternBuild(NULL, FC_FAMILY, FcTypeString, (const FcChar8 *)(itsName.utf8().data()), FC_WEIGHT, FcTypeInteger, itsWeight,
...
{
    if(itsInstalled)
        return XftFontOpen(KFI_DISPLAY(pix), 0, FC_FAMILY, FcTypeString, (const FcChar8 *)(itsName.utf8().data()), FC_WEIGHT, FcTypeInteger,
                           itsWeight, FC_SLANT, FcTypeInteger, itsSlant,
#ifndef KFI_FC_NO_WIDTHS
                           FC_WIDTH, FcTypeInteger, itsWidth,
#endif
                           FC_PIXEL_SIZE, FcTypeDouble, (double)size, NULL);
    else
    {
        FcPattern *pattern = FcPatternBuild(NULL, FC_FILE, FcTypeString, QFile::encodeName(itsName).data(), FC_INDEX, FcTypeInteger, itsIndex,
                                            FC_PIXEL_SIZE, FcTypeDouble, (double)size, NULL);
...
#endif
                           FcResultMatch == FcPatternGetString(f->pattern, FC_FAMILY, 0, &str) && str && QString::fromUtf8((char *)str) == itsName)
                        {
