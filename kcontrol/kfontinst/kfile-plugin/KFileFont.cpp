...
{
    QFile f(file);
    bool foundName = false, foundFamily = false;
    int intSpacing = FC_PROPORTIONAL,
#ifndef KFI_FC_NO_WIDTHS
        intWidth = FC_WIDTH_NORMAL,
#endif
        intWeight = FC_WEIGHT_NORMAL, intSlant = FC_SLANT_ROMAN, intItalic = FC_SLANT_ROMAN;
    ...
    if(f.open(IO_ReadOnly))
    {
        ...
        while(!stream.atEnd())
        {
            ...
            else if(0 == line.find("StartFontMetrics"))
                inMetrics = true;
        };
        f.close();
...
KFileFontPlugin::KFileFontPlugin(QObject *parent, const char *name, const QStringList &args) : KFilePlugin(parent, name, args)
{
    KGlobal::locale()->insertCatalogue(KFI_CATALOGUE);

    addMimeType("application/x-font-ttf"), addMimeType("application/x-font-type1");
...
{
    QString full, lastFull, family, foundry, weight,
#ifndef KFI_FC_NO_WIDTHS
        width,
#endif
        spacing, slant, fullAll, familyAll, foundryAll, weightAll,
#ifndef KFI_FC_NO_WIDTHS
        widthAll,
#endif
        spacingAll, slantAll;
    KURL url(info.url());
    QString fName;
    bool fontsProt = KFI_KIO_FONTS_PROTOCOL == url.protocol(), fileProt = "file" == url.protocol(), downloaded = false, status = false;
