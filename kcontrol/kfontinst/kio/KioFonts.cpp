...
// Version       : $Revision: 820166 $ $Date: 2008-06-13 14:14:08 +0200 (Fr, 13. Jun 2008) $
//
////////////////////////////////////////////////////////////////////////////////
//
...
static int getFontSize(const QString &file)
{
    ...
    if(urls.count())
    {
        KURL::List::Iterator uIt, uEnd = urls.end();

        for(uIt = urls.begin(); uIt != uEnd; ++uIt)
            files.append((*uIt).path());
    }

    QStringList::Iterator it(files.begin()), end(files.end());
...
{
    QValueList< FcPattern * >::Iterator it, end = patterns.end();
    int size = 0;
...
static bool createFolderUDSEntry(KIO::UDSEntry &entry, const QString &name, const QString &path, bool sys)
{
    ...
    notype:
        ...
        addAtom(entry, KIO::UDS_MIME_TYPE, 0, sys ? KFI_KIO_FONTS_PROTOCOL "/system-folder" : KFI_KIO_FONTS_PROTOCOL "/folder");
...
static KURL getRedirect(const KURL &u)
{
    ...
    KURL redirect(u);
    QString path(u.path()), sect(CKioFonts::getSect(path));
...
static QString getFontFolder(const QString &defaultDir, const QString &root, QStringList &dirs)
{
    ...
    {
        QStringList::Iterator it, end = dirs.end();
        bool found = false;
    ...
    if(checkExt(QFile::encodeName(fname), "pfm"))
    {
        ...
        if(f)
        {
            ...
            if(2 == fread(&version, 1, 2, f) &&                                   // Read version
               ...
               && 0 == fseek(f, constExtToFname, SEEK_CUR) && 4 == fread(&fontname, 1, 4, f) && fontname > constFontnameMin
               && fontname < constFontnameMax)
                ok = true;
            fclose(f);
        }
    }
...
struct FontList
{
    ...
    FontList(const QString &n = QString::null, const QString &p = QString::null) : name(n)
    {
        if(!p.isEmpty())
            paths.append(Path(p));
    }
    ...
    bool operator==(const FontList &f) const
    {
        return f.name == name;
    }
};
...
static bool getFontList(const QStringList &files, QMap< QString, QString > &map)
{
    ...
    for(; it != end; ++it)
    {
        QString name(Misc::getFile(*it)), path(Misc::getDir(*it));
    ...
    QValueList< FontList >::Iterator fIt(list.begin()), fEnd(list.end());

    for(; fIt != fEnd; ++fIt)
    {
        QValueList< FontList::Path >::Iterator pBegin((*fIt).paths.begin()), pIt(++pBegin), pEnd((*fIt).paths.end());
        --pBegin;
    ...
    , itsFontList(NULL)
{
    ...
    if(!itsRoot)
    {
        QString home(Misc::dirSyntax(QDir::homeDirPath())), defaultDir(Misc::dirSyntax(QDir::homeDirPath() + "/.fonts/")),
    ...
    QString sysDefault("/usr/local/share/fonts/"), sysDir(getFontFolder(sysDefault, "/usr/local/share/", dirs));
...
void CKioFonts::listDir(const KURL &url)
{
    ...
    if(updateFontList() && checkUrl(url, true))
    {
        ...
        if(itsRoot || QStringList::split('/', url.path(), false).count() != 0)
        {
            ...
            {
                QMap< QString, QValueList< FcPattern * > >::Iterator it = itsFolders[folder].fontMap.begin(), end = itsFolders[folder].fontMap.end();
    ...
    if(updateFontList() && checkUrl(url, true))
    {
        ...
        switch(pathList.count())
        {
            ...
            case 1:
                ...
                else if(isUserFolder(pathList[0]))
    ...
    if(updateFontList() && checkUrl(url) && getSourceFiles(url, srcFiles)) // Any error will be logged in getSourceFiles
    {
        ...
        else // Font is made up of multiple files - so create .tar.gz of them all!
        {
            ...
            if(tar.open(IO_WriteOnly))
            {
                ...
                QMap< QString, QString >::Iterator fIt(map.begin()), fEnd(map.end());
...
void CKioFonts::put(const KURL &u, int mode, bool overwrite, bool resume)
{
    ...
    KURL url(u);
    bool changed = confirmUrl(url), nrs = nonRootSys(url);
    EFolder destFolder(getFolder(url));
    QString dest = itsFolders[destFolder].location + modifyName(url.fileName()), passwd;
...
bool CKioFonts::putReal(const QString &destOrig, const QCString &destOrigC, bool origExists, int mode, bool resume)
{
...
void CKioFonts::copy(const KURL &src, const KURL &d, int mode, bool overwrite)
{
       ...
       && checkUrl(src) && checkAllowed(src))
    {
        ...
        if(getSourceFiles(src, srcFiles)) // Any error will be logged in getSourceFiles
        {
            ...
            if(fromFonts
                   ? confirmMultiple(src, srcFiles, FOLDER_SYS == destFolder ? FOLDER_USER : FOLDER_SYS, OP_COPY) && getFontList(srcFiles, map)
...
void CKioFonts::rename(const KURL &src, const KURL &d, bool overwrite)
{
    ...
    else
    {
        ...
        if(getSourceFiles(src, srcFiles)) // Any error will be logged in getSourceFiles
        {
            ...
            if(confirmMultiple(src, srcFiles, FOLDER_SYS == destFolder ? FOLDER_USER : FOLDER_SYS, OP_MOVE) && getFontList(srcFiles, map)
               && checkDestFiles(src, map, dest, destFolder, overwrite))
            {
                QMap< QString, QString >::Iterator fIt(map.begin()), fEnd(map.end());
                bool askPasswd = true, toSys = FOLDER_SYS == destFolder;
                QCString userId, groupId, destDir(QFile::encodeName(KProcess::quote(itsFolders[destFolder].location)));
                ...
                for(; fIt != fEnd; ++fIt)
                {
                    QCString cmd, destFile(QFile::encodeName(KProcess::quote(itsFolders[destFolder].location + fIt.data())));
...
void CKioFonts::del(const KURL &url, bool)
{
    ...
    if(checkUrl(url) && checkAllowed(url) && updateFontList() && (entries = getEntries(url)) && entries->count()
       && confirmMultiple(url, entries, getFolder(url), OP_DELETE))
    {
        QValueList< FcPattern * >::Iterator it, end = entries->end();
        ...
        else
        {
            for(it = entries->begin(); it != end; ++it)
            {
                ...
                if(0 != unlink(QFile::encodeName(file).data()))
                    error(EACCES == errno || EPERM == errno ? KIO::ERR_ACCESS_DENIED : EISDIR == errno ? KIO::ERR_IS_DIRECTORY
...
void CKioFonts::modified(EFolder folder, bool clearList, const CDirList &dirs)
{
    ...
    if(FOLDER_SYS != folder || itsCanStorePasswd || itsRoot)
    {
        if(dirs.count())
        {
            CDirList::ConstIterator it(dirs.begin()), end(dirs.end());
...
void CKioFonts::createRootRefreshCmd(QCString &cmd, const CDirList &dirs, bool reparseCfg)
{
    ...
    if(dirs.count())
    {
        ...
        for(; it != end; ++it)
        {
            ...
            else if(0 != itsNrsNonMainKfiParams[0])
...
void CKioFonts::doModified()
{
    ...
    if(itsFolders[FOLDER_SYS].modified.count())
    {
        if(itsRoot)
        {
            ...
            if(0 != itsKfiParams[0])
            {
                CDirList::ConstIterator it(itsFolders[FOLDER_SYS].modified.begin()), end(itsFolders[FOLDER_SYS].modified.end());
    ...
    if(!itsRoot && itsFolders[FOLDER_USER].modified.count())
    {
        ...
        if(0 != itsKfiParams[0])
        {
            CDirList::ConstIterator it(itsFolders[FOLDER_USER].modified.begin()), end(itsFolders[FOLDER_USER].modified.end());
    ...
    if(!itsRoot)
    {
        ...
        if(!isSysFolder(sect) && !isUserFolder(sect))
        {
            ...
            if(DEST_UNCHANGED != itsLastDest && itsLastDestTime && (abs(time(NULL) - itsLastDestTime) < constMaxLastDestTime))
                ...
                changeToSystem = KMessageBox::No == messageBox(QuestionYesNo, i18n("Do you wish to install the font into \"%1\" (in which "
                                                               ...
                                                               i18n("Where to Install"), i18n(KFI_KIO_FONTS_USER), i18n(KFI_KIO_FONTS_SYS));
        ...
        if(itsFontList)
        {
            ...
            for(int i = 0; i < itsFontList->nfont; i++)
            {
                ...
                if(!file.isEmpty())
                {
                    ...
                    QValueList< FcPattern * > &patterns = itsFolders[folder].fontMap[CFcEngine::createName(itsFontList->fonts[i])];
                    bool use = true;
...
{
    QMap< QString, QValueList< FcPattern * > >::Iterator it, end = itsFolders[folder].fontMap.end();

    for(it = itsFolders[folder].fontMap.begin(); it != end; ++it)
    {
        QValueList< FcPattern * >::Iterator patIt, patEnd = it.data().end();

        for(patIt = it.data().begin(); patIt != patEnd; ++patIt)
            if((full && CFcEngine::getFcString(*patIt, FC_FILE) == file) || (!full && Misc::getFile(CFcEngine::getFcString(*patIt, FC_FILE)) == file))
                return *patIt;
    }
...
bool CKioFonts::checkFile(const QString &file)
{
    ...
    if(checkExt(cFile, "ttf") || checkExt(cFile, "otf") || checkExt(cFile, "ttc") || checkExt(cFile, "pfa") || checkExt(cFile, "pfb") || isAAfm(file)
       || isAPfm(file))
        return true;
    ...
    error(KIO::ERR_SLAVE_DEFINED, i18n("<p>Only fonts may be installed.</p><p>If installing a fonts package (*%1), then "
                                       "extract the components, and install individually.</p>")
                                      .arg(constMultipleExtension));
    return false;
}
...
bool CKioFonts::getSourceFiles(const KURL &src, QStringList &files)
{
    if(KFI_KIO_FONTS_PROTOCOL == src.protocol())
    {
        ...
        if(files.count())
        {
            QStringList::Iterator sIt, sEnd = files.end();
    ...
    if(files.count())
    {
        QStringList::Iterator it, end = files.end();
...
bool CKioFonts::checkDestFiles(const KURL &src, QMap< QString, QString > &map, const KURL &dest, EFolder destFolder, bool overwrite)
{
    ...
    //
    if(dest.protocol() == src.protocol() && dest.directory() == src.directory()) // Check whether confirmUrl changed a "cp fonts:/System fonts:/"
...
bool CKioFonts::confirmMultiple(const KURL &url, const QStringList &files, EFolder folder, EOp op)
{
    ...
    QStringList::ConstIterator it, end = files.end();
    QStringList fonts;
    ...
    {
        QString out;
        QStringList::Iterator it, end = fonts.end();

        for(it = fonts.begin(); it != end; ++it)
            out += QString("<li>") + *it + QString("</li>");

        if(KMessageBox::No == messageBox(QuestionYesNo, OP_MOVE == op
...
bool CKioFonts::checkUrl(const KURL &u, bool rootOk)
{
    if(KFI_KIO_FONTS_PROTOCOL == u.protocol() && (!rootOk || (rootOk && "/" != u.path())))
    {
        ...
        if(itsRoot)
        {
            if((isSysFolder(sect) || isUserFolder(sect)) && (itsFolders[FOLDER_SYS].fontMap.end() == itsFolders[FOLDER_SYS].fontMap.find(sect)))
        ...
        else if(!isSysFolder(sect) && !isUserFolder(sect))
        {
            error(KIO::ERR_SLAVE_DEFINED, i18n("Please specify \"%1\" or \"%2\".").arg(i18n(KFI_KIO_FONTS_USER)).arg(i18n(KFI_KIO_FONTS_SYS)));
            return false;
        }
    }
...
bool CKioFonts::checkAllowed(const KURL &u)
{
    if(KFI_KIO_FONTS_PROTOCOL == u.protocol())
    {
        ...
        if(ds == QString(QChar('/') + i18n(KFI_KIO_FONTS_USER) + QChar('/')) || ds == QString(QChar('/') + i18n(KFI_KIO_FONTS_SYS) + QChar('/'))
...
void CKioFonts::createAfm(const QString &file, bool nrs, const QString &passwd)
{
    ...
    bool type1 = isAType1(file), pfm = !type1 && isAPfm(file); // No point checking if is pfm if its a type1
...
void CKioFonts::reparseConfig()
{
    ...
    if(itsRoot)
    {
        ...
        bool doX = cfg.readBoolEntry(KFI_CFG_X_KEY, KFI_DEFAULT_CFG_X), doGs = cfg.readBoolEntry(KFI_CFG_GS_KEY, KFI_DEFAULT_CFG_GS);
    ...
    else
    {
        ...
        bool rootDoX = rootCfg.readBoolEntry(KFI_CFG_X_KEY, KFI_DEFAULT_CFG_X), rootDoGs = rootCfg.readBoolEntry(KFI_CFG_GS_KEY, KFI_DEFAULT_CFG_GS);
