...
CXConfig::CXConfig(EType type, const QString &file) : itsType(type), itsFileName(file), itsOk(false), itsWritable(false)
{
...
bool CXConfig::readConfig()
{
    ...
    if(itsOk)
        itsWritable = Misc::fExists(itsFileName) ? Misc::fWritable(itsFileName) : Misc::dWritable(Misc::getDir(itsFileName));
    else
        itsWritable = false;
    ...
    do
    {
        ...
        if(s && s < *end)
        {
            ...
            else
            {
                ...
                if(s && *s == '\"' && s < *end)
                {
                    char *e = strchr(s + 1, '\"'), *nl = strchr(s + 1, '\n');
                ...
                if(x11.good())
                {
                    ...
                    if(NULL != (filesStart = locateSection(buffer, "\"Files\"")) && NULL != (filesEnd = locateEndSection(filesStart)))
                    {
                        ...
                        else
                        {
                            ...
                            if(fstream)
                            {
                                char *from = buffer, *modStart = NULL, *modEnd = NULL;
                                bool foundFt = false;
                                TPath *path;
...
{
    static const char *constKeys[] = {"alternate-servers",
                                      "cache-balance",
                                      "cache-hi-mark",
                                      "cache-low-mark",
                                      "catalogue",
    ...
    for(unsigned int key = 0; NULL != constKeys[key]; ++key)
        ...
        {
            unsigned int sLen = strlen(str), kLen = strlen(constKeys[key]);
        ...
        if(size < 32768) // Just incase...
        {
            ...
            if(buffer)
            {
                ...
                if(xfs.good())
                {
                    ...
                    char *cat = NULL;
                    bool found = false, formatError = false;
                                ...
                                if(!commentedOut(buffer, cat))
                                {
                                    ...
                                    for(i = 1; i < size - (cat - buffer) && !found && !formatError; ++i)
                                        if(!isWhitespace(cat[i]))
                                        {
                                            ...
                                            if(comChars)
                                                i += comChars;
                                            else if(cat[i] != '=' || i + 1 >= size - (cat - buffer))
                                                formatError = true;
                                            else
                                            {
                                                char *path;
...
CXConfig::TPath::EType CXConfig::TPath::getType(const QString &d)
{
    ...
    return 0 == str.find("unix/:") ? FONT_SERVER : "fontconfig" == str ? FONT_CONFIG : DIR;
}
}
