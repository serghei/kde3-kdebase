...
static KURL getDest(const KURL &url, bool system)
{
    return KURL(KFI::Misc::root()
                    ...
                    : QString("fonts:/") + QString(system ? i18n(KFI_KIO_FONTS_SYS) : i18n(KFI_KIO_FONTS_USER)) + QChar('/') + url.fileName());
}
...
CFontViewPart::CFontViewPart(QWidget *parent, const char *name)
{
    ...
    itsChangeTextAction = new KAction(i18n("Change Text..."), "text", KShortcut(), this, SLOT(changeText()), actionCollection(), "changeText");
...
{
    bool showFs = false, isFonts = KFI_KIO_FONTS_PROTOCOL == m_url.protocol();
...
{
    int resp = Misc::root() ? KMessageBox::Yes : KMessageBox::questionYesNoCancel(itsFrame, i18n("Where do you wish to install \"%1\" (%2)?\n"
                                                                                  ...
                                                                                  i18n("Install"), i18n(KFI_KIO_FONTS_USER), i18n(KFI_KIO_FONTS_SYS));
    ...
    if(KMessageBox::Cancel != resp)
    {
        ...
        if(KIO::NetAccess::copy(m_url, destUrl, itsFrame->parentWidget()))
        {
            ...
            if(urls.count())
            {
                KURL::List::Iterator it, end = urls.end();
...
void CFontViewPart::changeText()
{
    ...
    QString oldStr(itsPreview->engine().getPreviewString()),
        newStr(KInputDialog::getText(i18n("Preview String"), i18n("Please enter new string:"), oldStr, &status, itsFrame, "preview string dialog",
                                     &validator));
