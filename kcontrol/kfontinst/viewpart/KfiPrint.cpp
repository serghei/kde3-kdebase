...
namespace Print {
    ...
    void printItems(const QStringList &items, int size, QWidget *parent, CFcEngine &engine)
    {
        ...
        if(printer.setup(parent))
        {
            ...
            QSettings settings;
            bool entryExists, embedFonts, set = false;
            ...
            int margin = (int)((2 / 2.54) * metrics.logicalDpiY()), // 2 cm margins
                pageWidth = metrics.width() - (2 * margin), pageHeight = metrics.height() - (2 * margin), y = margin, oneSize[2] = {size, 0};
            ...
            QStringList::ConstIterator it(items.begin()), end(items.end());
