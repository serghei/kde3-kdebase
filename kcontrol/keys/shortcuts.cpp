...
ShortcutsModule::ShortcutsModule(QWidget *parent, const char *name) : QWidget(parent, name)
{
    initGUI();
}
...
{
    return i18n(
        "<h1>Key Bindings</h1> Using key bindings you can configure certain actions to be"
...
void ShortcutsModule::slotSelectScheme(int)
{
    ...
    else
    {
        ...
        if(!KKeyNative::keyboardHasWinKey() && config.readBoolEntry("Uses Win Modifier", false))
        {
            // TODO: change "Win" to Win's label.
            int ret = KMessageBox::warningContinueCancel(this, i18n("This scheme requires the \"%1\" modifier key, which is not "
...
void ShortcutsModule::slotSaveSchemeAs()
{
    ...
    do
    {
        ...
        sName = KInputDialog::getText(i18n("Save Key Scheme"), i18n("Enter a name for the key scheme:"), sName, &ok, this);
        ...
        if(ok)
        {
            ...
            for(int i = 0; i < (int)m_pcbSchemes->count(); i++)
            {
                if(sName.lower() == (m_pcbSchemes->text(i)).lower())
                {
                    ...
                    int result = KMessageBox::warningContinueCancel(0, i18n("A key scheme with the name '%1' already exists;\n"
                                                                            "do you want to overwrite it?\n")
                                                                           .arg(sName),
                                                                    i18n("Save Key Scheme"), i18n("Overwrite"));
