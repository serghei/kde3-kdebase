...
static const KCmdLineOptions options[] = {{"noninteractive", I18N_NOOP("Non interactive use: no message boxes"), 0},
...
static bool startNewKonqueror(QString url, QString mimetype, const QString &profile)
{
    ...
    allowed_parts << QString::fromLatin1("konq_iconview.desktop") << QString::fromLatin1("konq_multicolumnview.desktop")
                  << QString::fromLatin1("konq_sidebartng.desktop") << QString::fromLatin1("konq_infolistview.desktop")
                  << QString::fromLatin1("konq_treeview.desktop") << QString::fromLatin1("konq_detailedlistview.desktop");
    if(cfg.hasKey("SafeParts") && cfg.readEntry("SafeParts") != QString::fromLatin1("SAFE"))
    ...
    KTrader::OfferList offers = KTrader::self()->query(mimetype, QString::fromLatin1("KParts/ReadOnlyPart"), QString::null, QString::null);
    KService::Ptr serv;
    if(offers.count() > 0)
        serv = offers.first();
...
{ // prefer(?) preloaded ones
    ...
    str << currentScreen();
    if(!KApplication::dcopClient()->findObject("konqueror*", "KonquerorIface", "processCanBeReused( int )", data, ret, appObj, false, 3000))
        return "";
    return ret;
}
...
bool clientApp::createNewWindow(const KURL &url, bool newTab, bool tempFile, const QString &mimetype)
{
    ...
    if(newTab || cfg.readBoolEntry("KonquerorTabforExternalURL", false))
    {
        ...
        QByteArray data;
        QDataStream str(data, IO_WriteOnly);
        if(KApplication::dcopClient()->findObject("konqueror*", "konqueror-mainwindow*", "windowCanBeUsedForTab()", data, foundApp, foundObj, false,
                                                  3000))
        {
...
bool clientApp::openProfile(const QString &profileName, const QString &url, const QString &mimetype)
{
    ...
    {
        QString error;
        if(KApplication::startServiceByDesktopPath(QString::fromLatin1("konqueror.desktop"), QString::fromLatin1("--silent"), &error, &appId, NULL,
                                                   startup_id_str)
           > 0)
        {
            kdError() << "Couldn't start konqueror from konqueror.desktop: " << error << endl;
...
bool clientApp::doIt()
{
    ...
    else if(command == "exec")
    {
        ...
        else if(argc == 3)
        {
            ...
            const KTrader::OfferList offers =
                KTrader::self()->query(QString::fromLocal8Bit(args->arg(2)), QString::fromLatin1("Application"), QString::null, QString::null);
            if(offers.isEmpty())
                return 1;
    ...
    else if(command == "download")
    {
        ...
        if(srcLst.count() == 0)
            return m_ok;
        QString dst = KFileDialog::getSaveFileName((argc < 2) ? (QString::null) : (args->url(1).filename()));
