    ...
    : KonqSidebarPlugin(instance, parent, widgetParent, desktopName_, name)
{
    ...
    if(virt == 1)
        desktopName_ = ksc.readEntry("X-KDE-RelURL", "");
    ...
    connect(tree, SIGNAL(openURLRequest(const KURL &, const KParts::URLArgs &)), this, SIGNAL(openURLRequest(const KURL &, const KParts::URLArgs &)));

    connect(tree, SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &)), this,
            SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &)));

    connect(tree, SIGNAL(popupMenu(const QPoint &, const KURL &, const QString &, mode_t)), this,
            SIGNAL(popupMenu(const QPoint &, const KURL &, const QString &, mode_t)));

    connect(tree, SIGNAL(popupMenu(const QPoint &, const KFileItemList &)), this, SIGNAL(popupMenu(const QPoint &, const KFileItemList &)));

    connect(tree, SIGNAL(enableAction(const char *, bool)), this, SIGNAL(enableAction(const char *, bool)));
}
...
void *KonqSidebar_Tree::provides(const QString &)
{
    return 0;
}
...
QWidget *KonqSidebar_Tree::getWidget()
{
    return widget;
}
...
KDE_EXPORT bool add_konqsidebar_tree(QString *fn, QString *, QMap< QString, QString > *map)
{
    ...
    QString item = KInputDialog::getItem(i18n("Select Type"), i18n("Select type:"), names);
    if(!item.isEmpty())
    {
