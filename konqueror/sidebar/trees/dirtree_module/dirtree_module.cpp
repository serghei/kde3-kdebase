...
KonqSidebarDirTreeModule::~KonqSidebarDirTreeModule()
{
    ...
    if(m_dirLister)
    {
        disconnect(m_dirLister, SIGNAL(canceled(const KURL &)), this, SLOT(slotListingStopped(const KURL &)));
        delete m_dirLister;
    }
}
...
static void lookupItems(QDict< KonqSidebarTreeItem > &dict, const QString &key, KonqSidebarTreeItem *&item,
...
void KonqSidebarDirTreeModule::openSubFolder(KonqSidebarTreeItem *item)
{
    ...
    if(!m_dirLister) // created on demand
    {
        ...
        connect(m_dirLister, SIGNAL(completed(const KURL &)), this, SLOT(slotListingStopped(const KURL &)));
        connect(m_dirLister, SIGNAL(canceled(const KURL &)), this, SLOT(slotListingStopped(const KURL &)));
        connect(m_dirLister, SIGNAL(redirection(const KURL &, const KURL &)), this, SLOT(slotRedirection(const KURL &, const KURL &)));
    }


    if(!item->isTopLevelItem() && static_cast< KonqSidebarDirTreeItem * >(item)->hasStandardIcon())
    {
        ...
        while(oldItem)
        {
            ...
            if(!fileItem->isDir())
            {
                ...
                if(fileItem->url().isLocalFile()
                   && (((ptr = fileItem->determineMimeType()) != 0) && (ptr->is("inode/directory") || m_showArchivesAsFolders)
...
void KonqSidebarDirTreeModule::slotRefreshItems(const KFileItemList &entries)
{
    ...
    for(; kit.current(); ++kit)
    {
        ...
        do
        {
            ...
            {
                kdWarning(1201) << "KonqSidebarDirTreeModule::slotRefreshItems entry for " << kit.current()->url().url(-1)
                                << " matches against toplevel." << endl;
                break;
            }
