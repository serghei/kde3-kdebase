...
KonqSidebarHistoryItem::KonqSidebarHistoryItem(const KonqHistoryEntry *entry, KonqSidebarTreeItem *parentItem,
...
void KonqSidebarHistoryItem::update(const KonqHistoryEntry *entry)
{
    ...
    if(!title.stripWhiteSpace().isEmpty() && title != entry->url.url())
        setText(0, title);
    else
    {
...
QDragObject *KonqSidebarHistoryItem::dragObject(QWidget *parent, bool /*move*/)
{
    ...
    KBookmark bookmark = KBookmark::standaloneBookmark(m_entry->title, m_entry->url, icon);
...
QString KonqSidebarHistoryItem::toolTipText() const
{
    if(s_settings->m_detailedTips)
    {
        ...
        return tip.arg(KGlobal::locale()->formatDateTime(m_entry->lastVisited))
            .arg(KGlobal::locale()->formatDateTime(m_entry->firstVisited))
            .arg(m_entry->numberOfTimesVisited)
            .arg(m_entry->url.url());
    }
...
void KonqSidebarHistoryItem::paintCell(QPainter *p, const QColorGroup &cg, int column, int width, int alignment)
{
    QDateTime dt;
...
KonqSidebarHistoryGroupItem::KonqSidebarHistoryGroupItem(const KURL &url, KonqSidebarTreeTopLevelItem *topLevelItem)
    : KonqSidebarTreeItem(topLevelItem, topLevelItem), m_hasFavIcon(false), m_url(url)
{
}
...
QDragObject *KonqSidebarHistoryGroupItem::dragObject(QWidget *parent, bool /*move*/)
{
    ...
    KBookmark bookmark = KBookmark::standaloneBookmark(QString::null, m_url, icon);
    KBookmarkDrag *drag = KBookmarkDrag::newDrag(bookmark, parent);
    return drag;
}
