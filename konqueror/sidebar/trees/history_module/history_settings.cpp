...
KonqSidebarHistorySettings::KonqSidebarHistorySettings(QObject *parent, const char *name)
    : QObject(parent, name), DCOPObject("KonqSidebarHistorySettings")
{
    m_fontOlderThan.setItalic(true); // default
}

KonqSidebarHistorySettings::KonqSidebarHistorySettings() : QObject(), DCOPObject("KonqSidebarHistorySettings")
{
    m_fontOlderThan.setItalic(true); // default
}

KonqSidebarHistorySettings::KonqSidebarHistorySettings(const KonqSidebarHistorySettings &s) : QObject(), DCOPObject("KonqSidebarHistorySettings")
{
...
void KonqSidebarHistorySettings::readSettings(bool global)
{
    ...
    m_fontYoungerThan = config->readFontEntry("Font youngerThan", &m_fontYoungerThan);
    m_fontOlderThan = config->readFontEntry("Font olderThan", &m_fontOlderThan);
    if(global)
...
void KonqSidebarHistorySettings::applySettings()
{
    ...
    kapp->dcopClient()->send("konqueror*", "KonqSidebarHistorySettings", "notifySettingsChanged()", QByteArray());
}
