...
void KonqSidebarTree::loadModuleFactories()
{
    ...
    for(QStringList::ConstIterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        {
            kdWarning() << "Bad Configuration file for a dirtree module " << *it << endl;
            continue;
        }
...
KonqSidebarTree::KonqSidebarTree(KonqSidebar_Tree *parent, QWidget *parentWidget, int virt, const QString &path)
    : KListView(parentWidget), m_currentTopLevelItem(0), m_toolTip(this), m_scrollingLocked(false), m_collection(0)
{
    ...
    connect(this, SIGNAL(mouseButtonPressed(int, QListViewItem *, const QPoint &, int)), this,
            SLOT(slotMouseButtonPressed(int, QListViewItem *, const QPoint &, int)));
...
void KonqSidebarTree::scanDir(KonqSidebarTreeItem *parent, const QString &path, bool isRoot)
{
    ...
    if(isRoot)
    {
        ...
        if(copyConfig)
        {
            ...
            //            QString dirtree_dir = KGlobal::dirs()->findDirs("data","konqsidebartng/virtual_folders/"+m_dirtreeDir.relDir+"/").last();
            //            // most global
...
void KonqSidebarTree::enableActions(bool copy, bool cut, bool paste, bool trash, bool del, bool rename)
{
...
void KonqSidebarTree::showToplevelContextMenu()
{
    ...
    if(!m_collection)
    {
        ...
        (void)new KAction(i18n("Rename"), 0, this, SLOT(slotRename()), m_collection, "rename");
        (void)new KAction(i18n("Delete Link"), "editdelete", 0, this, SLOT(slotDelete()), m_collection, "delete_link");
...
void KonqSidebarTree::slotCreateFolder()
{
    ...
    while(true)
    {
        name = KInputDialog::getText(i18n("Create New Folder"), i18n("Enter folder name:"), name);
        if(name.isEmpty())
            return;
