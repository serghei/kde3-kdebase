...
KonqSidebar::KonqSidebar(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name, bool universalMode)
    : KParts::ReadOnlyPart(parent, name), KonqSidebarIface()
{
    ...
    m_widget =
        new Sidebar_Widget(parentWidget, this, widgetName, universalMode, parentWidget->topLevelWidget()->property("currentProfile").toString());
    ...
    connect(m_extension, SIGNAL(addWebSideBar(const KURL &, const QString &)), m_widget, SLOT(addWebSideBar(const KURL &, const QString &)));
...
{
    if(KonqFileSelectionEvent::test(ev) || KonqFileMouseOverEvent::test(ev) || KonqConfigEvent::test(ev))
    {
...
KonqSidebarFactory::KonqSidebarFactory() : KParts::Factory()
{
}
...
KParts::Part *KonqSidebarFactory::createPartObject(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name,
