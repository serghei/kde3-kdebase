...
class ButtonInfo : public QObject, public KonqSidebarIface {
    Q_OBJECT
public:
    ButtonInfo(const QString &file_, class KonqSidebarIface *part, class KDockWidget *dock_, const QString &url_, const QString &lib,
               const QString &dispName_, const QString &iconName_, QObject *parent)
        : QObject(parent), file(file_), dock(dock_), URL(url_), libName(lib), displayName(dispName_), iconName(iconName_), m_part(part)
    {
    ...
    virtual bool universalMode()
    {
        return m_part->universalMode();
    }
};
...
class addBackEnd : public QObject {
    Q_OBJECT
public:
    addBackEnd(QWidget *parent, class QPopupMenu *addmenu, bool universal, const QString &currentProfile, const char *name = 0);
    ~addBackEnd()
    {
        ;
    }
...
public:
    Sidebar_Widget(QWidget *parent, KParts::ReadOnlyPart *par, const char *name, bool universalMode, const QString &currentProfile);
    ~Sidebar_Widget();
