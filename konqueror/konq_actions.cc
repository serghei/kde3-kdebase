...
void KonqBidiHistoryAction::fillHistoryPopup(const QPtrList< HistoryEntry > &history, QPopupMenu *popup, bool onlyBack, bool onlyForward,
                                             bool checkCurrentItem, uint startPos)
{
    ...
    while(it.current())
    {
        ...
        if(checkCurrentItem && it.current() == current)
        {
            ...
            popup->insertItem(KonqPixmapProvider::self()->pixmapFor(it.current()->url.url()), text);
        if(++i > 10)
            break;
        if(!onlyForward)
            --it;
...
KonqBidiHistoryAction::KonqBidiHistoryAction(const QString &text, QObject *parent, const char *name) : KAction(text, 0, parent, name)
{
...
int KonqBidiHistoryAction::plug(QWidget *widget, int index)
{
    ...
    if(widget->inherits("QPopupMenu"))
    {
        ...
        connect(m_goMenu, SIGNAL(aboutToShow()), this, SIGNAL(menuAboutToShow()));
        connect(m_goMenu, SIGNAL(activated(int)), this, SLOT(slotActivated(int)));
...
KonqLogoAction::KonqLogoAction(const QString &text, int accel, QObject *parent, const char *name) : KAction(text, accel, parent, name)
{
}

KonqLogoAction::KonqLogoAction(const QString &text, int accel, QObject *receiver, const char *slot, QObject *parent, const char *name)
...
KonqLogoAction::KonqLogoAction(const QString &text, const QIconSet &pix, int accel, QObject *receiver, const char *slot, QObject *parent,
                               const char *name)
    : KAction(text, pix, accel, receiver, slot, parent, name)
{
}

KonqLogoAction::KonqLogoAction(const QStringList &icons, QObject *receiver, const char *slot, QObject *parent, const char *name)
...
KonqViewModeAction::KonqViewModeAction(const QString &text, const QString &icon, QObject *parent, const char *name)
    : KRadioAction(text, icon, 0, parent, name)
{
    m_menu = new QPopupMenu;

    connect(m_menu, SIGNAL(aboutToShow()), this, SLOT(slotPopupAboutToShow()));
    connect(m_menu, SIGNAL(activated(int)), this, SLOT(slotPopupActivated()));
    connect(m_menu, SIGNAL(aboutToHide()), this, SLOT(slotPopupAboutToHide()));
}
...
KonqMostOftenURLSAction::KonqMostOftenURLSAction(const QString &text, QObject *parent, const char *name) : KActionMenu(text, "goto", parent, name)
{
    setDelayed(false);
...
void KonqMostOftenURLSAction::parseHistory() // only ever called once
{
    ...
    connect(mgr, SIGNAL(entryAdded(const KonqHistoryEntry *)), SLOT(slotEntryAdded(const KonqHistoryEntry *)));
    connect(mgr, SIGNAL(entryRemoved(const KonqHistoryEntry *)), SLOT(slotEntryRemoved(const KonqHistoryEntry *)));
...
void KonqMostOftenURLSAction::slotFillMenu()
{
    ...
    while(entry)
    {
        ...
        QString text = entry->title.isEmpty() ? (entry->typedURL.isEmpty() ? entry->url.prettyURL() : entry->typedURL) : entry->title;

        popupMenu()->insertItem(KonqPixmapProvider::self()->pixmapFor(entry->url.url()), text, id);
...
int MostOftenList::compareItems(QPtrCollection::Item item1, QPtrCollection::Item item2)
{
