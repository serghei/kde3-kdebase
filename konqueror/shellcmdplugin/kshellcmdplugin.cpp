...
KShellCmdPlugin::KShellCmdPlugin(QObject *parent, const char *name, const QStringList &) : KParts::Plugin(parent, name)
{
    if(!kapp->authorize("shell_access"))
        return;

    new KAction(i18n("&Execute Shell Command..."), "run", CTRL + Key_E, this, SLOT(slotExecuteShellCommand()), actionCollection(),
                "executeshellcommand");
}
...
void KShellCmdPlugin::slotExecuteShellCommand()
{
    ...
    if(part->currentItem())
    {
        ...
        path = KURL::relativePath(url.path(), part->currentItem()->url().path());
    }
    else
    {
        path = url.path();
    ...
    bool ok;
    QString cmd = KInputDialog::getText(i18n("Execute Shell Command"), i18n("Execute shell command in current directory:"), KProcess::quote(path),
                                        &ok, part->widget());
    if(ok)
    {
        QString chDir;
