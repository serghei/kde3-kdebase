...
KonqViewManager::KonqViewManager(KonqMainWindow *mainWindow) : KParts::PartManager(mainWindow)
{
...
KonqView *KonqViewManager::splitView(Qt::Orientation orientation, const QString &serviceType, const QString &serviceName, bool newOneFirst,
                                     bool forceAutoEmbed)
{
#ifdef DEBUG_VIEWMGR
...
KonqView *KonqViewManager::splitWindow(Qt::Orientation orientation, const QString &serviceType, const QString &serviceName, bool newOneFirst)
{
...
KonqView *KonqViewManager::addTab(const QString &serviceType, const QString &serviceName, bool passiveMode, bool openAfterCurrentPage)
{
    ...
    {
        if(m_pMainWindow && m_pMainWindow->currentView() && m_pMainWindow->currentView()->frame())
        {
    ...
    KonqView *childView = setupView(static_cast< KonqFrameTabs * >(m_pDocContainer), newViewFactory, service, partServiceOffers, appServiceOffers,
                                    serviceType, passiveMode, openAfterCurrentPage);
...
void KonqViewManager::removeView(KonqView *view)
{
    ...
    if(parentContainer->frameType() == "Container")
    {
        ...
        if(m_pDocContainer == parentContainer)
...
KonqViewFactory KonqViewManager::createView(const QString &serviceType, const QString &serviceName, KService::Ptr &service,
                                            KTrader::OfferList &partServiceOffers, KTrader::OfferList &appServiceOffers, bool forceAutoEmbed)
{
    ...
    else
    {
        ...
        viewFactory = KonqFactory::createView(serviceType, serviceName, &service, &partServiceOffers, &appServiceOffers, forceAutoEmbed);
    }

    return viewFactory;
}

KonqView *KonqViewManager::setupView(KonqFrameContainerBase *parentContainer, KonqViewFactory &viewFactory, const KService::Ptr &service,
                                     const KTrader::OfferList &partServiceOffers, const KTrader::OfferList &appServiceOffers,
                                     const QString &serviceType, bool passiveMode, bool openAfterCurrentPage)
{
    ...
    KonqView *v = new KonqView(viewFactory, newViewFrame, m_pMainWindow, service, partServiceOffers, appServiceOffers, sType, passiveMode);
    // kdDebug(1202) << "KonqView created - v=" << v << " v->part()=" << v->part() << endl;

    QObject::connect(v, SIGNAL(sigPartChanged(KonqView *, KParts::ReadOnlyPart *, KParts::ReadOnlyPart *)), m_pMainWindow,
...
void KonqViewManager::saveViewProfile(const QString &fileName, const QString &profileName, bool saveURLs, bool saveWindowSize)
{
    ...
    QString path = locateLocal("data", QString::fromLatin1("konqueror/profiles/") + fileName, KGlobal::instance());
...
void KonqViewManager::saveViewProfile(KConfig &cfg, bool saveURLs, bool saveWindowSize)
{
    ...
    if(m_pMainWindow->childFrame() != 0L)
    {
        QString prefix = QString::fromLatin1(m_pMainWindow->childFrame()->frameType()) + QString::number(0);
...
void KonqViewManager::loadViewProfile(const QString &path, const QString &filename, const KURL &forcedURL, const KonqOpenURLRequest &req,
...
void KonqViewManager::loadViewProfile(KConfig &cfg, const QString &filename, const KURL &forcedURL, const KonqOpenURLRequest &req, bool resetWindow,
                                      bool openURL)
{
    if(docContainer() && docContainer()->frameType() == "Tabs")
    {
        ...
        if(tabContainer->count() > 1)
        {
            if(KMessageBox::warningContinueCancel(0, i18n("You have multiple tabs open in this window.\n"
                                                          "Loading a view profile will close them."),
                                                  i18n("Confirmation"), i18n("Load View Profile"), "LoadProfileTabsConfirm")
               == KMessageBox::Cancel)
                return;
        }
            ...
            if(view && view->part() && (view->part()->metaObject()->findProperty("modified") != -1))
            {
                ...
                if(prop.isValid() && prop.toBool())
                {
                    ...
                    if(KMessageBox::warningContinueCancel(
                           0, i18n("This tab contains changes that have not been submitted.\nLoading a profile will discard these changes."),
                           i18n("Discard Changes?"), i18n("&Discard Changes"), "discardchangesloadprofile")
    ...
    else
    {
        ...
        if(view && view->part() && (view->part()->metaObject()->findProperty("modified") != -1))
        {
            ...
            if(prop.isValid() && prop.toBool())
                if(KMessageBox::warningContinueCancel(
                       0, i18n("This page contains changes that have not been submitted.\nLoading a profile will discard these changes."),
                       i18n("Discard Changes?"), i18n("&Discard Changes"), "discardchangesloadprofile")
                   != KMessageBox::Continue)
                    return;
        }
    ...
    if(nextChildView == 0L)
        nextChildView = chooseNextView(0L);
    ...
    if(openURL && !forcedURL.isEmpty())
    {
        ...
        m_pMainWindow->openURL(nextChildView /* can be 0 for an empty profile */, forcedURL, _req.args.serviceType, _req, _req.args.trustedSource);
...
void KonqViewManager::setActivePart(KParts::Part *part, bool immediate)
{
    ...
    KParts::Part *mainWindowActivePart = (m_pMainWindow && m_pMainWindow->currentView()) ? m_pMainWindow->currentView()->part() : 0;
...
void KonqViewManager::loadItem(KConfig &cfg, KonqFrameContainerBase *parent, const QString &name, const KURL &defaultURL, bool openURL,
                               bool openAfterCurrentPage)
{
    ...
    if(name.startsWith("View") || name == "empty")
    {
        ...
        KonqViewFactory viewFactory =
            KonqFactory::createView(serviceType, serviceName, &service, &partServiceOffers, &appServiceOffers, true /*forceAutoEmbed*/);
        ...
        KonqView *childView =
            setupView(parent, viewFactory, service, partServiceOffers, appServiceOffers, serviceType, passiveMode, openAfterCurrentPage);
    ...
    else if(name.startsWith("Container"))
    {
        ...
        QValueList< int > sizes = cfg.readIntListEntry(QString::fromLatin1("SplitterSizes").prepend(prefix));
...
void KonqViewManager::setProfiles(KActionMenu *profiles)
{
    ...
    if(m_pamProfiles)
    {
        connect(m_pamProfiles->popupMenu(), SIGNAL(activated(int)), this, SLOT(slotProfileActivated(int)));
        connect(m_pamProfiles->popupMenu(), SIGNAL(aboutToShow()), this, SLOT(slotProfileListAboutToShow()));
    }
...
void KonqViewManager::slotProfileListAboutToShow()
{
    ...
    for(int id = 0; iter != accel_strings.end(); ++iter, ++id)
    {
...
#ifndef NDEBUG
void KonqViewManager::printSizeInfo(KonqFrameBase *frame, KonqFrameContainerBase *parent, const char *msg)
{
    QRect r;
...
void KonqViewManager::printFullHierarchy(KonqFrameContainerBase *container)
{
    ...
    if(container)
        container->printFrameInfo(QString::null);
    else
        m_pMainWindow->printFrameInfo(QString::null);
}
#endif
