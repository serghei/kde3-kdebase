...
KonqyPreloader::KonqyPreloader(const QCString &obj) : KDEDModule(obj)
{
    reconfigure();
    connect(kapp->dcopClient(), SIGNAL(applicationRemoved(const QCString &)), SLOT(appRemoved(const QCString &)));
    connect(&check_always_preloaded_timer, SIGNAL(timeout()), SLOT(checkAlwaysPreloaded()));
}
...
QCString KonqyPreloader::getPreloadedKonqy(int screen)
{
    if(instances.count() == 0)
        return "";
    for(InstancesList::Iterator it = instances.begin(); it != instances.end(); ++it)
    {
        if((*it).screen == screen)
        {
            QCString ret = (*it).id;
...
void KonqyPreloader::updateCount()
{
    ...
    if(KonqSettings::alwaysHavePreloaded() && KonqSettings::maxPreloadCount() > 0 && instances.count() == 0)
    {
        if(!check_always_preloaded_timer.isActive())
        {
            if(kapp->kdeinitExec(QString::fromLatin1("konqueror"), QStringList() << QString::fromLatin1("--preload"), NULL, NULL, "0") == 0)
            {
...
extern "C" KDE_EXPORT KDEDModule *create_konqy_preloader(const QCString &obj)
{
