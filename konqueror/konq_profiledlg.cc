...
KonqProfileItem::KonqProfileItem(KListView *parent, const QString &text) : QListViewItem(parent, text), m_profileName(text)
{
}
...
KonqProfileDlg::KonqProfileDlg(KonqViewManager *manager, const QString &preselectProfile, QWidget *parent)
    : KDialogBase(parent, "konq_profile_dialog", true, i18n("Profile Management"), KDialogBase::Close | BTN_RENAME | BTN_DELETE | BTN_SAVE, BTN_SAVE,
                  true, KGuiItem(i18n("&Rename Profile")), KGuiItem(i18n("&Delete Profile"), "editdelete"), KStdGuiItem::save())
{
    ...
    connect(m_pListView, SIGNAL(itemRenamed(QListViewItem *)), SLOT(slotItemRenamed(QListViewItem *)));
    ...
    connect(m_pListView, SIGNAL(selectionChanged(QListViewItem *)), this, SLOT(slotSelectionChanged(QListViewItem *)));

    connect(m_pProfileNameLineEdit, SIGNAL(textChanged(const QString &)), this, SLOT(slotTextChanged(const QString &)));
...
void KonqProfileDlg::slotUser3() // Save button
{
    ...
    m_pViewManager->saveViewProfile(name, m_pProfileNameLineEdit->text(), m_cbSaveURLs->isChecked(), m_cbSaveSize->isChecked());
