...
static char *conv_legacy(ConvRequest what, const char *prompt)
{
    char *p, *p2;
    int len;
    char buf[1024];
...
int main(int argc, char **argv)
{
    ...
    if(uid != geteuid())
    {
        ...
        if((c = read(lfd, fcont, sizeof(fcont) - 1)) > 0 && (fcont[c] = '\0', sscanf(fcont, "%ld", &nexttime) == 1))
        {
            time_t ct = time(0);
    ...
    ret = Authenticate(
        ...
        caller,
#endif
        method, username, sfd < 0 ? conv_legacy : conv_server);
