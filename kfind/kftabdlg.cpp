...
KfindTabWidget::KfindTabWidget(QWidget *parent, const char *name) : QTabWidget(parent, name), regExpDialog(0)
{
    ...
    const QString whatsfileindex = i18n(
    ...
    QGridLayout *grid = new QGridLayout(pages[0], 3, 2, KDialog::marginHint(), KDialog::spacingHint());
    ...
    connect(browseB, SIGNAL(clicked()), this, SLOT(getDirectory()));

    connect(nameBox, SIGNAL(activated(int)), this, SIGNAL(startSearch()));
    ...
    QGridLayout *grid1 = new QGridLayout(pages[1], 5, 6, KDialog::marginHint(), KDialog::spacingHint());
    ...
    for(KMimeType::List::ConstIterator it = m_types.begin(); it != m_types.end(); ++it)
    {
    ...
    QGridLayout *grid2 = new QGridLayout(pages[2], 5, 4, KDialog::marginHint(), KDialog::spacingHint());
    ...
    // Setup
    const QString whatsmetainfo = i18n(
        ...
        "</ul>"
        "</qt>");
    const QString whatsmetainfokey = i18n(
...
void KfindTabWidget::initMimeTypes()
{
    ...
    {
        KMimeType *type = *it;
        if((!type->comment().isEmpty()) && (!type->name().startsWith("kdedevice/")) && (!type->name().startsWith("all/")))
...
bool KfindTabWidget::isDateValid()
{
    ...
    QString str;
    if(!fromDate->getDate(&hi1).isValid() || !toDate->getDate(&hi2).isValid())
...
// Or is there an arch where this can happen?
  ...
  if (size < 0)  // overflow
    ...
    query->setContext(textEdit->text(), caseContextCb->isChecked(), binaryContextCb->isChecked(), regexpContentCb->isChecked());
}
...
{
    QString result = KFileDialog::getExistingDirectory(dirBox->text(dirBox->currentItem()).stripWhiteSpace(), this);
...
**/
KDigitValidator::KDigitValidator(QWidget *parent, const char *name) : QValidator(parent, name)
{
    r = new QRegExp("^[0-9]*$");
}
...
//             Static utility functions
//*******************************************************
static void save_pattern(QComboBox *obj, const QString &group, const QString &entry)
{
