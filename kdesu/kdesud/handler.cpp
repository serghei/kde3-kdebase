...
ConnectionHandler::ConnectionHandler(int fd) : SocketSecurity(fd), m_exitCode(0), m_hasExitCode(false), m_needExitCode(false), m_pid(0)
{
    m_Fd = fd;
    m_Priority = 50;
...
int ConnectionHandler::doCommand(QCString buf)
{
    ...
    switch(tok)
    {
        ...
        case Lexer::Tok_delCmd: // "DEL command:string user:string\n"
            ...
            {
                kdDebug(1205) << "Deleted command: " << command << ", user = " << user << endl;
                respond(Res_OK);
            }
            break;
