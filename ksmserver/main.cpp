...
void sanity_check(int argc, char *argv[])
{
    ...
    if(msg.isEmpty() && readOnly.isEmpty() && !writeTest(path))
    {
        if(errno == ENOSPC)
            ...
            msg =
                "Writing to the $HOME directory (%s) failed with\n    "
                "the error '"
                + QCString(strerror(errno)) + "'";
    }
    if(msg.isEmpty())
    {
    ...
    if(msg.isEmpty())
    {
        ...
        if(!writeTest(path))
        {
            if(errno == ENOSPC)
                ...
                msg =
                    "Writing to the temp directory (%s) failed with\n    "
                    "the error '"
                    + QCString(strerror(errno)) + "'";
        }
    }
    ...
    if(!msg.isEmpty())
    {
        ...
        QMessageBox::critical(0, "KDE Installation Problem!", QString::fromLatin1(qmsg.data()));
        exit(255);
    }
}
...
extern "C" KDE_EXPORT int kdemain(int argc, char *argv[])
{
    ...
    KAboutData aboutData("ksmserver", I18N_NOOP("The KDE Session Manager"), version, description, KAboutData::License_BSD,
    ...
    bool screenCountChanged = (config->readNumEntry("screenCount", realScreenCount) != realScreenCount);
