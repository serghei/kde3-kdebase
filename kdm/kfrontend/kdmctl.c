...
static int openctl(int fd, int err, const char *ctl, const char *dpy)
{
    struct sockaddr_un sa;

    sa.sun_family = AF_UNIX;
    if(dpy)
        snprintf(sa.sun_path, sizeof(sa.sun_path), "%s/dmctl-%s/socket", ctl, dpy);
    else
        snprintf(sa.sun_path, sizeof(sa.sun_path), "%s/dmctl/socket", ctl);
...
static const char *readcfg(const char *cfg)
{
    FILE *fp;
    const char *ctl;
    char *ptr, *ptr2;
    ...
    if(!(fp = fopen(cfg, "r")))
    {
        fprintf(stderr, "Cannot open kdm config file '%s'.\n", cfg);
        return 0;
    }
    ctl = "/var/run/xdmctl";
    while(fgets(buf, sizeof(buf), fp))
        if(!strncmp(buf, "FifoDir", 7))
        {
            ...
            while(*ptr && isspace(*ptr))
                ptr++;
            for(ptr2 = buf + strlen(buf); ptr2 > ptr && isspace(*(ptr2 - 1)); ptr2--)
                ;
            *ptr2 = 0;
            ctl = strdup(ptr);
            break;
...
static int exe(int fd, const char *in, int len)
{
    char buf[4096];
...
static int run(int fd, char **argv)
{
    unsigned len, l;
    char buf[1024];
...
int main(int argc, char **argv)
{
    ...
    {
        if(!openctl(fd, 0, "/var/run/xdmctl", dpy) && !openctl(fd, 0, "/var/run", dpy))
        {
