...
static void MkDSpec(DSpec *spec, const char *dname, const char *dclass)
{
    spec->dhost = dname;
...
static void GSendNStr(const char *buf, int len)
{
    int tlen = len + 1;
...
#ifdef XDMCP
static void GSendArr(int len, const char *data)
{
    GWrite(&len, sizeof(len));
    GWrite(data, len);
}
...
static int readFile(File *file, const char *fn, const char *what)
{
    int fd;
    off_t flen;
...
#ifdef XDMCP
static int PrequestPort(Value *retval)
{
    if(!VxdmcpEnable.ptr)
    {
        retval->ptr = (char *)0;
...
static Value emptyStr = {"", 1}, nullValue = {0, 0}, emptyArgv = {(char *)&nullValue, 0};

static int PnoPassUsers(Value *retval)
{
...
static int PautoLoginX(Value *retval)
{
...
static void ReadConf()
{
    ...
    for(s = file.buf, line = 0, cursec = 0, sectmoan = 1; s < file.eof; s++)
    {
        ...
        if(*s == '[')
        {
            ...
            for(cursec = rootsec; cursec; cursec = cursec->next)
                if(nlen == cursec->nlen && !memcmp(nstr, cursec->name, nlen))
                {
                    LogInfo(
                        "Multiple occurrences of section [%.*s] in %s. "
            ...
            for(i = 0; i < as(allSects); i++)
                if((int)strlen(allSects[i]->name) == clen && !memcmp(allSects[i]->name, cstr, clen))
                    goto newsec;
        illsec:
            cursec = 0;
    ...
    haveeq:
        ...
        for(i = 0; i < cursec->sect->numents; i++)
        {
            ...
            if((int)strlen(ce->name) == nlen && !memcmp(ce->name, nstr, nlen))
                goto keyok;
        }
    ...
    keyok:
        for(curent = cursec->entries; curent; curent = curent->next)
            ...
            {
                LogError(
                    "Multiple occurrences of key '%s' in section [%.*s]"
                    " of %s\n",
    ...
    for(cursec = rootsec; cursec; cursec = cursec->next)
        if(!cursec->dname)
            for(curent = cursec->entries; curent; curent = curent->next)
                if(curent->ent->id == id)
                {
                    Debug("line %d: %s = %'.*s\n", curent->line, curent->ent->name, curent->vallen, curent->val);
                    return curent;
                }
    return 0;
}
 ...
 */
static Entry *FindDEnt(int id, DSpec *dspec)
{
    ...
    for(cursec = rootsec; cursec; cursec = cursec->next)
        if(cursec->dname)
        {
            ...
            {
                if(cursec->dclassl == dspec->dclassl && !memcmp(cursec->dclass, dspec->dclass, dspec->dclassl))
                    score = 1;
                else
                    continue;
            }
            ...
            {
                if(cursec->dnuml == dspec->dnuml && !memcmp(cursec->dnum, dspec->dnum, dspec->dnuml))
                    score += 2;
                else
                    continue;
            }
            if(cursec->dhostl != 1 || cursec->dhost[0] != '*')
            {
                ...
                {
                    if(cursec->dhostl < dspec->dhostl && !memcmp(cursec->dhost, dspec->dhost + dspec->dhostl - cursec->dhostl, cursec->dhostl))
                        score += 8;
                    else
                        continue;
                }
                ...
                {
                    if(cursec->dhostl == dspec->dhostl && !memcmp(cursec->dhost, dspec->dhost, dspec->dhostl))
                        score += 12;
                    else
                        continue;
                }
    ...
    if(bestent)
        Debug("line %d: %.*s:%.*s_%.*s/%s = %'.*s\n", bestent->line, bestsec->dhostl, bestsec->dhost, bestsec->dnuml, bestsec->dnum, bestsec->dclassl,
              bestsec->dclass, bestent->ent->name, bestent->vallen, bestent->val);
    return bestent;
}

static const char *CvtValue(Ent *et, Value *retval, int vallen, const char *val, char **eopts)
{
    Value *ents;
    ...
    switch(et->id & C_TYPE_MASK)
    {
        case C_TYPE_INT:
            ...
            {
                if(!strcmp(buf, "true") || !strcmp(buf, "on") || !strcmp(buf, "yes") || !strcmp(buf, "1"))
                    retval->ptr = (char *)1;
                else if(!strcmp(buf, "false") || !strcmp(buf, "off") || !strcmp(buf, "no") || !strcmp(buf, "0"))
                    retval->ptr = (char *)0;
                else
                    return "boolean";
                return 0;
...
static void GetValue(Ent *et, DSpec *dspec, Value *retval, char **eopts)
{
    Entry *ent;
    const char *errs;
...
static int AddValue(ValArr *va, int id, Value *val)
{
    int nu;
...
static void CopyValues(ValArr *va, Sect *sec, DSpec *dspec, int isconfig)
{
    Value val;
    int i;
    ...
    for(i = 0; i < sec->numents; i++)
    {
        ...
        {
            if(((sec->ents[i].id & C_MTYPE_MASK) == C_ENUM) || !sec->ents[i].ptr || !((int (*)(Value *))sec->ents[i].ptr)(&val))
            {
                GetValue(sec->ents + i, dspec, &val, (char **)sec->ents[i].ptr);
            }
...
static void SendValues(ValArr *va)
{
    Value *cst;
    int i, nu;
...
#ifdef XDMCP
static char *ReadWord(File *file, int *len, int EOFatEOL)
{
...
static int HasGlobCharacters(char *s)
{
    for(;;)
        switch(*s++)
        {
...
static int ParseHost(int *nHosts, HostEntry ***hostPtr, int *nChars, char *hostOrAlias, int len, int parse)
{
...
#endif
        if(!((**hostPtr)->entry.displayAddress.hostAddress = Malloc(addr_len)))
        {
#if defined(IPv6) && defined(AF_INET6)
            freeaddrinfo(ai);
#endif
...
static int patternMatch(const char *string, const char *pattern)
{
    int p, s;
...
static int checkHostlist(HostEntry **hosts, int nh, AliasEntry *aliases, int na, int depth, int flags)
{
    HostEntry *h;
    AliasEntry *a;
    int hn, an, am;
        ...
        if(h->type == HOST_ALIAS)
        {
            ...
            for(a = aliases, an = 0, am = 0; an < na; an++, a = a->next)
                if(patternMatch(a->name, h->entry.aliasPattern))
                {
                    ...
                    {
                        LogError(
                            "XDMCP ACL: alias %%%s with unresolved hosts "
                            "in denying rule\n",
                            a->name);
                        return 1;
                    }
                    if(checkHostlist(a->pHosts, a->nhosts, aliases, na, depth + 1, flags))
                        return 1;
                }
            if(!am)
            {
                ...
                {
                    LogError(
                        "XDMCP ACL: unresolved alias pattern %%%s "
                        "in denying rule\n",
                        h->entry.aliasPattern);
                    return 1;
                }
...
static void ReadAccessFile(const char *fname)
{
    ...
    while((displayOrAlias = ReadWord(&file, &len, FALSE)))
    {
        if(*displayOrAlias == ALIAS_CHARACTER)
        {
            ...
            {
                if(ParseHost(&nHosts, &hostPtr, &nChars, hostOrAlias, len, PARSE_NO_BCAST))
                    (*aliasPtr)->nhosts++;
                else
                    (*aliasPtr)->hasBad = 1;
            }
        ...
        else if(!strcmp(displayOrAlias, LISTEN_STRING))
        {
            ...
            if(!(hostOrAlias = ReadWord(&file, &len, TRUE)) || !strcmp(hostOrAlias, WILDCARD_STRING)
               || !ParseHost(&nHosts, &hostPtr, &nChars, hostOrAlias, len, PARSE_NO_BCAST | PARSE_NO_PAT | PARSE_NO_ALIAS))
            {
                (*listenPtr)->iface = -1;
            }
            ...
            while((hostOrAlias = ReadWord(&file, &len, TRUE)))
            {
                if(ParseHost(&nHosts, &hostPtr, &nChars, hostOrAlias, len, PARSE_NO_BCAST | PARSE_NO_PAT | PARSE_NO_ALIAS))
                    (*listenPtr)->nmcasts++;
            }
        ...
        else
        {
            ...
            (*acPtr)->nentries = 1;
            if(!ParseHost(&nHosts, &hostPtr, &nChars, displayOrAlias, len, PARSE_NO_BCAST))
            {
                bad = 1;
            ...
            while((hostOrAlias = ReadWord(&file, &len, TRUE)))
            {
                ...
                {
                    if(ParseHost(&nHosts, &hostPtr, &nChars, hostOrAlias, len, PARSE_NO_PAT))
                        (*acPtr)->nhosts++;
                }
            }
            if(!bad)
    ...
    if(!nListens)
    {
        ...
        else
        {
            ...
            if(ParseHost(&nHosts, &hostPtr, &nChars, XDM_DEFAULT_MCAST_ADDR6, sizeof(XDM_DEFAULT_MCAST_ADDR6) - 1, PARSE_ALL))
                (*listenPtr)->nmcasts++;
#endif
            nListens++;
        }
    ...
    for(acl = acList, i = 0; i < nAcls; i++, acl = acl->next)
        if(checkHostlist(acl->pEntries, acl->nentries, aliasList, nAliases, 0, (acl->flags & a_notAllowed) ? CHECK_NOT : 0)
           || checkHostlist(acl->pHosts, acl->nhosts, aliasList, nAliases, 0, CHECK_NO_PAT))
            error = 1;
    ...
    for(i = 0; i < nHosts; i++, hostList = hostList->next)
    {
        ...
        switch(hostList->type)
        {
            ...
            case HOST_PATTERN:
                ...
                break;
            case HOST_ADDRESS:
                GSendArr(hostList->entry.displayAddress.hostAddrLen, hostList->entry.displayAddress.hostAddress);
