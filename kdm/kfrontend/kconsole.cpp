...
KConsole::KConsole(QWidget *_parent) : inherited(_parent), pty(0), notifier(0), fd(-1)
{
    setReadOnly(true);
    setWordWrap(NoWrap);
...
void KConsole::slotData()
{
    ...
    {
        bool as = !verticalScrollBar()->isVisible() || (verticalScrollBar()->value() == verticalScrollBar()->maxValue());
