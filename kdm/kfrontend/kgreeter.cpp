...
public:
    UserListView(QWidget *parent = 0, const char *name = 0) : KListView(parent, name), cachedSizeHint(-1, 0)
    {
...
protected:
    virtual QSize sizeHint() const
    {
        if(!cachedSizeHint.isValid())
        {
            ...
            cachedSizeHint.setWidth(style().pixelMetric(QStyle::PM_ScrollBarExtent) + frameWidth() * 2 + maxw);
        }
...
KGreeter::KGreeter(bool framed)
    : inherited(framed), dName(dname), userView(0), userList(0), nNormals(0), nSpecials(0), curPrev(-1), curSel(-1), prevValid(true), needLoad(false)
{
    ...
    if(_userList)
    {
        ...
        connect(userView, SIGNAL(clicked(QListViewItem *)), SLOT(slotUserClicked(QListViewItem *)));
        connect(userView, SIGNAL(doubleClicked(QListViewItem *)), SLOT(accept()));
    }
    if(_userCompletion)
    ...
    connect(sessMenu, SIGNAL(activated(int)), SLOT(slotSessionSelected(int)));
    insertSessions();
...
public:
    UserListViewItem(UserListView *parent, const QString &text, const QPixmap &pixmap, const QString &username)
        : KListViewItem(parent), login(username)
    {
        setPixmap(0, pixmap);
...
void KGreeter::insertUser(const QImage &default_pix, const QString &username, struct passwd *ps)
{
    if(userList)
    ...
    int dp = 0, nd = 0;
    if(_faceSource == FACE_USER_ONLY || _faceSource == FACE_PREFER_USER)
        dp = 1;
    if(_faceSource != FACE_USER_ONLY && _faceSource != FACE_ADMIN_ONLY)
        nd = 1;
    QImage p;
    do
    {
        dp ^= 1;
        QCString fn = !dp ? QCString(ps->pw_dir) + '/' : QFile::encodeName(_faceDir + '/' + username);
        fn += ".face.icon";
        int fd, ico;
...
public:
    UserList(char **in);
    bool hasUser(const char *str) const
    {
...
void KGreeter::insertUsers()
{
    ...
    if(userView)
    {
        ...
        if(default_pix.size() != ns)
            default_pix = default_pix.convertDepth(32).smoothScale(ns, QImage::ScaleMin);
    }
    if(_showUsers == SHOW_ALL)
    {
        ...
        {
            if(*ps->pw_dir && *ps->pw_shell && (ps->pw_uid >= (unsigned)_lowUserId || !ps->pw_uid && _showRoot) && ps->pw_uid <= (unsigned)_highUserId
               && !noUsers.hasUser(ps->pw_name) && !noUsers.hasGroup(ps->pw_gid))
            {
    ...
    else
    {
        ...
        if(users.hasGroups())
        {
            ...
            for(setpwent(); (ps = getpwent()) != 0;)
            {
                if(*ps->pw_dir && *ps->pw_shell && (ps->pw_uid >= (unsigned)_lowUserId || !ps->pw_uid && _showRoot)
                   && ps->pw_uid <= (unsigned)_highUserId && (users.hasUser(ps->pw_name) || users.hasGroup(ps->pw_gid)))
                {
                    QString username(QFile::decodeName(ps->pw_name));
                    if(!dupes.find(username))
                    {
        ...
        else
        {
            ...
            for(; it != users.users.end(); ++it)
                if((ps = getpwnam((*it).data())) && (ps->pw_uid || _showRoot))
...
void KGreeter::putSession(const QString &type, const QString &name, bool hid, const char *exe)
{
    int prio = exe ? (!strcmp(exe, "default") ? 0 : !strcmp(exe, "custom") ? 1 : !strcmp(exe, "failsafe") ? 3 : 2) : 2;
    ...
    for(char **dit = _sessionsDirs; *dit; ++dit)
    {
        ...
        for(QStringList::ConstIterator it = ents.begin(); it != ents.end(); ++it)
            if((*it).endsWith(".desktop"))
            {
                ...
                putSession(
                    (*it).left((*it).length() - 8), dsk.readEntry("Name"),
                    (dsk.readBoolEntry("Hidden", false) || (dsk.hasKey("TryExec") && KStandardDirs::findExe(dsk.readEntry("TryExec")).isEmpty())),
...
void KGreeter::slotUserClicked(QListViewItem *item)
{
    if(item)
    {
...
void KGreeter::slotSessionSelected(int id)
{
    if(id != curSel)
    {
...
void KGreeter::reject()
{
    verify->reject();
}

void KGreeter::accept()
{
    ...
    KGreeter::pluginSetup()
{
    ...
    if(_preselUser != PRESEL_NONE && verify->entityPresettable())
    {
        if(verify->entitiesLocal())
            ent = _preselUser == PRESEL_PREV ? stsFile->readEntry(dName) : _defaultUser;
        else
            ent = _preselUser == PRESEL_PREV ? stsFile->readEntry(dn) : verify->getConf(0, (pn + ".DefaultEntity").latin1(), QVariant()).toString();
        field = verify->entitiesFielded() ? verify->getConf(0, (pn + ".FocusField").latin1(), QVariant(0)).toInt() : _focusPasswd;
    }
...
void KGreeter::verifyPluginChanged(int id)
{
    curPlugin = id;
    pluginSetup();
}

void KGreeter::verifyClear()
{
...
void KGreeter::verifyOk()
{
    if(_preselUser == PRESEL_PREV && verify->entityPresettable())
        stsFile->writeEntry(verify->entitiesLocal() ? dName : dName + '_' + verify->pluginName(), verify->getEntity());
    if(curSel != -1)
    {
        GSendInt(G_PutDmrc);
        GSendStr("Session");
...
void KGreeter::verifyFailed()
{
    if(needLoad)
        slotLoadPrevWM();
}

void KGreeter::verifySetUser(const QString &user)
{
    curUser = user;
    slotUserEntered();
}
...
KStdGreeter::KStdGreeter() : KGreeter(), clock(0), pixLabel(0)
{
    QBoxLayout *main_box;
    ...
    {
        QLabel *complainLabel = new QLabel(i18n("Warning: this is an unsecured session"), this);
        QToolTip::add(complainLabel, i18n("This display requires no X authorization.\n"
    ...
    if(userView)
        ...
        prec = menuButton;
    KGStdVerify *sverify = new KGStdVerify(this, this, prec, QString::null, pluginList, KGreeterPlugin::Authenticate, KGreeterPlugin::Login);
...
KThemedGreeter::KThemedGreeter() : KGreeter(true), themer(0)
//	, clock( 0 )
{
    ...
    connect(themer, SIGNAL(activated(const QString &)), SLOT(slotThemeActivated(const QString &)));
    ...
    KGThemedVerify *tverify =
        new KGThemedVerify(this, themer, this, prec, QString::null, pluginList, KGreeterPlugin::Authenticate, KGreeterPlugin::Login);
...
bool KThemedGreeter::event(QEvent *e)
{
    if(themer)
...
void KThemedGreeter::pluginSetup()
{
    inherited::pluginSetup();
...
void KThemedGreeter::verifyFailed()
{
    //	goButton->setEnabled( false );
    inherited::verifyFailed();
}

void KThemedGreeter::verifyRetry()
{
    //	goButton->setEnabled( true );
}
...
void KThemedGreeter::updateStatus(bool fail, bool caps, int timedleft)
{
    if(pam_error)
    {
        if(fail)
...
void KThemedGreeter::slotThemeActivated(const QString &id)
{
    if(id == "login_button")
        accept();
...
void KThemedGreeter::slotSessMenu()
{
    sessMenu->popup(mapToGlobal(session_button->rect().center()));
}

void KThemedGreeter::slotActionMenu()
{
    if(system_button)
...
void KThemedGreeter::keyPressEvent(QKeyEvent *e)
{
    inherited::keyPressEvent(e);
    if(!(e->state() & KeyButtonMask) && (e->key() == Key_Return || e->key() == Key_Enter))
        accept();
}
