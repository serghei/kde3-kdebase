...
static int old_scripts, no_old_scripts, old_confs, no_old, no_backup, no_in_notice, use_destdir, mixed_scripts;
static const char *newdir = KDMCONF, *facesrc = KDMDATA "/pics/users", *oldxdm, *oldkde;
...
static void *mrealloc(void *optr, size_t sz)
{
    void *ptr;
...
static char *mstrdup(const char *optr)
{
    char *ptr;
...
static void OutCh_OCA(void *bp, char c)
{
...
static void StrCat(char **strp, const char *fmt, ...)
{
    char *str, *tstr;
    va_list args;
    int el;
...
static int readFile(File *file, const char *fn)
{
    off_t flen;
    int fd;
...
static int isTrue(const char *val)
{
    return !strcmp(val, "true") || !strcmp(val, "yes") || !strcmp(val, "on") || atoi(val);
}

static int mkdirp(const char *name, int mode, const char *what, int existok)
{
...
static void displace(const char *fn)
{
    if(!no_backup)
    {
        char bn[PATH_MAX + 4];
...
static char *locate(const char *exe)
{
    int len;
...
static const char *getfqval(const char *sect, const char *key, const char *defval)
{
    Section *cs;
    Entry *ce;
...
static void putfqval(const char *sect, const char *key, const char *value)
{
    Section *cs, **csp;
    Entry *ce, **cep;
...
static void putval(const char *key, const char *value)
{
    putfqval(csect, key, value);
}


static void wrconf(FILE *f)
{
    Section *cs;
    Entry *ce;
    StrList *sl = 0, *sp;
...
static char *prepName(const char *fn)
{
    const char *tname;
    char *nname;
...
static FILE *Create(const char *fn, int mode)
{
    char *nname;
    FILE *f;
...
static void WriteOut(const char *fn, int mode, time_t stamp, const char *buf, size_t len)
{
    char *nname;
    int fd;
    struct utimbuf utim;
...
static const char *resect(const char *sec, const char *name)
{
...
static int inNewDir(const char *name)
{
    return !memcmp(name, KDMCONF "/", sizeof(KDMCONF));
}

static int inList(StrList *sp, const char *s)
{
    for(; sp; sp = sp->next)
        if(!strcmp(sp->str, s))
            return 1;
...
static void addStr(StrList **sp, const char *s)
{
    for(; *sp; sp = &(*sp)->next)
        if(!strcmp((*sp)->str, s))
            return;
...
static void addedFile(const char *fn)
{
    addStr(&aflist, fn);
}

/* file from old config was parsed */
static void usedFile(const char *fn)
{
    addStr(&uflist, fn);
}

/* file from old config was copied with slight modifications */
static void editedFile(const char *fn)
{
    addStr(&eflist, fn);
}

/* file from old config was copied verbatim */
static void copiedFile(const char *fn)
{
    addStr(&cflist, fn);
}

/* file from old config is still being used */
static void linkedFile(const char *fn)
{
    addStr(&lflist, fn);
}
 ...
 */
static int copyfile(Entry *ce, const char *tname, int mode, int (*proc)(File *))
{
    const char *tptr;
    char *nname;
    File file;
    ...
    if(inList(cflist, ce->value) || inList(eflist, ce->value) || inList(lflist, ce->value))
    {
        rt = 1;
        goto doret;
    }
...
static void dlinkfile(const char *name)
{
    File file;
...
static void linkfile(Entry *ce)
{
    if(ce->written && *ce->value)
        dlinkfile(ce->value);
}

static void writefile(const char *tname, int mode, const char *cont)
{
...
static void handBgCfg(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active) /* can be only the X-*-Greeter one */
        writefile(def_BackgroundCfg, 0644, background ? background : def_background);
...
#ifdef HAVE_VTS
static char *mem_mem(char *mem, int lmem, const char *smem, int lsmem)
{
...
static void getInitTab(void)
{
    ...
    if(readFile(&it, "/etc/inittab"))
    {
        ...
        for(p = it.buf; p < it.eof; p = eol + 1)
        {
            ...
            if(*p != '#')
            {
                if((ep = mem_mem(p, eol - p, " tty", 4)) && ep < eol && isdigit(*ep))
                {
                    if(ep + 1 == eol || isspace(*(ep + 1)))
                        tty = *ep - '0';
                    else if(isdigit(*(ep + 1)) && (ep + 2 == eol || isspace(*(ep + 2))))
                        tty = (*ep - '0') * 10 + (*(ep + 1) - '0');
                    else
                        continue;
                    TTYmask |= 1 << (tty - 1);
...
static char *ReadWord(File *file, int EOFatEOL)
{
...
static const char *joinArgs(StrList *argv)
{
    StrList *av;
    const char *s, *rs;
    char *str;
...
static int parseDisplayType(const char *string, const char **atPos)
{
    ...
    *atPos = 0;
    ...
    {
        if(!memcmp(d->name, string, d->len) && (!string[d->len] || string[d->len] == '@'))
        {
...
static void absorb_xservers(const char *sect ATTR_UNUSED, char **value)
{
...
bustd:
    ...
    *serverPtr = 0;
    ...
    sdpys = rdpys = 0;
    for(se = serverList; se; se = se->next)
        StrCat(se->reserve ? &rdpys : &sdpys, se->class2 ? ",%s_%s" : ",%s", se->name, se->class2);
...
#ifdef HAVE_VTS
static void upd_servervts(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active)
...
static void upd_consolettys(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active)
    {            /* there is only the Global one */
#ifdef __linux__ /* XXX actually, sysvinit */
...
#ifdef XDMCP
static void cp_keyfile(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active) /* there is only the Global one */
        return;
    if(old_confs)
        linkfile(ce);
    else if(!copyfile(ce, "kdmkeys", 0600, 0))
        ce->active = 0;
}

static void mk_xaccess(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active) /* there is only the Global one */
...
static void mk_willing(Entry *ce, Section *cs ATTR_UNUSED)
{
    const char *fname;
...
static void cp_resources(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active) /* the X-*-Greeter one */
        return;
    if(old_confs)
        linkfile(ce);
    else if(!copyfile(ce, ce->value, 0644, 0 /*edit_resources*/))
        ce->active = 0;
}

static int delstr(File *fil, const char *pat)
{
    char *p, *pp, *bpp;
    const char *pap, *paap;
...
static int edit_setup(File *file)
{
    int chg = delstr(file,
                     "\n"
                     "(\n"
...
static void mk_setup(Entry *ce, Section *cs)
{
...
static int edit_startup(File *file)
{
    int chg1 = 0, chg2 = 0;

    if(mod_usebg && (delstr(file,
                            "\n"
                            "PIDFILE=/var/run/kdmdesktop-$DISPLAY.pid\n"
                            "if [[] -f $PIDFILE ] ; then\n"
                            "	   kill `cat $PIDFILE`\n"
...
static void mk_startup(Entry *ce, Section *cs)
{
    setsect(cs->name);
    if(old_scripts || mixed_scripts)
        linkfile(ce);
...
static int edit_reset(File *file)
{
    return
#ifdef _AIX
        delstr(file,
...
static void mk_reset(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(old_scripts || mixed_scripts)
        linkfile(ce);
    else
...
static void mk_session(Entry *ce, Section *cs ATTR_UNUSED)
{
    char *def_session;
    const char *tmpf;

    if((old_scripts || (ce->active && inNewDir(ce->value))) && oldver >= 0x202)
        linkfile(ce);
    else
    {
        tmpf = locate("mktemp") ? "`mktemp /tmp/xsess-env-XXXXXX`" : locate("tempfile") ? "`tempfile`" : "$HOME/.xsession-env-$DISPLAY";
...
static void upd_language(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!strcmp(ce->value, "C"))
        ce->value = (char *)"en_US";
}

static void upd_guistyle(Entry *ce, Section *cs ATTR_UNUSED)
{
...
static void upd_showusers(Entry *ce, Section *cs)
{
...
static void upd_minshowuid(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active)
    {
        ce->value = defminuid;
...
static void upd_maxshowuid(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active)
    {
        ce->value = defmaxuid;
...
static void upd_hiddenusers(Entry *ce, Section *cs ATTR_UNUSED)
{
    char *nv;
    ...
    for(;;)
    {
        ...
        if((pw = getpwnam(nbuf)))
        {
            if(!pw->pw_uid || (pw->pw_uid >= minuid && pw->pw_uid <= maxuid))
            {
                if(nv)
                    StrCat(&nv, ",%s", nbuf);
                else
...
static void upd_forgingseed(Entry *ce, Section *cs ATTR_UNUSED)
{
    if(!ce->active)
    {
...
static void upd_fifodir(Entry *ce, Section *cs ATTR_UNUSED)
{
    const char *dir;
    struct stat st;
...
static void upd_datadir(Entry *ce, Section *cs ATTR_UNUSED)
{
    char *oldsts, *newsts;
    const char *dir;
...
static void upd_facedir(Entry *ce, Section *cs ATTR_UNUSED)
{
    ...
    if(mkdirp(dir, 0755, "user face", 0))
    {
        ...
        if(oldkde)
        {
            ...
            while((pw = getpwent()))
                ...
                {
                    ASPrintf(&oldpic, "%s/../apps/kdm/pics/users/%s.png", oldkde, pw->pw_name);
...
static void mkdefconf(void)
{
    ...
    for(csp = &config, sc = 0; sc < as(dAllSects); csp = &(cs->next), sc++)
    {
        ...
        *csp = cs;
        ...
        for(cep = &(cs->ents), ec = 0; ec < dAllSects[sc].nents; cep = &(ce->next), ec++)
        {
            ce = mcalloc(sizeof(*ce));
            *cep = ce;
...
static RSection *ReadConf(const char *fname)
{
    char *nstr;
    ...
    for(s = file.buf, line = 0, cursec = 0, sectmoan = 1; s < file.eof; s++)
    {
        ...
        if(*s == '[')
        {
            ...
            for(cursec = rootsec; cursec; cursec = cursec->next)
                if(!memcmp(nstr, cursec->name, nlen) && !cursec->name[nlen])
                {
...
static int mergeKdmRcOld(const char *path)
{
    char *p;
    struct stat st;
...
static void applydefs(FDefs *chgdef, int ndefs, const char *path)
{
    char *p;
    int i;

    for(i = 0; i < ndefs; i++)
        if(!getfqval(chgdef[i].sect, chgdef[i].key, 0) && (!chgdef[i].cond || chgdef[i].cond()))
        {
...
static FDefs kdmdefs_eq_22[] = {
    {"General", "PidFile", "/var/run/xdm.pid", 0}, {"X-*-Core", "Setup", "%s/kdm/Xsetup", 0},     {"X-*-Core", "Startup", "%s/kdm/Xstartup", 0},
    {"X-*-Core", "Reset", "%s/kdm/Xreset", 0},     {"X-*-Core", "Session", "%s/kdm/Xsession", 0},
};

#ifdef XDMCP
static int if_xdmcp(void)
{
...
static int is22conf(const char *path)
{
    char *p;
    const char *val;
    int i, sl;
...
#ifdef XDMCP
static void P_EnableChooser(const char *sect ATTR_UNUSED, char **value)
{
    *value = (char *)(isTrue(*value) ? "DefaultLocal" : "LocalOnly");
}
#endif

static void P_UseLilo(const char *sect ATTR_UNUSED, char **value)
{
...
static int mergeKdmRcNewer(const char *path)
{
    char *p;
                    ...
                    for(ce = cs->ents; ce; ce = ce->next)
                    {
                        for(j = 0; j < kupsects[i].nents; j++)
                            if(!strcmp(ce->key, kupsects[i].ents[j].okey))
                            {
                                ...
                                {
                                    sec = sname;
                                    sprintf(sname, "%.*s-%s", cp - cs->name, cs->name, kupsects[i].ents[j].nsec);
                                }
...
#endif
    if(!*(cp = getfqval("General", "ConfigVersion", "")))
    { /* < 3.1 */
        mod_usebg = 1;
        if(is22conf(path))
        {
...
static void handleXdmVal(const char *dpy, const char *key, char *value, const XResEnt *ents, int nents)
{
    const char *kname;
    int i;
    ...
    for(i = 0; i < nents; i++)
        if(!strcmp(key, ents[i].xname) || (key[0] == toupper(ents[i].xname[0]) && !strcmp(key + 1, ents[i].xname + 1)))
        {
            ...
            {
                kname = knameb;
                sprintf(knameb, "%c%s", toupper(ents[i].xname[0]), ents[i].xname + 1);
            }
            if(ents[i].func)
...
static void P_List(const char *sect ATTR_UNUSED, char **value)
{
    int is, d, s;
    char *st;
...
static void P_authDir(const char *sect ATTR_UNUSED, char **value)
{
    int l;
    ...
    if((*value)[l - 1] == '/')
        (*value)[--l] = 0;
    if(!strncmp(*value, "/tmp/", 5) || !strncmp(*value, "/var/tmp/", 9))
    {
        printf("Warning: Resetting inappropriate value %s for AuthDir to default\n", *value);
        *value = 0;
        return;
    }
    if((l >= 4 && !strcmp(*value + l - 4, "/tmp")) || (l >= 6 && !strcmp(*value + l - 6, "/xauth")) || (l >= 8 && !strcmp(*value + l - 8, "/authdir"))
       || (l >= 10 && !strcmp(*value + l - 10, "/authfiles")))
        return;
    ASPrintf(value, "%s/authdir", *value);
}

static void P_openDelay(const char *sect, char **value)
{
    putfqval(sect, "ServerTimeout", *value);
}

static void P_noPassUsers(const char *sect, char **value ATTR_UNUSED)
{
    putfqval(sect, "NoPassEnable", "true");
}

static void P_autoUser(const char *sect, char **value ATTR_UNUSED)
{
    putfqval(sect, "AutoLoginEnable", "true");
}

#ifdef XDMCP
static void P_requestPort(const char *sect, char **value)
{
    if(!strcmp(*value, "0"))
    {
        *value = 0;
...
static void P_autoPass(const char *sect ATTR_UNUSED, char **value ATTR_UNUSED)
{
    kdmrcmode = 0600;
}
...
static Bool DumpEntry(XrmDatabase *db ATTR_UNUSED, XrmBindingList bindings, XrmQuarkList quarks, XrmRepresentation *type, XrmValuePtr value,
                      XPointer data ATTR_UNUSED)
{
    ...
    if(*type != XrmQString)
        return False;
    if(*bindings == XrmBindLoosely || strcmp(XrmQuarkToString(*quarks), "DisplayManager"))
        return False;
    bindings++, quarks++;
    if(!*quarks)
        return False;
    if(*bindings != XrmBindLoosely && !quarks[1])
    { /* DM.foo */
    ...
    else if(*bindings != XrmBindLoosely && quarks[1] && *bindings != XrmBindLoosely && !quarks[2])
    { /* DM.foo.bar */
        dpy = dpybuf + 4;
...
#ifdef XDMCP
    {"Xdmcp", "Xaccess", "%s/Xaccess", 0}, {"Xdmcp", "Willing", "", 0},
#endif
    {"X-*-Core", "Setup", "", 0},          {"X-*-Core", "Startup", "", 0}, {"X-*-Core", "Reset", "", 0}, {"X-*-Core", "Session", "", 0},
};

static int mergeXdmCfg(const char *path)
{
    char *p;
    XrmDatabase db;
    ...
    if((db = XrmGetFileDatabase(p)))
    {
        ...
        usedFile(p);
        free(p);
        XrmEnumerateDatabase(db, &empty, &empty, XrmEnumAllLevels, DumpEntry, (XPointer)0);
...
static void fwrapprintf(FILE *f, const char *msg, ...)
{
    char *txt, *ftxt, *line;
    va_list ap;
...
int main(int argc, char **argv)
{
    ...
    for(ap = 1; ap < argc; ap++)
    {
        if(!strcmp(argv[ap], "--help"))
        {
            printf(
                ...
                "    Default is to scan " KDE_CONFDIR
                " and\n"
    ...
    else
    {
        if(oldkde)
        {
            if(!(newer = mergeKdmRcNewer(oldkde)) && !mergeKdmRcOld(oldkde))
                fprintf(stderr, "Cannot read old kdmrc at specified location\n");
        }
        else if(!no_old_kde)
        {
            for(i = 0; i < as(oldkdes); i++)
            {
                if((newer = mergeKdmRcNewer(oldkdes[i])) || mergeKdmRcOld(oldkdes[i]))
                {
                    oldkde = oldkdes[i];
                    break;
                }
        ...
        if(!newer && !no_old_xdm)
        {
            ...
            if(oldxdm)
            {
                if(!mergeXdmCfg(oldxdm))
                    fprintf(stderr, "Cannot read old kdm-config/xdm-config at specified location\n");
            }
            else
    ...
    if(!old_scripts)
    {
        ...
        for(cs = config; cs; cs = cs->next)
            ...
            {
                for(ce = cs->ents; ce; ce = ce->next)
                    if(ce->active && (!strcmp(ce->spec->key, "Setup") || !strcmp(ce->spec->key, "Startup") || !strcmp(ce->spec->key, "Reset")))
                    {
...
#ifdef __linux__
    if(!stat("/etc/debian_version", &st))
    { /* debian */
        defminuid = "1000";
        defmaxuid = "29999";
    }
    else if(!stat("/usr/portage", &st))
    { /* gentoo */
        defminuid = "1000";
        defmaxuid = "65000";
    }
    else if(!stat("/etc/mandrake-release", &st))
    ...
    f = Create(nname, 0644);
    fprintf(f, "This automatically generated configuration consists of the following files:\n");
    fprintf(f, "- " KDMCONF "/kdmrc\n");
    for(fp = aflist; fp; fp = fp->next)
        fprintf(f, "- %s\n", fp->str);
    if(use_destdir && !no_in_notice)
        fwrapprintf(f, "All files destined for " KDMCONF
    ...
    if(uflist || eflist || cflist || lflist)
    {
        ...
        if(uflist)
        {
            fprintf(f, "Information from these files was extracted:\n");
            for(fp = uflist; fp; fp = fp->next)
                fprintf(f, "- %s\n", fp->str);
        }
        if(lflist)
        {
            fprintf(f, "These files were directly incorporated:\n");
            for(fp = lflist; fp; fp = fp->next)
                fprintf(f, "- %s\n", fp->str);
        }
        if(cflist)
        {
            fprintf(f, "These files were copied verbatim:\n");
            for(fp = cflist; fp; fp = fp->next)
                fprintf(f, "- %s\n", fp->str);
        }
        if(eflist)
        {
            fprintf(f, "These files were copied with modifications:\n");
            for(fp = eflist; fp; fp = fp->next)
                fprintf(f, "- %s\n", fp->str);
        }
        if(!no_backup && !use_destdir)
            fprintf(f, "Old files that would have been overwritten were renamed to <oldname>.bak.\n");
    }
    fprintf(f,
