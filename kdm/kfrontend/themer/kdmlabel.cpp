...
KdmLabel::KdmLabel(KdmItem *parent, const QDomNode &node, const char *name) : KdmItem(parent, node, name)
{
    itemType = "label";
...
void KdmLabel::setText(const QString &txt)
{
    label.text = txt;
    update();
}

QSize KdmLabel::sizeHint()
{
...
void KdmLabel::drawContents(QPainter *p, const QRect & /*r*/)
{
    // choose the correct label class
    struct LabelStruct::LabelClass *l = &label.normal;
    if(state == Sactive && label.active.present)
...
void KdmLabel::statusChanged()
{
    KdmItem::statusChanged();
    if(!label.active.present && !label.prelight.present)
        return;
    if((state == Sprelight && !label.prelight.present) || (state == Sactive && !label.active.present))
        return;
    needUpdate();
}

void KdmLabel::update()
{
...
} stocks[] = {{"language", I18N_NOOP("Language")},
...
QString KdmLabel::lookupStock(const QString &stock)
{
...
QString KdmLabel::lookupText(const QString &t)
{
    QString text = t;
