...
KdmItem::KdmItem(KdmItem *parent, const QDomNode &node, const char *name)
    : QObject(parent, name), boxManager(0), fixedManager(0), image(0), myWidget(0), myLayoutItem(0), buttonParent(0)
{
...
void KdmItem::inheritFromButton(KdmItem *button)
{
    if(button)
        buttonParent = button;
...
KdmItem *KdmItem::findNode(const QString &_id) const
{
    if(id == _id)
...
void KdmItem::setWidget(QWidget *widget)
{
...
void KdmItem::widgetGone()
{
    myWidget = 0;
}

void KdmItem::setLayoutItem(QLayoutItem *item)
{
    myLayoutItem = item;
    // XXX hiding not supported - it think it's pointless here
    if(myLayoutItem->widget())
        connect(myLayoutItem->widget(), SIGNAL(destroyed()), SLOT(layoutItemGone()));
    else if(myLayoutItem->layout())
        connect(myLayoutItem->layout(), SIGNAL(destroyed()), SLOT(layoutItemGone()));
}

void KdmItem::layoutItemGone()
{
    myLayoutItem = 0;
}

/* This is called as a result of KdmLayout::update, and directly on the root */
void KdmItem::setGeometry(const QRect &newGeometry, bool force)
{
...
void KdmItem::paint(QPainter *p, const QRect &rect)
{
    if(isHidden())
        return;
...
void KdmItem::mouseEvent(int x, int y, bool pressed, bool released)
{
...
QSize KdmItem::sizeHint()
{
    if(myWidget)
        return myWidget->size();
    if(myLayoutItem)
        return myLayoutItem->sizeHint();
    int w = pos.wType == DTpixel ? kAbs(pos.width) : -1, h = pos.hType == DTpixel ? kAbs(pos.height) : -1;
    return QSize(w, h);
}

QRect KdmItem::placementHint(const QRect &parentRect)
{
    QSize hintedSize = sizeHint();
    QSize boxHint;

    int x = parentRect.left(), y = parentRect.top(), w = parentRect.width(), h = parentRect.height();
    ...
    kdDebug() << "KdmItem::placementHint " << id << " x=" << x << " dx=" << dx << " w=" << w << " y=" << y << " dy=" << dy << " h=" << h << " "
              << parentRect << endl;
    y += dy;
    x += dx;
...
void KdmItem::addChildItem(KdmItem *item)
{
...
void KdmItem::parseAttribute(const QString &s, int &val, enum DataType &dType)
{
    if(s.isEmpty())
        return;
...
void KdmItem::parseFont(const QString &s, QFont &font)
{
...
void KdmItem::parseColor(const QString &s, QColor &color)
{
    if(s.at(0) != '#')
        return;
    bool ok;
...
void KdmItem::setBoxLayout(const QDomNode &node)
{
    if(!boxManager)
...
void KdmItem::setFixedLayout(const QDomNode &node)
{
    if(!fixedManager)
