...
KGVerify::KGVerify(KGVerifyHandler *_handler, KdmThemer *_themer, QWidget *_parent, QWidget *_predecessor, const QString &_fixedUser,
                   const PluginList &_pluginList, KGreeterPlugin::Function _func, KGreeterPlugin::Context _ctx)
    : inherited()
    , coreLock(0)
...
QPopupMenu *KGVerify::getPlugMenu()
{
    // assert( !cont );
    if(!plugMenu)
    {
        ...
        if(np > 1)
        {
            ...
            connect(plugMenu, SIGNAL(activated(int)), SLOT(slotPluginSelected(int)));
            for(uint i = 0; i < np; i++)
...
static void showWidgets(QLayoutItem *li)
{
    QWidget *w;
    QLayout *l;
    ...
    KGVerify::applyPreset()
{
    if(!presEnt.isEmpty())
    {
        Debug("%s->presetEntity(%\"s, %d)\n", pName.data(), presEnt.latin1(), presFld);
    ...
    KGVerify::scheduleAutoLogin(bool initial)
{
    if(timeable)
    {
        Debug("%s->presetEntity(%\"s, -1)\n", pName.data(), _autoLoginUser.latin1(), -1);
...
void KGVerify::setUser(const QString &user)
{
...
void KGVerify::start()
{
    ...
    Debug("%s->start()\n", pName.data());
    greet->start();
    if(!(func == KGreeterPlugin::Authenticate || ctx == KGreeterPlugin::ChangeTok || ctx == KGreeterPlugin::ExChangeTok))
    {
        cont = true;
        handleVerify();
    }
...
void KGVerify::setEnabled(bool on)
{
...
void // private static
    KGVerify::VMsgBox(QWidget *parent, const QString &user, QMessageBox::Icon type, const QString &mesg)
{
    FDialog::box(parent, type, user.isEmpty() ? mesg : i18n("Authenticating %1...\n\n").arg(user) + mesg);
}

static const char *msgs[] = {I18N_NOOP("You are required to change your password immediately (password aged)."),
    ...
    KGVerify::VInfoBox(QWidget *parent, const QString &user, const char *msg)
{
    ...
    if(rx.search(mesg) >= 0)
    {
        ...
        mesg = expire ? i18n("Your account expires tomorrow.", "Your account expires in %n days.", expire) : i18n("Your account expires today.");
    }
    else
    {
        ...
        if(rx.search(mesg) >= 0)
        {
            int expire = rx.cap(1).toInt();
            mesg =
                expire ? i18n("Your password expires tomorrow.", "Your password expires in %n days.", expire) : i18n("Your password expires today.");
        }
    }
        ...
        switch(ret)
        {
            ...
            case V_CHTOK_AUTH:
                Debug(" V_CHTOK_AUTH\n");
                {
                    ...
                    pgs = "classic";
                gotit:
                    KGChTok chtok(parent, user, init(pgs), 0, KGreeterPlugin::AuthChAuthTok, KGreeterPlugin::Login);
                    return chtok.exec();
                }
            case V_MSG_ERR:
                Debug(" V_MSG_ERR\n");
        ...
        if(ret == V_OK)
        {
            ...
            if(!fixedEntity.isEmpty())
            {
                ...
                if(ent != fixedEntity)
                {
                    Debug("%s->failed()\n", pName.data());
                    greet->failed();
                    MsgBox(sorrybox, i18n("Authenticated user (%1) does not match requested user (%2).\n").arg(ent).arg(fixedEntity));
                    goto retry;
                }
            }
            Debug("%s->succeeded()\n", pName.data());
...
void KGVerify::gplugReturnText(const char *text, int tag)
{
...
void KGVerify::gplugReturnBinary(const char *data)
{
    if(data)
    {
...
void KGVerify::gplugSetUser(const QString &user)
{
...
void KGVerify::gplugMsgBox(QMessageBox::Icon type, const QString &text)
{
    Debug("%s: gplugMsgBox(%d, %\"s)\n", pName.data(), type, text.latin1());
    MsgBox(type, text);
}

bool KGVerify::eventFilter(QObject *o, QEvent *e)
{
    switch(e->type())
    {
...
void KGVerify::updateLockStatus()
{
    unsigned int lmask;
    ...
    KGVerify::getConf(void *, const char *key, const QVariant &dflt)
{
    ...
    else
    {
        ...
        for(QStringList::ConstIterator it = _pluginOptions.begin(); it != _pluginOptions.end(); ++it)
...
PluginList KGVerify::init(const QStringList &plugins)
{
    ...
    for(QStringList::ConstIterator it = plugins.begin(); it != plugins.end(); ++it)
    {
        ...
        QString path = KLibLoader::self()->findLibrary(((*it)[0] == '/' ? *it : "kgreet_" + *it).latin1());
        if(path.isEmpty())
        {
...
KGStdVerify::KGStdVerify(KGVerifyHandler *_handler, QWidget *_parent, QWidget *_predecessor, const QString &_fixedUser, const PluginList &_pluginList,
                         KGreeterPlugin::Function _func, KGreeterPlugin::Context _ctx)
    : inherited(_handler, 0, _parent, _predecessor, _fixedUser, _pluginList, _func, _ctx), failedLabelState(0)
{
...
void KGStdVerify::updateStatus()
{
    ...
    if(failedLabelState != nfls)
    {
        ...
        if(nfls < 0)
        {
            ...
            failedLabel->setText(i18n("Automatic login in 1 second...", "Automatic login in %n seconds...", timedLeft));
        }
        else
        {
            switch(nfls)
            {
                ...
                case 2:
                    failedLabel->setPaletteForegroundColor(Qt::black);
                    failedLabel->setText(authTok ? i18n("Change failed") : fixedEntity.isEmpty() ? i18n("Login failed")
...
KGThemedVerify::KGThemedVerify(KGVerifyHandler *_handler, KdmThemer *_themer, QWidget *_parent, QWidget *_predecessor, const QString &_fixedUser,
                               const PluginList &_pluginList, KGreeterPlugin::Function _func, KGreeterPlugin::Context _ctx)
    : inherited(_handler, _themer, _parent, _predecessor, _fixedUser, _pluginList, _func, _ctx)
{
    updateLockStatus();
}
    ...
    KGThemedVerify::selectPlugin(int id)
{
    ...
    if(themer && (l = greet->getLayoutItem()))
    {
        if(!(n = themer->findNode("talker")))
            MsgBox(errorbox, i18n("Theme not usable with authentication method '%1'.").arg(i18n(greetPlugins[pluginList[id]].info->name)));
        else
        {
            n->setLayoutItem(l);
            showWidgets(l);
...
void KGThemedVerify::updateStatus()
{
    handler->updateStatus(enabled && failed, enabled && !suspended && capsLocked, timedLeft);
}


KGChTok::KGChTok(QWidget *_parent, const QString &user, const PluginList &pluginList, int curPlugin, KGreeterPlugin::Function func,
                 KGreeterPlugin::Context ctx)
    : inherited(_parent), verify(0)
{
...
void KGChTok::verifyPluginChanged(int)
{
    // cannot happen
}

void KGChTok::verifyOk()
{
    inherited::accept();
}

void KGChTok::verifyFailed()
{
    okButton->setEnabled(false);
    cancelButton->setEnabled(false);
}

void KGChTok::verifyRetry()
{
    okButton->setEnabled(true);
    cancelButton->setEnabled(true);
}

void KGChTok::verifySetUser(const QString &)
{
    // cannot happen
}
...
QXTimer::QXTimer() : inherited(0), left(-1)
{
    connect(&timer, SIGNAL(timeout()), SLOT(slotTimeout()));
}

void QXTimer::start(int msec)
{
    left = msec;
