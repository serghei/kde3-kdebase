...
static void GDebug(const char *fmt, ...)
{
    va_list args;
...
static void disposeSession(dpySpec *sess)
{
    free(sess->display);
    free(sess->from);
    if(sess->user)
...
static int ignoreErrors(Display *dpy ATTR_UNUSED, XErrorEvent *event ATTR_UNUSED)
{
...
static void killWindows(Display *dpy, Window window)
{
    ...
    while(XQueryTree(dpy, window, &root, &parent, &children, &nchildren) && nchildren > 0)
    {
...
static void abortReset(int n ATTR_UNUSED)
{
    longjmp(resetJmp, 1);
}
...
static void pseudoReset(Display *dpy)
{
    int screen;
...
static void syncTimeout(int n ATTR_UNUSED)
{
    longjmp(syncJump, 1);
}
...
static int PingLostIOErr(Display *dpy ATTR_UNUSED)
{
    longjmp(pingTime, 1);
}

static void PingLostSig(int n ATTR_UNUSED)
{
    longjmp(pingTime, 1);
}
...
static int xkb_init(Display *dpy)
{
    ...
    return XkbLibraryVersion(&xkb_lmaj, &xkb_lmin) && XkbQueryExtension(dpy, &xkb_opcode, &xkb_event, &xkb_error, &xkb_lmaj, &xkb_lmin);
}

static unsigned int xkb_modifier_mask_work(XkbDescPtr xkb, const char *name)
{
    int i;
...
static unsigned int xkb_modifier_mask(Display *dpy, const char *name)
{
    XkbDescPtr xkb;
...
static int xkb_get_modifier_state(Display *dpy, const char *name)
{
    unsigned int mask;
    XkbStateRec state;
...
static int xkb_set_modifier(Display *dpy, const char *name, int sts)
{
    unsigned int mask;
...
#ifdef HAVE_XTEST
static int xtest_get_modifier_state(Display *dpy, int key)
{
    XModifierKeymap *map;
...
static void xtest_fake_keypress(Display *dpy, int key)
{
...
void setCursor(Display *mdpy, int window, int shape)
{
    Cursor xcursor;
...
static void sigterm(int n ATTR_UNUSED)
{
    exit(EX_NORMAL);
}
