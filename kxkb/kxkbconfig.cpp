...
static const char *switchModes[SWITCH_POLICY_COUNT] = {"Global", "WinClass", "Window"};
...
bool KxkbConfig::load(int loadMode)
{
    ...
    if((m_useKxkb == false && loadMode == LOAD_ACTIVE_OPTIONS) || loadMode == LOAD_INIT_OPTIONS)
        return true;
    ...
    if(m_stickySwitching == true)
    {
        ...
        else if((int)m_layouts.count() - 1 < m_stickySwitchingDepth)
        {
