...
static const char *X11DirList[] = {XLIBDIR,
                                   "/usr/share/X11/",
                                   "/etc/X11/",
                                   "/usr/local/share/X11/",
                                   "/usr/X11R6/lib/X11/",
...
// Compiler will size array automatically.
static const char *rulesFileList[] = {"xkb/rules/xorg", "xkb/rules/xfree86"};
...
const QString X11Helper::findX11Dir()
{
...
const QString X11Helper::findXkbRulesFile(QString x11Dir, Display *dpy)
{
    QString rulesFile;
    XkbRF_VarDefsRec vd;
    char *tmp = NULL;
...
RulesInfo *X11Helper::loadRules(const QString &file, bool layoutsOnly)
{
    ...
    for(int i = 0; i < xkbRules->layouts.num_desc; ++i)
    {
        ...
        if(m_layoutsClean == true && layoutName.find(NON_CLEAN_LAYOUT_REGEXP) != -1 && layoutName.endsWith("/jp") == false)
        {
...
OldLayouts *X11Helper::loadOldLayouts(const QString &rulesFile)
{
    ...
    if(f.open(IO_ReadOnly))
    {
        ...
        while(!ts.eof())
        {
            ...
            else if(line.find(nonLatinLayoutsTag) == 0)
            {
...
*/
QStringList *X11Helper::getVariants(const QString &layout, const QString &x11Dir, bool oldLayouts)
{
...
QString X11Helper::getWindowClass(WId winId, Display *dpy)
{
    ...
    if((XGetWindowProperty(dpy, w, XA_WM_CLASS, 0L, 256L, 0, XA_STRING, &type_ret, &format_ret, &nitems_ret, &bytes_after_ret, &prop_ret) == Success)
       && (type_ret != None))
    {
