...
bool KXKBApp::setLayout(const LayoutUnit &layoutUnit, int group)
{
    ...
    res = m_extension->setLayout(kxkbConfig.m_model, layoutUnit.layout, layoutUnit.variant, layoutUnit.includeGroup);
    if(res)
...
{
    if(KxkbLabelController::START_MENU_ID <= id && id < KxkbLabelController::START_MENU_ID + (int)kxkbConfig.m_layouts.count())
    {
...
void KXKBApp::windowChanged(WId winId)
{
    ...
    if(m_prevWinId != X11Helper::UNKNOWN_WINDOW_ID)
    {   // saving layout/group from previous window
    ...
    if(winId != X11Helper::UNKNOWN_WINDOW_ID)
    {
        ...
        else if(layoutState.group != group)
        { // we need to change only the group
...
{
    KAboutData about("kxkb", I18N_NOOP("KDE Keyboard Tool"), "1.0", DESCRIPTION, KAboutData::License_LGPL,
