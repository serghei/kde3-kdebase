...
public:
    OptionListItem(OptionListItem *parent, const QString &text, Type tt, const QString &optionName);
    OptionListItem(QListView *parent, const QString &text, Type tt, const QString &optionName);
    ~OptionListItem()
    {
    }
...
void LayoutConfig::layoutSelChanged(QListViewItem *sel)
{
    ...
    if(!m_rules->isSingleGroup(kbdLayout) || kbdLayout.startsWith("us") || kbdLayout.startsWith("en"))
    {
        widget->chkLatin->setEnabled(false);
    }
    else
...
QString LayoutConfig::createOptionString()
{
    ...
    for(QDictIterator< char > it(m_rules->options()); it.current(); ++it)
    {
        ...
        if(option.contains(':'))
        {
            ...
            if(!item)
            {
                kdDebug() << "WARNING: skipping empty group for " << it.currentKey() << endl;
                continue;
            }
...
OptionListItem::OptionListItem(OptionListItem *parent, const QString &text, Type tt, const QString &optionName)
    : QCheckListItem(parent, text, tt), m_OptionName(optionName)
{
}

OptionListItem::OptionListItem(QListView *parent, const QString &text, Type tt, const QString &optionName)
    : QCheckListItem(parent, text, tt), m_OptionName(optionName)
{
}
