...
public:
    KonqPopupMenuGUIBuilder(QPopupMenu *menu) : KXMLGUIBuilder(0)
    {
        m_menu = menu;
    }
...
public:
    KonqPopupMenuPrivate() : m_parentWidget(0), m_itemFlags(KParts::BrowserExtension::DefaultPopupItems)
    {
    }
    QString m_urlTitle;
...
KonqPopupMenu::KonqPopupMenu(KBookmarkManager *mgr, const KFileItemList &items, KURL viewURL, KActionCollection &actions, KNewMenu *newMenu,
                             bool showProperties)
...
KonqPopupMenu::KonqPopupMenu(KBookmarkManager *mgr, const KFileItemList &items, KURL viewURL, KActionCollection &actions, KNewMenu *newMenu,
                             QWidget *parentWidget, bool showProperties)
    : QPopupMenu(parentWidget, "konq_popupmenu")
    , m_actions(actions)
    , m_ownActions(static_cast< QWidget * >(0), "KonqPopupMenu::m_ownActions")
    , m_pMenuNew(newMenu)
    , m_sViewURL(viewURL)
...
KonqPopupMenu::KonqPopupMenu(KBookmarkManager *mgr, const KFileItemList &items, const KURL &viewURL, KActionCollection &actions, KNewMenu *newMenu,
                             QWidget *parentWidget, KonqPopupFlags kpf, KParts::BrowserExtension::PopupFlags flags)
    : QPopupMenu(parentWidget, "konq_popupmenu")
    , m_actions(actions)
    , m_ownActions(static_cast< QWidget * >(0), "KonqPopupMenu::m_ownActions")
    , m_pMenuNew(newMenu)
...
int KonqPopupMenu::insertServicesSubmenus(const QMap< QString, ServiceList > &submenus, QDomElement &menu, bool isBuiltin)
{
    int count = 0;
...
int KonqPopupMenu::insertServices(const ServiceList &list, QDomElement &menu, bool isBuiltin)
{
    static int id = 1000;
    int count = 0;
    ...
    for(; it != list.end(); ++it)
    {
        if((*it).isEmpty())
        {
            if(!menu.firstChild().isNull() && menu.lastChild().toElement().tagName().lower() != "separator")
            {
        ...
        if(isBuiltin || (*it).m_display == true)
        {
            ...
            KAction *act = new KAction(QString((*it).m_strName).replace('&', "&&"), 0, this, SLOT(slotRunService()), &m_ownActions, name);
...
void KonqPopupMenu::setup(KonqPopupFlags kpf)
{
    ...
    bool isLocal = isReallyLocal || m_lstItems.first()->url().protocol() == "media" || m_lstItems.first()->url().protocol() == "system";
    ...
    for(; it.current(); ++it)
    {
        ...
        if(!bTrashIncluded
           && ((url.protocol() == "trash" && url.path().length() <= 1) || url.url() == "system:/trash" || url.url() == "system:/trash/"))
        {
            bTrashIncluded = true;
            isLocal = false;
        }
    ...
    // kdDebug() << "isLocal=" << isLocal << " url=" << url << " isCurrentTrash=" << isCurrentTrash << " isIntoTrash=" << isIntoTrash << "
    // bTrashIncluded=" << bTrashIncluded << endl;
    ...
    if((kpf & ShowProperties) && isKDesktop && !kapp->authorize("editable_desktop_icons"))
    {
    ...
    if(sReading)
    {
        ...
        const QStringList entries =
            KGlobal::dirs()->findAllResources("data", "konqueror/servicemenus/*.desktop", false /* recursive */, true /* unique */);
        ...
        for(; eIt != eEnd; ++eIt)
        {
            ...
            if(cfg.hasKey("X-KDE-ShowIfDcopCall"))
            {
                ...
                {
                    kdWarning() << "Desktop file " << *eIt << " contains an invalid X-KDE-ShowIfDcopCall - the function must take the exact "
                                                              "parameter (KURL::List) and must be specified."
                                << endl;
                    continue; // Be safe.
                }

                if(!kapp->dcopClient()->call(app, object, function.utf8(), dataToSend, replyType, replyData, true, 1000))
                    continue;
            ...
            if((cfg.hasKey("Actions") || cfg.hasKey("X-KDE-GetActionMenu")) && cfg.hasKey("ServiceTypes"))
            {
                ...
                for(QStringList::ConstIterator it = types.begin(); it != types.end() && !ok; ++it)
                {
                    ...
                    if(*it == "all/all" || *it == "allfiles" /*compat with KDE up to 3.0.3*/)
                    {
                        checkTheMimetypes = true;
                    }

                    // next, do we match all files?
                    if(!ok && !isDirectory && *it == "all/allfiles")
                    {
                        checkTheMimetypes = true;
                    }

                    // if we have a mimetype, see if we have an exact or a type globbed match
                    if(!ok && (!m_sMimeType.isEmpty() && *it == m_sMimeType)
                    ...
                    if(checkTheMimetypes)
                    {
                        ...
                        {
                            if(((*itex).right(1) == "*" && (*itex).left((*itex).find('/')) == mimeGroup) || ((*itex) == m_sMimeType))
                            {
                                ok = false;
                                break;
                            }
        ...
        {
            QString constraint =
                "Type == 'Application' and DesktopEntryName != 'kfmclient' and DesktopEntryName != 'kfmclient_dir' and DesktopEntryName != "
                "'kfmclient_html'";
        ...
        if(!isDirectory || isLocal)
        {
            ...
            if(!offers.isEmpty())
            {
                ...
                for(; it != offers.end(); it++)
                {
                    ...
                    act = new KAction(actionName, (*it)->pixmap(KIcon::Small), 0, this, SLOT(slotRunService()), &m_ownActions,
    ...
    int userItemCount = 0;
    if(s.user.count() + s.userSubmenus.count() + s.userPriority.count() + s.userPrioritySubmenus.count() > 1)
    {
    ...
    if(userItemCount > 0 && (s.user.count() > 0 || s.userSubmenus.count() > 0 || s.builtin.count() > 0)
    ...
    {
        act = new KAction(i18n("&Properties"), 0, this, SLOT(slotPopupProperties()), &m_ownActions, "properties");
        addAction(act);
    }

    while(!m_menuElement.lastChild().isNull() && m_menuElement.lastChild().toElement().tagName().lower() == "separator")
    ...
    if(isDirectory && isLocal)
    {
        ...
        {
            addSeparator();
            act = new KAction(i18n("Share"), 0, this, SLOT(slotOpenShareFileDialog()), &m_ownActions, "sharefile");
            addAction(act);
        }
    }
...
void KonqPopupMenu::addPlugins()
{
    ...
    plugin_offers =
        KTrader::self()->query(m_sMimeType.isNull() ? QString::fromLatin1("all/all") : m_sMimeType, "'KonqPopupMenu/Plugin' in ServiceTypes");
    ...
    for(; iterator != end; ++iterator, ++pluginCount)
    {
        ...
        KonqPopupMenuPlugin *plugin = KParts::ComponentFactory::createInstanceFromLibrary< KonqPopupMenuPlugin >(
            QFile::encodeName((*iterator)->library()), this, (*iterator)->name().latin1());
        if(!plugin)
            continue;
...
KonqPopupMenuPlugin::KonqPopupMenuPlugin(KonqPopupMenu *parent, const char *name) : QObject(parent, name)
{
}
