...
inline const char *dcopTypeName(const KonqCommand &)
{
    return "KonqCommand";
}
inline const char *dcopTypeName(const KonqCommand::Stack &)
{
...
public:
    ...
    virtual void kill(bool q)
    {
        KonqUndoManager::self()->stopUndo(true);
        KIO::Job::kill(q);
    }
};
    ...
    : QObject(job, "konqcmdrecorder")
{
    ...
    {
        connect(job, SIGNAL(copyingDone(KIO::Job *, const KURL &, const KURL &, bool, bool)), this,
                SLOT(slotCopyingDone(KIO::Job *, const KURL &, const KURL &, bool, bool)));
        connect(job, SIGNAL(copyingLinkDone(KIO::Job *, const KURL &, const QString &, const KURL &)), this,
                SLOT(slotCopyingLinkDone(KIO::Job *, const KURL &, const QString &, const KURL &)));
    }
...
KonqUndoManager::KonqUndoManager() : DCOPObject("KonqUndoManager")
{
...
void KonqUndoManager::undo()
{
    ...
    if(!fileCleanupStack.isEmpty())
    {
        ...
        if(!KonqOperations::askDeleteConfirmation(fileCleanupStack, KonqOperations::DEL, KonqOperations::FORCE_CONFIRMATION, 0 /* TODO parent */))
            return;
    }
...
void KonqUndoManager::undoStep()
{
    ...
    if(d->m_currentJob)
        connect(d->m_currentJob, SIGNAL(result(KIO::Job *)), this, SLOT(slotResult(KIO::Job *)));
}
...
void KonqUndoManager::undoMovingFiles()
{
    if(!d->m_current.m_opStack.isEmpty())
    {
        ...
        else if(d->m_current.m_type == KonqCommand::MOVE || d->m_current.m_type == KonqCommand::TRASH)
        {
...
{
    stream << op.m_valid << op.m_directory << op.m_renamed << op.m_link << op.m_src << op.m_dst << op.m_target;
    return stream;
}
