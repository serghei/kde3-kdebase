...
KonqBgndDialog::KonqBgndDialog(QWidget *parent, const QString &pixmapFile, const QColor &theColor, const QColor &defaultColor)
    : KDialogBase(parent, "KonqBgndDialog", false, i18n("Background Settings"), Ok | Cancel, Ok, true)
{
    ...
    connect(m_buttonGroup, SIGNAL(clicked(int)), this, SLOT(slotBackgroundModeChanged()));
    ...
    m_buttonColor->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Minimum);
    groupLayout->addWidget(m_buttonColor, 0, 1);

    connect(m_buttonColor, SIGNAL(changed(const QColor &)), this, SLOT(slotColorChanged()));
    ...
    connect(m_comboPicture->comboBox(), SIGNAL(activated(int)), this, SLOT(slotPictureChanged()));
    connect(m_comboPicture, SIGNAL(urlSelected(const QString &)), this, SLOT(slotPictureChanged()));
...
{
    KGlobal::dirs()->addResourceType("tiles", KGlobal::dirs()->kde_default("data") + "konqueror/tiles/");
    ...
    else
    {
        ...
        for(it = list.begin(); it != list.end(); it++)
            m_comboPicture->comboBox()->insertItem(((*it).at(0) == '/') ? // if absolute path
                                                       KURL(*it).fileName()
                                                                        : // then only fileName
                                                       *it);
    }
