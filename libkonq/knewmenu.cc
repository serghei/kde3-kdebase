...
KNewMenu::KNewMenu(KActionCollection *_collec, const char *name) : KActionMenu(i18n("Create New"), "filenew", _collec, name), menuItemsVersion(0)
{
...
KNewMenu::KNewMenu(KActionCollection *_collec, QWidget *parentWidget, const char *name)
    : KActionMenu(i18n("Create New"), "filenew", _collec, name), menuItemsVersion(0)
{
    d = new KNewMenuPrivate;
    d->m_actionCollection = _collec;
...
void KNewMenu::fillMenu()
{
    ...
    for(; templ != s_templatesList->end(); ++templ, ++i)
    {
        if((*templ).entryType != SEPARATOR)
        {
            ...
            if(!bSkip)
            {
                ...
                {
                    KAction *act = new KAction((*templ).text, (*templ).icon, 0, this, SLOT(slotNewDir()), d->m_actionCollection,
                                               QCString().sprintf("newmenu%d", i));
                ...
                {
                    KAction *act = new KAction((*templ).text, (*templ).icon, 0, this, SLOT(slotNewFile()), d->m_actionCollection,
                                               QCString().sprintf("newmenu%d", i));
                    act->setGroup("KNewMenu");
        ...
        { // Separate system from personal templates
    ...
    act->plug(popupMenu());
    if(linkURL)
        linkURL->plug(popupMenu());
    if(linkApp)
        linkApp->plug(popupMenu());
...
void KNewMenu::slotFillTemplates()
{
    ...
    if(!s_pDirWatch)
    {
        ...
        connect(s_pDirWatch, SIGNAL(dirty(const QString &)), this, SLOT(slotFillTemplates()));
        connect(s_pDirWatch, SIGNAL(created(const QString &)), this, SLOT(slotFillTemplates()));
        connect(s_pDirWatch, SIGNAL(deleted(const QString &)), this, SLOT(slotFillTemplates()));
    ...
    for(QStringList::Iterator it = files.begin(); it != files.end(); ++it)
    {
        ...
        if((*it)[0] != '.')
        {
            ...
            if((*it).endsWith("Directory.desktop") || (*it).endsWith("linkProgram.desktop") || (*it).endsWith("linkURL.desktop"))
...
void KNewMenu::slotNewFile()
{
    ...
    else
    {
        ...
        name = KInputDialog::getText(QString::null, entry.comment, text, &ok, d->m_parentWidget);
        if(!ok)
            return;
    }
    ...
    for(; it != popupFiles.end(); ++it)
    {
        ...
        if(m_isURLDesktopFile)
            connect(job, SIGNAL(renamed(KIO::Job *, const KURL &, const KURL &)), SLOT(slotRenamed(KIO::Job *, const KURL &, const KURL &)));
        KURL::List lst;
        lst.append(uSrc);
    ...
    else
    {
        ...
        if(destURL.isLocalFile())
        {
            if(m_isURLDesktopFile)
            {
                ...
                kdDebug(1203) << " destURL=" << destURL.path() << " "
...
KURLDesktopFileDlg::KURLDesktopFileDlg(const QString &textFileName, const QString &textUrl)
    : KDialogBase(Plain, QString::null, Ok | Cancel | User1, Ok, 0L /*parent*/, 0L, true, true, KStdGuiItem::clear())
{
...
void KURLDesktopFileDlg::initDialog(const QString &textFileName, const QString &defaultName, const QString &textUrl, const QString &defaultUrl)
{
    ...
    connect(m_leFileName, SIGNAL(textChanged(const QString &)), SLOT(slotNameTextChanged(const QString &)));
    ...
    connect(m_urlRequester->lineEdit(), SIGNAL(textChanged(const QString &)), SLOT(slotURLTextChanged(const QString &)));
