...
void KFileIVI::invalidateThumb(int state, bool redraw)
{
    ...
    d->icons = QIconSet();
    d->icons.setPixmap(KGlobal::iconLoader()->iconEffect()->apply(d->thumb, KIcon::Desktop, state), QIconSet::Large, mode);
    m_state = state;

    QIconViewItem::setPixmap(d->icons.pixmap(QIconSet::Large, mode), false, redraw);
}
...
void KFileIVI::setPixmapDirect(const QPixmap &pixmap, bool recalc, bool redraw)
{
    ...
    updatePixmapSize();
    QIconViewItem::setPixmap(d->icons.pixmap(QIconSet::Large, mode), recalc, redraw);
}
...
void KFileIVI::setDisabled(bool disabled)
{
    if(m_bDisabled != disabled)
    {
        ...
        setEffect(m_bDisabled ? KIcon::DisabledState : (active ? KIcon::ActiveState : KIcon::DefaultState));
    }
}
...
void KFileIVI::setThumbnailPixmap(const QPixmap &pixmap)
{
    ...
    d->icons = QIconSet();
    d->icons.setPixmap(KGlobal::iconLoader()->iconEffect()->apply(pixmap, KIcon::Desktop, KIcon::DefaultState), QIconSet::Large, QIconSet::Normal);

    m_state = KIcon::DefaultState;

    // Recalc when setting this pixmap!
    updatePixmapSize();
    QIconViewItem::setPixmap(d->icons.pixmap(QIconSet::Large, QIconSet::Normal), true);
}
...
void KFileIVI::setEffect(int state)
{
    ...
    bool haveEffect = effect->hasEffect(KIcon::Desktop, m_state) != effect->hasEffect(KIcon::Desktop, state);
    ...
    if(haveEffect && effect->fingerprint(KIcon::Desktop, m_state) != effect->fingerprint(KIcon::Desktop, state))
    {
        ...
        if(m_bThumbnail)
        {
            if(d->icons.isGenerated(QIconSet::Large, mode))
                d->icons.setPixmap(effect->apply(d->thumb, KIcon::Desktop, state), QIconSet::Large, mode);
        }
        else
        {
            if(d->icons.isGenerated(QIconSet::Large, mode))
                d->icons.setPixmap(m_fileitem->pixmap(m_size, state), QIconSet::Large, mode);
        }
