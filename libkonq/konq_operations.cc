...
KonqOperations::KonqOperations(QWidget *parent) : QObject(parent, "KonqOperations"), m_method(UNKNOWN), m_info(0L), m_pasteInfo(0L)
{
}
...
void KonqOperations::editMimeType(const QString &mimeType)
{
    ...
    KRun::runCommand(keditfiletype + " " + KProcess::quote(mimeType), keditfiletype, keditfiletype /*unused*/);
}
        ...
        switch(method)
        {
            case DEL:
                result = KMessageBox::warningContinueCancelList(
                    widget, i18n("Do you really want to delete this item?", "Do you really want to delete these %n items?", prettyList.count()),
                    prettyList, i18n("Delete Files"), KStdGuiItem::del(), keyName, KMessageBox::Dangerous);
                break;

            case SHRED:
                result = KMessageBox::warningContinueCancelList(
                    widget, i18n("Do you really want to shred this item?", "Do you really want to shred these %n items?", prettyList.count()),
                    prettyList, i18n("Shred Files"), KGuiItem(i18n("Shred"), "editshred"), keyName, KMessageBox::Dangerous);
                break;

            case MOVE:
            default:
                result = KMessageBox::warningContinueCancelList(
                    widget, i18n("Do you really want to move this item to the trash?", "Do you really want to move these %n items to the trash?",
                                 prettyList.count()),
                    prettyList, i18n("Move to Trash"), KGuiItem(i18n("Verb", "&Trash"), "edittrash"), keyName, KMessageBox::Dangerous);
        }
        if(!keyName.isEmpty())
        {
...
void KonqOperations::doDrop(const KFileItem *destItem, const KURL &dest, QDropEvent *ev, QWidget *parent)
{
    ...
    else
    {
        ...
        KonqOperations *op = new KonqOperations(parent);
        KIO::CopyJob *job = KIO::pasteMimeSource(ev, dest, i18n("File name for dropped contents:"), parent);
...
void KonqOperations::doFileCopy()
{
    ...
    bool linkOnly = false;
    if(isDesktopFile && !kapp->authorize("run_desktop_files") && (m_destURL.path(1) == KGlobalSettings::desktopPath()))
    {
        linkOnly = true;
    }
    ...
    else if((((m_info->keybstate & ControlMask) == 0) && ((m_info->keybstate & ShiftMask) == 0)) || linkOnly)
    {
        ...
        if(iconView && iconView->maySetWallpaper() && lst.count() == 1)
        {
            ...
            if((!KImageIO::type(url.path()).isEmpty()) || (KImageIO::isSupported(mime->name(), KImageIO::Reading)) || mime->is("image/svg+xml"))
            {
                bSetWallpaper = true;
            }
        }
    ...
    switch(action)
    {
        case QDropEvent::Move:
            ...
            (void)new KonqCommandRecorder(m_method == TRASH ? KonqCommand::TRASH : KonqCommand::MOVE, lst, m_destURL, job);
        ...
        case QDropEvent::Link:
            ...
            return;
        default:
            kdError(1203) << "Unknown action " << (int)action << endl;
    }
    delete this;
}
...
void KonqOperations::setOperation(KIO::Job *job, int method, const KURL::List & /*src*/, const KURL &dest)
{
    ...
    if(job)
    {
        ...
        if(copyJob && iconView)
        {
            connect(copyJob, SIGNAL(aboutToCreate(KIO::Job *, const QValueList< KIO::CopyInfo > &)), this,
                    SLOT(slotAboutToCreate(KIO::Job *, const QValueList< KIO::CopyInfo > &)));
            connect(this, SIGNAL(aboutToCreate(const QPoint &, const QValueList< KIO::CopyInfo > &)), iconView,
                    SLOT(slotAboutToCreate(const QPoint &, const QValueList< KIO::CopyInfo > &)));
        }
    }
    else // for link
        slotResult(0L);
...
void KonqOperations::newDir(QWidget *parent, const KURL &baseURL)
{
    ...
    name = KInputDialog::getText(i18n("New Folder"), i18n("Enter folder name:"), name, &ok, parent);
    if(ok && !name.isEmpty())
    {
        KURL url;
        if((name[0] == '/') || (name[0] == '~'))
...
KonqMultiRestoreJob::KonqMultiRestoreJob(const KURL::List &urls, bool showProgressInfo)
    : KIO::Job(showProgressInfo), m_urls(urls), m_urlsIterator(m_urls.begin()), m_progress(0)
{
...
void KonqMultiRestoreJob::slotStart()
{
    ...
    if(m_urlsIterator != m_urls.end())
    {
        ...
        KURL new_url = url;
        if(new_url.protocol() == "system" && new_url.path().startsWith("/trash"))
        {
