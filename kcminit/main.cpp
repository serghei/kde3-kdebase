...
static KCmdLineOptions options[] = {{"list", I18N_NOOP("List modules that are run at startup"), 0},
...
KCMInit::KCMInit(KCmdLineArgs *args) : DCOPObject("kcminit")
{
    QCString arg;
    if(args->count() == 1)
    {
    ...
    if(!arg.isEmpty())
    {
        ...
        if(!serv || serv->library().isEmpty() || serv->init().isEmpty())
        {
    ...
    config.setGroup("X11");
    bool multihead = !config.readBoolEntry("disableMultihead", false) && (ScreenCount(qt_xdisplay()) > 1);
...
extern "C" KDE_EXPORT int kdemain(int argc, char *argv[])
{
    ...
    KAboutData aboutData("kcminit", I18N_NOOP("KCMInit"), "", I18N_NOOP("KCMInit - runs startups initialization for Control Modules."));
