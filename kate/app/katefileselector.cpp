...
KateFileSelectorToolBarParent::KateFileSelectorToolBarParent(QWidget *parent) : QFrame(parent), m_tb(0)
{
}
...
KateFileSelector::KateFileSelector(KateMainWindow *mainWindow, KateViewManager *viewManager, QWidget *parent, const char *name)
    : QVBox(parent, name), mainwin(mainWindow), viewmanager(viewManager)
{
    ...
    connect(dir, SIGNAL(viewChanged(KFileView *)), this, SLOT(selectorViewChanged(KFileView *)));
    ...
    // bookmarks action!
    KActionMenu *acmBookmarks = new KActionMenu(i18n("Bookmarks"), "bookmark", mActionCollection, "bookmarks");
    ...
    acSyncDir = new KAction(i18n("Current Document Folder"), "curfiledir", 0, this, SLOT(setActiveDocumentDir()), mActionCollection, "sync_dir");
    ...
    connect(viewmanager, SIGNAL(viewChanged()), this, SLOT(kateViewChanged()));
    ...
    // whatsthis help
    QWhatsThis::add(cmbPath, i18n("<p>Here you can enter a path for a folder to display."
    ...
    QWhatsThis::add(filter, i18n("<p>Here you can enter a name filter to limit which files are displayed."
                                 "<p>To clear the filter, toggle off the filter button to the left."
                                 "<p>To reapply the last filter used, toggle on the filter button."));
    QWhatsThis::add(btnFilter, i18n("<p>This button clears the name filter when toggled off, or "
...
public:
    ActionLBItem(QListBox *lb = 0, const QPixmap &pm = QPixmap(), const QString &text = QString::null, const QString &str = QString::null)
        : QListBoxPixmap(lb, pm, text), _str(str){};
    QString idstring()
    {
        return _str;
    };
...
KFSConfigPage::KFSConfigPage(QWidget *parent, const char *name, KateFileSelector *kfs)
    : Kate::ConfigPage(parent, name), fileSelector(kfs), m_changed(false)
{
    ...
    */
    QString lhwt(
        i18n("<p>Decides how many locations to keep in the history of the location "
             "combo box."));
    QWhatsThis::add(lbPathHist, lhwt);
    QWhatsThis::add(sbPathHistLength, lhwt);
    QString fhwt(
        i18n("<p>Decides how many filters to keep in the history of the filter "
             "combo box."));
    QWhatsThis::add(lbFilterHist, fhwt);
    QWhatsThis::add(sbFilterHistLength, fhwt);
    QString synwt(
        i18n("<p>These options allow you to have the File Selector automatically "
    ...
    QWhatsThis::add(cbSesLocation, i18n("<p>If this option is enabled (default), the location will be restored "
    ...
    QWhatsThis::add(cbSesFilter, i18n("<p>If this option is enabled (default), the current filter will be "
...
void KFSConfigPage::init()
{
    ...
    allActions << "up"
               ...
               << "detailed view" /*<< "view menu" <<
                "show hidden" << "properties"*/ << "bookmarks"
               << "sync_dir";
    QRegExp re("&(?=[^&])");
    KAction *ac;
    QListBox *lb;
