...
KateExternalTool::KateExternalTool(const QString &name, const QString &command, const QString &icon, const QString &tryexec,
                                   const QStringList &mimetypes, const QString &acname, const QString &cmdname, int save)
    : name(name), command(command), icon(icon), tryexec(tryexec), mimetypes(mimetypes), acname(acname), cmdname(cmdname), save(save)
{
    // if ( ! tryexec.isEmpty() )
    hasexec = checkExec();
}
...
void KateExternalToolsCommand::reload()
{
    ...
    for(QStringList::Iterator it = tools.begin(); it != tools.end(); ++it)
    {
        ...
        KateExternalTool t = KateExternalTool(config.readEntry("name", ""), config.readEntry("command", ""), config.readEntry("icon", ""),
                                              config.readEntry("executable", ""), config.readListEntry("mimetypes"), config.readEntry("acname", ""),
...
bool KateExternalToolsCommand::exec(Kate::View *view, const QString &cmd, QString &)
{
    ...
    KateExternalToolsMenuAction *a = dynamic_cast< KateExternalToolsMenuAction * >(dmw->action("tools_external"));
    if(!a)
        return false;
...
KateExternalToolAction::KateExternalToolAction(QObject *parent, const char *name, KateExternalTool *t) : KAction(parent, name), tool(t)
{
    setText(t->name);
    if(!t->icon.isEmpty())
...
void KateExternalToolAction::slotRun()
{
    ...
    {
        KMessageBox::sorry((KateMainWindow *)parent()->parent(), i18n("Failed to expand the command '%1'.").arg(cmd), i18n("Kate External Tools"));
        return;
    }
...
KateExternalToolsMenuAction::KateExternalToolsMenuAction(const QString &text, QObject *parent, const char *name, KateMainWindow *mw)
    : KActionMenu(text, parent, name), mainwindow(mw)
{
...
void KateExternalToolsMenuAction::reload()
{
    ...
    for(QStringList::Iterator it = tools.begin(); it != tools.end(); ++it)
    {
        ...
        KateExternalTool *t = new KateExternalTool(
            config->readEntry("name", ""), config->readEntry("command", ""), config->readEntry("icon", ""), config->readEntry("executable", ""),
            config->readListEntry("mimetypes"), config->readEntry("acname", ""), config->readEntry("cmdname", ""), config->readNumEntry("save", 0));
...
public:
    ToolItem(QListBox *lb, const QPixmap &icon, KateExternalTool *tool) : QListBoxPixmap(lb, icon, tool->name), tool(tool)
    {
        ;
    }
...
KateExternalToolServiceEditor::KateExternalToolServiceEditor(KateExternalTool *tool, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, i18n("Edit External Tool"), KDialogBase::Ok | KDialogBase::Cancel), tool(tool)
{
    ...
    QWhatsThis::add(leName, i18n("The name will be displayed in the 'Tools->External' menu"));
    ...
    QWhatsThis::add(teCommand, i18n("<p>The script to execute to invoke the tool. The script is passed "
    ...
    QWhatsThis::add(leExecutable, i18n("The executable used by the command. This is used to check if a tool "
    ...
    QWhatsThis::add(leMimetypes, i18n("A semicolon-separated list of mime types for which this tool should "
    ...
    QWhatsThis::add(btnMTW, i18n("Click for a dialog that can help you creating a list of mimetypes."));
    ...
    if(tool)
        cmbSave->setCurrentItem(tool->save);
    QWhatsThis::add(cmbSave, i18n("You can elect to save the current or all [modified] documents prior to "
    ...
    QWhatsThis::add(leCmdLine, i18n("If you specify a name here, you can invoke the command from the view "
...
KateExternalToolsConfigWidget::KateExternalToolsConfigWidget(QWidget *parent, const char *name) : Kate::ConfigPage(parent, name), m_changed(false)
{
    ...
    QWhatsThis::add(lbTools, i18n("This list shows all the configured tools, represented by their menu text."));
...
void KateExternalToolsConfigWidget::reload()
{
    ...
    for(QStringList::Iterator it = tools.begin(); it != tools.end(); ++it)
    {
        ...
        else
        {
            ...
            KateExternalTool *t = new KateExternalTool(config->readEntry("name", ""), config->readEntry("command", ""), config->readEntry("icon", ""),
                                                       config->readEntry("executable", ""), config->readListEntry("mimetypes"),
                                                       config->readEntry("acname"), config->readEntry("cmdname"), config->readNumEntry("save", 0));
...
void KateExternalToolsConfigWidget::slotNew()
{
    ...
    if(editor.exec())
    {
        KateExternalTool *t = new KateExternalTool(editor.leName->text(), editor.teCommand->text(), editor.btnIcon->icon(),
                                                   editor.leExecutable->text(), QStringList::split(QRegExp("\\s*;\\s*"), editor.leMimetypes->text()));
...
{
    if(!dynamic_cast< ToolItem * >(lbTools->selectedItem()))
        return;
