...
KateApp::KateApp(KCmdLineArgs *args) : KApplication(), m_args(args), m_shouldExit(false)
{
...
bool KateApp::openURL(const KURL &url, const QString &encoding, bool isTempFile)
{
    ...
    if(noDir)
    {
        ...
        KMessageBox::sorry(mainWindow, i18n("The file '%1' could not be opened: it is not a normal file, it is a folder.").arg(url.url()));
