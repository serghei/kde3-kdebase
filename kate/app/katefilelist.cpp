...
public:
    ToolTip(QWidget *parent, KateFileList *lv) : QToolTip(parent), m_listView(lv)
    {
    }
    virtual ~ToolTip(){};
...
// BEGIN KateFileList
KateFileList::KateFileList(KateMainWindow *main, KateViewManager *_viewManager, QWidget *parent, const char *name)
    : KListView(parent, name), m_sort(KateFileList::sortByID)
{
    m_main = main;
    ...
    connect(this, SIGNAL(selectionChanged(QListViewItem *)), this, SLOT(slotActivateView(QListViewItem *)));
    connect(viewManager, SIGNAL(viewChanged()), this, SLOT(slotViewChanged()));
    connect(this, SIGNAL(contextMenuRequested(QListViewItem *, const QPoint &, int)), this, SLOT(slotMenu(QListViewItem *, const QPoint &, int)));
}
...
void KateFileList::setupActions()
{
    ...
    sortAction = new KSelectAction(i18n("Sort &By"), 0, m_main->actionCollection(), "filelist_sortby");
    QStringList l;
...
void KateFileList::slotDocumentCreated(Kate::Document *doc)
{
    ...
    connect(doc, SIGNAL(modifiedOnDisc(Kate::Document *, bool, unsigned char)), this,
...
KateFileListItem::KateFileListItem(QListView *lv, Kate::Document *_doc)
    : QListViewItem(lv, _doc->docName()), doc(_doc), m_viewhistpos(0), m_edithistpos(0), m_docNumber(_doc->documentNumber())
{
}
