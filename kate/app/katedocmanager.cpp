...
KateDocManager::KateDocManager(QObject *parent) : QObject(parent), m_saveMetaInfos(true), m_daysMetaInfos(0)
{
...
bool KateDocManager::closeDocument(class Kate::Document *doc, bool closeURL)
{
    ...
    if(closeURL && m_tempFiles.contains(documentNumber))
    {
            ...
            i18n("Delete File?") ) == KMessageBox::Yes*/)
        {
            ...
            kdWarning(13001) << "The supposedly temporary file " << m_tempFiles[documentNumber].first.prettyURL()
...
bool KateDocManager::queryCloseDocuments(KateMainWindow *w)
{
    ...
    for(QPtrListIterator< Kate::Document > it(m_docList); it.current(); ++it)
    {
        ...
        {
            int msgres = KMessageBox::warningYesNoCancel(w, i18n("<p>The document '%1' has been modified, but not saved."
            ...
            {
                KEncodingFileDialog::Result r = KEncodingFileDialog::getSaveURLAndEncoding(KTextEditor::encodingInterface(doc)->encoding(),
                                                                                           QString::null, QString::null, w, i18n("Save As"));
...
void KateDocManager::restoreDocumentList(KConfig *config)
{
    ...
    QProgressDialog *pd = new QProgressDialog(i18n("Reopening files from the last session..."), QString::null, count, 0, "openprog");
