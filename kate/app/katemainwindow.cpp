...
void KateMainWindow::setupActions()
{
    ...
    documentOpenWith->setWhatsThis(
        i18n("Open the current document using another application registered for its file type, or an application of your choice."));
    ...
    // session actions
    new KAction(i18n("Menu entry Session->New", "&New"), "filenew", 0, KateSessionManager::self(), SLOT(sessionNew()), actionCollection(),
                "sessions_new");
...
void KateMainWindow::documentMenuAboutToShow()
{
    ...
    while(item)
    {
        ...
        documentMenu->insertItem(doc->isModified() ? i18n("'document name [*]', [*] means modified", "%1 [*]").arg(name) : name, m_viewManager,
                                 SLOT(activateView(int)), 0, ((KateFileListItem *)item)->documentNumber());
...
void KateMainWindow::slotMail()
{
    ...
    for(; it.current(); ++it)
    {
        ...
        if(doc->url().isEmpty())
        {
            ...
            int r = KMessageBox::questionYesNo(this, i18n("<p>The current document has not been saved, and "
        ...
        if(doc->isModified())
        {
            ...
            int r = KMessageBox::warningYesNoCancel(this, i18n("<p>The current file:<br><strong>%1</strong><br>has been "
                                                               "modified. Modifications will not be available in the attachment."
                                                               "<p>Do you want to save it before sending it?")
                                                              .arg(doc->url().prettyURL()),
...
void KateMainWindow::updateCaption(Kate::Document *doc)
{
    ...
    setCaption(sessName + KStringHandler::lsqueeze(c, 64), m_viewManager->activeView()->getDoc()->isModified());
}
