...
KateSession::KateSession(KateSessionManager *manager, const QString &fileName, const QString &name)
    : m_sessionFileRel(fileName), m_sessionName(name), m_documents(0), m_manager(manager), m_readConfig(0), m_writeConfig(0)
{
    init();
}
...
public:
    KateSessionChooserItem(KListView *lv, KateSession::Ptr s) : QListViewItem(lv, s->sessionName()), session(s)
    {
        QString docs;
