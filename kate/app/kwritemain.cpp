...
KWrite::KWrite(KTextEditor::Document *doc) : m_view(0), m_recentFiles(0), m_paShowPath(0), m_paShowStatusBar(0)
{
    if(!doc)
    {
...
void KWrite::setupActions()
{
    ...
    KAction *a = new KAction(i18n("&New Window"), "window_new", 0, this, SLOT(newView()), actionCollection(), "view_new_view");
    a->setWhatsThis(i18n("Create another view containing the current document"));

    a = new KAction(i18n("Choose Editor..."), 0, this, SLOT(changeEditor()), actionCollection(), "settings_choose_editor");
...
void KWrite::slotOpen()
{
    if(KTextEditor::encodingInterface(m_view->document()))
    {
        KEncodingFileDialog::Result r = KEncodingFileDialog::getOpenURLsAndEncoding(
            KTextEditor::encodingInterface(m_view->document())->encoding(), m_view->document()->url().url(), QString::null, this, i18n("Open File"));
...
void KWrite::slotOpen(const KURL &url)
{
    ...
    {
        KWrite *t = new KWrite();
        if(KTextEditor::encodingInterface(t->m_view->document()))
            KTextEditor::encodingInterface(t->m_view->document())->setEncoding(encoding);
        t->loadURL(url);
    }
    else
    {
        if(KTextEditor::encodingInterface(m_view->document()))
            KTextEditor::encodingInterface(m_view->document())->setEncoding(encoding);
        loadURL(url);
    }
}
...
static KCmdLineOptions options[] = {{"stdin", I18N_NOOP("Read the contents of stdin"), 0},
...
extern "C" KDE_EXPORT int kdemain(int argc, char **argv)
{
    ...
    KAboutData aboutData("kwrite", I18N_NOOP("KWrite"), kWriteVersion.latin1(), I18N_NOOP("KWrite - Text Editor"), KAboutData::License_LGPL_V2,
