...
public:
    KateDocItem(Kate::Document *doc, const QString &status, KListView *lv) : QCheckListItem(lv, doc->url().prettyURL(), CheckBox), document(doc)
    {
        setText(1, status);
        if(!doc->isModified())
            setOn(On);
...
KateMwModOnHdDialog::KateMwModOnHdDialog(DocVector docs, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, i18n("Documents Modified on Disk"), User1 | User2 | User3, User3, false, KGuiItem(i18n("&Ignore"), "fileclose"),
                  KGuiItem(i18n("&Overwrite"), "filesave"), KGuiItem(i18n("&Reload"), "reload"))
{
    setButtonWhatsThis(User1, i18n("Removes the modified flag from the selected documents and closes the "
                                   "dialog if there are no more unhandled documents."));
    setButtonWhatsThis(User2, i18n("Overwrite selected documents, discarding the disk changes and closes the "
                                   "dialog if there are no more unhandled documents."));
    setButtonWhatsThis(User3, i18n("Reloads the selected documents from disk and closes the dialog if there "
    ...
    QLabel *t = new QLabel(i18n("<qt>The documents listed below has changed on disk.<p>Select one "
                                "or more at the time and press an action button until the list is empty.</qt>"),
                           lo1);
    ...
    QWhatsThis::add(btnDiff, i18n("Calculates the difference between the the editor contents and the disk "
...
void KateMwModOnHdDialog::slotPDone(KProcess *p)
{
    ...
    {
        KMessageBox::sorry(this, i18n("The diff command failed. Please make sure that "
