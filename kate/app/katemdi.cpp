...
Splitter::Splitter(Orientation o, QWidget *parent, const char *name) : QSplitter(o, parent, name)
{
}
...
ToggleToolViewAction::ToggleToolViewAction(const QString &text, const KShortcut &cut, ToolView *tv, QObject *parent, const char *name)
    : KToggleAction(text, cut, parent, name), m_tv(tv)
{
...
GUIClient::GUIClient(MainWindow *mw) : QObject(mw), KXMLGUIClient(mw), m_mw(mw)
{
    ...
    m_showSidebarsAction = new KToggleAction(i18n("Show Side&bars"), CTRL | ALT | SHIFT | Key_F, actionCollection(), "kate_mdi_sidebar_visibility");
    m_showSidebarsAction->setCheckedState(i18n("Hide Side&bars"));
    m_showSidebarsAction->setChecked(m_mw->sidebarsVisible());
    connect(m_showSidebarsAction, SIGNAL(toggled(bool)), m_mw, SLOT(setSidebarsVisible(bool)));
...
void GUIClient::registerToolView(ToolView *tv)
{
    ...
    KToggleAction *a = new ToggleToolViewAction(i18n("Show %1").arg(tv->text), sc, tv, actionCollection(), aname.latin1());
...
ToolView::ToolView(MainWindow *mainwin, Sidebar *sidebar, QWidget *parent)
    : QVBox(parent), m_mainWin(mainwin), m_sidebar(sidebar), m_visible(false), persistent(false)
{
}
    ...
    if(ev->type() == QEvent::ContextMenu)
    {
        ...
        if(bt)
        {
            ...
            if(w)
            {
                ...
                p->insertItem(w->persistent ? SmallIconSet("window_nofullscreen") : SmallIconSet("window_fullscreen"),
                              w->persistent ? i18n("Make Non-Persistent") : i18n("Make Persistent"), 10);
                ...
                connect(p, SIGNAL(activated(int)), this, SLOT(buttonPopupActivate(int)));
...
MainWindow::MainWindow(QWidget *parentWidget, const char *name)
    : KParts::MainWindow(parentWidget, name), m_sidebarsVisible(true), m_restoreConfig(0), m_guiClient(new GUIClient(this))
{
...
void MainWindow::setSidebarsVisible(bool visible)
{
    ...
    {
        KMessageBox::information(this, i18n("<qt>You are about to hide the sidebars. With "
    ...
    if(m_restoreConfig->hasGroup(m_restoreGroup))
    {
        ...
        for(unsigned int i = 0; i < m_toolviews.size(); ++i)
        {
            KMultiTabBar::KMultiTabBarPosition newPos = (KMultiTabBar::KMultiTabBarPosition)m_restoreConfig->readNumEntry(
                QString("Kate-MDI-ToolView-%1-Position").arg(m_toolviews[i]->id), m_toolviews[i]->sidebar()->position());
