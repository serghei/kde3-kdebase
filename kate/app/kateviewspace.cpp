...
// BEGIN KateViewSpace
KateViewSpace::KateViewSpace(KateViewSpaceContainer *viewManager, QWidget *parent, const char *name) : QVBox(parent, name), m_viewManager(viewManager)
{
...
{
    disconnect(v->getDoc(), SIGNAL(modifiedChanged()), mStatusBar, SLOT(modifiedChanged()));
...
bool KateViewSpace::showView(uint documentNumber)
{
    ...
    for(; it.current(); --it)
    {
        ...
        {
            if(currentView())
                disconnect(currentView()->getDoc(), SIGNAL(modifiedChanged()), mStatusBar, SLOT(modifiedChanged()));

            Kate::View *kv = it.current();
            connect(kv->getDoc(), SIGNAL(modifiedChanged()), mStatusBar, SLOT(modifiedChanged()));
...
KateVSStatusBar::KateVSStatusBar(KateViewSpace *parent, const char *name) : KStatusBar(parent, name), m_viewSpace(parent)
{
...
{
    m_lineColLabel->setText(i18n(" Line: %1 Col: %2 ").arg(KGlobal::locale()->formatNumber(r + 1, 0)).arg(KGlobal::locale()->formatNumber(c + 1, 0)));
...
void KateVSStatusBar::updateMod(bool mod)
{
    ...
    if(v)
    {
        const KateDocumentInfo *info = KateDocManager::self()->documentInfo(v->getDoc());

        bool modOnHD = info && info->modifiedOnDisc;

        m_modifiedLabel->setPixmap(mod ? info && modOnHD ? m_modmodPm : m_modPm : info && modOnHD ? m_modDiscPm : m_noPm);
    }
}
