...
KateViewManager::KateViewManager(KateMainWindow *parent) : QObject(parent), showFullPath(false), m_mainWindow(parent)
{
    // while init
    m_init = true;
...
void KateViewManager::setupActions()
{
     ...
     * tabbing
     */
    a = new KAction(i18n("New Tab"), "tab_new", 0, this, SLOT(slotNewTab()), m_mainWindow->actionCollection(), "view_new_tab");

    m_closeTab =
        new KAction(i18n("Close Current Tab"), "tab_remove", 0, this, SLOT(slotCloseTab()), m_mainWindow->actionCollection(), "view_close_tab");

    m_activateNextTab = new KAction(i18n("Activate Next Tab"), QApplication::reverseLayout() ? KStdAccel::tabPrev() : KStdAccel::tabNext(), this,
                                    SLOT(activateNextTab()), m_mainWindow->actionCollection(), "view_next_tab");

    m_activatePrevTab = new KAction(i18n("Activate Previous Tab"), QApplication::reverseLayout() ? KStdAccel::tabNext() : KStdAccel::tabPrev(), this,
                                    SLOT(activatePrevTab()), m_mainWindow->actionCollection(), "view_prev_tab");

    /**
     * view splitting
     */
    a = new KAction(i18n("Split Ve&rtical"), "view_right", CTRL + SHIFT + Key_L, this, SLOT(slotSplitViewSpaceVert()),
                    m_mainWindow->actionCollection(), "view_split_vert");

    a->setWhatsThis(i18n("Split the currently active view vertically into two views."));

    a = new KAction(i18n("Split &Horizontal"), "view_bottom", CTRL + SHIFT + Key_T, this, SLOT(slotSplitViewSpaceHoriz()),
    ...
    m_closeView = new KAction(i18n("Cl&ose Current View"), "view_remove", CTRL + SHIFT + Key_R, this, SLOT(slotCloseCurrentViewSpace()),
                              m_mainWindow->actionCollection(), "view_close_current_space");

    m_closeView->setWhatsThis(i18n("Close the currently active splitted view"));

    goNext = new KAction(i18n("Next View"), Key_F8, this, SLOT(activateNextView()), m_mainWindow->actionCollection(), "go_next");
    ...
    connect(b, SIGNAL(clicked()), this, SLOT(slotNewTab()));
    ...
    connect(b, SIGNAL(clicked()), this, SLOT(slotCloseTab()));
...
{
    if(m_viewSpaceContainerList.count() <= 1)
        return;
    if(!m_currentContainer)
        return;
...
{
    if(m_mainWindow->tabWidget()->count() <= 1)
        return;
...
void KateViewManager::slotDocumentOpen()
{
    ...
    if(cv)
    {
        KEncodingFileDialog::Result r = KEncodingFileDialog::getOpenURLsAndEncoding(
            ...
            (cv ? cv->document()->url().url() : QString::null), QString::null, m_mainWindow, i18n("Open File"));
...
void KateViewManager::slotDocumentClose()
{
    ...
    if((KateDocManager::self()->documents() == 1) && !activeView()->getDoc()->isModified() && activeView()->getDoc()->url().isEmpty()
