...
DCOPBrowserItem::DCOPBrowserItem(QListView *parent, DCOPBrowserItem::Type type) : QListViewItem(parent), type_(type)
{
}

DCOPBrowserItem::DCOPBrowserItem(QListViewItem *parent, DCOPBrowserItem::Type type) : QListViewItem(parent), type_(type)
{
}

DCOPBrowserItem::Type DCOPBrowserItem::type() const
{
    return type_;
}
...
DCOPBrowserApplicationItem::DCOPBrowserApplicationItem(QListView *parent, const QCString &app) : DCOPBrowserItem(parent, Application), app_(app)
{
    setExpandable(true);
    ...
    QByteArray data;
    int callId = kapp->dcopClient()->callAsync(app_, mainWindowName.utf8(), "icon()", data, this,
                                               SLOT(retreiveIcon(int, const QCString &, const QByteArray &)));
...
void DCOPBrowserApplicationItem::setOpen(bool o)
{
...
void DCOPBrowserApplicationItem::populate()
{
...
void DCOPBrowserApplicationItem::slotGotWindowName(int /*callId*/, const QCString & /*replyType*/, const QByteArray &replyData)
{
    ...
    if(sl.count() >= 1)
    {
        ...
        {
            QByteArray data;
            kapp->dcopClient()->callAsync(app_, mainWindowName.utf8(), "icon()", data, this,
                                          SLOT(retreiveIcon(int, const QCString &, const QByteArray &)));
        }
    }
}
...
DCOPBrowserInterfaceItem::DCOPBrowserInterfaceItem(DCOPBrowserApplicationItem *parent, const QCString &app, const QCString &object, bool def)
    : DCOPBrowserItem(parent, Interface), app_(app), object_(object)
{
    setExpandable(true);
...
void DCOPBrowserInterfaceItem::setOpen(bool o)
{
    DCOPBrowserItem::setOpen(o);
...
void DCOPBrowserInterfaceItem::populate()
{
    KApplication::setOverrideCursor(waitCursor);
...
DCOPBrowserFunctionItem::DCOPBrowserFunctionItem(DCOPBrowserInterfaceItem *parent, const QCString &app, const QCString &object,
                                                 const QCString &function)
    : DCOPBrowserItem(parent, Function), app_(app), object_(object), function_(function)
{
    setExpandable(false);
    setText(0, QString::fromUtf8(function_));
}

void DCOPBrowserFunctionItem::setOpen(bool o)
{
    DCOPBrowserItem::setOpen(o);
}
...
KDCOPWindow::KDCOPWindow(QWidget *parent, const char *name) : KMainWindow(parent, name)
{
    ...
    connect(mainView->lv, SIGNAL(doubleClicked(QListViewItem *)), SLOT(slotCallFunction(QListViewItem *)));

    connect(mainView->lv, SIGNAL(currentChanged(QListViewItem *)), SLOT(slotCurrentChanged(QListViewItem *)));
    ...
    exeaction = new KAction(i18n("&Execute"), "exec", CTRL + Key_E, this, SLOT(slotCallFunction()), actionCollection(), "execute");

    exeaction->setEnabled(false);
    exeaction->setToolTip(i18n("Execute the selected DCOP call."));

    langmode = new KSelectAction(i18n("Language Mode"), CTRL + Key_M, this, SLOT(slotMode()), actionCollection(), "langmode");
    ...
    slotMode();
    connect(dcopClient, SIGNAL(applicationRegistered(const QCString &)), SLOT(slotApplicationRegistered(const QCString &)));

    connect(dcopClient, SIGNAL(applicationRemoved(const QCString &)), SLOT(slotApplicationUnregistered(const QCString &)));
...
void KDCOPWindow::slotCallFunction(QListViewItem *it)
{
    ...
    {
        KMessageBox::error(this, i18n("No parameters found."), i18n("DCOP Browser Error"));
    ...
    KDialogBase mydialog(this, "KDCOP Parameter Entry", true, QString::null, KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Ok, true);

    mydialog.setCaption(i18n("Call Function %1").arg(fitem->function()));
    ...
    QGridLayout *grid = new QGridLayout(frame, types.count() + 2, 3, 0, KDialog::spacingHint());
    ...
    if(types.count())
    {
        ...
        for(QStringList::ConstIterator it = types.begin(); it != types.end(); ++it)
        {
            ...
            else if(type == "unsigned" || type == "uint" || type == "unsigned int" || type == "Q_UINT32")
            {
                QLabel *n = new QLabel(name, frame);
                grid->addWidget(n, i, 0);
            ...
            else if(type == "ulong" || type == "unsigned long" || type == "unsigned long int" || type == "Q_UINT64")
            {
                QLabel *n = new QLabel(name, frame);
                grid->addWidget(n, i, 0);
    ...
    bool callOk = dcopClient->call(fitem->app(), fitem->object(), normalisedSignature.utf8(), data, replyType, replyData);

    if(!callOk)
    {
        kdDebug() << "call failed( " << fitem->app().data() << ", " << fitem->object().data() << ", " << normalisedSignature << " )" << endl;
        ...
        {
            msg =
                msg.arg(i18n("<p>Application is still registered with DCOP;"
                             " I do not know why this call failed.</p>"));
        }
        else
        {
            msg = msg.arg(i18n("<p>The application appears to have unregistered with DCOP.</p>"));
        }
    ...
    {
        QString coolSignature = QString::fromUtf8(fitem->app()) + "." + QString::fromUtf8(fitem->object()) + "." + normalisedSignature;
...
bool KDCOPWindow::demarshal(QCString &replyType, QDataStream &reply, QListBox *theList)
{
    QStringList ret;
    QPixmap pret;
    bool isValid = true;
    ...
    else if(replyType == "uint" || replyType == "unsigned int" || replyType == "Q_UINT32")
    {
        uint i;
        reply >> i;
        ret << QString::number(i);
    ...
    {
        ret << i18n("Do not know how to demarshal %1").arg(QString::fromUtf8(replyType));
        isValid = false;
    }
...
void KDCOPWindow::slotApplicationRegistered(const QCString &appName)
{
    QListViewItemIterator it(mainView->lv);

    for(; it.current(); ++it)
    {
        DCOPBrowserApplicationItem *item = static_cast< DCOPBrowserApplicationItem * >(it.current());
...
void KDCOPWindow::slotApplicationUnregistered(const QCString &appName)
{
    QListViewItemIterator it(mainView->lv);
...
bool KDCOPWindow::getParameters(const QString &_unNormalisedSignature, QString &normalisedSignature, QStringList &types, QStringList &names)
{
    ...
    normalisedSignature = unNormalisedSignature.left(left) + "(" + types.join(",") + ")";
