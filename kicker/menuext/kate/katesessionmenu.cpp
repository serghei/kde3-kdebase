...
KateSessionMenu::KateSessionMenu(QWidget *parent, const char *name, const QStringList &) : KPanelMenu(parent, name), m_parent(parent)
{
}
...
void KateSessionMenu::slotExec(int id)
{
    ...
    {
        bool ok(false);
        QString name = KInputDialog::getText(i18n("Session Name"), i18n("Please enter a name for the new session"), QString::null, &ok, 0, 0,
                                             new Validator(m_parent));
        if(!ok)
            return;

        if(name.isEmpty()
           && KMessageBox::questionYesNo(0, i18n("An unnamed session will not be saved automatically. "
                                                 "Do you want to create such a session?"),
                                         i18n("Create anonymous session?"), KStdGuiItem::yes(), KStdGuiItem::cancel(),
                                         "kate_session_button_create_anonymous")
                  == KMessageBox::No)
            return;

        if(m_sessions.contains(name)
           && KMessageBox::warningYesNo(0, i18n("You allready have a session named %1. Do you want to open that session?").arg(name),
                                        i18n("Session exists"))
                  == KMessageBox::No)
            return;
        else
