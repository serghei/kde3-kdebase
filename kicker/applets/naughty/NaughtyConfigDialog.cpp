...
NaughtyConfigDialog::NaughtyConfigDialog(const QStringList &items, uint updateInterval, uint threshold, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, i18n("Configuration"), KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Ok, true)
{
    ...
    listBox_ =
        new KEditListBox(i18n("&Programs to Ignore"), v, "naughty config dialog ignore listbox", false, KEditListBox::Add | KEditListBox::Remove);
...
uint NaughtyConfigDialog::updateInterval() const
{
    return uint(kini_updateInterval_->value());
}

uint NaughtyConfigDialog::threshold() const
{
    return uint(kini_threshold_->value());
}

QStringList NaughtyConfigDialog::ignoreList() const
{
    QStringList retval;
