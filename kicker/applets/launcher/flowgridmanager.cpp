...
FlowGridManager::FlowGridManager(QSize p_item_size, QSize p_space_size, QSize p_border_size, QSize p_frame_size, Qt::Orientation orient,
                                 int num_items, Slack slack_x, Slack slack_y)
{
...
{
    if(slack_x == _slackX && slack_y == _slackY)
        return;
    _slackX = slack_x;
    _slackY = slack_y;
    _dirty = true;
...
QPoint FlowGridManager::cell(int index) const
{
    ...
    int x = index % _gridDim.width(), y = index / _gridDim.width();
    return QPoint(x, y);
}
...
int FlowGridManager::indexNearest(QPoint p) const
{
    ...
    int x = c.x() / _gridSpacing.width(), y = c.y() / _gridSpacing.height();
...
{
    if((!_pFrameSize.isValid()) || (!_pItemSize.isValid()) || _numItems == 0)
    {
        _clear();
        return;
    }
    int height = _getHH(_pFrameSize), pItemHeight = _getHH(_pItemSize), pSpaceHeight = _getHH(_pSpaceSize), pBorderHeight = _getHH(_pBorderSize),
...
{
    return csg1.gridDim() == csg2.gridDim() && csg1.origin() == csg2.origin() && csg1.gridSpacing() == csg2.gridSpacing()
