...
public:
    MediumTypeItem(QListView *parent, const QString name, const QString mimetype) : QCheckListItem(parent, name, CheckBox), mMimeType(mimetype)
    {
    }
...
public:
    MediumItem(QListView *parent, const QString name, const KFileItem medium) : QCheckListItem(parent, name, CheckBox), mMedium(medium)
    {
    }
...
QStringList PreferencesDialog::excludedMediumTypes()
{
    ...
    for(MediumTypeItem *it = static_cast< MediumTypeItem * >(mpMediumTypesListView->firstChild()); it;
        it = static_cast< MediumTypeItem * >(it->nextSibling()))
    {
        if(!it->isOn())
            excludedTypes << it->mimeType();
    }
...
QStringList PreferencesDialog::excludedMedia()
{
    ...
    for(MediumItem *it = static_cast< MediumItem * >(mpMediaListView->firstChild()); it; it = static_cast< MediumItem * >(it->nextSibling()))
    {
        if(!it->isOn())
            excluded << it->itemURL();
    }
