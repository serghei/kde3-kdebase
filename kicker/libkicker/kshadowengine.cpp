...
KShadowEngine::KShadowEngine() : m_shadowSettings(new KShadowSettings)
{
}
...
KShadowEngine::KShadowEngine(KShadowSettings *fx) : m_shadowSettings(0L)
{
...
double KShadowEngine::defaultDecay(QImage &source, int i, int j)
{
    ...
    double alphaShadow;
    alphaShadow = (qGray(source.pixel(i - 1, j - 1)) * DIAGONAL_FACTOR + qGray(source.pixel(i - 1, j)) * AXIS_FACTOR
                   + qGray(source.pixel(i - 1, j + 1)) * DIAGONAL_FACTOR + qGray(source.pixel(i, j - 1)) * AXIS_FACTOR + 0
                   + qGray(source.pixel(i, j + 1)) * AXIS_FACTOR + qGray(source.pixel(i + 1, j - 1)) * DIAGONAL_FACTOR
                   + qGray(source.pixel(i + 1, j)) * AXIS_FACTOR + qGray(source.pixel(i + 1, j + 1)) * DIAGONAL_FACTOR)
                  / m_shadowSettings->multiplicationFactor();
...
void KTextShadowEngine::drawText(QPainter &p, const QRect &tr, int tf, const QString &str, const QSize &size)
{
    ...
    if(shadCol.red() + shadCol.green() + shadCol.blue() <= 3 * 256 / 2 - 1)
    {
