...
static KCmdLineOptions options[] = {{"r", I18N_NOOP("Personalizer is restarted by itself"), 0},
                                    {"before-session", I18N_NOOP("Personalizer is running before KDE session"), 0},
                                    KCmdLineLastOption};

int main(int argc, char *argv[])
{
    KAboutData aboutData("kpersonalizer", I18N_NOOP("KPersonalizer"), VERSION, description, KAboutData::License_GPL, "(c) 2001, Ralf Nolden", 0, 0,
                         "nolden@kde.org");
    ...
    if(!args->isSet("r"))
    { // we'll first show the first page
