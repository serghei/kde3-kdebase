...
KStylePage::KStylePage(QWidget *parent, const char *name) : KStylePageDlg(parent, name)
{
    ...
    connect(klv_styles, SIGNAL(selectionChanged()), this, SLOT(slotCurrentChanged()));
...
void KStylePage::saveColors(bool curSettings)
{
    ...
    ////////////////////////////////////////////////////
    // KDE-1.x support
    KSimpleConfig *kconfig = new KSimpleConfig(QDir::homeDirPath() + "/.kderc");
...
void KStylePage::saveIcons(bool curSettings)
{
    ...
    if(curSettings)
    {
        if((kde->isSelected() || platinum->isSelected() || keramik->isSelected()) && icon_crystalsvg_exist)
            theme = "crystalsvg";
        else if((classic->isSelected() || cde->isSelected() || win->isSelected()) && (QColor::numBitPlanes() > 8) && icon_kdeclassic_exist)
            theme = "kdeclassic";
        else if((classic->isSelected() || cde->isSelected() || win->isSelected()) && (QColor::numBitPlanes() <= 8) && icon_Locolor_exist)
            theme = "Locolor";
    }
    // save, what we got
...
void KStylePage::getAvailability()
{
    ...
    kde_keramik_exist = kde_hc_exist = kde_def_exist = cde_exist = kde_plastik_exist = win_exist = platinum_exist = false;
    QStringList styles = QStyleFactory::keys();
    for(QStringList::iterator it = styles.begin(); it != styles.end(); it++)
    {
        if(*it == "Keramik")
            kde_keramik_exist = true;
        else if(*it == "HighColor")
            kde_hc_exist = true;
        else if(*it == "Default")
            kde_def_exist = true;
        else if(*it == "Plastik")
            kde_plastik_exist = true;
        else if(*it == "Motif")
            cde_exist = true;
        else if(*it == "Windows")
            win_exist = true;
            ...
            platinum_exist = true;
        else if(*it == "Light, 3rd revision")
            kde_light_exist = true;
    }
    // and disable the ListItems, if they are not.
    if(!(kde_plastik_exist || kde_light_exist))
        kde->setVisible(false);
    if(!(kde_hc_exist || kde_def_exist))
        classic->setVisible(false);
    if(!kde_keramik_exist || QPixmap::defaultDepth() <= 8)
        keramik->setVisible(false);
    if(!cde_exist)
    ...
    kwin_keramik_exist = kwin_system_exist = kwin_plastik_exist = kwin_default_exist = kwin_win_exist = kwin_cde_exist = kwin_quartz_exist = false;
    KStandardDirs *kstd = KGlobal::dirs();
    ...
    for(QStringList::iterator it = icons.begin(); it != icons.end(); it++)
    {
        ...
        if(icontheme.isHidden() || !icontheme.isValid())
            continue;
        if(*it == "crystalsvg")
            icon_crystalsvg_exist = true;
        else if(*it == "kdeclassic")
            icon_kdeclassic_exist = true;
        else if(*it == "Locolor")
            icon_Locolor_exist = true;
    }
}
...
QPalette KStylePage::createPalette()
{
    ...
    QColorGroup disabledgrp(cc->windowForeground, cc->background, cc->background.light(150), cc->background.dark(), cc->background.dark(120),
                            cc->background.dark(120), cc->windowBackground);
    QColorGroup colgrp(cc->windowForeground, cc->background, cc->background.light(150), cc->background.dark(), cc->background.dark(120),
                       cc->foreground, cc->windowBackground);
