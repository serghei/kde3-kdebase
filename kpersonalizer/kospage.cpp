...
void KOSPage::saveCheckState(bool currSettings)
{
    if(currSettings)
    {
        ...
        if(rb_kde->isChecked())
        {
            writeKDE();
            emit selectedOS(
                "KDE"); // send a signal to be caught by the KStylePage to set the according style by default depending on the OS selection
        }
        else if(rb_unix->isChecked())
        {
            writeUNIX();
            emit selectedOS(
                "CDE"); // send a signal to be caught by the KStylePage to set the according style by default depending on the OS selection
        }
        else if(rb_windows->isChecked())
        {
            writeWindows();
            emit selectedOS(
                "win"); // send a signal to be caught by the KStylePage to set the according style by default depending on the OS selection
        }
        else if(rb_mac->isChecked())
        {
            writeMacOS();
            emit selectedOS(
                "mac"); // send a signal to be caught by the KStylePage to set the according style by default depending on the OS selection
        }
    ...
    { // User has pressed "cancel & dismiss", so his old settings are written back
        writeUserDefaults();
    }
}
...
void KOSPage::slotKDEDescription()
{
    ...
    textview_ospage->setText(
        i18n("<b>Window activation:</b> <i>Focus on click</i><br>"
...
void KOSPage::slotUnixDescription()
{
    ...
    textview_ospage->setText("");
    textview_ospage->setText(
        i18n("<b>Window activation:</b> <i>Focus follows mouse</i><br>"
             "<b>Titlebar double-click:</b> <i>Shade window</i><br>"
             "<b>Mouse selection:</b> <i>Single click</i><br>"
