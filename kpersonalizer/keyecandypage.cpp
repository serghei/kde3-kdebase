...
KEyeCandyPage::KEyeCandyPage(QWidget *parent, const char *name) : KEyeCandyPageDlg(parent, name)
{
    ...
    // Level 1
    desktop_wallpaper = new QCheckListItem(klv_features, i18n("Desktop Wallpaper"), QCheckListItem::CheckBox);
    desktop_window_effects = new QCheckListItem(klv_features, i18n("Window Moving/Resizing Effects"), QCheckListItem::CheckBox);
    desktop_window_moving_contents = new QCheckListItem(klv_features, i18n("Display Contents in Moving/Resizing Windows"), QCheckListItem::CheckBox);
    // Level 2
    backgrounds_konqueror = new QCheckListItem(klv_features, i18n("File Manager Background Picture"), QCheckListItem::CheckBox);
    backgrounds_panel = new QCheckListItem(klv_features, i18n("Panel Background Picture"), QCheckListItem::CheckBox);
    // Level 3
    icon_zooming_panel = new QCheckListItem(klv_features, i18n("Panel Icon Popups"), QCheckListItem::CheckBox);
    icon_effect_gamma = new QCheckListItem(klv_features, i18n("Icon Highlighting"), QCheckListItem::CheckBox);
    icon_mng_animation = new QCheckListItem(klv_features, i18n("File Manager Icon Animation"), QCheckListItem::CheckBox);

    sound_scheme = new QCheckListItem(klv_features, i18n("Sound Theme"), QCheckListItem::CheckBox);

    // Level 4
    ///////////////////////////////////////////////////////////////////////////////////
    /// DEPENDEND ON RESOLUTION; DEFAULT: DON`T USE IN LEVELS
    icon_effect_size_desktop = new QCheckListItem(klv_features, i18n("Large Desktop Icons"), QCheckListItem::CheckBox);
    icon_effect_size_panel = new QCheckListItem(klv_features, i18n("Large Panel Icons"), QCheckListItem::CheckBox);
    /// DEPENDEND ON RESOLUTION; DEFAULT: DON`T USE IN LEVELS
    ///////////////////////////////////////////////////////////////////////////////////
    ...
    // Level 5
    preview_images = new QCheckListItem(klv_features, i18n("Preview Images"), QCheckListItem::CheckBox);
    pushbutton_icons = new QCheckListItem(klv_features, i18n("Icons on Buttons"), QCheckListItem::CheckBox);

    // Level 6
    animated_combo = new QCheckListItem(klv_features, i18n("Animated Combo Boxes"), QCheckListItem::CheckBox);

    // Level 7
    fading_tooltips = new QCheckListItem(klv_features, i18n("Fading Tooltips"), QCheckListItem::CheckBox);

    // Level 8
    preview_text = new QCheckListItem(klv_features, i18n("Preview Text Files"), QCheckListItem::CheckBox);
    // Level 9
    fading_menus = new QCheckListItem(klv_features, i18n("Fading Menus"), QCheckListItem::CheckBox);
    preview_other = new QCheckListItem(klv_features, i18n("Preview Other Files"), QCheckListItem::CheckBox);
...
/** this function enables/disables the window effects for Shading, Minimize and Restore. The contents in moving/resized windows is set in
 * enableWindowContens(bool ) */
...
void KEyeCandyPage::enablePreview(bool currSettings)
{
    ...
    if(prevOther)
    { // remove text/image if not checked
        if(!prevImage)
        {
...
void KEyeCandyPage::enableAntialiasingFonts(bool enable, bool reset)
{
    if(!reset)
    {
        if(((sys->isXfromXFreeInc() && (sys->getXRelease() > 40000000)) || sys->isXfromXOrg()) && sys->getRenderSupport())
        {
...
void KEyeCandyPage::saveCheckState(bool currSettings)
{ // currSettings= true -> take the checkboxes, otherwise take user values set
    if(currSettings)
    {
