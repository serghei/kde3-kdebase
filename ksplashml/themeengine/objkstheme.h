...
public:
    ...
    QString themeEngine() const
    {
        return (mThemeEngine);
    }
    KConfig *themeConfig() const
    {
        return (mThemeConfig);
    }
    ...
    bool managedMode() const
    {
        return (mManagedMode);
    }
    QString icon(int i)
    {
        return (m_icons[i - 1].isNull() ? (QString::null) : m_icons[i - 1]);
    }
    QString text(int i)
    {
        return (m_text[i - 1].isNull() ? (QString::null) : m_text[i - 1]);
    }
    QString locateThemeData(const QString &resource);
    int xineramaScreen() const
    {
        return mXineramaScreen;
    }
