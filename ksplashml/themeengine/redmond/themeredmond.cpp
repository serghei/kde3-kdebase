...
CfgRedmond::CfgRedmond(QWidget *p, KConfig *c) : ThemeEngineConfig(p, c)
{
...
ThemeRedmond::ThemeRedmond(QWidget *parent, const char *name, const QStringList &args) : ThemeEngine(parent, name, args)
{
    _readSettings();
    _initUi();
}
...
void ThemeRedmond::_initUi()
{
    ...
    */
    ...
    if(mShowWelcomeText)
    {
        ...
        {
            mWelcomeTextPosition = QPoint((screen.width() / 2) - fmet.width() - 25, (screen.height() / 2) - (fmet.height() / 2) + fmet.height());
        }
    }
    ...
    if(mShowUsernameText)
    {
        ...
        if(pos == QPoint(0, 0))
        {
            pos = QPoint((screen.width() / 2) + mPixmap.width() + 20, (screen.height() / 2) - (fmet.height() / 2) + fmet.height());
        }
...
void ThemeRedmond::paintEvent(QPaintEvent *pe)
{
    ...
    bitBlt(this, r.x(), r.y(), &mImage, r.x(), r.y(), r.width(), r.height());
    ...
    if(mShowActionText)
    {
        ...
        {
            mMsgPos = QPoint((screen.width() / 2) + mPixmap.width() + 20, (screen.height() / 2) + (int)(fmet.height() * 0.85) + 15);
        }
