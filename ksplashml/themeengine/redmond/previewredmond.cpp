...
void PreviewRedmond::_updateCache()
{
    ...
    QPoint welcomeTextPos(rect().width() / 2 - p.fontMetrics().width(m_welcomeString), rect().height() / 2 + p.fontMetrics().height() / 2);
    ...
    if(m_showUserString)
    {
        ...
        QPoint userTextPos(rect().width() / 2 + m_icon.width() + 20, rect().height() / 2 + p.fontMetrics().height() / 2);
    ...
    {
        QPoint statusTextPos(rect().width() / 2 + m_icon.width() + 20, rect().height() / 2 + (int)(p.fontMetrics().height() * 0.85) + 15);
