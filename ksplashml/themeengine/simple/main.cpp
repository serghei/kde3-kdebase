...
int main(int argc, char *argv[])
{
    ...
    int unused;
    bool use_xinerama = XineramaQueryExtension(dpy, &unused, &unused) && XineramaIsActive(dpy);
    if(use_xinerama)
    {
        int screenCount;
    ...
    Window win = XCreateWindow(dpy, DefaultRootWindow(dpy), sx + (sw - w) / 2, sy + (sh - h) / 2, w, h, 0, CopyFromParent, CopyFromParent,
    ...
    for(;;)
    {
        while(XPending(dpy))
        {
            ...
            if(ev.type == ClientMessage && ev.xclient.window == DefaultRootWindow(dpy) && ev.xclient.message_type == kde_splash_progress)
            {
                // based on ksplash
