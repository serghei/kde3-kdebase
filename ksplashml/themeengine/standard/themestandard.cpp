...
ThemeStandard::ThemeStandard(QWidget *parent, const char *name, const QStringList &args)
    ...
    , mIconCount(0)
    , mStdIconWidth(-1)
    , mIconPos(WndIcon::HBottomLeft)
    , mSbAtTop(false)
    , mSbVisible(true)
    , mSbPbVisible(true)
    , mSbFontName("helvetica")
    , mSbFontSz(16)
    , mSbFontBold(true)
    , mSbFontItalic(false)
    , mSbFont(QFont())
...
void ThemeStandard::slotSetPixmap(const QString &pxn)
{
    ...
    mIcon =
        new WndIcon(++mIconCount, mStdIconWidth, mStatusBarHeight, mTheme->xineramaScreen(), px, QString::null, mIconPos, mSbAtTop, mIconsJumping);
    mIcon->show();
