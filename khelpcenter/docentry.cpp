...
DocEntry::DocEntry(const QString &name, const QString &url, const QString &icon)
{
    init();
...
QString DocEntry::identifier() const
{
    if(mIdentifier.isEmpty())
        mIdentifier = KApplication::randomString(15);
    return mIdentifier;
}
...
bool DocEntry::readFromFile(const QString &fileName)
{
    ...
    mSearchEnabledDefault = file.readBoolEntry("X-DOC-SearchEnabledDefault", false);
...
void DocEntry::addChild(DocEntry *entry)
{
    ...
    for(i = 0; i < mChildren.count(); ++i)
    {
        ...
        {
            if(entry->weight() >= mChildren[i]->weight() && entry->weight() < mChildren[i + 1]->weight())
            {
...
{
    return !search().isEmpty() && docExists() && indexExists(Prefs::indexDirectory());
}
