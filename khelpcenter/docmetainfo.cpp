...
QString DocMetaInfo::languageName(const QString &langcode)
{
    ...
    QString cfgfile = locate("locale", QString::fromLatin1("%1/entry.desktop").arg(langcode));
...
void DocMetaInfo::traverseEntry(DocEntry *entry, DocEntryTraverser *traverser)
{
    ...
    {
        if((*it)->isDirectory() && !(*it)->hasChildren() && (*it)->khelpcenterSpecial().isEmpty())
            continue;
...
void DocMetaInfo::startTraverseEntry(DocEntry *entry, DocEntryTraverser *traverser)
{
    //  kdDebug() << "DocMetaInfo::startTraverseEntry() " << entry->name() << endl;

    if(!traverser)
    {
        kdDebug() << "DocMetaInfo::startTraverseEntry(): ERROR. No Traverser." << endl;
        return;
    }
