...
KHTMLSearchConfig::KHTMLSearchConfig(QWidget *parent, const char *name) : KCModule(parent, name), indexProc(0)
{
    ...
    hbox->addWidget(gb);
    QWhatsThis::add(gb,
                    i18n("Here you can select which parts of the documentation should be included in the fulltext search index. Available options "
                         "are the KDE Help pages, the installed man pages, and the installed info pages. You can select any number of these."));
    ...
    indexMan->setEnabled(false), connect(indexMan, SIGNAL(clicked()), this, SLOT(configChanged()));
    ...
    hbox->addWidget(gb);
    QWhatsThis::add(
        gb, i18n("Here you can add additional paths to search for documentation. To add a path, click on the <em>Add...</em> button and select the "
                 "folder from where additional documentation should be searched. You can remove folders by clicking on the <em>Delete</em> button."));
    ...
    connect(searchPaths, SIGNAL(highlighted(const QString &)), this, SLOT(pathSelected(const QString &)));
...
void KHTMLSearchConfig::loadLanguages()
{
    ...
    QStringList langs = KGlobal::dirs()->findAllResources("locale", QString::fromLatin1("*/entry.desktop"));
    langs.sort();
...
{
    return i18n(
        "<h1>Help Index</h1> This configuration module lets you configure the ht://dig engine which can be used for fulltext search in the KDE "
...
void KHTMLSearchConfig::generateIndex()
{
    ...
    *indexProc << exe << "--lang" << language->currentTag();
    ...
    connect(indexProc, SIGNAL(processExited(KProcess *)), this, SLOT(indexTerminated(KProcess *)));
