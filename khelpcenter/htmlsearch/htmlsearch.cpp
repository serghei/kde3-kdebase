...
HTMLSearch::HTMLSearch() : QObject(), _proc(0)
{
}
...
bool HTMLSearch::createConfig(const QString &_lang)
{
    ...
    if(f.open(IO_WriteOnly))
    {
        ...
        ts << "external_parsers:\t"
           << "text/xml\t" << locate("data", "khelpcenter/meinproc_wrapper") << endl;
        f.close();
        return true;
    }
...
bool HTMLSearch::generateIndex(QString _lang, QWidget *parent)
{
    ...
    while(!done)
    {
        ...
        *_proc << exe << "-v"
        ...
        connect(_proc, SIGNAL(receivedStdout(KProcess *, char *, int)), this, SLOT(htdigStdout(KProcess *, char *, int)));

        connect(_proc, SIGNAL(processExited(KProcess *)), this, SLOT(htdigExited(KProcess *)));
    ...
    *_proc << exe << "-c" << dataPath(_lang) + "/htdig.conf";
    ...
    connect(_proc, SIGNAL(processExited(KProcess *)), this, SLOT(htmergeExited(KProcess *)));
...
QString HTMLSearch::search(QString _lang, QString words, QString method, int matches, QString format, QString sort)
{
    if(_lang == "C")
        _lang = "en";
    ...
    *_proc << exe << "-c" << dataPath(_lang) + "/htdig.conf"
    ...
    connect(_proc, SIGNAL(receivedStdout(KProcess *, char *, int)), this, SLOT(htsearchStdout(KProcess *, char *, int)));
    connect(_proc, SIGNAL(processExited(KProcess *)), this, SLOT(htsearchExited(KProcess *)));
