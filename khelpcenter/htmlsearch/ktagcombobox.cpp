...
KTagComboBox::KTagComboBox(QWidget *parent, const char *name) : QComboBox(parent, name), popup(0), old_popup(0)
{
...
void KTagComboBox::keyPressEvent(QKeyEvent *e)
{
    ...
    if((e->key() == Key_F4 && e->state() == 0) || (e->key() == Key_Down && (e->state() & AltButton)) || (e->key() == Key_Space))
    {
        if(count())
        {
...
void KTagComboBox::clear()
{
    ...
    connect(popup, SIGNAL(activated(int)), SLOT(internalActivate(int)));
    connect(popup, SIGNAL(highlighted(int)), SLOT(internalHighlight(int)));
}
