...
History::History() : QObject(), m_goBuffer(0)
{
...
void History::setupActions(KActionCollection *coll)
{
    ...
    m_backAction = new KToolBarPopupAction(backForward.first, ALT + Key_Left, this, SLOT(back()), coll, "back");
    connect(m_backAction->popupMenu(), SIGNAL(activated(int)), SLOT(backActivated(int)));
    connect(m_backAction->popupMenu(), SIGNAL(aboutToShow()), SLOT(fillBackMenu()));
    m_backAction->setEnabled(false);

    m_forwardAction = new KToolBarPopupAction(backForward.second, ALT + Key_Right, this, SLOT(forward()), coll, "forward");
    connect(m_forwardAction->popupMenu(), SIGNAL(activated(int)), SLOT(forwardActivated(int)));
    connect(m_forwardAction->popupMenu(), SIGNAL(aboutToShow()), SLOT(fillForwardMenu()));
    m_forwardAction->setEnabled(false);
}

void History::installMenuBarHook(KMainWindow *mainWindow)
{
    QPopupMenu *goMenu = dynamic_cast< QPopupMenu * >(mainWindow->guiFactory()->container("go_web", mainWindow));
    if(goMenu)
    {
        connect(goMenu, SIGNAL(aboutToShow()), SLOT(fillGoMenu()));
        connect(goMenu, SIGNAL(activated(int)), SLOT(goMenuActivated(int)));
...
void History::createEntry()
{
    ...
    if(current)
    {
        ...
        {
            if(!m_entries.removeLast())
            { // and remove from the end (faster and easier)
                Q_ASSERT(0);
                return;
            }
            else
...
void History::updateCurrentEntry(View *view)
{
    ...
    kdDebug() << "History::updateCurrentEntry(): " << view->title() << " (URL: " << url.url() << ")" << endl;
