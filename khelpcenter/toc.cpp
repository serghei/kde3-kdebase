...
public:
    TOCChapterItem(TOC *toc, NavigatorItem *parent, QListViewItem *after, const QString &title, const QString &name);
...
public:
    TOCSectionItem(TOC *toc, TOCChapterItem *parent, QListViewItem *after, const QString &title, const QString &name);
...
{
    if(!QFile::exists(m_cacheFile) || sourceFileCTime() != cachedCTime())
        return NeedRebuild;
...
void TOC::buildCache()
{
    ...
    connect(meinproc, SIGNAL(processExited(KProcess *)), this, SLOT(meinprocExited(KProcess *)));
...
TOCChapterItem::TOCChapterItem(TOC *toc, NavigatorItem *parent, QListViewItem *after, const QString &title, const QString &name)
    : TOCItem(toc, parent, after, title), m_name(name)
{
    setOpen(false);
    entry()->setUrl(url());
}
...
TOCSectionItem::TOCSectionItem(TOC *toc, TOCChapterItem *parent, QListViewItem *after, const QString &title, const QString &name)
    : TOCItem(toc, parent, after, title), m_name(name)
{
