...
Navigator::Navigator(View *view, QWidget *parent, const char *name) : QWidget(parent, name), mIndexDialog(0), mView(view), mSelected(false)
{
    ...
    connect(mSearchEngine, SIGNAL(searchFinished()), SLOT(slotSearchFinished()));
    ...
    connect(mSearchEdit, SIGNAL(textChanged(const QString &)), SLOT(checkSearchButton()));
    ...
    connect(mTabWidget, SIGNAL(currentChanged(QWidget *)), SLOT(slotTabChanged(QWidget *)));
...
void Navigator::setupContentsTab()
{
    ...
    connect(mContentsTree, SIGNAL(clicked(QListViewItem *)), SLOT(slotItemSelected(QListViewItem *)));
    connect(mContentsTree, SIGNAL(returnPressed(QListViewItem *)), SLOT(slotItemSelected(QListViewItem *)));
...
void Navigator::setupSearchTab()
{
    ...
    connect(mSearchWidget, SIGNAL(searchResult(const QString &)), SLOT(slotShowSearchResult(const QString &)));
    connect(mSearchWidget, SIGNAL(scopeCountChanged(int)), SLOT(checkSearchButton()));
    connect(mSearchWidget, SIGNAL(showIndexDialog()), SLOT(showIndexDialog()));
...
void Navigator::setupGlossaryTab()
{
    ...
    connect(mGlossaryTree, SIGNAL(entrySelected(const GlossaryEntry &)), this, SIGNAL(glossSelected(const GlossaryEntry &)));
...
NavigatorItem *Navigator::insertScrollKeeperDocs(NavigatorItem *topItem, NavigatorItem *after)
{
...
void Navigator::selectItem(const KURL &url)
{
    ...
    if(url != homeURL())
    {
        for(QListViewItem *item = mContentsTree->firstChild(); item; item = item->nextSibling())
        {
...
void Navigator::slotItemSelected(QListViewItem *currentItem)
{
    ...
    kdDebug(1400) << "Navigator::slotItemSelected(): " << item->entry()->name() << endl;
    ...
    else
    {
        ...
        {
            kdDebug(1400) << "slotItemSelected(): Got help URL " << url.url() << endl;
            if(!item->toc())
            {
                TOC *tocTree = item->createTOC();
                kdDebug(1400) << "slotItemSelected(): Trying to build TOC for " << item->entry()->name() << endl;
...
void Navigator::showOverview(NavigatorItem *item, const KURL &url)
{
    ...
    if(childCount > 0)
    {
        ...
        if(item)
            ...
            child = mContentsTree->firstChild();
...
{
    mSearchButton->setEnabled(!mSearchEdit->text().isEmpty() && mSearchWidget->scopeCount() > 0);
...
bool Navigator::checkSearchIndex()
{
    ...
    cfg->setGroup("Search");
    if(cfg->readBoolEntry("IndexExists", false))
        return true;

    if(mIndexDialog && mIndexDialog->isShown())
        return true;

    QString text = i18n(
        "A search index does not yet exist. Do you want "
        "to create the index now?");

    int result = KMessageBox::questionYesNo(this, text, QString::null, i18n("Create"), i18n("Do Not Create"), "indexcreation");
...
void Navigator::showIndexDialog()
{
    if(!mIndexDialog)
    {
        ...
        connect(mIndexDialog, SIGNAL(searchIndexUpdated()), mSearchWidget, SLOT(updateScopeList()));
    }
    mIndexDialog->show();
    mIndexDialog->raise();
}
