...
KClassicGreeter::KClassicGreeter(KGreeterPluginHandler *_handler, KdmThemer *themer, QWidget *parent, QWidget *pred, const QString &_fixedEntity,
                                 Function _func, Context _ctx)
    : QObject(), KGreeterPlugin(_handler), fixedUser(_fixedEntity), func(_func), ctx(_ctx), exp(-1), pExp(-1), running(false)
{
    ...
    KClassicGreeter::returnData()
{
    ...
    {
        case 0:
            handler->gplugReturnText((loginEdit ? loginEdit->text() : fixedUser).local8Bit(), KGreeterPluginHandler::IsUser);
            break;
        case 1:
    ...
    KClassicGreeter::textPrompt(const char *prompt, bool echo, bool nonBlocking)
{
    ...
    else
    {
        ...
        {
            if(pr.find(QRegExp("\\b(re-?(enter|type)|again|confirm|repeat)\\b", false)) >= 0)
                exp = 3;
            else if(pr.find(QRegExp("\\bnew\\b", false)) >= 0)
                exp = 2;
            else
            { // QRegExp( "\\b(old|current)\\b", false ) is too strict
...
void KClassicGreeter::setActive(bool enable)
{
    if(loginEdit)
...
void KClassicGreeter::setActive2(bool enable)
{
    if(passwd1Edit)
    {
...
void KClassicGreeter::slotLoginLostFocus()
{
    if(!running)
        return;
    if(exp > 0)
...
void KClassicGreeter::slotActivity()
{
    if(running)
...
KDE_EXPORT kgreeterplugin_info kgreeterplugin_info = {
    I18N_NOOP("Username + password (classic)"), "classic", kgreeterplugin_info::Local | kgreeterplugin_info::Presettable, init, done, create};
