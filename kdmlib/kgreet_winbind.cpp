...
static void splitEntity(const QString &ent, QString &dom, QString &usr)
{
...
KWinbindGreeter::KWinbindGreeter(KGreeterPluginHandler *_handler, KdmThemer *themer, QWidget *parent, QWidget *pred, const QString &_fixedEntity,
                                 Function _func, Context _ctx)
    : QObject(), KGreeterPlugin(_handler), func(_func), ctx(_ctx), exp(-1), pExp(-1), running(false)
{
    ...
    if(themer && (!(user_entry = themer->findNode("user-entry")) || !(pw_entry = themer->findNode("pw-entry"))
    ...
    if(func != ChAuthTok)
    {
        if(fixedUser.isEmpty())
        {
            ...
            connect(domainCombo, SIGNAL(activated(const QString &)), SLOT(slotChangedDomain(const QString &)));
            connect(domainCombo, SIGNAL(activated(const QString &)), SLOT(slotLoginLostFocus()));
            connect(domainCombo, SIGNAL(activated(const QString &)), SLOT(slotActivity()));
        ...
        connect(passwdEdit, SIGNAL(textChanged(const QString &)), SLOT(slotActivity()));
        ...
        {
            passwdLabel = new QLabel(passwdEdit, func == Authenticate ? i18n("&Password:") : i18n("Current &password:"), parent);
            if(pred)
            {
...
void KWinbindGreeter::slotChangedDomain(const QString &dom)
{
    ...
    KWinbindGreeter::returnData()
{
    ...
    {
        case 0:
            handler->gplugReturnText(getEntity().local8Bit(), KGreeterPluginHandler::IsUser);
            break;
        case 1:
            handler->gplugReturnText(passwdEdit->password(), KGreeterPluginHandler::IsPassword | KGreeterPluginHandler::IsSecret);
            break;
        case 2:
            handler->gplugReturnText(passwd1Edit->password(), KGreeterPluginHandler::IsSecret);
            break;
        default: // case 3:
            handler->gplugReturnText(passwd2Edit->password(), KGreeterPluginHandler::IsNewPassword | KGreeterPluginHandler::IsSecret);
            break;
    }
}
...
{
    if(!err && QString(text).find(QRegExp("^Changing password for [^ ]+$")) >= 0)
        return true;
    return false;
}
    ...
    KWinbindGreeter::textPrompt(const char *prompt, bool echo, bool nonBlocking)
{
    ...
    else
    {
        ...
        {
            handler->gplugReturnText("", KGreeterPluginHandler::IsOldPassword | KGreeterPluginHandler::IsSecret);
            return;
        }
        else if(pr.find(QRegExp("\\b(re-?(enter|type)|again|confirm|repeat)\\b", false)) >= 0)
            exp = 3;
        else if(pr.find(QRegExp("\\bnew\\b", false)) >= 0)
            exp = 2;
        else
        {
            handler->gplugMsgBox(QMessageBox::Critical, i18n("Unrecognized prompt \"%1\"").arg(prompt));
...
void KWinbindGreeter::setActive(bool enable)
{
    if(domainCombo)
        domainCombo->setEnabled(enable);
    if(loginEdit)
...
void KWinbindGreeter::setActive2(bool enable)
{
    if(passwd1Edit)
    {
...
void KWinbindGreeter::slotLoginLostFocus()
{
    if(!running)
        return;
...
void KWinbindGreeter::slotActivity()
{
    if(running)
        handler->gplugActivity();
}

void KWinbindGreeter::slotStartDomainList()
{
...
void KWinbindGreeter::slotReadDomainList()
{
    QString line;
...
void KWinbindGreeter::slotEndDomainList()
{
    delete m_domainLister;
    m_domainLister = 0;
    ...
    for(QStringList::const_iterator it = mDomainListing.begin(); it != mDomainListing.end(); ++it)
    {
...
static bool init(const QString &, QVariant (*getConf)(void *, const char *, const QVariant &), void *ctx)
{
...
static KGreeterPlugin *create(KGreeterPluginHandler *handler, KdmThemer *themer, QWidget *parent, QWidget *predecessor, const QString &fixedEntity,
...
KDE_EXPORT kgreeterplugin_info kgreeterplugin_info = {I18N_NOOP("Winbind / Samba"),
                                                      "classic",
