...
#define KDE_GNUC_PREREQ(maj, min) ((__GNUC__ << 16) + __GNUC_MINOR__ >= ((maj) << 16) + (min))
#else
...
NPError g_NPN_DestroyStream(NPP instance, NPStream *stream, NPReason reason)
{
...
NPError g_NPN_GetURL(NPP instance, const char *url, const char *target)
{
    ...
    if(inst)
    {
        inst->requestURL(QString::fromLatin1(url), QString::null, QString::fromLatin1(target), 0);
    }
...
NPError g_NPN_GetURLNotify(NPP instance, const char *url, const char *target, void *notifyData)
{
    ...
    if(inst)
    {
        ...
        inst->requestURL(QString::fromLatin1(url), QString::null, QString::fromLatin1(target), notifyData, true);
    }
...
NPError g_NPN_PostURLNotify(NPP instance, const char *url, const char *target, uint32 len, const char *buf, NPBool file, void *notifyData)
{
   ...
   f.close();
    ...
    else if(!strcmp(target, "_current") || !strcmp(target, "_self") || !strcmp(target, "_top"))
    {
    ...
    {
        inst->postURL(QString::fromLatin1(url), postdata, args.contentType(), QString::fromLatin1(target), notifyData, args, true);
    }
    else
    {
...
NPError g_NPN_PostURL(NPP instance, const char *url, const char *target, uint32 len, const char *buf, NPBool file)
{
   ...
   f.close();
    ...
    if(inst && !inst->normalizedURL(QString::fromLatin1(url)).isNull())
    {
        inst->postURL(QString::fromLatin1(url), postdata, args.contentType(), QString::fromLatin1(target), 0L, args, false);
    }
    else
    {
        // Unsupported / insecure
...
void NSPluginInstance::forwarder(Widget w, XtPointer cl_data, XEvent *event, Boolean *cont)
{
    Q_UNUSED(w);
...
NSPluginInstance::NSPluginInstance(NPP privateData, NPPluginFuncs *pluginFuncs, KLibrary *handle, int width, int height, QString src,
                                   QString /*mime*/, QString appId, QString callbackId, bool embed, WId xembed, QObject *parent, const char *name)
    : DCOPObject(), QObject(parent, name)
{
    ...
    if(_xembed_window == 0)
    {
        ...
        Arg args[7];
        Cardinal nargs = 0;
        XtSetArg(args[nargs], XtNwidth, width);
        nargs++;
        XtSetArg(args[nargs], XtNheight, height);
        nargs++;
        XtSetArg(args[nargs], XtNborderWidth, 0);
        nargs++;

        String n, c;
        XtGetApplicationNameAndClass(qt_xdisplay(), &n, &c);

        _toplevel = XtAppCreateShell("drawingArea", c, applicationShellWidgetClass, qt_xdisplay(), args, nargs);
        ...
        XtSetArg(args[nargs], XtNvisual, QPaintDevice::x11AppVisual());
        nargs++;
        XtSetArg(args[nargs], XtNdepth, QPaintDevice::x11AppDepth());
        nargs++;
        XtSetArg(args[nargs], XtNcolormap, QPaintDevice::x11AppColormap());
        nargs++;
...
// From mozilla - not sure if it's needed yet, nor what to use for embedder
      ...
      _XtRegisterWindow(embedderid, _toplevel);
        ...
        // Register forwarder
        XtAddEventHandler(_toplevel, (KeyPressMask | KeyReleaseMask), False, forwarder, (XtPointer) this);
        XtAddEventHandler(_form, (KeyPressMask | KeyReleaseMask), False, forwarder, (XtPointer) this);
        XSync(qt_xdisplay(), false);
    }
}
...
void NSPluginInstance::destroy()
{
    if(!_destroyed)
    {
        ...
        if(_form != 0)
        {
            XtRemoveEventHandler(_form, (KeyPressMask | KeyReleaseMask), False, forwarder, (XtPointer) this);
            XtRemoveEventHandler(_toplevel, (KeyPressMask | KeyReleaseMask), False, forwarder, (XtPointer) this);
        ...
        else
        {
            if(!url.isEmpty())
            {
                ...
                if(req.post)
                {
                    ...
                    connect(s, SIGNAL(finished(NSPluginStreamBase *)), SLOT(streamFinished(NSPluginStreamBase *)));
                    _streams.append(s);
...
QString NSPluginInstance::normalizedURL(const QString &url) const
{
    ...
    if(!cfg.readBoolEntry("HTTP URLs Only", false) || inURL.protocol() == "http" || inURL.protocol() == "https" || inURL.protocol() == "javascript")
    {
        return inURL.url();
    }
...
void NSPluginInstance::requestURL(const QString &url, const QString &mime, const QString &target, void *notify, bool forceNotify, bool reload)
{
...
void NSPluginInstance::postURL(const QString &url, const QByteArray &data, const QString &mime, const QString &target, void *notify,
...
void NSPluginInstance::resizePlugin(Q_INT32 w, Q_INT32 h)
{
    ...
    if(_form != 0)
    {
        ...
        Arg args[7];
        Cardinal nargs = 0;
        XtSetArg(args[nargs], XtNwidth, _width);
        nargs++;
        XtSetArg(args[nargs], XtNheight, _height);
        nargs++;
        XtSetArg(args[nargs], XtNvisual, QPaintDevice::x11AppVisual());
        nargs++;
        XtSetArg(args[nargs], XtNdepth, QPaintDevice::x11AppDepth());
...
extern "C" KDE_EXPORT int XQueryKeymap(Display *dpy, char k[32])
{
...
NSPluginViewer::NSPluginViewer(QCString dcopId, QObject *parent, const char *name) : DCOPObject(dcopId), QObject(parent, name)
{
    _classes.setAutoDelete(true);
    connect(KApplication::dcopClient(), SIGNAL(applicationRemoved(const QCString &)), this, SLOT(appUnregistered(const QCString &)));
}
...
NSPluginClass::NSPluginClass(const QString &library, QObject *parent, const char *name) : DCOPObject(), QObject(parent, name)
{
    // initialize members
...
DCOPRef NSPluginClass::newInstance(QString url, QString mimeType, Q_INT8 embed, QStringList argn, QStringList argv, QString appId, QString callbackId,
                                   Q_INT8 reload, Q_INT8 doPost, QByteArray postData, Q_UINT32 xembed)
{
    ...
    for(unsigned int i = 0; i < argc; i++)
    {
        ...
        if(!strcasecmp(_argn[i], "WIDTH"))
            width = argv[i].toInt();
        if(!strcasecmp(_argn[i], "HEIGHT"))
            height = argv[i].toInt();
        if(!strcasecmp(_argn[i], "__KHTML__PLUGINBASEURL"))
            baseURL = _argv[i];
    ...
    // create plugin instance
    NPError error = _pluginFuncs.newp(mime, npp, embed ? NP_EMBED : NP_FULL, argc, _argn, _argv, 0);
    ...
    NSPluginInstance *inst = new NSPluginInstance(npp, &_pluginFuncs, _handle, width, height, baseURL, mimeType, appId, callbackId, embed,
                                                  wants_xembed ? xembed : 0, this);
...
NSPluginStreamBase::NSPluginStreamBase(NSPluginInstance *instance)
    : QObject(instance), _instance(instance), _stream(0), _tempFile(0L), _pos(0), _queue(0), _queuePos(0), _error(false)
{
    _informed = false;
}
...
void NSPluginStreamBase::inform()
{
    ...
    if(!_informed)
    {
        ...
        // inform the plugin
        _instance->NPNewStream(_mimeType.isEmpty() ? (char *)"text/plain" : (char *)_mimeType.ascii(), _stream, false, &_streamType);
...
NSPluginBufStream::NSPluginBufStream(class NSPluginInstance *instance) : NSPluginStreamBase(instance)
{
    _timer = new QTimer(this);
...
bool NSPluginBufStream::get(const QString &url, const QString &mimeType, const QByteArray &buf, void *notifyData, bool singleShot)
{
...
NSPluginStream::NSPluginStream(NSPluginInstance *instance) : NSPluginStreamBase(instance), _job(0)
{
...
bool NSPluginStream::get(const QString &url, const QString &mimeType, void *notify, bool reload)
{
    // create new stream
    if(create(url, mimeType, notify))
    {
        ...
        connect(_job, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(data(KIO::Job *, const QByteArray &)));
        connect(_job, SIGNAL(result(KIO::Job *)), SLOT(result(KIO::Job *)));
        connect(_job, SIGNAL(totalSize(KIO::Job *, KIO::filesize_t)), SLOT(totalSize(KIO::Job *, KIO::filesize_t)));
        connect(_job, SIGNAL(mimetype(KIO::Job *, const QString &)), SLOT(mimetype(KIO::Job *, const QString &)));
        connect(_job, SIGNAL(redirection(KIO::Job *, const KURL &)), SLOT(redirection(KIO::Job *, const KURL &)));
    }
...
bool NSPluginStream::post(const QString &url, const QByteArray &data, const QString &mimeType, void *notify, const KParts::URLArgs &args)
{
    // create new stream
    if(create(url, mimeType, notify))
    {
