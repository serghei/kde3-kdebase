...
void deletePluginMimeTypes()
{
    ...
    for(unsigned int i = 0; i < dirs.count(); i++)
    {
        if(!dirs[i].contains("."))
        {
            ...
            QDir files(dirs.absFilePath(dirs[i]), QString::null, QDir::Name | QDir::IgnoreCase, QDir::Files);
            if(files.exists(dir))
            {
...
void registerPlugin(const QString &name, const QString &description, const QString &file, const QString &mimeInfo)
{
    // global stuff
...
int tryCheck(int write_fd, const QString &absFile)
{
    ...
    if(!_handle)
    {
        kdDebug(1433) << " - open failed with message " << KLibLoader::self()->lastErrorMessage() << ", skipping " << endl;
        return 1;
    }
    ...
    NPError (*func_GetValue)(void *, NPPVariable, void *) = (NPError(*)(void *, NPPVariable, void *))_handle->symbol("NP_GetValue");
    if(func_GetValue)
    {

        // get name
        char *buf = 0;
        NPError err = func_GetValue(0, NPPVpluginNameString, (void *)&buf);
        if(err == NPERR_NO_ERROR)
            name = QString::fromLatin1(buf);
        kdDebug() << "name = " << name << endl;

        // get name
        NPError nperr = func_GetValue(0, NPPVpluginDescriptionString, (void *)&buf);
    ...
    char *(*func_GetMIMEDescription)() = (char *(*)())_handle->symbol("NP_GetMIMEDescription");
...
void scanDirectory(QString dir, QStringList &mimeInfoList, QTextStream &cache)
{
...
void removeExistingExtensions(QString &extension)
{
    ...
    for(QStringList::Iterator it = exts.begin(); it != exts.end(); ++it)
    {
        ...
        KMimeType::Ptr mime = KMimeType::findByURL(KURL("file:///foo." + ext), 0, true, true);
        if(mime->name() == "application/octet-stream" || mime->comment().left(8) == "Netscape")
        {
...
static KCmdLineOptions options[] = {{"verbose", I18N_NOOP("Show progress output for GUI"), 0}, KCmdLineLastOption};


int main(int argc, char **argv)
{
    KAboutData aboutData("nspluginscan", I18N_NOOP("nspluginscan"), "0.3", "nspluginscan", KAboutData::License_GPL,
...
// calls (and thus does not have SA_RESTART)
    ...
    int i = 0;
    for(QStringList::Iterator it = searchPaths.begin(); it != searchPaths.end(); ++it, ++i)
    {
