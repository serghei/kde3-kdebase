    ...
    : KDialogBase(parent, name, modal, i18n("Change your Face"), Ok | Cancel, Ok, true)
{
  ...
  morePics->addWidget( acquireBtn );
    ...
    if(facesDir.exists())
    {
        QStringList picslist = facesDir.entryList(QDir::Files);
        for(QStringList::Iterator it = picslist.begin(); it != picslist.end(); ++it)
            new QIconViewItem(m_FacesWidget, "/" + (*it) == KCFGUserAccount::customFaceFile() ? i18n("(Custom)") : (*it).section(".", 0, 0),
...
void ChFaceDlg::addCustomPixmap(QString imPath, bool saveCopy)
{
    ...
    if(saveCopy)
    {
        ...
        KonqOperations::copy(this, KonqOperations::COPY, KURL::List(KURL(userfaces.absPath() + "/.userinfo-tmp")),
                             KURL(userfaces.absPath() + "/" + QFileInfo(imPath).fileName().section(".", 0, 0)));
#if 0
...
void ChFaceDlg::slotGetCustomImage()
{
    ...
    KFileDialog *dlg = new KFileDialog(QDir::homeDirPath(), KImageIO::pattern(KImageIO::Reading), this, 0, true, checkWidget);
