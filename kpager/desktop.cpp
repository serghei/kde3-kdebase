...
Desktop::Desktop(int desk, QString desktopName, QWidget *parent, const char *name) : QWidget(parent, name)
{
    ...
    if(m_desk == 1)
        Desktop::m_windowPixmaps.setAutoDelete(true);
    KConfig *cfg = KGlobal::config();
    m_transparentMode = static_cast< WindowTransparentMode >(cfg->readNumEntry("windowTransparentMode", c_defWindowTransparentMode));
    resize(67, 50);
}
...
{
    QRect tmp(r);
    r.setRect(deskX() + tmp.x() * deskWidth() / kapp->desktop()->width(), deskY() + tmp.y() * deskHeight() / kapp->desktop()->height(),
              tmp.width() * deskWidth() / kapp->desktop()->width(), tmp.height() * deskHeight() / kapp->desktop()->height());
}
...
void Desktop::paintWindow(QPainter &p, const KWin::WindowInfo *info, bool onDesktop)
{
    switch(static_cast< WindowDrawMode >(KPagerConfigDialog::m_windowDrawMode))
    {
        ...
        case(Icon):
            ...
            break;
        case(Pixmap):
            paintWindowPixmap(p, info, onDesktop);
            break;
    }
}
...
QPixmap *Desktop::paintNewWindow(const KWin::WindowInfo *info)
{
    ...
    r = QRect(r.x() * width() / dw, 2 + r.y() * height() / dh, r.width() * width() / dw, r.height() * height() / dh);
...
void Desktop::startDrag(const QPoint &p)
{
    ...
    PagerWindowDrag *wdrag = new PagerWindowDrag(info->win(), deltax, deltay, m_desk, this);
...
{
    if(PagerWindowDrag::canDecode(ev))
        ev->accept();
}
...
void Desktop::paintEvent(QPaintEvent *)
{
    ...
    {
        if((!m_isCommon && !m_bgSmallPixmap) || (m_isCommon && !m_bgCommonSmallPixmap))
            loadBgPixmap();

        if((!m_isCommon && m_bgSmallPixmap && !m_bgSmallPixmap->isNull())
           || (m_isCommon && m_bgCommonSmallPixmap && !m_bgCommonSmallPixmap->isNull()))
        {
            QPixmap tmp;
            if(m_isCommon)
    ...
    if(KPagerConfigDialog::m_showWindows)
    {
        ...
        for(it = pager()->kwin()->stackingOrder().begin(); it != pager()->kwin()->stackingOrder().end(); ++it)
        {
...
void Desktop::paintWindowPlain(QPainter &p, const KWin::WindowInfo *info, bool onDesktop)
{
    ...
    if(isActive)
        brush = colorGroup().brush(QColorGroup::Highlight);
    else
        brush = colorGroup().brush(QColorGroup::Button);

    if(m_transparentMode == AllWindows || (m_transparentMode == MaximizedWindows && (info->state() & NET::Max)))
...
void Desktop::paintWindowIcon(QPainter &p, const KWin::WindowInfo *info, bool onDesktop)
{
    ...
    int dh = QApplication::desktop()->height();
    r = QRect(r.x() * width() / dw, 2 + r.y() * height() / dh, r.width() * width() / dw, r.height() * height() / dh);
    QPixmap icon = KWin::icon(info->win(), int(r.width() * 0.8), int(r.height() * 0.8), true);
    ...
    p.drawPixmap(r.topLeft() + QPoint(int(r.width() * 0.1), int(r.height() * 0.1)), icon);
}

void Desktop::paintWindowPixmap(QPainter &p, const KWin::WindowInfo *info, bool onDesktop)
{
    ...
    int dw = QApplication::desktop()->width();
    int dh = QApplication::desktop()->height();
    rSmall = QRect(r.x() * width() / dw, 2 + r.y() * height() / dh, r.width() * width() / dw, r.height() * height() / dh);
    ...
    if(!pixmap || isDirty || m_grabWindows)
    {
        if(isCurrent())
        {
            QPixmap tmp = QPixmap::grabWindow(info->win(), 0, 0, r.width(), r.height());
...
const Desktop::WindowTransparentMode Desktop::c_defWindowTransparentMode = Desktop::AllWindows;
#include "desktop.moc"
