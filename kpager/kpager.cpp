...
KPagerMainWindow::KPagerMainWindow(QWidget *parent, const char *name) : DCOPObject("KPagerIface"), KMainWindow(parent, name)
{
    m_reallyClose = false;
...
KPager::KPager(KPagerMainWindow *parent, const char *name)
    : QFrame(parent, name, WStyle_Customize | WStyle_NoBorder | WStyle_Tool), m_layout(0), m_mnu(0), m_smnu(0), m_dmnu(0)
{
    ...
    connect(m_winmodule, SIGNAL(activeWindowChanged(WId)), SLOT(slotActiveWindowChanged(WId)));
    connect(m_winmodule, SIGNAL(windowAdded(WId)), SLOT(slotWindowAdded(WId)));
    connect(m_winmodule, SIGNAL(windowRemoved(WId)), SLOT(slotWindowRemoved(WId)));
    connect(m_winmodule, SIGNAL(windowChanged(WId, unsigned int)), SLOT(slotWindowChanged(WId, unsigned int)));
    connect(m_winmodule, SIGNAL(stackingOrderChanged()), SLOT(slotStackingOrderChanged()));
    connect(m_winmodule, SIGNAL(desktopNamesChanged()), SLOT(slotDesktopNamesChanged()));
    connect(m_winmodule, SIGNAL(numberOfDesktopsChanged(int)), SLOT(slotNumberOfDesktopsChanged(int)));
    connect(m_winmodule, SIGNAL(currentDesktopChanged(int)), SLOT(slotCurrentDesktopChanged(int)));
...
const QString KPager::lWidth()
{
    ...
    {
        case(Classical):
            return "layoutClassicalWidth";
            break;
        case(Horizontal):
            return "layoutHorizontalWidth";
            break;
        case(Vertical):
            return "layoutVerticalWidth";
            break;
    };
    return "Width";
}
...
const QString KPager::lHeight()
{
    ...
    {
        case(Classical):
            return "layoutClassicalHeight";
            break;
        case(Horizontal):
            return "layoutHorizontalHeight";
            break;
        case(Vertical):
            return "layoutVerticalHeight";
            break;
    };
    return "Height";
}
...
void KPager::updateLayout()
{
    ...
    {
        case(Classical):
            m_layout = new QGridLayout(this, 2, 0);
            break;
        case(Horizontal):
            m_layout = new QGridLayout(this, 0, 1);
            break;
        case(Vertical):
            m_layout = new QGridLayout(this, 1, 0);
            break;
    };
    ...
    {
        case(Classical):
            resize(w * (ndesks / 2 + (ndesks % 2)), h * 2);
            break;
        case(Horizontal):
            resize(w * ndesks, h);
            break;
...
void KPager::slotActiveWindowChanged(WId win)
{
    ...
    {
        if((inf1 && inf1->isOnDesktop(i)) || (inf2 && inf2->isOnDesktop(i)))
...
void KPager::slotWindowChanged(WId win, unsigned int prop)
{
    ...
    if(repaint)
        for(int i = 1; i <= (int)m_desktops.count(); ++i)
        {
            if((inf && (inf->isOnDesktop(i))) || onAllDesktops || desktop == i)
            {
                m_desktops[i - 1]->repaint(false);
            }
        }
...
void KPager::clientPopupAboutToShow()
{
    ...
    if(m_showStickyOption) // Add translation
        m_mnu->changeItem(StickyOp, (m_winfo.onAllDesktops()) ? QString("Un&sticky") : QString("&Sticky"));
}
...
void KPager::desktopPopupAboutToShow()
{
    ...
    {
        id = m_dmnu->insertItem(QString("&") + QString::number(i) + QString(" ") + m_winmodule->desktopName(i), i);
