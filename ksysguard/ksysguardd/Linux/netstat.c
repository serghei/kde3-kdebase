...
static const char *raw_type[] = {"", "stream", "dgram", "raw", "rdm", "seqpacket", "packet"};

static const char *raw_state[] = {"free", "unconnected", "connecting", "connected", "disconnecting"};

static const char *conn_state[] = {"",          "established", "syn_sent",   "syn_recv", "fin_wait1", "fin_wait2",
...
void printSocketInfo(SocketInfo *socket_info)
{
    fprintf(CurrentClient, "%s\t%s\t%s\t%s\t%s\t%d\n", socket_info->local_addr, socket_info->local_port, socket_info->remote_addr,
            socket_info->remote_port, socket_info->state, socket_info->uid);
}
...
int updateNetStatTcpUdpRaw(const char *cmd)
{
    ...
    while(fgets(buffer, sizeof(buffer), netstat) != NULL)
    {
        if(strcmp(buffer, ""))
        {
            sscanf(buffer, "%*d: %x:%x %x:%x %x %*x:%*x %*x:%*x %d", &local_addr, &local_port, &remote_addr, &remote_port, &state, &uid);
...
int updateNetStatUnix(void)
{
    ...
    while(fgets(buffer, sizeof(buffer), file) != NULL)
    {
        if(strcmp(buffer, ""))
        {
            sscanf(buffer, "%*x: %d %*d %*d %d %d %d %255s", &ref_count, &type, &state, &inode, path);
...
void printNetStatUnix(const char *cmd)
{
    ...
    {
        fprintf(CurrentClient, "%d\t%s\t%s\t%d\t%s\n", unix_info->refcount, unix_info->type, unix_info->state, unix_info->inode, unix_info->path);
    }
