...
static int updateProcess(int pid)
{
    ...
    if(fscanf(fd,
              ...
              "%*d %*d %*d %d %*u %*u %*d %u %u",
              &status, (int *)&ps->ppid, (int *)&ps->gid, &ps->ttyNo, &userTime, &sysTime, &ps->niceLevel, &ps->vmSize, &ps->vmRss)
       != 9)
    {
        fclose(fd);
        return -1;
    ...
    if(strcmp(ps->name, "kdeinit") == 0 && strncmp(ps->cmdline, "kdeinit: ", KDEINITLEN) == 0 && strcmp(ps->cmdline + KDEINITLEN, "Running...") != 0)
    {
        size_t len;
