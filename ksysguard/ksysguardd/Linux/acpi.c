...
int updateAcpiBattery(void)
{
    ...
    for(i = 0; i < AcpiBatteryNum; i++)
    {
        ...
        if((n = read(fd, AcpiBatInfoBuf, ACPIBATTERYINFOBUFSIZE - 1)) == ACPIBATTERYINFOBUFSIZE - 1)
        {
        ...
        close(fd);
        p = AcpiBatInfoBuf;
        while((p != NULL) && (sscanf(p, "last full capacity: %d ", &AcpiBatCapacity) != 1))
        {
            p = strchr(p, '\n');
            if(p)
                p++;
        ...
        if((n = read(fd, AcpiBatStateBuf, ACPIBATTERYSTATEBUFSIZE - 1)) == ACPIBATTERYSTATEBUFSIZE - 1)
        {
            log_error("Internal buffer too small to read \'%s\'", s);
            close(fd);
            return -1;
        ...
        close(fd);
        p = AcpiBatStateBuf;
        while((p != NULL) && (sscanf(p, "remaining capacity: %d ", &AcpiBatRemainingCapacity) != 1))
        {
            p = strchr(p, '\n');
            if(p)
                p++;
        ...
        p = AcpiBatStateBuf;
        while((p != NULL) && (sscanf(p, "present rate: %d ", &AcpiBatteryUsage[i]) != 1))
        {
            p = strchr(p, '\n');
            if(p)
                p++;
...
void initAcpiThermal(struct SensorModul *sm)
{
    ...
    while((de = readdir(d)) != NULL)
    {
        ...
        AcpiThermalZones++;
        snprintf(th_ref, sizeof(th_ref), "acpi/thermal_zone/%s/temperature", de->d_name);
        registerMonitor(th_ref, "integer", printThermalZoneTemperature, printThermalZoneTemperatureInfo, sm);
    }
...
static int getCurrentTemperature(const char *cmd)
{
    ...
    snprintf(th_file, sizeof(th_file), THERMAL_ZONE_DIR "/%.*s/" TEMPERATURE_FILE, len_zone_name, zone_name);
...
void initAcpiFan(struct SensorModul *sm)
{
    ...
    while((de = readdir(d)) != NULL)
    {
        ...
        AcpiFans++;
        snprintf(th_ref, sizeof(th_ref), "acpi/fan/%s/state", de->d_name);
        registerMonitor(th_ref, "integer", printFanState, printFanStateInfo, sm);
    }
...
static int getFanState(const char *cmd)
{
    ...
    snprintf(fan_state_file, sizeof(fan_state_file), FAN_DIR "/%.*s/" FAN_STATE_FILE, len_fan_name, fan_name);
