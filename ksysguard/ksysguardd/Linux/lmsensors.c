        ...
        while((sf = sensors_get_features(scn, &nr1)) != 0)
        {
            ...
            switch(sf->type)
            {
                case SENSORS_FEATURE_IN:
                    ssubf = sensors_get_subfeature(scn, sf, SENSORS_SUBFEATURE_IN_INPUT);
                    break;

                case SENSORS_FEATURE_FAN:
                    ssubf = sensors_get_subfeature(scn, sf, SENSORS_SUBFEATURE_FAN_INPUT);
                    break;

                case SENSORS_FEATURE_TEMP:
                    ssubf = sensors_get_subfeature(scn, sf, SENSORS_SUBFEATURE_TEMP_INPUT);
                    break;
                default:
                    ssubf = NULL;
            }
            ...
            p->fullName = (char *)malloc(strlen("lmsensors/") + strlen(scn->prefix) + 1 + strlen(label) + 1);
...
void printLmSensorInfo(const char *cmd)
{
    ...
    {
#else
    if(sensors_get_label(*s->scn, s->sfd->number, &label) != 0)
    { /*error*/
#endif
        fprintf(CurrentClient, "0\n");
        return;
