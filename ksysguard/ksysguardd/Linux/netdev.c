...
#define DEFVARS(a, b, c, d, e) unsigned long long a;

#define FORALL(a)                                                                                                                                    \
    a(recBytes, "receiver/data", "Received Data", "kBytes/s", 1024) a(recPacks, "receiver/packets", "Received Packets", "1/s", 1)                    \
        a(recErrs, "receiver/errors", "Receiver Errors", "1/s", 1) a(recDrop, "receiver/drops", "Receiver Drops", "1/s", 1)                          \
            a(recFifo, "receiver/fifo", "Receiver FIFO Overruns", "1/s", 1) a(recFrame, "receiver/frame", "Receiver Frame Errors", "1/s", 1)         \
            ...
            if(pos)
            {
                ...
                *pos = '\0';
                ...
                sscanf(buf + 7,
                       ...
                       "%llu %llu %llu %llu %llu %llu %llu %llu",
                       &recBytes, &recPacks, &recErrs, &recDrop, &recFifo, &recFrame, &recCompressed, &recMulticast, &sentBytes, &sentPacks,
    ...
    for(i = 0; sscanf(netDevBufP, format, buf) == 1; ++i)
    {
        ...
        if(sscanf(buf, devFormat, tag))
        {
            ...
            if(pos)
            {
                char mon[MON_SIZE];
                *pos = '\0';
                snprintf(NetDevs[i].name, sizeof(NetDevs[i].name), "%s", tag);
                FORALL(REGISTERSENSOR);
                sscanf(pos + 1,
                       "%llu %llu %llu %llu %llu %llu %llu %llu"
                       "%llu %llu %llu %llu %llu %llu %llu %llu",
                       &NetDevs[i].recBytes, &NetDevs[i].recPacks, &NetDevs[i].recErrs, &NetDevs[i].recDrop, &NetDevs[i].recFifo,
                       &NetDevs[i].recFrame, &NetDevs[i].recCompressed, &NetDevs[i].recMulticast, &NetDevs[i].sentBytes, &NetDevs[i].sentPacks,
                       &NetDevs[i].sentErrs, &NetDevs[i].sentDrop, &NetDevs[i].sentFifo, &NetDevs[i].sentColls, &NetDevs[i].sentCarrier,
...
int updateNetDev(void)
{
    ...
    face  | bytes    packets errs drop fifo frame compressed multicast| bytes    packets errs drop fifo colls carrier compressed
      ...
      eth0:123648812  655251    0    0    0     0          0         0 246847871  889636    0    0    0     0       0          0       Inter-|
    Receive                                                |  Transmit
    face  | bytes    packets errs drop fifo frame compressed multicast|bytes    packets errs drop fifo colls carrier compressed
        lo:275135772 1437448    0    0    0     0          0         0 275135772 1437448    0    0    0     0       0          0
...
#define PRINTFUNC(a, b, c, d, e)                                                                                                                     \
    ...
    {                                                                                                                                                \
        ...
        for(i = 0; i < MAXNETDEVS; ++i)                                                                                                              \
            ...
            {                                                                                                                                        \
                fprintf(CurrentClient, "%lu\n", (unsigned long)(NetDevs[i].a / (NetDevs[i].a##Scale * timeInterval)));                               \
                return;                                                                                                                              \
            }                                                                                                                                        \
                                                                                                                                                     \
