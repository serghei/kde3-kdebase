...
static int initStatDisk(char *tag, char *buf, const char *label, const char *shortLabel, int idx, cmdExecutor ex, cmdExecutor iq);
...
static void updateCPULoad(const char *line, CPULoadInfo *load)
{
    ...
    sscanf(line, "%*s %lu %lu %lu %lu", &currUserTicks, &currNiceTicks, &currSysTicks, &currIdleTicks);

    totalTicks =
        (currUserTicks - load->userTicks) + (currSysTicks - load->sysTicks) + (currNiceTicks - load->niceTicks) + (currIdleTicks - load->idleTicks);
...
static int processDiskIO(const char *buf)
{
    ...
    while(p && *p)
    {
        if(sscanf(p, "(%d,%d):(%lu,%lu,%lu,%lu,%lu)", &major, &minor, &total, &rio, &rblk, &wio, &wblk) != 7)
            return -1;
...
static int process26DiskIO(const char *buf)
{
    ...
    int major, minor;
    char devname[16];
    unsigned long total, rio, rmrg, rblk, rtim, wio, wmrg, wblk, wtim, ioprog, iotim, iotimw;
    DiskIOInfo *ptr = DiskIO;
    DiskIOInfo *last = 0;
    char sensorName[128];

    switch(sscanf(buf, "%d %d %s %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu", &major, &minor, devname, &rio, &rmrg, &rblk, &rtim, &wio, &wmrg, &wblk,
                  &wtim, &ioprog, &iotim, &iotimw))
    {
        case 7:
...
static void processStat(void)
{
    ...
    timeInterval = currSampling.tv_sec - lastSampling.tv_sec + (currSampling.tv_usec - lastSampling.tv_usec) / 1000000.0;
...
void initStat(struct SensorModul *sm)
{
    ...
    while(sscanf(statBufP, format, buf) == 1)
    {
        ...
        else if(strcmp("page", tag) == 0)
        {
            ...
            registerMonitor("cpu/pageIn", "integer", printPageIn, printPageInInfo, StatSM);
            registerMonitor("cpu/pageOut", "integer", printPageOut, printPageOutInfo, StatSM);
        }
        else if(strcmp("intr", tag) == 0)
        {
            unsigned int i;
            ...
            for(i = 0; p && i < NumOfInts; i++)
            {
                ...
                registerMonitor(cmdName, "integer", printInterruptx, printInterruptxInfo, StatSM);
            }
        }
        else if(strcmp("ctxt", tag) == 0)
        {
            sscanf(buf + 5, "%lu", &OldCtxt);
            registerMonitor("cpu/context", "integer", printCtxt, printCtxtInfo, StatSM);
        }
    }
...
void printDiskRBlk(const char *cmd)
{
    ...
    fprintf(CurrentClient, "%lu\n", (unsigned long)(DiskLoad[id].s[3].delta / timeInterval * 2));
}
...
void printDiskWBlk(const char *cmd)
{
    ...
    sscanf(cmd + 9, "%d", &id);
    /* a block is 512 bytes or 1/2 kBytes */
    fprintf(CurrentClient, "%lu\n", (unsigned long)(DiskLoad[id].s[4].delta / timeInterval * 2));
}
...
void printDiskIO(const char *cmd)
{
    ...
    if(strcmp(name, "total") == 0)
        ...
        fprintf(CurrentClient, "%lu\n", (unsigned long)(ptr->rblk.delta / (timeInterval * 2)));
    else if(strcmp(name, "wblk") == 0)
        fprintf(CurrentClient, "%lu\n", (unsigned long)(ptr->wblk.delta / (timeInterval * 2)));
    else
    {
        fprintf(CurrentClient, "0\n");
