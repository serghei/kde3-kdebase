...
static int updateProcess(pid_t pid)
{
    ...
    {
        if((ps = (ProcessInfo *)malloc(sizeof(ProcessInfo))) == NULL)
        {
    ...
    if((ps->nThreads = psinfo.pr_nlwp) != 0)
    {
        ...
        ps->Time = psinfo.pr_lwp.pr_time.tv_sec * 100 + psinfo.pr_lwp.pr_time.tv_nsec * 10000000;
        ps->Load = (double)psinfo.pr_lwp.pr_pctcpu / (double)0x8000 * 100.0;
    }
    else
    {
        ps->Prio = 0;
...
void printProcessListInfo(const char *cmd)
{
    fprintf(CurrentClient,
            "Name\tPID\tPPID\tGID\tStatus\tUser\tThreads"
...
void printProcessList(const char *cmd)
{
    ...
    for(ps = first_ctnr(ProcessList); ps; ps = next_ctnr(ProcessList))
    {
        fprintf(CurrentClient, "%s\t%ld\t%ld\t%ld\t%s\t%s\t%d\t%d\t%d\t%.2f\t%d\t%s\n", ps->Command, (long)ps->pid, (long)ps->ppid, (long)ps->gid,
                ps->State, ps->userName, ps->nThreads, ps->Size, ps->RSSize, ps->Load, ps->Prio, ps->CmdLine);
    }
