...
int getnetdevlist(void)
{
    ...
    for(ifr = (struct ifreq *)ifc.ifc_buf; ifr < (struct ifreq *)(ifc.ifc_buf + ifc.ifc_len); ifr++)
    {
...
void initNetDev(struct SensorModul *sm)
{
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        ...
        registerMonitor(mon, "integer", printIPackets, printIPacketsInfo, sm);
        sprintf(mon, "network/%s/opackets", IfInfo[i].Name);
        registerMonitor(mon, "integer", printOPackets, printOPacketsInfo, sm);
        /*
        ...
        if(!(IfInfo[i].flags & IFF_LOOPBACK))
        {
             ...
             *  recv errors
             */
            sprintf(mon, "network/%s/ierrors", IfInfo[i].Name);
            registerMonitor(mon, "integer", printIErrors, printIErrorsInfo, sm);
            /*
             *  xmit errors
             */
            sprintf(mon, "network/%s/oerrors", IfInfo[i].Name);
            registerMonitor(mon, "integer", printOErrors, printOErrorsInfo, sm);
            /*
             *  collisions
             */
            sprintf(mon, "network/%s/collisions", IfInfo[i].Name);
            registerMonitor(mon, "integer", printCollisions, printCollisionsInfo, sm);
            /*
             *  multicast xmits
             */
            sprintf(mon, "network/%s/multixmt", IfInfo[i].Name);
            registerMonitor(mon, "integer", printMultiXmits, printMultiXmitsInfo, sm);
            /*
             *  multicast recvs
             */
            sprintf(mon, "network/%s/multircv", IfInfo[i].Name);
            registerMonitor(mon, "integer", printMultiRecvs, printMultiRecvsInfo, sm);
            /*
             *  broadcast xmits
             */
            sprintf(mon, "network/%s/brdcstxmt", IfInfo[i].Name);
            registerMonitor(mon, "integer", printBcastXmits, printBcastXmitsInfo, sm);
            /*
             *  broadcast recvs
             */
            sprintf(mon, "network/%s/brdcstrcv", IfInfo[i].Name);
            registerMonitor(mon, "integer", printBcastRecvs, printBcastRecvsInfo, sm);
        }
    }
#endif
}
...
void printIPackets(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDipackets > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printOPackets(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDopackets > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printIErrors(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDierrors > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printOErrors(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDoerrors > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printCollisions(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDcollisions > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printMultiXmits(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDmultixmt > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printMultiRecvs(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDmultircv > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printBcastXmits(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDbrdcstxmt > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
...
void printBcastRecvs(const char *cmd)
{
    ...
    *ptr = '\0';
    ...
    for(i = 0; i < NetDevCount; i++)
    {
        if((IfInfo[i].OLDbrdcstrcv > 0) && (strcmp(IfInfo[i].Name, name) == 0))
        {
