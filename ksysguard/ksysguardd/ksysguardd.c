...
#ifdef HAVE_DNSSD
void publish_callback(DNSServiceRef ref, DNSServiceFlags f, DNSServiceErrorType errorCode, const char *name, const char *type, const char *domain,
                      void *context)
{
    if(errorCode != kDNSServiceErr_NoError)
        log_error("Publishing DNS-SD service failed with error %i", errorCode);
}
#endif
...
#ifdef HAVE_DNSSD
    if(BindToAllInterfaces)
        if(DNSServiceRegister(&Ref, 0, 0, 0, "_ksysguard._tcp", RegisterDomain ? RegisterDomain : "local.", NULL, htons(SocketPort), 0, 0,
                              publish_callback, 0)
...
#ifdef HAVE_DNSSD
        if(ServiceSocket != -1 && FD_ISSET(ServiceSocket, fds))
...
int main(int argc, char *argv[])
{
   ...
   */
    ...
    if(geteuid() != 0)
    {
        ...
        {
            fprintf(stderr,
                    "ksysguardd can't be started because of permission conflicts!\n"
