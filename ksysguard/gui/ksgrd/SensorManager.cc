...
SensorManager::SensorManager()
{
    ...
    for(int i = 0; i < 32; i++)
    {
        mDict.insert("cpu" + QString::number(i), new QString(QString(i18n("CPU%1")).arg(i)));
        mDict.insert("disk" + QString::number(i), new QString(QString(i18n("Disk%1")).arg(i)));
    }

    for(int i = 0; i < 6; i++)
    {
        mDict.insert("fan" + QString::number(i), new QString(QString(i18n("Fan%1")).arg(i)));
        mDict.insert("temp" + QString::number(i), new QString(QString(i18n("Temperature%1")).arg(i)));
    }
    ...
    {
        num.sprintf("%.2d", i);
        mDict.insert("int" + num, new QString(QString(i18n("Int%1")).arg(i - 1, 3)));
    }
...
bool SensorManager::engageHost(const QString &hostName)
{
    ...
    if(hostName.isEmpty() || mAgents.find(hostName) == 0)
    {
        ...
        if(mHostConnector->exec())
        {
            ...
            if(hostName.isEmpty())
                retVal = engage(mHostConnector->currentHostName(), shell, command, port);
            else
...
bool SensorManager::engage(const QString &hostName, const QString &shell, const QString &command, int port)
{
    ...
    if((agent = mAgents.find(hostName)) == 0)
    {
        ...
        connect(agent, SIGNAL(reconfigure(const SensorAgent *)), SLOT(reconfigure(const SensorAgent *)));
...
void SensorManager::hostLost(const SensorAgent *agent)
{
    ...
    if(mBroadcaster)
    {
        ...
        event->setData(new QString(i18n("Connection to %1 has been lost.").arg(agent->hostName())));
...
void SensorManager::saveProperties(KConfig *cfg)
{
