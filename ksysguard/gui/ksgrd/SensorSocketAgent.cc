...
SensorSocketAgent::SensorSocketAgent(SensorManager *sm) : SensorAgent(sm)
{
...
void SensorSocketAgent::error(int id)
{
    switch(id)
    {
        case KNetwork::KSocketBase::ConnectionRefused:
            SensorMgr->notify(i18n("Connection to %1 refused").arg(hostName()));
            break;
        case KNetwork::KSocketBase::LookupFailure:
            SensorMgr->notify(i18n("Host %1 not found").arg(hostName()));
            break;
        case KNetwork::KSocketBase::Timeout:
            SensorMgr->notify(i18n("Timeout at host %1").arg(hostName()));
            break;
        case KNetwork::KSocketBase::NetFailure:
            SensorMgr->notify(i18n("Network failure host %1").arg(hostName()));
            break;
        default:
