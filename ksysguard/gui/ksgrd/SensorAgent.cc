...
SensorAgent::SensorAgent(SensorManager *sm) : mSensorManager(sm)
{
...
#if SA_TRACE
    kdDebug(1215) << "-> " << req << "(" << mInputFIFO.count() << "/" << mProcessingFIFO.count() << ")" << endl;
#endif
    executeCommand();
...
void SensorAgent::processAnswer(const QString &buffer)
{
    ...
    {
#if SA_TRACE
        kdDebug(1215) << "<= " << mAnswerBuffer.left(end) << "(" << mInputFIFO.count() << "/" << mProcessingFIFO.count() << ")" << endl;
#endif
        if(!mDaemonOnLine)
        {
...
#if SA_TRACE
        kdDebug(1215) << ">> " << req->request().ascii() << "(" << mInputFIFO.count() << "/" << mProcessingFIFO.count() << ")" << endl;
#endif
...
SensorRequest::SensorRequest(const QString &request, SensorClient *client, int id) : mRequest(request), mClient(client), mId(id)
{
}
