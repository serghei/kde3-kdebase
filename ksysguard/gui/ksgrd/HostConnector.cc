...
HostConnector::HostConnector(QWidget *parent, const char *name)
    : KDialogBase(Plain, i18n("Connect Host"), Help | Ok | Cancel, Ok, parent, name, true, true)
{
    ...
    QButtonGroup *group = new QButtonGroup(0, Qt::Vertical, i18n("Connection Type"), page);
    ...
    QWhatsThis::add(mUseDaemon, i18n("Select this if you want to connect to a ksysguard daemon that is running on the machine you want to connect "
                                     "to, and is listening for client requests."));
    ...
    connect(mUseCustom, SIGNAL(toggled(bool)), mCommands, SLOT(setEnabled(bool)));
    connect(mUseDaemon, SIGNAL(toggled(bool)), mPort, SLOT(setEnabled(bool)));
    connect(mHostNames->lineEdit(), SIGNAL(textChanged(const QString &)), this, SLOT(slotHostNameChanged(const QString &)));
