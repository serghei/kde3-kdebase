...
SensorShellAgent::SensorShellAgent(SensorManager *sm) : SensorAgent(sm), mDaemon(0)
{
}
...
bool SensorShellAgent::start(const QString &host, const QString &shell, const QString &command, int)
{
    ...
    connect(mDaemon, SIGNAL(processExited(KProcess *)), SLOT(daemonExited(KProcess *)));
    connect(mDaemon, SIGNAL(receivedStdout(KProcess *, char *, int)), SLOT(msgRcvd(KProcess *, char *, int)));
    connect(mDaemon, SIGNAL(receivedStderr(KProcess *, char *, int)), SLOT(errMsgRcvd(KProcess *, char *, int)));
    connect(mDaemon, SIGNAL(wroteStdin(KProcess *)), SLOT(msgSent(KProcess *)));
...
void SensorShellAgent::errMsgRcvd(KProcess *, char *buffer, int buflen)
{
    ...
    kdDebug(1215) << "SensorShellAgent: Warning, received text over stderr!" << endl << buf << endl;
}
