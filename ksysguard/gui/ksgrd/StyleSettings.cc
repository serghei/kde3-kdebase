...
StyleSettings::StyleSettings(QWidget *parent, const char *name)
    : KDialogBase(Tabbed, i18n("Global Style Settings"), Help | Ok | Apply | Cancel, Ok, parent, name, true, true)
{
    ...
    connect(mColorListBox, SIGNAL(selectionChanged(QListBoxItem *)), SLOT(selectionChanged(QListBoxItem *)));
    connect(mColorListBox, SIGNAL(doubleClicked(QListBoxItem *)), SLOT(editSensorColor()));
    connect(mEditColorButton, SIGNAL(clicked()), SLOT(editSensorColor()));
