...
Workspace::Workspace(QWidget *parent, const char *name) : QTabWidget(parent, name)
{
    ...
    connect(this, SIGNAL(currentChanged(QWidget *)), SLOT(updateCaption(QWidget *)));
...
Workspace::~Workspace()
{
    ...
    disconnect(this, SIGNAL(currentChanged(QWidget *)), this, SLOT(updateCaption(QWidget *)));
}
...
void Workspace::newWorkSheet()
{
    ...
    if(dlg.exec())
    {
        ...
        showPage(sheet);
        connect(sheet, SIGNAL(sheetModified(QWidget *)), SLOT(updateCaption(QWidget *)));
...
bool Workspace::saveOnQuit()
{
    ...
    for(; it.current(); ++it)
        if((*it)->modified())
        {
            ...
            {
                int res = KMessageBox::warningYesNoCancel(this, i18n("The worksheet '%1' contains unsaved data.\n"
...
{
    KFileDialog dlg(0, i18n("*.sgrd|Sensor Files"), this, "LoadFileDialog", true);
...
void Workspace::saveWorkSheet(WorkSheet *sheet)
{
    ...
    {
        KFileDialog dlg(0, i18n("*.sgrd|Sensor Files"), this, "LoadFileDialog", true);
        fileName = dlg.getSaveFileName(mWorkDir + "/" + tabLabel(sheet) + ".sgrd", "*.sgrd", 0, i18n("Save Current Worksheet As"));
        if(fileName.isEmpty())
            return;
...
void Workspace::saveWorkSheetAs(WorkSheet *sheet)
{
    ...
    do
    {
        ...
        fileName = dlg.getSaveFileName(mWorkDir + "/" + tabLabel(currentPage()) + ".sgrd", "*.sgrd");
        if(fileName.isEmpty())
            return;
...
void Workspace::showProcesses()
{
    ...
    if(!processSensor || !processSensor->isA("ProcessController"))
        return;
