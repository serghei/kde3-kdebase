...
KDE_EXPORT KPanelApplet *init(QWidget *parent, const QString &configFile)
{
    ...
    return new KSysGuardApplet(configFile, KPanelApplet::Normal, KPanelApplet::Preferences, parent, "ksysguardapplet");
}
}

KSysGuardApplet::KSysGuardApplet(const QString &configFile, Type type, int actions, QWidget *parent, const char *name)
...
void KSysGuardApplet::dropEvent(QDropEvent *e)
{
    ...
    if(QTextDrag::decode(e, dragObject))
    {
        ...
        if(mDockList[dock]->isA("QFrame"))
        {
            if(sensorType == "integer" || sensorType == "float")
            {
                ...
                switch(popup.exec(QCursor::pos()))
                {
                    ...
                    case 3:
                        wdg = new DancingBars(this, "DancingBars", sensorDescr, 100, 100, true);
                        break;
                }
                ...
                if(wdg)
                {
                    ...
                    connect(wdg, SIGNAL(modified(bool)), SLOT(sensorDisplayModified(bool)));
        ...
        if(!mDockList[dock]->isA("QFrame"))
            ((KSGRD::SensorDisplay *)mDockList[dock])->addSensor(hostName, sensorName, sensorType, sensorDescr);
    }
...
bool KSysGuardApplet::load()
{
    ...
    for(i = 0; i < dnList.count(); ++i)
    {
        ...
        KSGRD::SensorMgr->engage(element.attribute("name"), element.attribute("shell"), element.attribute("command"), port);
    }
    ...
    for(i = 0; i < dnList.count(); ++i)
    {
        ...
        if(i >= mDockCount)
        {
            kdDebug(1215) << "Dock number " << i << " out of range " << mDockCount << endl;
            return false;
        }
        ...
        connect(newDisplay, SIGNAL(modified(bool)), SLOT(sensorDisplayModified(bool)));
    }
...
void KSysGuardApplet::addEmptyDisplay(QWidget **dock, uint pos)
{
    ...
    QToolTip::add(dock[pos], i18n("Drag sensors from the KDE System Guard into this cell."));
