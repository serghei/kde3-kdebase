...
ProcessController::ProcessController(QWidget *parent, const char *name, const QString &title, bool nf) : KSGRD::SensorDisplay(parent, name, title, nf)
{
    ...
    connect(pList, SIGNAL(killProcess(int, int)), this, SLOT(killProcess(int, int)));
    connect(pList, SIGNAL(reniceProcess(const QValueList< int > &, int)), this, SLOT(reniceProcess(const QValueList< int > &, int)));
    connect(pList, SIGNAL(listModified(bool)), this, SLOT(setModified(bool)));
    ...
    connect(xbTreeView, SIGNAL(toggled(bool)), this, SLOT(setTreeView(bool)));


    /* When the both cbFilter and pList are constructed we can connect the
     * missing link. */
    connect(cbFilter, SIGNAL(activated(int)), this, SLOT(filterModeChanged(int)));

    // Create the 'Refresh' button.
    bRefresh = new KPushButton(KGuiItem(i18n("&Refresh"), "reload"), this, "bRefresh");
...
bool ProcessController::addSensor(const QString &hostName, const QString &sensorName, const QString &sensorType, const QString &title)
{
    if(sensorType != "table")
        return (false);
...
void ProcessController::updateList()
{
...
{
    sendRequest(sensors().at(0)->hostName(), QString("kill %1 %2").arg(pid).arg(sig), 3);
...
void ProcessController::killProcess()
{
    ...
    {
        QString msg = i18n("Do you want to kill the selected process?", "Do you want to kill the %n selected processes?", selectedAsStrings.count());

        KDialogBase *dlg = new KDialogBase(i18n("Kill Process"), KDialogBase::Yes | KDialogBase::Cancel, KDialogBase::Yes, KDialogBase::Cancel,
                                           this->parentWidget(), "killconfirmation", true, true, KGuiItem(i18n("Kill")));

        bool dontAgain = false;

        int res = KMessageBox::createKMessageBox(dlg, QMessageBox::Question, msg, selectedAsStrings, i18n("Do not ask again"), &dontAgain,
                                                 KMessageBox::Notify);
    ...
    for(it = selectedPIds.begin(); it != selectedPIds.end(); ++it)
        sendRequest(sensors().at(0)->hostName(), QString("kill %1 %2").arg(*it).arg(MENU_ID_SIGKILL), 3);
...
void ProcessController::reniceProcess(const QValueList< int > &pids, int niceValue)
{
    for(QValueList< int >::ConstIterator it = pids.constBegin(), end = pids.constEnd(); it != end; ++it)
        sendRequest(sensors().at(0)->hostName(), QString("setpriority %1 %2").arg(*it).arg(niceValue), 5);
    sendRequest(sensors().at(0)->hostName(), "ps", 2); // update the display afterwards
}

void ProcessController::answerReceived(int id, const QString &answer)
{
        ...
        case 3:
        {
            ...
            switch(vals[0].toInt())
            {
                ...
                case 1: // unknown error
                    KSGRD::SensorMgr->notify(i18n("Error while attempting to kill process %1.").arg(vals[1]));
                    break;
                case 2:
                    KSGRD::SensorMgr->notify(i18n("Insufficient permissions to kill "
                                                  "process %1.")
                                                 .arg(vals[1]));
                    break;
                case 3:
                    KSGRD::SensorMgr->notify(i18n("Process %1 has already disappeared.").arg(vals[1]));
                    break;
                case 4:
        ...
        case 5:
        {
            ...
            switch(vals[0].toInt())
            {
                case 0: // successful renice operation
                    break;
                case 1: // unknown error
                    KSGRD::SensorMgr->notify(i18n("Error while attempting to renice process %1.").arg(vals[1]));
                    break;
                case 2:
                    KSGRD::SensorMgr->notify(i18n("Insufficient permissions to renice "
                                                  "process %1.")
                                                 .arg(vals[1]));
                    break;
                case 3:
...
void ProcessController::sensorError(int, bool err)
{
...
bool ProcessController::restoreSettings(QDomElement &element)
{
    bool result = addSensor(element.attribute("hostName"), element.attribute("sensorName"),
                            (element.attribute("sensorType").isEmpty() ? "table" : element.attribute("sensorType")), QString::null);
...
bool ProcessController::saveSettings(QDomDocument &doc, QDomElement &element, bool save)
{
