...
SensorDisplay::SensorDisplay(QWidget *parent, const char *name, const QString &title, bool nf, bool isApplet) : QWidget(parent, name)
{
    mIsApplet = isApplet;
...
{
    if(event->type() == QEvent::MouseButtonPress && ((QMouseEvent *)event)->button() == RightButton)
    {
        QPopupMenu pm;
        if(mIsApplet)
        {
    ...
    else if(event->type() == QEvent::MouseButtonRelease && ((QMouseEvent *)event)->button() == LeftButton)
    {
        setFocus();
    }
...
void SensorDisplay::sendRequest(const QString &hostName, const QString &command, int id)
{
...
{
    QWhatsThis::add(this, i18n("<qt><p>This is a sensor display. To customize a sensor display click "
...
QColor SensorDisplay::restoreColor(QDomElement &element, const QString &attr, const QColor &fallback)
{
    bool ok;
...
void SensorDisplay::saveColor(QDomElement &element, const QString &attr, const QColor &color)
{
    int r, g, b;
    color.rgb(&r, &g, &b);
...
bool SensorDisplay::addSensor(const QString &hostName, const QString &name, const QString &type, const QString &description)
{
...
void SensorDisplay::setSensorOk(bool ok)
{
    ...
    else
    {
        ...
        QPixmap errorIcon = KGlobal::iconLoader()->loadIcon("connect_creating", KIcon::Desktop, KIcon::SizeSmall);
        if(!mPlotterWdg)
            return;
...
SensorProperties::SensorProperties(const QString &hostName, const QString &name, const QString &type, const QString &description)
