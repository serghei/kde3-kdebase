...
LogFile::LogFile(QWidget *parent, const char *name, const QString &title) : KSGRD::SensorDisplay(parent, name, title)
{
...
bool LogFile::addSensor(const QString &hostName, const QString &sensorName, const QString &sensorType, const QString &title)
{
...
bool LogFile::restoreSettings(QDomElement &element)
{
    QFont font;
    QColorGroup cgroup = monitor->colorGroup();
    ...
    addSensor(element.attribute("hostName"), element.attribute("sensorName"),
              (element.attribute("sensorType").isEmpty() ? "logfile" : element.attribute("sensorType")), element.attribute("title"));
...
bool LogFile::saveSettings(QDomDocument &doc, QDomElement &element, bool save)
{
...
void LogFile::updateMonitor()
{
    sendRequest(sensors().at(0)->hostName(), QString("%1 %2").arg(sensors().at(0)->name()).arg(logFileID), 19);
}

void LogFile::answerReceived(int id, const QString &answer)
{
