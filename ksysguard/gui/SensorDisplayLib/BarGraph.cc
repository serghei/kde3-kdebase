...
BarGraph::BarGraph(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
bool BarGraph::removeBar(uint idx)
{
    if(idx >= bars)
    {
        kdDebug(1215) << "BarGraph::removeBar: idx " << idx << " out of range " << bars << endl;
        return false;
    }
...
void BarGraph::paintEvent(QPaintEvent *)
{
    ...
    if(bars > 0)
    {
        ...
        {
            int topVal = (int)((float)barHeight / maxValue * (samples[b] - minValue));
            ...
            {
                if((upperLimitActive && samples[b] > upperLimit) || (lowerLimitActive && samples[b] < lowerLimit))
            ...
            if(showLabels)
            {
                p.drawText(b * barWidth + 3, h - (2 * fm.lineSpacing()) - 2, barWidth - 2 * 3, fm.lineSpacing(), Qt::AlignCenter, footers[b]);
                p.drawText(b * barWidth + 3, h - fm.lineSpacing() - 2, barWidth - 2 * 3, fm.lineSpacing(), Qt::AlignCenter,
