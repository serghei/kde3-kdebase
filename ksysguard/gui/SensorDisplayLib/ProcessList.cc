...
ProcessList::ProcessList(QWidget *parent, const char *name) : KListView(parent, name)
{
    iconCache.setAutoDelete(true);

    columnDict.setAutoDelete(true);
    columnDict.insert("running", new QString(i18n("process status", "running")));
    columnDict.insert("sleeping", new QString(i18n("process status", "sleeping")));
    columnDict.insert("disk sleep", new QString(i18n("process status", "disk sleep")));
    columnDict.insert("zombie", new QString(i18n("process status", "zombie")));
    columnDict.insert("stopped", new QString(i18n("process status", "stopped")));
    ...
    connect(parent, SIGNAL(setFilterMode(int)), this, SLOT(setFilterMode(int)));
    ...
    connect(header(), SIGNAL(sizeChange(int, int, int)), this, SLOT(sizeChanged(int, int, int)));
    connect(header(), SIGNAL(indexChange(int, int, int)), this, SLOT(indexChanged(int, int, int)));
...
const QValueList< int > &ProcessList::getSelectedPIds()
{
    selectedPIds.clear();
...
const QStringList &ProcessList::getSelectedAsStrings()
{
...
bool ProcessList::update(const QString &list)
{
...
void ProcessList::setTreeView(bool tv)
{
...
bool ProcessList::load(QDomElement &el)
{
...
bool ProcessList::save(QDomDocument &doc, QDomElement &display)
{
...
void ProcessList::sortingChanged(int col)
{
    if(col == sortColumn)
...
bool ProcessList::matchesFilter(KSGRD::SensorPSLine *p) const
{
...
void ProcessList::deleteLeaves(void)
{
    for(;;)
    {
        unsigned int i;
        for(i = 0; i < pl.count() && (!isLeafProcess(pl.at(i)->pid()) || matchesFilter(pl.at(i))); i++)
            ;
        if(i == pl.count())
            return;
...
bool ProcessList::isLeafProcess(int pid)
{
...
void ProcessList::extendTree(QPtrList< KSGRD::SensorPSLine > *pl, ProcessLVI *parent, int ppid)
{
...
void ProcessList::addProcess(KSGRD::SensorPSLine *p, ProcessLVI *pli)
{
    QString name = p->name();
    if(aliases[name])
        name = *aliases[name];
    ...
    if(!iconCache[name])
    {
        pix = KGlobal::iconLoader()->loadIcon(name, KIcon::Small, KIcon::SizeSmall, KIcon::DefaultState, 0L, true);
        if(pix.isNull() || !pix.mask())
            pix = KGlobal::iconLoader()->loadIcon("unknownapp", KIcon::User, KIcon::SizeSmall);
...
void ProcessList::updateMetaInfo(void)
{
    selectedPIds.clear();
...
void ProcessList::removeColumns(void)
{
    for(int i = columns() - 1; i >= 0; --i)
        removeColumn(i);
}

void ProcessList::addColumn(const QString &label, const QString &type)
{
    ...
    {
        kdDebug(1215) << "Unknown type " << type << " of column " << label << " in ProcessList!" << endl;
        return;
    }
    ...
    if(currentWidth.count() - 1 == col)
    {
        ...
        for(uint i = 0; i < col; ++i)
        {
            ...
            setColumnWidthMode(i, currentWidth[i] == 0 ? QListView::Manual : QListView::Maximum);
...
void ProcessList::handleRMBPressed(QListViewItem *lvi, const QPoint &p, int col)
{
    if(!lvi)
        return;
        ...
        default:
            ...
            if(id < 100)
            {
                ...
                QString msg = i18n("Do you really want to send signal %1 to the selected process?",
                                   "Do you really want to send signal %1 to the %n selected processes?", selectedPIds.count())
...
void ProcessList::selectAllItems(bool select)
{
    selectedPIds.clear();
...
void ProcessList::selectAllChilds(int pid, bool select)
{
    QListViewItemIterator it(this, QListViewItemIterator::Visible);
