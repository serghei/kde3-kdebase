...
SignalPlotter::SignalPlotter(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
void SignalPlotter::updateDataBuffers()
{
    ...
    uint newSampleNum = static_cast< uint >(((width() - 2) / mHorizontalScale) + 2.5);
    ...
    for(uint i = 0; i < mBeamData.count(); ++i)
    {
        ...
        memcpy(nd + (newSampleNum - overlap), mBeamData.at(i) + (mSamples - overlap), overlap * sizeof(double));
...
void SignalPlotter::paintEvent(QPaintEvent *)
{
    ...
    if(mShowHorizontalLines)
    {
        ...
        for(uint y = 1; y <= mHorizontalLinesCount; y++)
        {
            ...
            if(mShowLabels && h > (mFontSize + 1) * (mHorizontalLinesCount + 1) && w > 60)
            {
