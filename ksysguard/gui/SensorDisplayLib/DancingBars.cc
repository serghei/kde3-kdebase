...
DancingBars::DancingBars(QWidget *parent, const char *name, const QString &title, int, int, bool noFrame_, bool isApplet)
...
void DancingBars::applySettings()
{
    ...
    mPlotter->setLimits(mSettingsDialog->useLowerLimit() ? mSettingsDialog->lowerLimit() : 0, mSettingsDialog->useLowerLimit(),
                        mSettingsDialog->useUpperLimit() ? mSettingsDialog->upperLimit() : 0, mSettingsDialog->useUpperLimit());
    ...
    for(uint i = 0; i < sensors().count(); i++)
    {
        ...
        {
            if((*it)[0] == sensors().at(i)->hostName() && (*it)[1] == KSGRD::SensorMgr->translateSensor(sensors().at(i)->name()))
            {
...
bool DancingBars::addSensor(const QString &hostName, const QString &name, const QString &type, const QString &title)
{
...
bool DancingBars::removeSensor(uint pos)
{
    if(pos >= mBars)
    {
        kdDebug(1215) << "DancingBars::removeSensor: idx out of range (" << pos << ")" << endl;
        return false;
    }
...
void DancingBars::answerReceived(int id, const QString &answer)
{
    ...
    if(id < 100)
    {
        ...
        {
            kdDebug(1215) << "ERROR: DancingBars lost sample (" << mFlags << ", " << mBars << ")" << endl;
            sensorError(id, true);
        }
...
bool DancingBars::restoreSettings(QDomElement &element)
{
    ...
    mPlotter->changeRange(element.attribute("min", "0").toDouble(), element.attribute("max", "0").toDouble());

    mPlotter->setLimits(element.attribute("lowlimit", "0").toDouble(), element.attribute("lowlimitactive", "0").toInt(),
                        element.attribute("uplimit", "0").toDouble(), element.attribute("uplimitactive", "0").toInt());

    mPlotter->normalColor = restoreColor(element, "normalColor", KSGRD::Style->firstForegroundColor());
    mPlotter->alarmColor = restoreColor(element, "alarmColor", KSGRD::Style->alarmColor());
    mPlotter->backgroundColor = restoreColor(element, "backgroundColor", KSGRD::Style->backgroundColor());
    mPlotter->fontSize = element.attribute("fontSize", QString("%1").arg(KSGRD::Style->fontSize())).toInt();
    ...
    for(uint i = 0; i < dnList.count(); ++i)
    {
        ...
        addSensor(el.attribute("hostName"), el.attribute("sensorName"),
                  (el.attribute("sensorType").isEmpty() ? "integer" : el.attribute("sensorType")), el.attribute("sensorDescr"));
    }
...
bool DancingBars::saveSettings(QDomDocument &doc, QDomElement &element, bool save)
{
