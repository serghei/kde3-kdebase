...
FancyPlotter::FancyPlotter(QWidget *parent, const char *name, const QString &title, double, double, bool nf, bool isApplet)
...
void FancyPlotter::applySettings()
{
    ...
    else
    {
        ...
        mPlotter->changeRange(0, mSettingsDialog->minValue(), mSettingsDialog->maxValue());
    }
...
void FancyPlotter::applyStyle()
{
    ...
    for(uint i = 0; i < mPlotter->beamColors().count() && i < KSGRD::Style->numSensorColors(); ++i)
...
bool FancyPlotter::addSensor(const QString &hostName, const QString &name, const QString &type, const QString &title)
{
    return addSensor(hostName, name, type, title, KSGRD::Style->sensorColor(mBeams));
}

bool FancyPlotter::addSensor(const QString &hostName, const QString &name, const QString &type, const QString &title, const QColor &color)
{
    ...
    for(uint i = 0; i < mBeams; ++i)
    {
        tooltip += QString("%1%2:%3").arg(i != 0 ? "\n" : "").arg(sensors().at(mBeams - i - 1)->hostName()).arg(sensors().at(mBeams - i - 1)->name());
    }
...
bool FancyPlotter::removeSensor(uint pos)
{
    if(pos >= mBeams)
    {
        kdDebug(1215) << "FancyPlotter::removeSensor: idx out of range (" << pos << ")" << endl;
        return false;
    }
...
void FancyPlotter::answerReceived(int id, const QString &answer)
{
    ...
    else if(id >= 100)
    {
        ...
        if(!mPlotter->useAutoRange() && mPlotter->minValue() == 0.0 && mPlotter->maxValue() == 0.0)
        {
...
bool FancyPlotter::restoreSettings(QDomElement &element)
{
    ...
    {
        mPlotter->setUseAutoRange(false);
        mPlotter->changeRange(0, element.attribute("min").toDouble(), element.attribute("max").toDouble());
    }

    mPlotter->setShowVerticalLines(element.attribute("vLines", "1").toUInt());
    mPlotter->setVerticalLinesColor(restoreColor(element, "vColor", KSGRD::Style->firstForegroundColor()));
    ...
    mPlotter->setHorizontalLinesColor(restoreColor(element, "hColor", KSGRD::Style->secondForegroundColor()));
    ...
    mPlotter->setFontSize(element.attribute("fontSize", QString("%1").arg(KSGRD::Style->fontSize())).toUInt());

    mPlotter->setBackgroundColor(restoreColor(element, "bColor", KSGRD::Style->backgroundColor()));
    ...
    for(uint i = 0; i < dnList.count(); ++i)
    {
        ...
        addSensor(el.attribute("hostName"), el.attribute("sensorName"),
                  (el.attribute("sensorType").isEmpty() ? "integer" : el.attribute("sensorType")), "",
                  restoreColor(el, "color", KSGRD::Style->sensorColor(i)));
    }
...
bool FancyPlotter::saveSettings(QDomDocument &doc, QDomElement &element, bool save)
{
...
FPSensorProperties::FPSensorProperties(const QString &hostName, const QString &name, const QString &type, const QString &description,
                                       const QColor &color)
    : KSGRD::SensorProperties(hostName, name, type, description), mColor(color)
{
}
