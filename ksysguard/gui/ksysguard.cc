...
/**
  ...
  TaskMan widget.
 */
TopLevel::TopLevel(const char *name) : KMainWindow(0, name), DCOPObject("KSysGuardIface")
{
    ...
    connect(mServiceBrowser, SIGNAL(serviceAdded(DNSSD::RemoteService::Ptr)), this, SLOT(serviceAdded(DNSSD::RemoteService::Ptr)));

    mWorkSpace = new Workspace(mSplitter);
    connect(mWorkSpace, SIGNAL(announceRecentURL(const KURL &)), SLOT(registerRecentURL(const KURL &)));
    connect(mWorkSpace, SIGNAL(setCaption(const QString &, bool)), SLOT(setCaption(const QString &, bool)));
    connect(KSGRD::Style, SIGNAL(applyStyleToWorksheet()), mWorkSpace, SLOT(applyStyle()));
    ...
    new KAction(i18n("&New Worksheet..."), "tab_new", 0, mWorkSpace, SLOT(newWorkSheet()), actionCollection(), "new_worksheet");

    new KAction(i18n("Import Worksheet..."), "fileopen", 0, mWorkSpace, SLOT(loadWorkSheet()), actionCollection(), "import_worksheet");

    mActionOpenRecent = new KRecentFilesAction(i18n("&Import Recent Worksheet"), "fileopen", 0, mWorkSpace, SLOT(loadWorkSheet(const KURL &)),
                                               actionCollection(), "recent_import_worksheet");

    new KAction(i18n("&Remove Worksheet"), "tab_remove", 0, mWorkSpace, SLOT(deleteWorkSheet()), actionCollection(), "remove_worksheet");

    new KAction(i18n("&Export Worksheet..."), "filesaveas", 0, mWorkSpace, SLOT(saveWorkSheetAs()), actionCollection(), "export_worksheet");

    KStdAction::quit(this, SLOT(close()), actionCollection());

    new KAction(i18n("C&onnect Host..."), "connect_established", 0, this, SLOT(connectHost()), actionCollection(), "connect_host");
    new KAction(i18n("D&isconnect Host"), "connect_no", 0, this, SLOT(disconnectHost()), actionCollection(), "disconnect_host");
    ...
    new KAction(i18n("&Worksheet Properties"), "configure", 0, mWorkSpace, SLOT(configure()), actionCollection(), "configure_sheet");

    new KAction(i18n("Load Standard Sheets"), "revert", 0, this, SLOT(resetWorkSheets()), actionCollection(), "revert_all_worksheets");

    new KAction(i18n("Configure &Style..."), "colorize", 0, this, SLOT(editStyle()), actionCollection(), "configure_style");
...
{
    if(KMessageBox::warningContinueCancel(this, i18n("Do you really want to restore the default worksheets?"), i18n("Reset All Worksheets"),
                                          i18n("Reset"), "AskResetWorkSheets")
       == KMessageBox::Cancel)
        return;
...
void TopLevel::initStatusBar()
{
     ...
     * answerReceived(). */
    KSGRD::SensorMgr->sendRequest("localhost", "mem/swap/used?", (KSGRD::SensorClient *)this, 5);
    updateStatusBar();
...
void TopLevel::timerEvent(QTimerEvent *)
{
    if(statusBar()->isVisibleTo(this))
    {
        ...
        KSGRD::SensorMgr->sendRequest("localhost", "pscount", (KSGRD::SensorClient *)this, 0);
        KSGRD::SensorMgr->sendRequest("localhost", "mem/physical/free", (KSGRD::SensorClient *)this, 1);
        KSGRD::SensorMgr->sendRequest("localhost", "mem/physical/used", (KSGRD::SensorClient *)this, 2);
        KSGRD::SensorMgr->sendRequest("localhost", "mem/swap/free", (KSGRD::SensorClient *)this, 3);
        KSGRD::SensorMgr->sendRequest("localhost", "mem/swap/used", (KSGRD::SensorClient *)this, 4);
    }
}
    ...
    switch(id)
    {
        ...
        case 2:
            ...
            s = i18n("Memory: %1 %2 used, %3 %4 free")
                    .arg(KGlobal::locale()->formatNumber(mUsed, 0))
                    .arg(unit)
                    .arg(KGlobal::locale()->formatNumber(mFree, 0))
                    .arg(unit);
...
void TopLevel::setSwapInfo(long used, long free, const QString &unit)
{
    ...
    else
    {
        msg = i18n("Swap: %1 %2 used, %3 %4 free")
                  .arg(KGlobal::locale()->formatNumber(used, 0))
                  .arg(unit)
                  .arg(KGlobal::locale()->formatNumber(free, 0))
                  .arg(unit);
    }
...
static const KCmdLineOptions options[] = {{"showprocesses", I18N_NOOP("Show only process list of local host"), 0},
...
int main(int argc, char **argv)
{
    ...
    KAboutData aboutData("ksysguard", I18N_NOOP("KDE System Guard"), KSYSGUARD_VERSION, Description, KAboutData::License_GPL,
    ...
    if(args->isSet("showprocesses"))
    {
        ...
        if(app->dcopClient()->registerAs("ksysguard_taskmanager", false) == "ksysguard_taskmanager")
        {
        ...
        {
            QByteArray data;
            app->dcopClient()->send("ksysguard_taskmanager", "KSysGuardIface", "showOnCurrentDesktop()", data);
        }
    }
    else
    {
