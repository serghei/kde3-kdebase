...
public:
    HostItem(SensorBrowser *parent, const QString &text, int id, KSGRD::SensorAgent *host)
        : QListViewItem(parent, text), mInited(false), mId(id), mHost(host), mParent(parent)
    {
        setExpandable(true);
    }
...
SensorBrowser::SensorBrowser(QWidget *parent, KSGRD::SensorManager *sm, const char *name) : KListView(parent, name), mSensorManager(sm)
{
    mHostInfoList.setAutoDelete(true);

    connect(mSensorManager, SIGNAL(update()), SLOT(update()));
    connect(this, SIGNAL(clicked(QListViewItem *)), SLOT(newItemSelected(QListViewItem *)));
    connect(this, SIGNAL(returnPressed(QListViewItem *)), SLOT(newItemSelected(QListViewItem *)));
...
void SensorBrowser::newItemSelected(QListViewItem *item)
{
    ...
    if(item && item->pixmap(0) && showAnnoyingPopup)
    {
        ...
        KMessageBox::information(this, i18n("Drag sensors to empty fields in a worksheet."), QString::null, "ShowSBUseInfo");
    }
}
    ...
    for(uint i = 0; i < lines.count(); ++i)
    {
        ...
        for(uint j = 0; j < absolutePath.count(); ++j)
        {
            ...
            if(!found)
            {
                ...
                {
                    QPixmap pix = KGlobal::iconLoader()->loadIcon("ksysguardd", KIcon::Desktop, KIcon::SizeSmall);
                    lvi->setPixmap(0, pix);
...
void SensorBrowser::viewportMouseMoveEvent(QMouseEvent *e)
{
    ...
    mDragText = (*it)->hostName() + " " + (*it)->sensorName(item) + " " + (*it)->sensorType(item) + " " + (*it)->sensorDescription(item);
...
/**
 Helper classes
 */
SensorInfo::SensorInfo(QListViewItem *lvi, const QString &name, const QString &desc, const QString &type)
...
HostInfo::HostInfo(int id, const KSGRD::SensorAgent *agent, const QString &name, QListViewItem *lvi)
...
void HostInfo::addSensor(QListViewItem *lvi, const QString &name, const QString &desc, const QString &type)
{
