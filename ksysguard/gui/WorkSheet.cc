...
WorkSheet::WorkSheet(QWidget *parent, const char *name) : QWidget(parent, name)
{
    mGridLayout = 0;
    mRows = mColumns = 0;
    mDisplayList = 0;
...
WorkSheet::WorkSheet(uint rows, uint columns, uint interval, QWidget *parent, const char *name) : QWidget(parent, name)
{
    mRows = mColumns = 0;
    mGridLayout = 0;
    mDisplayList = 0;
...
bool WorkSheet::load(const QString &fileName)
{
    ...
    {
        KMessageBox::sorry(this, i18n("The file %1 does not contain valid XML.").arg(mFileName));
        return false;
    }
    ...
    if(!(rowsOk && columnsOk))
    {
        KMessageBox::sorry(this, i18n("The file %1 has an invalid worksheet size.").arg(mFileName));
        return false;
    }
    ...
    for(i = 0; i < dnList.count(); ++i)
    {
        ...
        {
            kdDebug(1215) << "Row or Column out of range (" << row << ", " << column << ")" << endl;
            return false;
        }
...
KSGRD::SensorDisplay *WorkSheet::addDisplay(const QString &hostName, const QString &sensorName, const QString &sensorType, const QString &sensorDescr,
...
void WorkSheet::replaceDisplay(uint row, uint column, KSGRD::SensorDisplay *newDisplay)
{
    ...
    else
    {
        ...
        connect(newDisplay, SIGNAL(modified(bool)), SLOT(setModified(bool)));
    }
