...
bool scanDesktopFile(QPtrList< AppLnkCache > &appCache, const QString &templ, QString destDir)
{
    ...
    QString pexec = KGlobal::dirs()->findExe(exec, QString(::getenv("PATH")) + ":/usr/X11R6/bin:/usr/games");
    if(pexec.isEmpty())
    {
        kdDebug(DBG_CODE) << "looking for " << exec.local8Bit() << "\t\tnot found" << endl;
        return false;
    }
    ...
    kdDebug(DBG_CODE) << "looking for " << exec.local8Bit() << "\t\tfound" << endl;
    return true;
}
