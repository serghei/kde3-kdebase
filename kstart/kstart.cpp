...
void KStart::sendRule()
{
    ...
    if(windowtitle || windowclass)
    {
        ...
        message +=
            "types=" + QCString().setNum(-1U & ~(NET::TopMenuMask | NET::ToolbarMask | NET::DesktopMask | NET::SplashMask | NET::MenuMask)) + "\n";
    }
    else
    {
    ...
    if((desktop > 0 && desktop <= kwinmodule->numberOfDesktops()) || desktop == NETWinInfo::OnAllDesktops)
    {
...
void KStart::windowAdded(WId w)
{
    ...
    if(windowclass)
    {
        ...
        if(!XGetClassHint(qt_xdisplay(), w, &hint))
            return;
        QCString cls = windowclass.contains(' ') ? QCString(hint.res_name) + ' ' + hint.res_class : QCString(hint.res_class);
        cls = cls.lower();
        XFree(hint.res_name);
...
static bool wstate_withdrawn(WId winid)
{
    ...
    unsigned char *data;
    int r = XGetWindowProperty(qt_xdisplay(), winid, qt_wm_state, 0, 2, FALSE, AnyPropertyType, &type, &format, &length, &after, &data);
    bool withdrawn = TRUE;
...
int main(int argc, char *argv[])
{
    ...
    KAboutData aboutData("kstart", I18N_NOOP("KStart"), KSTART_VERSION,
                         I18N_NOOP(""
                                   ...
                                   "and so on."),
                         KAboutData::License_GPL, "(C) 1997-2000 Matthias Ettrich (ettrich@kde.org)");
