...
const long ClientWinMask = KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | KeymapStateMask | ButtonMotionMask
                           | PointerMotionMask | // need this, too!
                           EnterWindowMask | LeaveWindowMask | FocusChangeMask | ExposureMask | StructureNotifyMask | SubstructureRedirectMask;
...
public:
    ...
    {
        return kwin_shape_version > 0;
    }
    static int version()
    {
        return kwin_shape_version;
    } // as 16*major+minor, i.e. two hex digits
...
#define KWIN_CHECK_PREDICATE(name, check)                                                                                                            \
    struct name                                                                                                                                      \
    {                                                                                                                                                \
        inline bool operator()(const Client *cl)                                                                                                     \
        {                                                                                                                                            \
            return check;                                                                                                                            \
        };                                                                                                                                           \
    }
...
#define KWIN_PROCEDURE(name, action)                                                                                                                 \
    struct name                                                                                                                                      \
    {                                                                                                                                                \
        inline void operator()(Client *cl)                                                                                                           \
        {                                                                                                                                            \
            action;                                                                                                                                  \
        };                                                                                                                                           \
    }
...
inline int timestampCompare(Time time1, Time time2) // like strcmp()
{
    return NET::timestampCompare(time1, time2);
}

inline Time timestampDiff(Time time1, Time time2) // returns time2 - time1
{
