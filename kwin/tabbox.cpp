...
TabBox::TabBox(Workspace *ws, const char *name) : QFrame(0, name, Qt::WNoAutoErase), current_client(NULL), wspace(ws)
{
    ...
    outline_left =
        XCreateWindow(qt_xdisplay(), qt_xrootwin(), 0, 0, 1, 1, 0, CopyFromParent, CopyFromParent, CopyFromParent, CWOverrideRedirect, &attr);
    outline_right =
        XCreateWindow(qt_xdisplay(), qt_xrootwin(), 0, 0, 1, 1, 0, CopyFromParent, CopyFromParent, CopyFromParent, CWOverrideRedirect, &attr);
    outline_top =
        XCreateWindow(qt_xdisplay(), qt_xrootwin(), 0, 0, 1, 1, 0, CopyFromParent, CopyFromParent, CopyFromParent, CWOverrideRedirect, &attr);
        ...
        else
        {
            for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
            {
                if(workspace()->hasClient(*it)) // safety
                {
                    ...
                    p.drawText(x + 5 + iconWidth + 8, y, r.width() - 5 - iconWidth - 8, lineHeight, Qt::AlignLeft | Qt::AlignVCenter | Qt::SingleLine,
                               s);
...
static bool areKeySymXsDepressed(bool bAll, const uint keySyms[], int nKeySyms)
{
    ...
    for(int iKeySym = 0; iKeySym < nKeySyms; iKeySym++)
    {
        ...
        kdDebug(125) << iKeySym << ": keySymX=0x" << QString::number(keySymX, 16) << " i=" << i << " mask=0x" << QString::number(mask, 16)
...
void Workspace::CDEWalkThroughWindows(bool forward)
{
    ...
    {
        if((*it)->isOnCurrentDesktop() && !(*it)->isSpecialWindow() && (*it)->isShown(false) && (*it)->wantsTabFocus() && !(*it)->keepAbove()
           && !(*it)->keepBelow())
        {
            c = *it;
            break;
    ...
    } while(nc && nc != c && ((!options_traverse_all && !nc->isOnDesktop(currentDesktop())) || nc->isMinimized() || !nc->wantsTabFocus()
                              || nc->keepAbove() || nc->keepBelow()));
    if(nc)
    {
        if(c && c != nc)
            lowerClient(c);
...
void Workspace::tabBoxKeyPress(const KKeyNative &keyX)
{
    ...
    if(tab_grab)
    {
        ...
        if(forward || backward)
        {
            kdDebug(125) << "== " << cutWalkThroughWindows.toStringInternal() << " or " << cutWalkThroughWindowsReverse.toStringInternal() << endl;
...
{
    unsigned int mk =
        ev.state & (KKeyNative::modX(KKey::SHIFT) | KKeyNative::modX(KKey::CTRL) | KKeyNative::modX(KKey::ALT) | KKeyNative::modX(KKey::WIN));
