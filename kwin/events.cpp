...
WinInfo::WinInfo(Client *c, Display *display, Window window, Window rwin, const unsigned long pr[], int pr_size)
...
void RootInfo::changeActiveWindow(Window w, NET::RequestSource src, Time timestamp, Window active_window)
{
    if(Client *c = workspace->findClient(WindowMatchPredicate(w)))
    {
        ...
        else                                    // NET::FromApplication
        {
            ...
            else if(active_window != None && (c2 = workspace->findClient(WindowMatchPredicate(active_window))) != NULL
                    && workspace->allowClientActivation(c2, timestampCompare(timestamp, c2->userTime() > 0 ? timestamp : c2->userTime())))
                workspace->activateClient(c);
            else
                c->demandAttention();
        }
...
bool Workspace::workspaceEvent(XEvent *e)
{
    ...
    {
        case CreateNotify:
            if(e->xcreatewindow.parent == root && !QWidget::find(e->xcreatewindow.window) && !e->xcreatewindow.override_redirect)
            {
        ...
        case UnmapNotify:
        {
            ...
            if(removeSystemTrayWin(e->xunmap.window, true))
            {
                ...
                if(XCheckTypedWindowEvent(qt_xdisplay(), w, ReparentNotify, &ev))
                {
        ...
        case ConfigureRequest:
        {
            if(e->xconfigurerequest.parent == root)
            {
                ...
                wc.sibling = None;
                wc.stack_mode = Above;
                unsigned int value_mask = e->xconfigurerequest.value_mask & (CWX | CWY | CWWidth | CWHeight | CWBorderWidth);
...
bool Client::windowEvent(XEvent *e)
{
    if(e->xany.window == window()) // avoid doing stuff on frame or wrapper
    {
        ...
        if((dirty[WinInfo::PROTOCOLS] & NET::WMIconName) != 0)
            fetchIconicName();
        if((dirty[WinInfo::PROTOCOLS] & NET::WMStrut) != 0 || (dirty[WinInfo::PROTOCOLS2] & NET::WM2ExtendedStrut) != 0)
        {
    ...
    switch(e->type)
    {
        ...
        case ButtonPress:
            updateUserTime();
            workspace()->setWasUserInteraction();
            buttonPressEvent(e->xbutton.window, e->xbutton.button, e->xbutton.state, e->xbutton.x, e->xbutton.y, e->xbutton.x_root,
                             e->xbutton.y_root);
            break;
        case KeyRelease:
        ...
        case ButtonRelease:
            ...
            // e.g. if the user presses Alt+F2, the Alt release
            // would appear as user input to the currently active window
            buttonReleaseEvent(e->xbutton.window, e->xbutton.button, e->xbutton.state, e->xbutton.x, e->xbutton.y, e->xbutton.x_root,
                               e->xbutton.y_root);
            break;
        case MotionNotify:
            motionNotifyEvent(e->xmotion.window, e->xmotion.state, e->xmotion.x, e->xmotion.y, e->xmotion.x_root, e->xmotion.y_root);
        ...
        case EnterNotify:
            ...
            motionNotifyEvent(e->xcrossing.window, e->xcrossing.state, e->xcrossing.x, e->xcrossing.y, e->xcrossing.x_root, e->xcrossing.y_root);
...
void Client::unmapNotifyEvent(XUnmapEvent *e)
{
    ...
    switch(mappingState())
    {
        ...
        case NormalState:
            ...
            XEvent ev;
            if(XCheckTypedWindowEvent(qt_xdisplay(), window(), DestroyNotify, &ev)) // TODO I don't like this much
            {
...
void Client::enterNotifyEvent(XCrossingEvent *e)
{
    ...
    if(e->mode == NotifyNormal || (!options->focusPolicyIsReasonable() && e->mode == NotifyUngrab))
    {
        ...
        if(options->autoRaise && !isDesktop() && !isDock() && !isTopMenu() && workspace()->focusChangeEnabled()
        ...
        if(options->focusPolicy != Options::FocusFollowsMouse || currentPos != workspace()->focusMousePosition())
        {
...
void Client::leaveNotifyEvent(XCrossingEvent *e)
{
    ...
    if(e->mode == NotifyNormal)
    {
        ...
        if(!lostMouse && e->detail != NotifyInferior)
        {
            ...
            unsigned int d5;
            Window w, child;
            if(XQueryPointer(qt_xdisplay(), frameId(), &w, &child, &d1, &d2, &d3, &d4, &d5) == False || child == None)
...
void Client::grabButton(int modifier)
{
    ...
    for(int i = 0; i < 8; ++i)
        XGrabButton(qt_xdisplay(), AnyButton, modifier | mods[i], wrapperId(), FALSE, ButtonPressMask, GrabModeSync, GrabModeAsync, None, None);
}
...
void Client::ungrabButton(int modifier)
{
    unsigned int mods[8] = {0, XCapL, XNumL, XNumL | XCapL, XScrL, XScrL | XCapL, XScrL | XNumL, XScrL | XNumL | XCapL};
    for(int i = 0; i < 8; ++i)
        XUngrabButton(qt_xdisplay(), AnyButton, modifier | mods[i], wrapperId());
}
#undef XCapL
#undef XNumL
#undef XScrL
...
void Client::updateMouseGrab()
{
    ...
    if(isActive() && !workspace()->forcedGlobalMouseGrab()) // see Workspace::establishTabBoxGrab()
    {
        ...
        XGrabButton(qt_xdisplay(), AnyButton, AnyModifier, wrapperId(), FALSE, ButtonPressMask, GrabModeSync, GrabModeAsync, None, None);
...
{
    if(decoration == NULL || o != decoration->widget())
        return false;
    if(e->type() == QEvent::MouseButtonPress)
    {
        QMouseEvent *ev = static_cast< QMouseEvent * >(e);
        return buttonPressEvent(decorationId(), qtToX11Button(ev->button()), qtToX11State(ev->state()), ev->x(), ev->y(), ev->globalX(),
                                ev->globalY());
    }
    if(e->type() == QEvent::MouseButtonRelease)
    {
        QMouseEvent *ev = static_cast< QMouseEvent * >(e);
        return buttonReleaseEvent(decorationId(), qtToX11Button(ev->button()), qtToX11State(ev->state()), ev->x(), ev->y(), ev->globalX(),
                                  ev->globalY());
    }
    if(e->type() == QEvent::MouseMove) // FRAME i fake z enter/leave?
    {
        QMouseEvent *ev = static_cast< QMouseEvent * >(e);
        return motionNotifyEvent(decorationId(), qtToX11State(ev->state()), ev->x(), ev->y(), ev->globalX(), ev->globalY());
    }
    if(e->type() == QEvent::Wheel)
    {
        QWheelEvent *ev = static_cast< QWheelEvent * >(e);
        bool r = buttonPressEvent(decorationId(), ev->delta() > 0 ? Button4 : Button5, qtToX11State(ev->state()), ev->x(), ev->y(), ev->globalX(),
                                  ev->globalY());
        r = r || buttonReleaseEvent(decorationId(), ev->delta() > 0 ? Button4 : Button5, qtToX11State(ev->state()), ev->x(), ev->y(), ev->globalX(),
                                    ev->globalY());
        return r;
    }
...
bool Client::buttonPressEvent(Window w, int button, int state, int x, int y, int x_root, int y_root)
{
    ...
    { // FRAME neco s tohohle by se melo zpracovat, nez to dostane dekorace
        updateUserTime();
        workspace()->setWasUserInteraction();
        uint keyModX = (options->keyCmdAllModKey() == Qt::Key_Meta) ? KKeyNative::modX(KKey::WIN) : KKeyNative::modX(KKey::ALT);
        bool bModKeyHeld = keyModX != 0 && (state & KKeyNative::accelModMaskX()) == keyModX;

        if(isSplash() && button == Button1 && !bModKeyHeld)
        ...
        { // inactive inner window
            ...
            // active inner window
            if(isActive() && w == wrapperId() && options->clickRaise && button < 4) // exclude wheel
            {
...
void Client::processDecorationButtonPress(int button, int /*state*/, int x, int y, int x_root, int y_root)
{
    ...
    if(button == Button1 && com != Options::MouseOperationsMenu // actions where it's not possible to get the matching
...
static bool waitingMotionEvent()
{
    ...
    if(next_motion_time != CurrentTime && timestampCompare(qt_x_time, next_motion_time) < 0)
        return true;
    was_motion = false;
...
void Client::NETMoveResize(int x_root, int y_root, NET::Direction direction)
{
    ...
    {
        static const Position convert[] = {PositionTopLeft,     PositionTop,    PositionTopRight,   PositionRight,
