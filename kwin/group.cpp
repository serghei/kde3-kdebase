...
bool performTransiencyCheck()
{
    ...
    for(ClientList::ConstIterator it1 = clients.begin(); it1 != clients.end(); ++it1)
    {
        if((*it1)->deleting)
            continue;
        ...
        else
        {
            ...
            for(ClientList::ConstIterator it2 = mains.begin(); it2 != mains.end(); ++it2)
            {
                if(transiencyCheckNonExistent && !Workspace::self()->clients.contains(*it2) && !Workspace::self()->desktops.contains(*it2))
                {
        ...
        for(ClientList::ConstIterator it2 = trans.begin(); it2 != trans.end(); ++it2)
        {
            if(transiencyCheckNonExistent && !Workspace::self()->clients.contains(*it2) && !Workspace::self()->desktops.contains(*it2))
            {
    ...
    for(GroupList::ConstIterator it1 = groups.begin(); it1 != groups.end(); ++it1)
    {
        ClientList members = (*it1)->members();
        for(ClientList::ConstIterator it2 = members.begin(); it2 != members.end(); ++it2)
        {
            if((*it2)->in_group != *it1)
            {
...
Group::Group(Window leader_P, Workspace *workspace_P)
    : leader_client(NULL), leader_wid(leader_P), _workspace(workspace_P), leader_info(NULL), user_time(-1U), refcount(0)
{
    if(leader_P != None)
    {
        leader_client = workspace_P->findClient(WindowMatchPredicate(leader_P));
        unsigned long properties[2] = {0, NET::WM2StartupId};
        leader_info = new NETWinInfo(qt_xdisplay(), leader_P, workspace()->rootWin(), properties, 2);
    }
...
{
    assert(leader != None);
    for(GroupList::ConstIterator it = groups.begin(); it != groups.end(); ++it)
        if((*it)->leader() == leader)
            return *it;
    return NULL;
}
    ...
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
    {
        ...
        if((*it)->wmClientLeader() == c->wmClientLeader())
        {
            ...
            else
            {
                ...
                for(unsigned int pos = 0; pos < old_group.count(); ++pos)
                {
...
void Workspace::updateMinimizedOfTransients(Client *c)
{
    ...
    if(c->isMinimized() || c->isShade())
    {
        ...
        {
            if(!(*it)->isMinimized() && !(*it)->isTopMenu()) // topmenus are not minimized, they're hidden
            {
    ...
    { // else unmiminize the transients
        for(ClientList::ConstIterator it = c->transients().begin(); it != c->transients().end(); ++it)
        {
            if((*it)->isMinimized() && !(*it)->isTopMenu())
            {
...
bool Client::belongToSameApplication(const Client *c1, const Client *c2, bool active_hack)
{
    ...
    else if(c1->pid() != c2->pid() || c1->wmClientMachine(false) != c2->wmClientMachine(false))
        ; // different processes
    else if(c1->wmClientLeader() != c2->wmClientLeader()
            && c1->wmClientLeader() != c1->window()  // if WM_CLIENT_LEADER is not set, it returns window(),
            && c2->wmClientLeader() != c2->window()) // don't use in this test then
...
void Client::cleanGrouping()
{
    ...
    //         ++it )
    //        kdDebug() << "MN2:" << *it << endl;
    for(ClientList::ConstIterator it = transients_list.begin(); it != transients_list.end();)
    {
    ...
    in_group = NULL;
    for(ClientList::ConstIterator it = group_members.begin(); it != group_members.end(); ++it)
        (*it)->removeTransient(this);
...
{
    TRANSIENCY_CHECK(this);
    for(ClientList::ConstIterator it1 = group()->members().begin(); it1 != group()->members().end(); ++it1)
    {
        if(!(*it1)->groupTransient()) // check all group transients in the group
            continue;                 // TODO optimize to check only the changed ones?
        for(ClientList::ConstIterator it2 = group()->members().begin(); it2 != group()->members().end();
        ...
        {          // so don't make them transient for the ones that are transient for it
            if(*it1 == *it2)
                continue;
            for(Client *cl = (*it2)->transientFor(); cl != NULL; cl = cl->transientFor())
            {
                if(cl == *it1)
            ...
            for(ClientList::ConstIterator it3 = group()->members().begin(); it3 != group()->members().end(); ++it3)
            {
...
Window Client::verifyTransientFor(Window new_transient_for, bool defined)
{
    ...
    while(new_transient_for != None && new_transient_for != workspace()->rootWin()
    ...
    if(Client *new_transient_for_client = workspace()->findClient(WindowMatchPredicate(new_transient_for)))
    {
        ...
        {
            kdDebug(1212) << "Client " << this << " has WM_TRANSIENT_FOR poiting to non-toplevel window " << before_search << ", child of "
                          << new_transient_for_client << ", adjusting." << endl;
    ...
    if(new_transient_for != workspace()->rootWin() && workspace()->findClient(WindowMatchPredicate(new_transient_for)) == NULL)
...
bool Client::hasTransientInternal(const Client *cl, bool indirect, ConstClientList &set) const
{
    ...
    if(set.contains(this))
        return false;
    set.append(this);
    for(ClientList::ConstIterator it = transients().begin(); it != transients().end(); ++it)
...
void Client::checkGroup(Group *set_group, bool force)
{
    ...
    {
        for(ClientList::Iterator it = transients_list.begin(); it != transients_list.end();)
        ...
        if(groupTransient())
        {
            ...
            if(old_group != NULL)
            {
                for(ClientList::ConstIterator it = old_group->members().begin(); it != old_group->members().end(); ++it)
                    (*it)->removeTransient(this);
            }
