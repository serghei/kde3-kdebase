...
static void drawB2Rect(KPixmap *pix, const QColor &primary, bool down)
{
    ...
    {
        KPixmapEffect::gradient(*pix, hColor, lColor, KPixmapEffect::DiagonalGradient);
    }
    else
        pix->fill(primary);
    int x2 = pix->width() - 1;
...
static void create_pixmaps()
{
    ...
    for(i = 0; i < NUM_PIXMAPS; i++)
    {
        ...
        switch(i / NumStates)
        {
            ...
            case P_RESIZE:
                break;
            case P_ICONIFY:
                pixmap[i]->resize(10, 10);
                break;
            case P_SHADE:
            case P_CLOSE:
                pixmap[i]->resize(bsize, bsize);
                break;
            default:
                pixmap[i]->resize(16, 16);
                break;
        }
    }
    ...
    mask.fillRect(normalizeMask.width() - 12, normalizeMask.height() - 12, 12, 12, one);
...
QValueList< B2ClientFactory::BorderSize > B2ClientFactory::borderSizes() const
{
    ...
    return QValueList< BorderSize >() << BorderTiny << BorderNormal << BorderLarge << BorderVeryLarge << BorderHuge;
}
...
B2Client::B2Client(KDecorationBridge *b, KDecorationFactory *f) : KDecoration(b, f), bar_x_ofs(0), in_unobs(0)
{
}
...
void B2Client::calcHiddenButtons()
{
    ...
    B2Button *btnArray[] = {button[BtnShade], button[BtnSticky],  button[BtnHelp],  button[BtnResize],
...
void B2Client::paintEvent(QPaintEvent *e)
{
    ...
    KDecoration::ColorType frameColorGroup = colored_frame ? KDecoration::ColorTitleBar : KDecoration::ColorFrame;
    ...
    {
        // inner window rect
        p.drawRect(thickness - 1, t.bottom(), width() - 2 * (thickness - 1), fHeight - bDepth + 2);

        if(thickness >= 3)
        {
            // frame shade panel
            qDrawShadePanel(&p, 1, t.bottom() - thickness + 2, width() - 2, fHeight - 2 - bb + thickness, fillColor, false);
            if(thickness == 4)
            {
                p.setPen(fillColor.background());
                p.drawRect(thickness - 2, t.bottom() - 1, width() - 2 * (thickness - 2), fHeight + 4 - bDepth);
            }
            else if(thickness > 4)
            {
                qDrawShadePanel(&p, thickness - 2, t.bottom() - 1, width() - 2 * (thickness - 2), fHeight + 4 - bDepth, fillColor, true);
                if(thickness >= 5)
                {
                    // draw frame interior
                    p.fillRect(2, t.bottom() - thickness + 3, width() - 4, thickness - 4, fillBrush);
                    p.fillRect(2, height() - bDepth + 2, width() - 4, thickness - 4, fillBrush);
                    p.fillRect(2, t.bottom() - 1, thickness - 4, fHeight - bDepth + 4, fillBrush);
                    p.fillRect(width() - thickness + 2, t.bottom() - 1, thickness - 4, fHeight - bDepth + 4, fillBrush);
                }
            }
        }
    }
    ...
    if(mustDrawHandle())
    {
        ...
        p.drawLine(width() - 1, height() - thickness - 4, width() - 1, height() - 1);
        p.drawLine(hx, height() - 1, width() - 1, height() - 1);
        p.drawLine(hx, height() - 4, hx, height() - 1);

        p.fillRect(hx + 1, height() - thickness - 3, hw - 2, thickness + 2, fillBrush);

        p.setPen(fillColor.dark());
        p.drawLine(width() - 2, height() - thickness - 4, width() - 2, height() - 2);
        p.drawLine(hx + 1, height() - 2, width() - 2, height() - 2);

        p.setPen(fillColor.light());
        p.drawLine(hx + 1, height() - thickness - 2, hx + 1, height() - 3);
        p.drawLine(hx + 1, height() - thickness - 3, width() - 3, height() - thickness - 3);
    }
...
void B2Client::doShape()
{
    ...
    if(t.right() < width() - 1)
    {
        mask -= QRect(width() - 1, t.height() - thickness, 1, 1); // top right point
        mask -= QRect(t.right() + 1, 0, width() - t.right() - 1, t.height() - thickness);
    }
    // bottom right point
...
KDecoration::Position B2Client::mousePosition(const QPoint &p) const
{
    ...
    {
        if(p.x() > bar_x_ofs + thickness && p.x() < lx - thickness && p.y() > thickness)
...
void B2Client::maximizeChange()
{
    ...
    bottomSpacer->changeSize(10, thickness + (mustDrawHandle() ? 4 : 0), QSizePolicy::Expanding, QSizePolicy::Minimum);
...
void B2Client::menuButtonPressed()
{
    ...
    bool dbl = (lastClient == this && time.elapsed() <= QApplication::doubleClickInterval());
    lastClient = this;
    time.start();
    if(!dbl)
    {
...
static void redraw_pixmaps()
{
    ...
    for(i = 0; i < NumStates; i++)
    {
        ...
        pix->fill(Qt::black);
        bitBlt(pix, 0, 0, &thinBox, 0, 0, thinBox.width(), thinBox.height(), Qt::CopyROP, true);
    }
    ...
    for(i = 0; i < NumStates; i++)
    {
        ...
        bitBlt(pix, pix->width() - 12, pix->width() - 12, &largeBox, 0, 0, 12, 12, Qt::CopyROP, true);
        bitBlt(pix, 0, 0, &smallBox, 0, 0, 10, 10, Qt::CopyROP, true);

        bitBlt(pixmap[P_ICONIFY * NumStates + i], 0, 0, &smallBox, 0, 0, 10, 10, Qt::CopyROP, true);
    }
    ...
    for(i = 0; i < NumStates; i++)
    {
        ...
        *pixmap[P_RESIZE * NumStates + i] = *pixmap[P_CLOSE * NumStates + i];
        ...
        drawB2Rect(&smallBox, is_act ? aGrp.button() : iGrp.button(), is_down);
        bitBlt(pixmap[P_RESIZE * NumStates + i], 0, 0, &smallBox, 0, 0, 10, 10, Qt::CopyROP, true);
    }
    ...
    for(int j = 0; j < 3; j++)
    {
        ...
        switch(j)
        {
            case 0:
                pix = P_CLOSE;
                light = close_white_bits;
                dark = close_dgray_bits;
                break;
            case 1:
                pix = P_MENU;
                light = menu_white_bits;
                dark = menu_dgray_bits;
                break;
            default:
                pix = P_HELP;
                light = help_light_bits;
                dark = help_dark_bits;
                break;
        }
        ...
        for(i = 0; i < NumStates; i++)
        {
            ...
            kColorBitmaps(&p, (i < 3) ? aGrp : iGrp, off, off, 16, 16, true, light, NULL, NULL, dark, NULL, NULL);
            p.end();
        }
    }
    ...
    for(i = 0; i < NumStates; i++)
    {
        ...
        kColorBitmaps(&p, (i < 3) ? aGrp : iGrp, 0, 0, 16, 16, true, white, gray, NULL, dgray, NULL, NULL);
        p.end();
    }
    ...
    for(i = 0; i < P_NUM_BUTTON_TYPES; i++)
    {
        ...
        hilighted = ie.apply(*pixmap[offset + Norm], KIcon::Small, KIcon::ActiveState);
        *pixmap[offset + Hover] = hilighted;

        hilighted = ie.apply(*pixmap[offset + INorm], KIcon::Small, KIcon::ActiveState);
    ...
    if(QPixmap::defaultDepth() > 8)
    {
        QColor titleColor[4] = {options()->color(KDecoration::ColorTitleBar, true), options()->color(KDecoration::ColorFrame, true),

                                options()->color(KDecoration::ColorTitleBlend, false), options()->color(KDecoration::ColorTitleBar, false)};
        ...
        for(i = 0; i < 2; i++)
        {
            if(titleColor[2 * i] != titleColor[2 * i + 1])
            {
                ...
                KPixmapEffect::gradient(*titleGradient[i], titleColor[2 * i], titleColor[2 * i + 1], KPixmapEffect::VerticalGradient);
            }
            else
            {
                delete titleGradient[i];
...
bool B2Client::drawbound(const QRect &geom, bool clear)
{
    ...
    if(!visible_bound)
    {
        ...
        bound_shape.putPoints(0, 8, g.left(), frameTop, barLeft, frameTop, barLeft, g.top(), barRight, g.top(), barRight, frameTop, g.right(),
                              frameTop, g.right(), g.bottom(), g.left(), g.bottom());
    }
    else
    {
        *visible_bound = geom;
...
B2Button::B2Button(B2Client *_client, QWidget *parent, const QString &tip, const int realizeBtns) : QButton(parent, 0), hover(false)
{
...
void B2Button::drawButton(QPainter *p)
{
    ...
    if(useMiniIcon)
    {
        QPixmap miniIcon = client->icon().pixmap(QIconSet::Small, client->isActive() ? QIconSet::Normal : QIconSet::Disabled);
        p->drawPixmap((width() - miniIcon.width()) / 2, (height() - miniIcon.height()) / 2, miniIcon);
    }
    else
    {
        int type;
        ...
        p->drawPixmap((width() - icon[type]->width()) / 2, (height() - icon[type]->height()) / 2, *icon[type]);
    }
}
...
B2Titlebar::B2Titlebar(B2Client *parent)
    : QWidget(parent->widget(), 0, WStyle_Customize | WRepaintNoErase), client(parent), set_x11mask(false), isfullyobscured(false), shift_move(false)
{
    setBackgroundMode(NoBackground);
    captionSpacer = new QSpacerItem(buttonSize, buttonSize + 4, QSizePolicy::Expanding, QSizePolicy::Fixed);
}
...
bool B2Titlebar::x11Event(XEvent *e)
{
    ...
    {
        set_x11mask = true;
        XSelectInput(qt_xdisplay(), winId(), KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | KeymapStateMask | ButtonMotionMask
                                                 | EnterWindowMask | LeaveWindowMask | FocusChangeMask | ExposureMask | PropertyChangeMask
                                                 | StructureNotifyMask | SubstructureRedirectMask | VisibilityChangeMask);
    }
    switch(e->type)
    {
...
void B2Titlebar::drawTitlebar(QPainter &p, bool state)
{
    ...
    // titlebar fill
    const QColorGroup cg = options()->colorGroup(KDecoration::ColorTitleBar, state);
    QBrush brush(cg.background());
    if(gradient)
        brush.setPixmap(*gradient);
    qDrawShadeRect(&p, 1, 1, t.right() - 1, t.height() - 1, cg, false, 1, 0, &brush);
...
{
    if(client->isActive())
        bitBlt(this, 0, 0, &titleBuffer, 0, 0, titleBuffer.width(), titleBuffer.height(), Qt::CopyROP, true);
    else
    {
        QPainter p(this);
