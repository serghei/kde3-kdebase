...
Rules::Rules(const QString &str, bool temporary) : temporary_state(temporary ? 2 : 0)
{
    KTempFile file;
    QFile *f = file.file();
    if(f != NULL)
...
Rules::Rules(KConfig &cfg) : temporary_state(0)
{
    readFromCfg(cfg);
}
...
bool Rules::isEmpty() const
{
    return (placementrule == UnusedForceRule && positionrule == UnusedSetRule && sizerule == UnusedSetRule && minsizerule == UnusedForceRule
            ...
            && ignorepositionrule == UnusedForceRule && desktoprule == UnusedSetRule && typerule == UnusedForceRule
            && maximizevertrule == UnusedSetRule && maximizehorizrule == UnusedSetRule && minimizerule == UnusedSetRule && shaderule == UnusedSetRule
            && skiptaskbarrule == UnusedSetRule && skippagerrule == UnusedSetRule && aboverule == UnusedSetRule && belowrule == UnusedSetRule
            && fullscreenrule == UnusedSetRule && noborderrule == UnusedSetRule && fsplevelrule == UnusedForceRule
            && acceptfocusrule == UnusedForceRule && moveresizemoderule == UnusedForceRule && closeablerule == UnusedForceRule
            && strictgeometryrule == UnusedForceRule && shortcutrule == UnusedSetRule && disableglobalshortcutsrule == UnusedForceRule);
}
...
bool Rules::matchWMClass(const QCString &match_class, const QCString &match_name) const
{
    ...
    { // TODO optimize?
        QCString cwmclass = wmclasscomplete ? match_name + ' ' + match_class : match_class;
...
bool Rules::matchClientMachine(const QCString &match_machine) const
{
    if(clientmachinematch != UnimportantMatch)
    {
        ...
        if(match_machine != "localhost" && isLocalMachine(match_machine) && matchClientMachine("localhost"))
            return true;
        if(clientmachinematch == RegExpMatch && QRegExp(clientmachine).search(match_machine) == -1)
            return false;
...
void WindowRules::discardTemporary()
{
    ...
    for(QValueVector< Rules * >::Iterator it = rules.begin(); it != rules.end();)
    {
...
WindowRules Workspace::findWindowRules(const Client *c, bool ignore_temporary)
{
    ...
    for(QValueList< Rules * >::Iterator it = rules.begin(); it != rules.end();)
    {
...
void Workspace::writeWindowRules()
{
    ...
    int i = 1;
    for(QValueList< Rules * >::ConstIterator it = rules.begin(); it != rules.end(); ++it)
    {
        if((*it)->isTemporary())
            continue;
        cfg.setGroup(QString::number(i));
