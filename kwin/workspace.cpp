    ...
    , forced_global_mouse_grab(false)
{
    ...
    startup = new KStartupInfo(KStartupInfo::DisableKWinModule | KStartupInfo::AnnounceSilenceChanges, this);

    // select windowmanager privileges
    XSelectInput(qt_xdisplay(), root,
                 KeyPressMask | PropertyChangeMask | ColormapChangeMask | SubstructureRedirectMask | SubstructureNotifyMask
    ...
    XChangeProperty(qt_xdisplay(), qt_xrootwin(), atoms->kwin_running, atoms->kwin_running, 32, PropModeAppend, (unsigned char *)&data, 1);
...
void Workspace::init()
{
    ...
    null_focus_window =
        XCreateWindow(qt_xdisplay(), qt_xrootwin(), -1, -1, 1, 1, 0, CopyFromParent, InputOnly, CopyFromParent, CWOverrideRedirect, &attr);
    XMapWindow(qt_xdisplay(), null_focus_window);

    unsigned long protocols[5] = {
        NET::Supported | NET::SupportingWMCheck | NET::ClientList | NET::ClientListStacking | NET::DesktopGeometry | NET::NumberOfDesktops
            | NET::CurrentDesktop | NET::ActiveWindow | NET::WorkArea | NET::CloseWindow | NET::DesktopNames | NET::KDESystemTrayWindows | NET::WMName
            | NET::WMVisibleName | NET::WMDesktop | NET::WMWindowType | NET::WMState | NET::WMStrut | NET::WMIconGeometry | NET::WMIcon | NET::WMPid
            | NET::WMMoveResize | NET::WMKDESystemTrayWinFor | NET::WMFrameExtents | NET::WMPing,
        NET::NormalMask | NET::DesktopMask | NET::DockMask | NET::ToolbarMask | NET::MenuMask | NET::DialogMask | NET::OverrideMask | NET::TopMenuMask
            | NET::UtilityMask | NET::SplashMask | 0,
        NET::Modal |
            //        NET::Sticky |  // large desktops not supported (and probably never will be)
            NET::MaxVert | NET::MaxHoriz | NET::Shaded | NET::SkipTaskbar | NET::KeepAbove |
            //        NET::StaysOnTop |  the same like KeepAbove
            NET::SkipPager | NET::Hidden | NET::FullScreen | NET::KeepBelow | NET::DemandsAttention | 0,
        NET::WM2UserTime | NET::WM2StartupId | NET::WM2AllowedActions | NET::WM2RestackWindow | NET::WM2MoveResizeWindow | NET::WM2ExtendedStrut
            | NET::WM2KDETemporaryRules | NET::WM2ShowingDesktop | NET::WM2DesktopLayout | 0,
        NET::ActionMove | NET::ActionResize | NET::ActionMinimize | NET::ActionShade |
            //        NET::ActionStick | // Sticky state is not supported
            NET::ActionMaxVert | NET::ActionMaxHoriz | NET::ActionFullScreen | NET::ActionChangeDesktop | NET::ActionClose | 0,
    };

    rootInfo = new RootInfo(this, qt_xdisplay(), supportWindow->winId(), "KWin", protocols, 5, qt_xscreen());
    ...
    if(new_active_client == NULL && activeClient() == NULL && should_get_focus.count() == 0) // no client activated in manage()
    {
...
void Workspace::updateFocusChains(Client *c, FocusChainChange change)
{
    ...
    { // now on all desktops, add it to focus_chains it is not already in
        ...
        { // making first/last works only on current desktop, don't affect all desktops
            if(i == currentDesktop() && (change == FocusChainMakeFirst || change == FocusChainMakeLast))
            {
                focus_chain[i].remove(c);
            ...
            { // add it after the active one
                if(active_client != NULL && active_client != c && !focus_chain[i].isEmpty() && focus_chain[i].last() == active_client)
    ...
    {
        if(active_client != NULL && active_client != c && !global_focus_chain.isEmpty() && global_focus_chain.last() == active_client)
...
void Workspace::updateCurrentTopMenu()
{
    ...
    if(active_client)
    {
        ...
        for(;;)
        {
            ...
            for(ClientList::ConstIterator it = menu_client->transients().begin(); it != menu_client->transients().end(); ++it)
                if((*it)->isTopMenu())
                {
                    menubar = *it;
                    break;
        ...
        { // try to find any topmenu from the application (#72113)
            for(ClientList::ConstIterator it = active_client->group()->members().begin(); it != active_client->group()->members().end(); ++it)
                if((*it)->isTopMenu())
                {
                    menubar = *it;
                    break;
    ...
    if(!menubar && !block_desktop_menubar && options->desktopTopMenu())
    {
        ...
        if(desktop != NULL)
        {
            for(ClientList::ConstIterator it = desktop->transients().begin(); it != desktop->transients().end(); ++it)
                if((*it)->isTopMenu())
                {
                    menubar = *it;
                    break;
        ...
        if(menubar == NULL)
        {
            for(ClientList::ConstIterator it = topmenus.begin(); it != topmenus.end(); ++it)
...
void Workspace::updateToolWindows(bool also_hide)
{
        ...
        if((*it)->isUtility() || (*it)->isMenu() || (*it)->isToolbar())
        {
            ...
            if(!show && also_hide)
            {
                ...
                if(mainclients.isEmpty())
                    show = true;
                for(ClientList::ConstIterator it2 = mainclients.begin(); it2 != mainclients.end(); ++it2)
                {
    ...
    for(ClientList::ConstIterator it = to_show.fromLast(); it != to_show.end(); --it) // from topmost
        // TODO since this is in stacking order, the order of taskbar entries changes :(
        (*it)->hideClient(false);
    if(also_hide)
    {
        for(ClientList::ConstIterator it = to_hide.begin(); it != to_hide.end(); ++it) // from bottommost
            (*it)->hideClient(true);
...
void ObscuringWindows::create(Client *c)
{
    ...
    else
    {
        ...
        obs_win = XCreateWindow(qt_xdisplay(), qt_xrootwin(), c->x(), c->y(), c->width(), c->height(), 0, CopyFromParent, InputOutput, CopyFromParent,
                                CWBackPixmap | CWOverrideRedirect, &a);
    }
...
bool Workspace::setCurrentDesktop(int new_desktop)
{
    ...
    if(options->focusPolicyIsReasonable())
    {
        ...
        if(movingClient != NULL && active_client == movingClient && focus_chain[currentDesktop()].contains(active_client)
        ...
        if(!c)
        {
            for(ClientList::ConstIterator it = focus_chain[currentDesktop()].fromLast(); it != focus_chain[currentDesktop()].end(); --it)
            {
...
void Workspace::sendClientToDesktop(Client *c, int desk, bool dont_activate)
{
    ...
    {
        if(c->wantsTabFocus() && options->focusPolicyIsReasonable() && !was_on_desktop // for stickyness changes
           && !dont_activate)
            requestFocus(c);
        else
    ...
    for(ClientList::ConstIterator it = transients_stacking_order.begin(); it != transients_stacking_order.end(); ++it)
...
void Workspace::updateDesktopLayout()
{
    ...
    layoutOrientation = (rootInfo->desktopLayoutOrientation() == NET::OrientationHorizontal ? Qt::Horizontal : Qt::Vertical);
...
void Workspace::slotGrabWindow()
{
    if(active_client)
    {
        ...
        if(Shape::available())
        {
            ...
            int count, order;
            XRectangle *rects = XShapeGetRectangles(qt_xdisplay(), active_client->frameId(), ShapeBounding, &count, &order);
...
WId Workspace::getMouseEmulationWindow()
{
    ...
    do
    {
        ...
        XQueryPointer(qt_xdisplay(), w, &root, &child, &root_x, &root_y, &lx, &ly, &state);
...
void Workspace::createBorderWindows()
{
    ...
    attributes.cursor = XCreateFontCursor(qt_xdisplay(), XC_sb_up_arrow);
    electric_top_border =
        XCreateWindow(qt_xdisplay(), qt_xrootwin(), 0, 0, r.width(), 1, 0, CopyFromParent, InputOnly, CopyFromParent, valuemask, &attributes);
    XMapWindow(qt_xdisplay(), electric_top_border);

    attributes.cursor = XCreateFontCursor(qt_xdisplay(), XC_sb_down_arrow);
    electric_bottom_border = XCreateWindow(qt_xdisplay(), qt_xrootwin(), 0, r.height() - 1, r.width(), 1, 0, CopyFromParent, InputOnly,
                                           CopyFromParent, valuemask, &attributes);
    XMapWindow(qt_xdisplay(), electric_bottom_border);

    attributes.cursor = XCreateFontCursor(qt_xdisplay(), XC_sb_left_arrow);
    electric_left_border =
        XCreateWindow(qt_xdisplay(), qt_xrootwin(), 0, 0, 1, r.height(), 0, CopyFromParent, InputOnly, CopyFromParent, valuemask, &attributes);
    XMapWindow(qt_xdisplay(), electric_left_border);

    attributes.cursor = XCreateFontCursor(qt_xdisplay(), XC_sb_right_arrow);
    electric_right_border = XCreateWindow(qt_xdisplay(), qt_xrootwin(), r.width() - 1, 0, 1, r.height(), 0, CopyFromParent, InputOnly, CopyFromParent,
                                          valuemask, &attributes);
    XMapWindow(qt_xdisplay(), electric_right_border);
    // Set XdndAware on the windows, so that DND enter events are received (#86998)
    Atom version = 4; // XDND version
    XChangeProperty(qt_xdisplay(), electric_top_border, atoms->xdnd_aware, XA_ATOM, 32, PropModeReplace, (unsigned char *)&version, 1);
    XChangeProperty(qt_xdisplay(), electric_bottom_border, atoms->xdnd_aware, XA_ATOM, 32, PropModeReplace, (unsigned char *)&version, 1);
    XChangeProperty(qt_xdisplay(), electric_left_border, atoms->xdnd_aware, XA_ATOM, 32, PropModeReplace, (unsigned char *)&version, 1);
    XChangeProperty(qt_xdisplay(), electric_right_border, atoms->xdnd_aware, XA_ATOM, 32, PropModeReplace, (unsigned char *)&version, 1);
}
...
void Workspace::clientMoved(const QPoint &pos, Time now)
{
    ...
    if((pos.x() != electricLeft) && (pos.x() != electricRight) && (pos.y() != electricTop) && (pos.y() != electricBottom))
        return;
    ...
    if((electric_current_border == border) && (timestampDiff(electric_time_last, now) < treshold_reset)
    ...
    {
        case 1:
            QCursor::setPos(pos.x() + mouse_warp, pos.y());
            break;
        case 2:
            QCursor::setPos(pos.x() - mouse_warp, pos.y());
            break;
        case 3:
            QCursor::setPos(pos.x(), pos.y() + mouse_warp);
            break;
        case 4:
            QCursor::setPos(pos.x(), pos.y() - mouse_warp);
            break;
...
bool Workspace::electricBorder(XEvent *e)
{
    ...
    if(e->type == ClientMessage)
    {
        if(e->xclient.message_type == atoms->xdnd_position
           && (e->xclient.window == electric_top_border || e->xclient.window == electric_bottom_border || e->xclient.window == electric_left_border
...
void Workspace::helperDialog(const QString &message, const Client *c)
{
    ...
    {
        QString shortcut =
            QString("%1 (%2)").arg(keys->label("Window Operations Menu")).arg(keys->shortcut("Window Operations Menu").seq(0).toString());
        args << "--msgbox"
             << i18n(
...
void Workspace::startKompmgr()
{
    ...
    if(!kompmgr->start(KProcess::OwnGroup, KProcess::Stderr))
    {
        ...
        KProcess proc;
        proc << "kdialog"
             << "--error" << i18n("The Composite Manager could not be started.\\nMake sure you have \"kompmgr\" in a $PATH directory.") << "--title"
...
// this is for inernal purpose (crashhandling) only, usually you want to use workspace->stopKompmgr(); QTimer::singleShot(200, workspace,
// SLOT(startKompmgr()));
{
    if(!allowKompmgrRestart) // uh-ohh
    {
        ...
        KProcess proc;
        proc << "kdialog"
             << "--error" << i18n("The Composite Manager crashed twice within a minute and is therefore disabled for this session.") << "--title"
             << i18n("Composite Manager Failure");
        proc.start(KProcess::DontCare);
        return;
    }
    if(!kompmgr)
...
void Workspace::handleKompmgrOutput(KProcess *, char *buffer, int buflen)
{
    ...
    if(output.contains("Started", false))
        ...
        message = i18n(
            "<qt><b>kompmgr cannot find the Xrender extension</b><br>You are using either an outdated or a crippled version of XOrg.<br>Get XOrg "
            "&ge; 6.8 from www.freedesktop.org.<br></qt>");
    else if(output.contains("No composite extension", false))
        message = i18n(
            "<qt><b>Composite extension not found</b><br>You <i>must</i> use XOrg &ge; 6.8 for translucency and shadows to work.<br>Additionally, "
            "you need to add a new section to your X config file:<br>"
    ...
    if(!message.isEmpty())
    {
        KProcess proc;
        proc << "kdialog"
             << "--error" << message << "--title" << i18n("Composite Manager Failure");
        proc.start(KProcess::DontCare);
    }
}
...
void Workspace::setShadowSize(unsigned long winId, unsigned int shadowSizePercent)
{
    ...
    if(shadowSizePercent > 400)
        shadowSizePercent = 400;
    for(ClientList::ConstIterator it = stackingOrder().begin(); it != stackingOrder().end(); it++)
        if(winId == (*it)->window())
        {
...
void Workspace::setShowingDesktop(bool showing)
{
    ...
    if(showing_desktop)
    {
        ...
        for(ClientList::ConstIterator it = cls.begin(); it != cls.end(); ++it)
        {
        ...
        for(ClientList::ConstIterator it = showing_desktop_clients.begin(); it != showing_desktop_clients.end(); ++it)
            (*it)->minimize(true);
        --block_focus;
