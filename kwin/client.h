...
private:
    ...
    {
        assert(false);
    } // SELI remove after Client is no longer QWidget
    void hide()
    {
        assert(false);
    }
...
public:
    GeometryUpdatesPostponer(Client *c) : cl(c)
    {
        cl->postponeGeometryUpdates(true);
    }
...
#ifdef NDEBUG
inline kndbgstream &operator<<(kndbgstream &stream, const Client *)
{
    return stream;
}
inline kndbgstream &operator<<(kndbgstream &stream, const ClientList &)
{
    return stream;
}
inline kndbgstream &operator<<(kndbgstream &stream, const ConstClientList &)
{
    return stream;
}
#else
