...
void Client::updateDecoration(bool check_workspace_pos, bool force)
{
    ...
    if(!noBorder())
    {
        ...
        options->onlyDecoTranslucent ? setDecoHashProperty(border_top, border_right, border_bottom, border_left) : unsetDecoHashProperty();
...
void Client::checkBorderSizes()
{
    ...
    if(new_left == border_left && new_right == border_right && new_top == border_top && new_bottom == border_bottom)
        return;
...
void Client::updateShape()
{
    ...
    if(shape())
    {
        XShapeCombineShape(qt_xdisplay(), frameId(), ShapeBounding, clientPos().x(), clientPos().y(), window(), ShapeBounding, ShapeSet);
        setShapable(TRUE);
    }
    ...
    {                            // There appears to be no way to find out if a window has input
        ...
        if(helper_window == None)
            helper_window = XCreateSimpleWindow(qt_xdisplay(), qt_xrootwin(), 0, 0, 1, 1, 0, 0, 0);
        XResizeWindow(qt_xdisplay(), helper_window, width(), height());
        XShapeCombineShape(qt_xdisplay(), helper_window, ShapeInput, 0, 0, frameId(), ShapeBounding, ShapeSet);
        XShapeCombineShape(qt_xdisplay(), helper_window, ShapeInput, clientPos().x(), clientPos().y(), window(), ShapeBounding, ShapeSubtract);
        XShapeCombineShape(qt_xdisplay(), helper_window, ShapeInput, clientPos().x(), clientPos().y(), window(), ShapeInput, ShapeUnion);
        XShapeCombineShape(qt_xdisplay(), frameId(), ShapeInput, 0, 0, helper_window, ShapeInput, ShapeSet);
    }
}
...
void Client::setMask(const QRegion &reg, int mode)
{
    _mask = reg;
    if(reg.isNull())
        XShapeCombineMask(qt_xdisplay(), frameId(), ShapeBounding, 0, 0, None, ShapeSet);
    else if(mode == X::Unsorted)
        XShapeCombineRegion(qt_xdisplay(), frameId(), ShapeBounding, 0, 0, reg.handle(), ShapeSet);
    else
    {
        ...
        XShapeCombineRectangles(qt_xdisplay(), frameId(), ShapeBounding, 0, 0, xrects, rects.count(), ShapeSet, mode);
        delete[] xrects;
    }
    updateShape();
}
...
void Client::setMappingState(int s)
{
    ...
    XChangeProperty(qt_xdisplay(), window(), qt_wm_state, qt_wm_state, 32, PropModeReplace, (unsigned char *)data, 2);
...
void Client::killProcess(bool ask, Time timestamp)
{
    ...
    { // SELI TODO handle the window created by handler specially (on top,urgent?)
        ...
        *process_killer << KStandardDirs::findExe("kwin_killer_helper") << "--pid" << QCString().setNum(pid) << "--hostname" << machine
                        << "--windowname" << caption().utf8() << "--applicationname" << resourceClass() << "--wid" << QCString().setNum(window())
                        << "--timestamp" << QCString().setNum(timestamp);
        connect(process_killer, SIGNAL(processExited(KProcess *)), SLOT(processKillerExited()));
...
void Client::setSkipTaskbar(bool b, bool from_outside)
{
    ...
    if(was_wants_tab_focus != wantsTabFocus())
        workspace()->updateFocusChains(this, isActive() ? Workspace::FocusChainMakeFirst : Workspace::FocusChainUpdate);
}
...
{
    if((b && isOnAllDesktops()) || (!b && !isOnAllDesktops()))
        return;
    if(b)
...
KWIN_COMPARE_PREDICATE(FetchNameInternalPredicate, const Client *,
...
void Client::getIcons()
{
    ...
    { // then mainclients
        ClientList mainclients = mainClients();
        for(ClientList::ConstIterator it = mainclients.begin(); it != mainclients.end() && icon_pix.isNull(); ++it)
        {
            icon_pix = (*it)->icon();
...
Window Client::staticWmClientLeader(WId w)
{
    ...
    status = XGetWindowProperty(qt_xdisplay(), w, atoms->wm_client_leader, 0, 10000, FALSE, XA_WINDOW, &type, &format, &nitems, &extra, &data);
...
{
    return isDesktop() || isDock() || isSplash() || isTopMenu() || isToolbar(); // TODO
}
...
NET::WindowType Client::windowType(bool direct, int supported_types) const
{
    ...
    if(wt == NET::Menu)
    {
        ...
        if(x() == 0 && y() < 0 && y() > -10 && height() < 100 && abs(width() - workspace()->clientArea(FullArea, this).width()) < 10)
            wt = NET::TopMenu;
    }
...
void Client::setOpacity(bool translucent, uint opacity)
{
    ...
    if(!translucent || opacity == 0xFFFFFFFF)
    {
        ...
        XDeleteProperty(qt_xdisplay(), window(), atoms->net_wm_window_opacity); // ??? frameId() is necessary for visible changes, window() is the
                                                                                // winId() that would be set by apps - we set both to be sure the app
                                                                                // knows what's currently displayd
    }
    else
    {
        if(opacity == opacity_)
...
// extra syncscreen flag allows to avoid double syncs when active state changes (as it will usually change for two windows)
{
    ...
    else
    {
        if(ruleOpacityInactive())
            ...
            setOpacity(options->translucentInactiveWindows && !(keepAbove() && options->keepAboveAsActive), options->inactiveWindowOpacity);
                ...
                // potential "to activate" client found
                {
                    ...
                    if((*it)->touches(this)) // first test, if the new client touches the just activated one
                    {
                        ...
                        if((*it)->ruleOpacityInactive())
                            ...
                            (*it)->setOpacity(options->translucentInactiveWindows && !((*it)->keepAbove() && options->keepAboveAsActive),
...
{
    long data = (topHeight < 255 ? topHeight : 255) << 24 | (rightWidth < 255 ? rightWidth : 255) << 16
                | (bottomHeight < 255 ? bottomHeight : 255) << 8 | (leftWidth < 255 ? leftWidth : 255);
...
kdbgstream &operator<<(kdbgstream &stream, const ConstClientList &list)
{
    stream << "LIST:(";
    bool first = true;
    for(ConstClientList::ConstIterator it = list.begin(); it != list.end(); ++it)
    {
        if(!first)
            stream << ":";
        first = false;
