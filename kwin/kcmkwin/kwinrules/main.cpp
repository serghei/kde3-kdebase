...
{
    KWin::WindowInfo info = KWin::windowInfo(wid, NET::WMName | NET::WMWindowType, NET::WM2WindowClass | NET::WM2WindowRole | NET::WM2ClientMachine);
    ...
    for(QValueList< Rules * >::ConstIterator it = rules.begin(); it != rules.end(); ++it)
    {
        ...
        if(!rule->matchType(type) || !rule->matchRole(role) || !rule->matchTitle(title) || !rule->matchClientMachine(machine))
            continue;
...
extern "C" KDE_EXPORT int kdemain(int argc, char *argv[])
{
    ...
    KCmdLineArgs::init(argc, argv, "kwin_rules_dialog", I18N_NOOP("KWin"), I18N_NOOP("KWin helper utility"), "1.0");
