...
ButtonDrag::ButtonDrag(Button btn, QWidget *parent, const char *name) : QStoredDrag(BUTTONDRAGMIMETYPE, parent, name)
{
    QByteArray data;
...
Button::Button(const QString &n, const QBitmap &i, QChar t, bool d, bool s) : name(n), icon(i), type(t), duplicate(d), supported(s)
{
}
...
ButtonSource::ButtonSource(QWidget *parent, const char *name) : KListView(parent, name)
{
...
ButtonDropSiteItem::ButtonDropSiteItem(const Button &btn) : m_button(btn)
{
}
...
ButtonDropSite::ButtonDropSite(QWidget *parent, const char *name) : QFrame(parent, name), m_selected(0)
{
    setAcceptDrops(TRUE);
...
ButtonSourceItem::ButtonSourceItem(QListView *parent, const Button &btn) : QListViewItem(parent), m_button(btn), m_dirty(true)
{
    setButton(btn);
}
...
ButtonPositionWidget::ButtonPositionWidget(QWidget *parent, const char *name) : QWidget(parent, name), m_factory(0)
{
    ...
    label->setText(
        i18n("To add or remove titlebar buttons, simply <i>drag</i> items "
