...
KWinDecorationModule::KWinDecorationModule(QWidget *parent, const char *name, const QStringList &)
    : DCOPObject("KWinClientDecoration"), KCModule(KWinDecoFactory::instance(), parent, name), kwinConfig("kwinrc"), pluginObject(0)
{
    ...
    cbShowToolTips = new QCheckBox(i18n("&Show window button tooltips"), buttonPage);
    QWhatsThis::add(cbShowToolTips, i18n("Enabling this checkbox will show window button tooltips. "
                                         "If this checkbox is off, no window button tooltips will be shown."));

    cbUseCustomButtonPositions = new QCheckBox(i18n("Use custom titlebar button &positions"), buttonPage);
    ...
    connect(decorationList, SIGNAL(activated(const QString &)), SLOT(slotChangeDecoration(const QString &)));
    ...
    KAboutData *about = new KAboutData(I18N_NOOP("kcmkwindecoration"), I18N_NOOP("Window Decoration Control Module"), 0, 0, KAboutData::License_GPL,
...
static const char *const border_names[KDecorationDefines::BordersCount] = {I18N_NOOP("Tiny"),       I18N_NOOP("Normal"), I18N_NOOP("Large"),
                                                                           I18N_NOOP("Very Large"), I18N_NOOP("Huge"),   I18N_NOOP("Very Huge"),
                                                                           I18N_NOOP("Oversized")};

int KWinDecorationModule::borderSizeToIndex(BorderSize size, QValueList< BorderSize > sizes)
{
    int pos = 0;
    for(QValueList< BorderSize >::ConstIterator it = sizes.begin(); it != sizes.end(); ++it, ++pos)
        if(size <= *it)
            break;
    return pos;
}

KDecorationDefines::BorderSize KWinDecorationModule::indexToBorderSize(int index, QValueList< BorderSize > sizes)
{
    QValueList< BorderSize >::ConstIterator it = sizes.begin();
    for(; it != sizes.end(); ++it, --index)
        if(index == 0)
            break;
    return *it;
}
...
void KWinDecorationModule::resetPlugin(KConfig *conf, const QString &currentDecoName)
{
    ...
    if(plugins->loadPlugin(currentName) && preview->recreateDecoration(plugins))
...
void KWinDecorationModule::readConfig(KConfig *conf)
{
    ...
    currentLibraryName = conf->readEntry("PluginLib", ((QPixmap::defaultDepth() > 8) ? "kwin_plastik" : "kwin_quartz"));
...
{
    bool ok = kapp->dcopClient()->send("kwin*", "KWinInterface", "reconfigure()", QByteArray());
    if(!ok)
