    ...
    : KCModule(parent, "kcmkwm"), config(_config), standAlone(_standAlone)
{
    ...
    wtstr = i18n(
        "This is the delay after which the window that the mouse pointer is over will automatically"
        " come to the front.");
    ...
    wtstr = i18n(
        "Hold down the Alt key and press the Tab key repeatedly to walk"
    ...
    wtstr = i18n(
        "Leave this option disabled if you want to limit walking through"
    ...
    wtstr = i18n(
        "Enable this option if you want keyboard or active desktop border navigation beyond"
    ...
    wtstr = i18n(
        "Enable this option if you wish to see the current desktop"
    ...
    : KCModule(parent, "kcmkwm"), config(_config), standAlone(_standAlone)
{
    ...
    wtstr = i18n(
        "Sets the time in milliseconds before the window unshades "
    ...
    wtstr = i18n(
        "<p>This option specifies how much KWin will try to prevent unwanted focus stealing "
    ...
    : KCModule(parent, "kcmkwm"), config(_config), standAlone(_standAlone)
{
    ...
    {
        KActiveLabel *label =
            new KActiveLabel(i18n("<qt><b>It seems that alpha channel support is not available.</b><br><br>"
...
void KTranslucencyConfig::save(void)
{
    ...
    config->writeEntry("DockShadowSize",
                       (int)(200.0 * dockWindowShadowSize->value() / (activeWindowShadowSize->value() + inactiveWindowShadowSize->value())));
    config->writeEntry("ActiveWindowShadowSize",
                       (int)(200.0 * activeWindowShadowSize->value() / (activeWindowShadowSize->value() + inactiveWindowShadowSize->value())));
    config->writeEntry("InactiveWindowShadowSize",
                       (int)(200.0 * inactiveWindowShadowSize->value() / (activeWindowShadowSize->value() + inactiveWindowShadowSize->value())));
...
{
    if(alphaActivated)
        KMessageBox::information(this, i18n("<qt>Translucency support is new and may cause problems<br> including crashes (sometimes the "
                                            "translucency engine, seldom even X).</qt>"),
                                 i18n("Warning"));
}
