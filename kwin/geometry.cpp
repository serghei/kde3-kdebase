    ...
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
    {
        ...
        if((*it)->isOnAllDesktops())
            for(int i = 1; i <= numberOfDesktops(); ++i)
            {
                ...
                for(int iS = 0; iS < nscreens; iS++)
                    new_sareas[i][iS] = new_sareas[i][iS].intersect((*it)->adjustedClientArea(desktopArea, screens[iS]));
            }
        else
        {
            ...
            for(int iS = 0; iS < nscreens; iS++)
            {
                ...
                new_sareas[(*it)->desktop()][iS] = new_sareas[(*it)->desktop()][iS].intersect((*it)->adjustedClientArea(desktopArea, screens[iS]));
            }
        }
    }
#if 0
...
QRect Workspace::clientArea(clientAreaOption opt, const QPoint &p, int desktop) const
{
    ...
    QRect warea = workarea[desktop].isNull() ? QApplication::desktop()->geometry() : workarea[desktop];
    switch(opt)
    {
        case MaximizeArea:
...
QPoint Workspace::adjustClientPosition(Client *c, QPoint pos)
{
    ...
    if(options->windowSnapZone || options->borderSnapZone)
    {
        ...
        if(snap)
        {
            ...
            {
                if((*l)->isOnDesktop(currentDesktop()) && !(*l)->isMinimized() && (*l) != c)
                {
                    lx = (*l)->x();
                    ly = (*l)->y();
                    lrx = lx + (*l)->width();
...
QRect Client::adjustedClientArea(const QRect &desktopArea, const QRect &area) const
{
    ...
    QRect stareaR = QRect(desktopArea.right() - str.right_width + 1, str.right_start, str.right_width, str.right_end - str.right_start + 1);
    QRect stareaT = QRect(str.top_start, 0, str.top_end - str.top_start + 1, str.top_width);
    QRect stareaB = QRect(str.bottom_start, desktopArea.bottom() - str.bottom_width + 1, str.bottom_end - str.bottom_start + 1, str.bottom_width);
...
void Client::configureRequest(int value_mask, int rx, int ry, int rw, int rh, int gravity, bool from_tool)
{
    ...
    if(value_mask & (CWX | CWY))
    {
        ...
        // manager
        if(new_pos.x() == x() + clientPos().x() && new_pos.y() == y() + clientPos().y() && gravity == NorthWestGravity && !from_tool)
        {
            new_pos.setX(x());
            new_pos.setY(y());
        }
        ...
        if(maximizeMode() != MaximizeFull || ns != size())
        {
            QRect orig_geometry = geometry();
            ...
            QRect area = workspace()->clientArea(WorkArea, this);
            if(!from_tool && (!isSpecialWindow() || isToolbar()) && !isFullScreen() && area.contains(orig_geometry))
                keepInArea(area);
    ...
    if(value_mask & (CWWidth | CWHeight) && !(value_mask & (CWX | CWY))) // pure resize
    {
        int nw = clientSize().width();
        int nh = clientSize().height();
        if(value_mask & CWWidth)
...
{ // changeMaximize() flips the state, so change from set->flip
    changeMaximize(max_mode & MaximizeVertical ? !vertically : vertically, max_mode & MaximizeHorizontal ? !horizontally : horizontally, false);
}
...
void Client::changeMaximize(bool vertical, bool horizontal, bool adjust)
{
    ...
    Q_ASSERT(!(vertical && horizontal) || (((max_mode & MaximizeVertical) != 0) == ((max_mode & MaximizeHorizontal) != 0)));
    ...
    if(!adjust)
    {
        if((vertical && !(old_mode & MaximizeVertical)) || (horizontal && !(old_mode & MaximizeHorizontal)))
    ...
    switch(max_mode)
    {
        ...
        case MaximizeVertical:
        {
            if(old_mode & MaximizeHorizontal) // actually restoring from MaximizeFull
            {
                ...
                setGeometry(QRect(QPoint(x(), clientArea.top()), adjustedSize(QSize(width(), clientArea.height()), SizemodeFixedH)),
                            ForceGeometrySet);
        ...
        case MaximizeHorizontal:
        {
            if(old_mode & MaximizeVertical) // actually restoring from MaximizeFull
            {
                ...
                setGeometry(QRect(QPoint(clientArea.left(), y()), adjustedSize(QSize(clientArea.width(), height()), SizemodeFixedW)),
                            ForceGeometrySet);
...
bool Client::startMoveResize()
{
    ...
    move_resize_grab_window = XCreateWindow(qt_xdisplay(), workspace()->rootWin(), r.x(), r.y(), r.width(), r.height(), 0, CopyFromParent, InputOnly,
    ...
    if(XGrabPointer(qt_xdisplay(), move_resize_grab_window, False,
                    ButtonPressMask | ButtonReleaseMask | PointerMotionMask | EnterWindowMask | LeaveWindowMask, GrabModeAsync, GrabModeAsync,
                    move_resize_grab_window, cursor.handle(), qt_x_time)
       == Success)
        has_grab = true;
...
{
    if((mode == PositionCenter && !isMovable()) || (mode != PositionCenter && (isShade() || !isResizable())))
        return;
