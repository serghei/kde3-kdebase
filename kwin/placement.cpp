...
{
    const char *const policies[] = {"NoPlacement", "Default",      "XXX should never see", "Random",       "Smart",     "Cascade",
                                    "Centered",    "ZeroCornered", "UnderMouse",           "OnMainWindow", "Maximizing"};
...
void Client::growVertical()
{
    ...
    if(geometry().size() == adjsize && geom.size() != adjsize && xSizeHint.height_inc > 1) // take care of size increments
    {
        ...
        // check that it hasn't grown outside of the area, due to size increments
        if(workspace()->clientArea(MovementArea, QPoint(geometry().center().x(), (y() + newbottom) / 2), desktop()).bottom() >= newbottom)
...
int Workspace::packPositionLeft(const Client *cl, int oldx, bool left_edge) const
{
    ...
    if(oldx <= newx) // try another Xinerama screen
        newx = clientArea(MovementArea, QPoint(cl->geometry().left() - 1, cl->geometry().center().y()), cl->desktop()).left();
    if(oldx <= newx)
        return oldx;
...
int Workspace::packPositionRight(const Client *cl, int oldx, bool right_edge) const
{
    ...
    if(oldx >= newx) // try another Xinerama screen
        newx = clientArea(MovementArea, QPoint(cl->geometry().right() + 1, cl->geometry().center().y()), cl->desktop()).right();
    if(oldx >= newx)
        return oldx;
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
    {
        ...
        if(x < newx && x > oldx && !(cl->geometry().top() > (*it)->geometry().bottom() || cl->geometry().bottom() < (*it)->geometry().top()))
            newx = x;
    }
    return newx;
}
...
int Workspace::packPositionUp(const Client *cl, int oldy, bool top_edge) const
{
    ...
    if(oldy <= newy) // try another Xinerama screen
        newy = clientArea(MovementArea, QPoint(cl->geometry().center().x(), cl->geometry().top() - 1), cl->desktop()).top();
    if(oldy <= newy)
        return oldy;
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
    {
...
int Workspace::packPositionDown(const Client *cl, int oldy, bool bottom_edge) const
{
    ...
    if(oldy >= newy) // try another Xinerama screen
        newy = clientArea(MovementArea, QPoint(cl->geometry().center().x(), cl->geometry().bottom() + 1), cl->desktop()).bottom();
    if(oldy >= newy)
        return oldy;
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
    {
        ...
        if(y < newy && y > oldy && !(cl->geometry().left() > (*it)->geometry().right() || cl->geometry().right() < (*it)->geometry().left()))
            newy = y;
    }
    return newy;
}
