...
static void set_fade(Display *dpy, win *w, double start, double finish, double step, void (*callback)(Display *dpy, win *w, Bool gone), Bool gone,
                     Bool exec_callback, Bool override, Bool wholeWin)
{
    fade *f;
...
static XImage *make_shadow(Display *dpy, double opacity, int width, int height)
{
    ...
    if(!data)
        return 0;
    ximage =
        XCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)), 8, ZPixmap, 0, (char *)data, swidth, sheight, 8, swidth * sizeof(unsigned char));
    if(!ximage)
    {
        free(data);
        return 0;
...
static Picture shadow_picture(Display *dpy, double opacity, Picture alpha_pict, int width, int height, int *wp, int *hp)
{
    XImage *shadowImage;
    Pixmap shadowPixmap;
    Pixmap finalPixmap;
    ...
    shadowPicture = XRenderCreatePicture(dpy, shadowPixmap, XRenderFindStandardFormat(dpy, PictStandardA8), 0, 0);
    if(!shadowPicture)
    {
        XDestroyImage(shadowImage);
...
static Picture root_tile(Display *dpy)
{
    ...
    {
        if(XGetWindowProperty(dpy, root, XInternAtom(dpy, backgroundProps[p], False), 0, 4, False, AnyPropertyType, &actual_type, &actual_format,
    ...
    pa.repeat = True;
    picture = XRenderCreatePicture(dpy, pixmap, sXRenderFindVisualFormat(dpy, DefaultVisual(dpy, scr)), CPRepeat, &pa);
    if(fill)
    {
        XRenderColor c;
...
static void paint_root(Display *dpy)
{
    ...
    XRenderComposite(dpy, PictOpSrc, rootTile, None, rootBuffer, 0, 0, 0, 0, 0, 0, root_width, root_height);
}
...
static XserverRegion deco_region(Display *dpy, win *w)
{
    XserverRegion title;
...
static XserverRegion content_region(Display *dpy, win *w)
{
...
static void paint_all(Display *dpy, XserverRegion region)
{
    ...
    if(!rootBuffer)
    {
        ...
        rootBuffer = XRenderCreatePicture(dpy, rootPixmap, sXRenderFindVisualFormat(dpy, DefaultVisual(dpy, scr)), 0, 0);
    ...
    for(w = list; w; w = w->next)
    {
        ...
        if((w->mode == WINDOW_SOLID) || ((w->mode == WINDOW_TRANS) && w->decoHash))
        {
            ...
            else
            {
                ...
                set_ignore(dpy, NextRequest(dpy));
                /*solid part*/
                XRenderComposite(dpy, PictOpSrc, w->picture, None, rootBuffer, _LEFTWIDTH_(w->decoHash), _TOPHEIGHT_(w->decoHash), 0, 0,
                                 x + _LEFTWIDTH_(w->decoHash), y + _TOPHEIGHT_(w->decoHash),
    ...
    for(w = t; w; w = w->prev_trans)
    {
        ...
        if(w->shadowSize > 0)
        {
            switch(compMode)
            {
                ...
                case CompServerShadows:
                    ...
                    XRenderComposite(dpy, PictOpOver, w->shadowPict ? w->shadowPict : transBlackPicture, w->picture, rootBuffer, 0, 0, 0, 0,
                                     w->a.x + w->shadow_dx, w->a.y + w->shadow_dy, w->shadow_width, w->shadow_height);
                    break;
                case CompClientShadows:
                    if(w->shadow && w->windowType != winDesktopAtom)
                    {
                        XFixesSetPictureClipRegion(dpy, rootBuffer, 0, 0, w->borderClip);
                        XRenderComposite(dpy, PictOpOver, blackPicture, w->shadow, rootBuffer, 0, 0, 0, 0, w->a.x + w->shadow_dx,
        ...
        if(w->opacity != OPAQUE && !w->alphaPict)
            w->alphaPict = solid_picture(dpy, False, (double)w->opacity / OPAQUE, shadowColor.red, shadowColor.green, shadowColor.blue);
        if(w->mode == WINDOW_TRANS)
        {
            int x, y, wid, hei;
            ...
            else
            {
                ...
                |--------------------------|--|*/
                /*top*/
                XRenderComposite(dpy, PictOpOver, w->picture, w->alphaPict, rootBuffer, 0, 0, 0, 0, x, y, wid, _TOPHEIGHT_(w->decoHash));
                /*right*/
                XRenderComposite(dpy, PictOpOver, w->picture, w->alphaPict, rootBuffer, wid - _RIGHTWIDTH_(w->decoHash), _TOPHEIGHT_(w->decoHash), 0,
                                 0, x + wid - _RIGHTWIDTH_(w->decoHash), y + _TOPHEIGHT_(w->decoHash), _RIGHTWIDTH_(w->decoHash),
                                 hei - _TOPHEIGHT_(w->decoHash));
                /*bottom*/
                XRenderComposite(dpy, PictOpOver, w->picture, w->alphaPict, rootBuffer, 0, hei - _BOTTOMHEIGHT_(w->decoHash), 0, 0, x,
                                 y + hei - _BOTTOMHEIGHT_(w->decoHash), wid - _RIGHTWIDTH_(w->decoHash), _BOTTOMHEIGHT_(w->decoHash));
                /*left*/
                XRenderComposite(dpy, PictOpOver, w->picture, w->alphaPict, rootBuffer, 0, _TOPHEIGHT_(w->decoHash), 0, 0, x,
                                 y + _TOPHEIGHT_(w->decoHash), _LEFTWIDTH_(w->decoHash),
                                 hei - _TOPHEIGHT_(w->decoHash) - _BOTTOMHEIGHT_(w->decoHash));
            }
        }
...
static void map_win(Display *dpy, Window id, unsigned long sequence, Bool fade)
{
...
static unsigned int get_shadow_prop(Display *dpy, win *w)
{
    Atom actual;
    int format;
    unsigned long n, left;
    ...
    int result = XGetWindowProperty(dpy, w->id, shadowAtom, 0L, 1L, False, XA_CARDINAL, &actual, &format, &n, &left, &data);
    if(result == Success && data != NULL && format == 32)
    {
        unsigned int i;
        i = *(long *)data;
...
static unsigned int get_shade_prop(Display *dpy, win *w)
{
    Atom actual;
    int format;
    unsigned long n, left;
    ...
    unsigned char *data = NULL;
    int result = XGetWindowProperty(dpy, w->id, shadeAtom, 0L, 1L, False, XA_CARDINAL, &actual, &format, &n, &left, &data);
    if(result == Success && data != NULL && format == 32)
    {
        unsigned int i;
        i = *(long *)data;
...
static Bool get_shapable_prop(Display *dpy, win *w)
{
    Atom actual;
    int format;
    unsigned long n, left;
    ...
    unsigned char *data = NULL;
    int result = XGetWindowProperty(dpy, w->id, shapableAtom, 0L, 1L, False, XA_CARDINAL, &actual, &format, &n, &left, &data);
    if(result == Success && data != NULL && format == 32)
    {
        unsigned int i;
        i = *(long *)data;
...
static unsigned int get_decoHash_prop(Display *dpy, win *w)
{
    Atom actual;
    int format;
    unsigned long n, left;
    ...
    unsigned char *data = NULL;
    int result = XGetWindowProperty(dpy, w->id, decoHashAtom, 0L, 1L, False, XA_CARDINAL, &actual, &format, &n, &left, &data);
    if(result == Success && data != NULL && format == 32)
    {
        unsigned int i;
        i = *(long *)data;
...
static unsigned int get_dim_prop(Display *dpy, win *w)
{
    Atom actual;
    int format;
    unsigned long n, left;
    ...
    unsigned char *data = NULL;
    int result = XGetWindowProperty(dpy, w->id, dimAtom, 0L, 1L, False, XA_CARDINAL, &actual, &format, &n, &left, &data);
...
static unsigned int get_deskchange_prop(Display *dpy, Window id)
{
    Atom actual;
    int format;
    unsigned long n, left;
    ...
    unsigned char *data = NULL;
    int result = XGetWindowProperty(dpy, id, deskChangeAtom, 0L, 1L, False, XA_CARDINAL, &actual, &format, &n, &left, &data);
    if(result == Success && data != NULL)
    {
        unsigned int i;
        memcpy(&i, data, sizeof(unsigned int));
...
#endif
        if(w->damage_bounds.x <= 0 && w->damage_bounds.y <= 0 && w->a.width <= w->damage_bounds.x + w->damage_bounds.width
...
static void expose_root(Display *dpy, Window root, XRectangle *rects, int nrects)
{
...
void setShadowColor(char *value)
...
void setValue(Option option, char *value)
{
    switch(option)
    { /*please keep that upside-down, because this way adding a new option is easier (all in one view)*/
...
static void give_me_a_name(void)
{
    Window w;

    w = XCreateSimpleWindow(dpy, RootWindow(dpy, 0), 0, 0, 1, 1, 0, None, None);
    Xutf8SetWMProperties(dpy, w, "kcompmgr", "kcompmgr", NULL, 0, NULL, NULL, NULL);
}
...
int main(int argc, char **argv)
{
    ...
    rootPicture = XRenderCreatePicture(dpy, root, sXRenderFindVisualFormat(dpy, DefaultVisual(dpy, scr)), CPSubwindowMode, &pa);
    blackPicture =
        solid_picture(dpy, True, 1, (double)(shadowColor.red) / 0xff, (double)(shadowColor.green) / 0xff, (double)(shadowColor.blue) / 0xff);
    if(compMode == CompServerShadows)
    ...
    else
    {
        ...
        XSelectInput(dpy, root, SubstructureNotifyMask | ExposureMask | StructureNotifyMask | PropertyChangeMask | VisibilityChangeMask);
                            ...
                            if(w)
                            {
                                ...
                                if(tmp)
                                {
                                    if(tmp == 1)
                                    {
                                        ...
                                        w->opacity =
                                            w->opacity - 1; /*assuming that no human being will ever be able to shade an invisable window ;) */
                                        determine_mode(dpy, w);
                                    }
                                    else if(tmp == 2)
                                    {
                        ...
                        else if(ev.xproperty.atom == opacityAtom || ev.xproperty.atom == shadowAtom)
                        {
                            ...
                            if(w)
                            {
                                ...
                                if(ev.xproperty.atom == opacityAtom)
                                {
                                    ...
                                    /*THis will most probably happen if window is in fade - resulting in that the fade process isn't updated or broken
                                     * -> we may have a wrong opacity in the future*/
                                    /*if (tmp == w->opacity)
