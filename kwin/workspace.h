...
public:
    SystemTrayWindow() : win(0), winFor(0)
    {
    }
    SystemTrayWindow(WId w) : win(w), winFor(0)
    {
    }
    SystemTrayWindow(WId w, WId wf) : win(w), winFor(wf)
    {
    }
...
public:
    StackingUpdatesBlocker(Workspace *w) : ws(w)
    {
...
template < typename T > inline Client *Workspace::findClient(T predicate)
{
...
template < typename T1, typename T2 > inline void Workspace::forEachClient(T1 procedure, T2 predicate)
{
...
template < typename T > inline void Workspace::forEachClient(T procedure)
{
