...
QPopupMenu *Workspace::clientPopup()
{
    if(!popup)
    {
        ...
        if(!KGlobal::config()->isImmutable() && !kapp->authorizeControlModules(Workspace::configModules(true)).isEmpty())
        {
...
bool Client::performMouseCommand(Options::MouseCommand command, QPoint globalPos, bool handled)
{
    ...
    switch(command)
    {
        ...
        case Options::MouseUnrestrictedMove:
        {
            ...
            unrestrictedMoveResize = (command == Options::MouseActivateRaiseAndUnrestrictedMove || command == Options::MouseUnrestrictedMove);
            setCursor(mode);
            if(!startMoveResize())
            {
                buttonDown = false;
...
{
    Client *c = active_popup_client ? active_popup_client : active_client;
    if(i >= 1 && i <= numberOfDesktops() && c && !c->isDesktop() && !c->isDock() && !c->isTopMenu())
...
void Workspace::showWindowMenu(const QRect &pos, Client *cl)
{
    ...
    if(active_popup_client != NULL) // recursion
        return;
    if(cl->isDesktop() || cl->isDock() || cl->isTopMenu())
        return;
