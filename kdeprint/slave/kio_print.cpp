...
QString buildMenu(const QStringList &items, const QStringList &links, int active)
{
    ...
    for(QStringList::ConstIterator it1 = items.begin(), it2 = links.begin(); it1 != items.end() && it2 != links.end(); ++it1, ++it2, index++)
    {
        if(index == active)
            ...
            s.append("<td height=20 class=\"menu\">&nbsp; <a class=\"menu\" href=\"")
                .append(*it2)
                .append("\">")
                .append(*it1)
                .append("</a>&nbsp;</td>");
...
int kdemain(int argc, char **argv)
{
    ...
    KAboutData about("kio_print", "kio_print", "fake_version", "KDEPrint IO slave", KAboutData::License_GPL, "(c) 2003, Michael Goffioul");
...
KIO_Print::KIO_Print(const QCString &pool, const QCString &app) : KIO::SlaveBase("print", pool, app)
{
}
...
void KIO_Print::get(const KURL &url)
{
    ...
    if(elems.count() > 2 || (path.isEmpty() && group != "printers" && group != "classes" && group != "specials") || (mprinter == 0 && path.isEmpty()))
    {
        error(KIO::ERR_DOES_NOT_EXIST, url.path());
        return;
    }
...
void KIO_Print::showPrinterInfo(KMPrinter *printer)
{
    ...
    else
    {
        ...
        content = content.arg(i18n("Properties of %1").arg(printer->printerName()))
                      ...
                      .arg(buildMenu(QStringList::split('|', i18n("General|Driver|Active jobs|Completed jobs"), false),
                                     QStringList::split('|', "?general|?driver|?jobs|?completed_jobs", true), 0))
                      .arg(QString::null)
                      .arg(printer->pixmap())
                      .arg(printer->name())
                      ...
                      .arg(i18n("Interface (Backend)"))
                      .arg(printer->device())
                      .arg(i18n("Driver"))
                      .arg(i18n("Manufacturer"))
                      .arg(printer->manufacturer())
                      .arg(i18n("Model"))
                      .arg(printer->model())
                      .arg(i18n("Driver Information"))
    ...
    else
    {
        ...
        content = content.arg(i18n("Properties of %1").arg(printer->printerName()))
                      ...
                      .arg(buildMenu(QStringList::split('|', i18n("General|Active jobs|Completed jobs"), false),
                                     QStringList::split('|', "?general|?jobs|?completed_jobs", true), 0))
                      .arg(QString::null)
                      .arg(printer->pixmap())
                      .arg(printer->name())
                      .arg(i18n("General Properties"))
...
void KIO_Print::showSpecialInfo(KMPrinter *printer)
{
    ...
    content = content.arg(i18n("Properties of %1").arg(printer->printerName()))
                  .arg(i18n("Properties of %1").arg(printer->printerName()))
                  .arg(buildMenu(QStringList::split('|', i18n("General"), false), QStringList::split('|', "?general", true), 0))
                  .arg(QString::null)
                  .arg(printer->pixmap())
                  .arg(printer->name())
                  .arg(i18n("General Properties"))
...
void KIO_Print::showJobs(KMPrinter *prt, bool completed)
{
    ...
    if(prt)
    {
        content = content.arg(i18n("Jobs of %1").arg(prt->printerName()))
                      .arg(i18n("Jobs of %1").arg(prt->printerName()))
                      .arg(prt->isClass() ? buildMenu(QStringList::split('|', i18n("General|Active jobs|Completed jobs"), false),
                                                      QStringList::split('|', "?general|?jobs|?completed_jobs", true), (completed ? 2 : 1))
                                          : buildMenu(QStringList::split('|', i18n("General|Driver|Active jobs|Completed jobs"), false),
                                                      QStringList::split('|', "?general|?driver|?jobs|?completed_jobs", true), (completed ? 3 : 2)))
                      .arg(QString::null)
                      .arg(prt->pixmap())
                      .arg(prt->printerName());
    }
    ...
    {
        content = content.arg(i18n("All jobs"))
                      .arg(buildMenu(QStringList::split('|', i18n("Active jobs|Completed jobs"), false),
                                     QStringList::split('|', "?jobs|?completed_jobs", true), (completed ? 1 : 0)))
                      .arg("fileprint")
                      .arg(i18n("All jobs"));
    }
...
void KIO_Print::showDriver(KMPrinter *prt)
{
    ...
    content = content.arg(i18n("Driver of %1").arg(prt->printerName()))
                  .arg(i18n("Driver of %1").arg(prt->printerName()))
                  .arg(buildMenu(QStringList::split('|', i18n("General|Driver|Active jobs|Completed jobs"), false),
                                 QStringList::split('|', "?general|?driver|?jobs|?completed_jobs", true), 1))
                  .arg(QString::null)
                  .arg(prt->pixmap())
