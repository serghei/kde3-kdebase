...
void KNetAttach::setInformationText(const QString &type)
{
    ...
    {
        text = i18n(
            "Enter a name for this <i>WebFolder</i> as well as a server address, port and folder path to use and press the <b>Save & Connect</b> "
            "button.");
    }
    else if(type == "Fish")
    {
        text = i18n(
            "Enter a name for this <i>Secure shell connection</i> as well as a server address, port and folder path to use and press the <b>Save & "
            "Connect</b> button.");
    }
    else if(type == "FTP")
    {
...
{
    setFinishEnabled(_folderParameters, !_host->text().stripWhiteSpace().isEmpty() && !_path->text().stripWhiteSpace().isEmpty()
