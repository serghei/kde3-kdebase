...
TreeItem::TreeItem(QListViewItem *parent, QListViewItem *after, const QString &menuId, bool __init)
    : QListViewItem(parent, after), _hidden(false), _init(__init), _layoutDirty(false), _menuId(menuId), m_folderInfo(0), m_entryInfo(0)
{
}
    ...
    , m_layoutDirty(false)
{
    ...
    connect(this, SIGNAL(selectionChanged(QListViewItem *)), SLOT(itemSelected(QListViewItem *)));

    connect(this, SIGNAL(rightButtonPressed(QListViewItem *, const QPoint &, int)), SLOT(slotRMBPressed(QListViewItem *, const QPoint &)));
...
{
    if(e->provides("application/x-kmenuedit-internal") && (e->source() == const_cast< TreeView * >(this)))
        return true;
    KURL::List urls;
    if(KURLDrag::decode(e, urls) && (urls.count() == 1) && urls[0].isLocalFile() && urls[0].path().endsWith(".desktop"))
        return true;
    return false;
}
...
void TreeView::newsubmenu()
{
    ...
    bool ok;
    QString caption = KInputDialog::getText(i18n("New Submenu"), i18n("Submenu name:"), QString::null, &ok, this);
...
void TreeView::newitem()
{
    ...
    bool ok;
    QString caption = KInputDialog::getText(i18n("New Item"), i18n("Item name:"), QString::null, &ok, this);
...
bool TreeView::save()
{
    ...
    {
        KMessageBox::sorry(this, "<qt>" + i18n("Menu changes could not be saved because of the following problem:") + "<br><br>" + m_menuFile->error()
                                     + "</qt>");
    }
    return success;
}
