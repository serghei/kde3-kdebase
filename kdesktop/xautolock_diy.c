...
static void addToQueue(Window window)
{
...
static void processQueue(time_t age)
{
    if(queue.head)
    {
        time_t now = time(0);
...
static void selectEvents(Window window, Bool substructureOnly)
{
     ...
     */
    if(!XQueryTree(queue.display, window, &root, &parent, &children, &nofChildren))
    {
        return;
    }
 ...
 */
  ...
  else
  {
...
#else
    {
        ...
        if(!substructureOnly)
        {
            mask |= ((attribs.all_event_masks | attribs.do_not_propagate_mask) & KeyPressMask);
        }
