...
KFileIVIDesktop::KFileIVIDesktop(KonqIconViewWidget *iconview, KFileItem *fileitem, int size, KShadowEngine *shadow)
    : KFileIVI(iconview, fileitem, size), m_selectedImage(0L), m_normalImage(0L), _selectedUID(0), _normalUID(0)
{
    m_shadow = shadow;
    oldText = "";
...
void KFileIVIDesktop::calcRect(const QString &_text)
{
    ...
    if(!iconView() || !m_shadow || !wordWrap() || !(static_cast< KDesktopShadowSettings * >(m_shadow->shadowSettings()))->isEnabled())
        return;
...
void KFileIVIDesktop::paintFocus(QPainter *p, const QColorGroup &cg)
{
    ...
    if(!m_shadow || !wordWrap() || !(static_cast< KDesktopShadowSettings * >(m_shadow->shadowSettings()))->isEnabled())
    {
    ...
    iconView()->style().drawPrimitive(
        QStyle::PE_FocusRect, p,
        QRect(textRect(false).x(), textRect(false).y(), textRect(false).width() - spread, textRect(false).height() - spread + 1), cg,
        isSelected() ? QStyle::Style_FocusAtBorder : QStyle::Style_Default, QStyleOption(isSelected() ? cg.highlight() : cg.base()));

    if(this != iconView()->currentItem())
    {
        iconView()->style().drawPrimitive(QStyle::PE_FocusRect, p,
                                          QRect(pixmapRect(false).x(), pixmapRect(false).y(), pixmapRect(false).width(), pixmapRect(false).height()),
...
void KFileIVIDesktop::drawShadowedText(QPainter *p, const QColorGroup &cg)
{
    ...
    int align = ((KIconView *)iconView())->itemTextPos() == QIconView::Bottom ? AlignHCenter : AlignAuto;
    ...
    else
    {
        ...
        else
        {
            ...
            shadow = (settings->bgColor().isValid()) ? settings->bgColor() : (qGray(text.rgb()) > 127) ? black : white;
            if(rebuild)
            {
        ...
        p->drawImage(shadowX, shadowY, (isSelected()) ? *selectedImage() : *normalImage(), 0, 0, -1, -1, DITHER_FLAGS);
    }
...
QImage *KFileIVIDesktop::buildShadow(QPainter *p, const int align, QColor &shadowColor)
{
    QPainter pixPainter;
    int spread = shadowThickness();

    QPixmap textPixmap(textRect(FALSE).width() + spread * 2 + 2, textRect(FALSE).height() + spread * 2 + 2);
