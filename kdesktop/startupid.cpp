...
StartupId::StartupId(QWidget *parent, const char *name)
    : QWidget(parent, name), startup_info(KStartupInfo::CleanOnCantDetect), startup_widget(NULL), blinking(true), bouncing(false)
{
    ...
    connect(&startup_info, SIGNAL(gotNewStartup(const KStartupInfoId &, const KStartupInfoData &)),
            SLOT(gotNewStartup(const KStartupInfoId &, const KStartupInfoData &)));
    connect(&startup_info, SIGNAL(gotStartupChange(const KStartupInfoId &, const KStartupInfoData &)),
            SLOT(gotStartupChange(const KStartupInfoId &, const KStartupInfoData &)));
    connect(&startup_info, SIGNAL(gotRemoveStartup(const KStartupInfoId &, const KStartupInfoData &)),
...
{
    if(e->type == ClientMessage && e->xclient.window == qt_xrootwin() && e->xclient.message_type == kde_splash_progress)
    {
...
void StartupId::stop_startupid()
{
    ...
    startup_widget = NULL;
    if(blinking)
        for(int i = 0; i < NUM_BLINKING_PIXMAPS; ++i)
...
void StartupId::start_startupid(const QString &icon_P)
{
    ...
    const QColor startup_colors[StartupId::NUM_BLINKING_PIXMAPS] = {Qt::black, Qt::darkGray, Qt::lightGray, Qt::white, Qt::white};


    QPixmap icon_pixmap = KGlobal::iconLoader()->loadIcon(icon_P, KIcon::Small, 0, KIcon::DefaultState, 0, true); // return null pixmap if not found
...
const int frame_to_yoffset[] = {-5, -1, 2, 5, 8, 10, 12, 13, 15, 15, 15, 15, 14, 12, 10, 8, 5, 2, -1, -5};
const int frame_to_pixmap[] = {0, 0, 0, 1, 2, 2, 1, 0, 3, 4, 4, 3, 0, 1, 2, 2, 1, 0, 0, 0};
}
...
void StartupId::update_startupid()
{
    ...
    if(cursor_size <= 16)
        ...
        X_DIFF = 32 + 7;
    int Y_DIFF = X_DIFF;
    if(startup_widget->x() != c_pos.x() + X_DIFF || startup_widget->y() != c_pos.y() + Y_DIFF + yoffset)
