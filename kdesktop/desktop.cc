    ...
    , m_waitForKicker(0)
{
    ...
    kapp->dcopClient()->connectDCOPSignal(kicker_name, kicker_name, "desktopIconsAreaChanged(QRect, int)", "KDesktopIface",
                                          "desktopIconsAreaChanged(QRect, int)", false);
...
void KDesktop::initRoot()
{
    ...
    else if(m_bDesktopEnabled && !m_pIconView)
    {
        ...
        connect(m_pIconView, SIGNAL(imageDropEvent(QDropEvent *)), this, SLOT(handleImageDropEvent(QDropEvent *)));
        connect(m_pIconView, SIGNAL(colorDropEvent(QDropEvent *)), this, SLOT(handleColorDropEvent(QDropEvent *)));
        connect(m_pIconView, SIGNAL(newWallpaper(const KURL &)), this, SLOT(slotNewWallpaper(const KURL &)));
        connect(m_pIconView, SIGNAL(wheelRolled(int)), this, SLOT(slotSwitchDesktops(int)));
        ...
        // set the size of the area for desktop icons placement
        {
            ...
            if(kapp->dcopClient()->call(kicker_name, kicker_name, "desktopIconsArea(int)", data, replyType, result, false, 2000))
            {
...
void KDesktop::backgroundInitDone()
{
...
void KDesktop::runAutoStart()
{
    ...
    for(; it != end; ++it)
    {
        ...
        if((*it).right(1) != "~" && (*it).right(4) != ".bak" && ((*it)[0] != '%' || (*it).right(1) != "%")
           && ((*it)[0] != '#' || (*it).right(1) != "#"))
        {
            KURL url;
...
void KDesktop::initConfig()
{
    ...
    m_eWheelDirection = (KDesktopSettings::wheelDirection() == forward_string) ? Forward : Reverse;
}
...
void KDesktop::popupExecuteCommand(const QString &command)
{
    ...
    else
    {
        ...
        m_miniCli->move(rect.x() + (rect.width() - m_miniCli->width()) / 2, rect.y() + (rect.height() - m_miniCli->height()) / 2);
...
void KDesktop::slotSetVRoot()
{
    ...
    while(1)
    {
        /*int ret = */ XQueryTree(qt_xdisplay(), top, &rootReturn, &parentReturn, &children, &numChildren);
        if(children)
            XFree((char *)children);
        if(parentReturn == rw)
        {
    ...
    if(set_vroot)
        XChangeProperty(qt_xdisplay(), top, vroot, XA_WINDOW, 32, PropModeReplace, (unsigned char *)vroot_data, 1);
    else
...
void KDesktop::handleColorDropEvent(QDropEvent *e)
{
    ...
    {
        case 1:
            bgMgr->setColor(c, true);
            break;
        case 2:
            bgMgr->setColor(c, false);
            break;
        default:
            return;
    }
...
{
    logout(KApplication::ShutdownConfirmDefault, KApplication::ShutdownTypeNone);
}

void KDesktop::logout(KApplication::ShutdownConfirm confirm, KApplication::ShutdownType sdtype)
{
...
{
    logout(KApplication::ShutdownConfirmNo, KApplication::ShutdownTypeNone);
}

void KDesktop::slotHaltNoCnf()
{
    logout(KApplication::ShutdownConfirmNo, KApplication::ShutdownTypeHalt);
}

void KDesktop::slotRebootNoCnf()
{
    logout(KApplication::ShutdownConfirmNo, KApplication::ShutdownTypeReboot);
}
