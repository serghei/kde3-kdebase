...
Minicli::Minicli(QWidget *parent, const char *name) : KDialog(parent, name, false, WType_TopLevel), m_autoCheckedRunInTerm(false)
{
    ...
    connect(m_dlg->cbCommand, SIGNAL(textChanged(const QString &)), SLOT(slotCmdChanged(const QString &)));

    connect(m_dlg->cbCommand, SIGNAL(returnPressed()), m_dlg->pbRun, SLOT(animateClick()));
...
void Minicli::accept()
{
    ...
    if(!cmd.isEmpty() && (cmd[0].isNumber() || (cmd[0] == '(')) && (QRegExp("[a-zA-Z\\]\\[]").search(cmd) == -1))
    {
        ...
        if(m_dlg->cbRunAsOther)
        {
            ...
            if(pw == 0L)
            {
                KMessageBox::sorry(this, i18n("<qt>The user <b>%1</b> "
                                              "does not exist on this system.</qt>")
                                             .arg(m_dlg->leUsername->text()));
                return 1;
            }
        }
        else
...
void Minicli::parseLine(bool final)
{
    ...
    bool isTerminalApp = ((m_filterData->uriType() == KURIFilterData::EXECUTABLE) && m_terminalAppList.contains(m_filterData->uri().url()));
    ...
    if(m_iconName == "www")
    {
        ...
        if(!overlay.isNull())
        {
            ...
            if(icon.mask())
            {
                ...
                bitBlt(&mask, x, y, overlay.mask() ? const_cast< QBitmap * >(overlay.mask()) : &overlay, 0, 0, overlay.width(), overlay.height(),
...
{
    if(m_dlg->cbPriority->isChecked() && (m_iPriority > 50) || (m_iScheduler != StubProcess::SchedNormal))
    {
    ...
    else if(m_dlg->cbRunAsOther->isEnabled() && m_dlg->cbRunAsOther->isChecked() && !m_dlg->leUsername->text().isEmpty())
    {
        m_dlg->lePassword->setEnabled(true);
        m_dlg->lbPassword->setEnabled(true);
    }
...
{
    return ((m_dlg->cbPriority->isChecked() && ((m_iPriority > 50) || (m_iScheduler != StubProcess::SchedNormal)))
...
void Minicli::slotRealtime(bool enabled)
{
    ...
    if(enabled)
    {
        if(KMessageBox::warningContinueCancel(this, i18n("Running a realtime application can be very dangerous. "
                                                         "If the application misbehaves, the system might hang "
                                                         "unrecoverably.\nAre you sure you want to continue?"),
                                              i18n("Warning - Run Command"), KGuiItem(i18n("&Run Realtime")), QString::null,
