...
PasswordDlg::PasswordDlg(LockProcess *parent, GreeterPluginHandle *plugin)
    : QDialog(parent, "password dialog", true, WX11BypassWM), mPlugin(plugin), mCapsLocked(-1), mUnlockingFailed(false)
{
    ...
    KUser user;
    QLabel *greetLabel = new QLabel(user.fullName().isEmpty() ? i18n("<nobr><b>The session is locked</b><br>")
                                                              : i18n("<nobr><b>The session was locked by %1</b><br>").arg(user.fullName()),
                                    frame);
    ...
    greet = plugin->info->create(this, 0, this, mLayoutButton, QString::null, KGreeterPlugin::Authenticate, KGreeterPlugin::ExUnlock);
...
#endif
               "-m", mPlugin->info->method, "-S", fdbuf, (char *)0);
        exit(20);
    }
    ::close(sfd[1]);
    sFd = sfd[0];
...
public:
    LockListViewItem(QListView *parent, const QString &sess, const QString &loc, int _vt) : QListViewItem(parent), vt(_vt)
    {
        setText(0, sess);
        setText(1, loc);
    }
...
void PasswordDlg::slotSwitchUser()
{
    ...
    if(dm.localSessions(sess))
    {
        ...
        lv->setMinimumWidth(fw + hds.width() + (ns > 10 ? style().pixelMetric(QStyle::PM_ScrollBarExtent) : 0));
        lv->setFixedHeight(fw + hds.height() + itm->height() * (ns < 6 ? 6 : ns > 10 ? 10 : ns));
