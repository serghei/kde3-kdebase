...
//
//
static const char *colornames[TABLE_COLORS] = {"fgnormal",  "bgnormal",  "bg0",  "bg1",  "bg2",  "bg3",  "bg4",  "bg5",  "bg6",  "bg7",
                                               "fgintense", "bgintense", "bg0i", "bg1i", "bg2i", "bg3i", "bg4i", "bg5i", "bg6i", "bg7i"};
...
ColorSchema::ColorSchema(const QString &pathname) : m_fileRead(false), lastRead(new QDateTime())
{
...
ColorSchema::ColorSchema() : m_fileRead(false), fRelPath(QString::null), lastRead(0L)
{
    setDefaultSchema();
    m_numb = 0;
}

ColorSchema::ColorSchema(KConfig &c) : m_fileRead(false), fRelPath(QString::null), lastRead(0L)
{
    clearSchema();
...
/* static */ QString ColorSchema::colorName(int i)
{
    ...
    {
        kdWarning() << "Request for color name " << i << " out of range." << endl;
        return QString::null;
    }
...
void ColorSchema::writeConfigColor(KConfig &c, const QString &name, const ColorEntry &e) const
{
...
void ColorSchema::readConfigColor(KConfig &c, const QString &name, ColorEntry &e)
{
    KConfigGroupSaver(&c, name);
    c.setGroup(name);
...
bool ColorSchema::rereadSchemaFile()
{
    ...
    if(!sysin)
    {
        ...
        kdWarning() << "Schema file " << fPath << " could not be opened (" << strerror(e) << ")" << endl;
        return false;
    }
...
bool ColorSchema::hasSchemaFileChanged() const
{
    ...
    {
        kdWarning() << "Schema file no longer exists." << endl;
        return false;
    }
}
...
ColorSchemaList::ColorSchemaList() : QPtrList< ColorSchema >()
{
...
bool ColorSchemaList::deleteOldSchemas(const QDateTime &now)
{
    ...
    while((p = it.current()))
    {
        ...
        {
            KONSOLEDEBUG << "Found deleted schema " << p->relPath() << endl;
            ++it;
            remove(p);
            r = true;
            if(!it.current())
