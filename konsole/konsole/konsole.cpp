...
public:
    KonsoleFontSelectAction(const QString &text, int accel, const QObject *receiver, const char *slot, QObject *parent, const char *name = 0)
        : KSelectAction(text, accel, receiver, slot, parent, name)
    {
    }
    KonsoleFontSelectAction(const QString &text, const QIconSet &pix, int accel, const QObject *receiver, const char *slot, QObject *parent,
                            const char *name = 0)
...
Konsole::Konsole(const char *name, int histon, bool menubaron, bool tabbaron, bool frameon, bool scrollbaron, QCString type, bool b_inRestore,
                 const int wanted_tabbar, const QString &workdir)
    : DCOPObject("konsole")
    , KMainWindow(0, name)
    , m_defaultSession(0)
    ...
    , s_workDir(workdir)
{
    ...
    connect(sessionNumberMapper, SIGNAL(mapped(int)), this, SLOT(newSessionTabbar(int)));
...
void Konsole::makeGUI()
{
    ...
    if(!(isRestored))
    {
        if(sender() && sender()->inherits("QPopupMenu") && static_cast< const QPopupMenu * >(sender())->count() == 1)
    ...
    // bookmarks menu
    if(bookmarkHandler)
        connect(bookmarkHandler, SIGNAL(openURL(const QString &, const QString &)), SLOT(enterURL(const QString &, const QString &)));
    if(bookmarkHandlerSession)
        connect(bookmarkHandlerSession, SIGNAL(openURL(const QString &, const QString &)), SLOT(newSession(const QString &, const QString &)));
    if(m_bookmarks)
    ...
    if(m_options)
    {
        ...
        // Tabbar
        selectTabbar = new KSelectAction(i18n("&Tab Bar"), 0, this, SLOT(slotSelectTabbar()), actions, "tabbar");
        ...
        // Scrollbar
        selectScrollbar = new KSelectAction(i18n("Sc&rollbar"), 0, this, SLOT(slotSelectScrollbar()), actions, "scrollbar");
        ...
        // Select Bell
        selectBell = new KSelectAction(i18n("&Bell"), SmallIconSet("bell"), 0, this, SLOT(slotSelectBell()), actions, "bell");
        QStringList bellitems;
        bellitems << i18n("System &Bell") << i18n("System &Notification") << i18n("&Visible Bell") << i18n("N&one");
        selectBell->setItems(bellitems);
        selectBell->plug(m_options);

        checkBitmapFonts();
        KActionMenu *m_fontsizes = new KActionMenu(i18n("Font"), SmallIconSet("text"), actions, 0L);
        m_fontsizes->insert(new KAction(i18n("&Enlarge Font"), SmallIconSet("fontsizeup"), 0, this, SLOT(biggerFont()), actions, "enlarge_font"));
        m_fontsizes->insert(new KAction(i18n("&Shrink Font"), SmallIconSet("fontsizedown"), 0, this, SLOT(smallerFont()), actions, "shrink_font"));
        m_fontsizes->insert(new KAction(i18n("Se&lect..."), SmallIconSet("font"), 0, this, SLOT(slotSelectFont()), actions, "select_font"));
        if(b_installBitmapFonts)
        {
            m_fontsizes->insert(
                new KAction(i18n("&Install Bitmap..."), SmallIconSet("font"), 0, this, SLOT(slotInstallBitmapFonts()), actions, "install_fonts"));
        }
        ...
        if(!b_fixedSize)
        {
            selectSize = new KonsoleFontSelectAction(i18n("S&ize"), 0, this, SLOT(slotSelectSize()), actions, "size");
            QStringList sizeitems;
            sizeitems << i18n("40x15 (&Small)") << i18n("80x24 (&VT100)") << i18n("80x25 (&IBM PC)") << i18n("80x40 (&XTerm)")
                      << i18n("80x52 (IBM V&GA)") << "" << i18n("&Custom...");
            selectSize->setItems(sizeitems);
            selectSize->plug(m_options);
        }

        KAction *historyType = new KAction(i18n("Hist&ory..."), "history", 0, this, SLOT(slotHistoryType()), actions, "history");
        historyType->plug(m_options);

        m_options->insertSeparator();

        KAction *save_settings = new KAction(i18n("&Save as Default"), "filesave", 0, this, SLOT(slotSaveSettings()), actions, "save_default");
    ...
    if(m_help)
    {
        ...
        m_help->insertItem(SmallIcon("idea"), i18n("&Tip of the Day"), this, SLOT(showTip()), 0, -1, 2);
    }
    ...
    if(m_rightButton)
    {
        ...
        KAction *selectionEnd = new KAction(i18n("Set Selection End"), 0, this, SLOT(slotSetSelectionEnd()), actions, "selection_end");
    ...
    m_tabPopupMenu->insertItem(i18n("&Rename Session..."), this, SLOT(slotTabRenameSession()));
    m_tabPopupMenu->insertSeparator();

    m_tabMonitorActivity = new KToggleAction(i18n("Monitor for &Activity"), SmallIconSet("activity"), 0, this, SLOT(slotTabToggleMonitor()), this);
    m_tabMonitorActivity->setCheckedState(KGuiItem(i18n("Stop Monitoring for &Activity")));
    m_tabMonitorActivity->plug(m_tabPopupMenu);

    m_tabMonitorSilence = new KToggleAction(i18n("Monitor for &Silence"), SmallIconSet("silence"), 0, this, SLOT(slotTabToggleMonitor()), this);
    m_tabMonitorSilence->setCheckedState(KGuiItem(i18n("Stop Monitoring for &Silence")));
    m_tabMonitorSilence->plug(m_tabPopupMenu);

    m_tabMasterMode = new KToggleAction(i18n("Send &Input to All Sessions"), "remote", 0, this, SLOT(slotTabToggleMasterMode()), this);
    ...
    connect(m_tabPopupTabsMenu, SIGNAL(activated(int)), SLOT(activateSession(int)));

    m_tabPopupMenu->insertSeparator();
    m_tabPopupMenu->insertItem(SmallIcon("fileclose"), i18n("C&lose Session"), this, SLOT(slotTabCloseSession()));
    ...
    if(m_options)
    {
        ...
        KToggleAction *dynamicTabHideOption = new KToggleAction(i18n("&Dynamic Hide"), 0, this, SLOT(slotTabbarToggleDynamicHide()), this);
        dynamicTabHideOption->setChecked(b_dynamicTabHide);
        dynamicTabHideOption->plug(m_tabbarPopupMenu);

        KToggleAction *m_autoResizeTabs = new KToggleAction(i18n("&Auto Resize Tabs"), 0, this, SLOT(slotToggleAutoResizeTabs()), this);
...
void Konsole::makeTabWidget()
{
    ...
    connect(tabwidget, SIGNAL(contextMenu(QWidget *, const QPoint &)), SLOT(slotTabContextMenu(QWidget *, const QPoint &)));
    connect(tabwidget, SIGNAL(contextMenu(const QPoint &)), SLOT(slotTabbarContextMenu(const QPoint &)));
...
bool Konsole::eventFilter(QObject *o, QEvent *ev)
{
    if(o == m_newSessionButton)
    {
        ...
        {
            QMouseEvent *mev = static_cast< QMouseEvent * >(ev);
            if((mev->pos() - m_newSessionButtonMousePressPos).manhattanLength() > KGlobalSettings::dndEventDelay())
            {
...
void Konsole::makeBasicGUI()
{
    ...
    m_copyClipboard = new KAction(i18n("&Copy"), "editcopy", 0, this, SLOT(slotCopyClipboard()), m_shortcuts, "edit_copy");
    m_pasteClipboard =
        new KAction(i18n("&Paste"), "editpaste", Qt::SHIFT + Qt::Key_Insert, this, SLOT(slotPasteClipboard()), m_shortcuts, "edit_paste");
    m_pasteSelection =
        new KAction(i18n("Paste Selection"), Qt::CTRL + Qt::SHIFT + Qt::Key_Insert, this, SLOT(slotPasteSelection()), m_shortcuts, "pasteselection");

    m_clearTerminal = new KAction(i18n("C&lear Terminal"), 0, this, SLOT(slotClearTerminal()), m_shortcuts, "clear_terminal");
    m_resetClearTerminal =
        new KAction(i18n("&Reset && Clear Terminal"), 0, this, SLOT(slotResetClearTerminal()), m_shortcuts, "reset_clear_terminal");
    m_findHistory = new KAction(i18n("&Find in History..."), "find", 0, this, SLOT(slotFindHistory()), m_shortcuts, "find_history");
    m_findHistory->setEnabled(b_histEnabled);

    m_findNext = new KAction(i18n("Find &Next"), "next", 0, this, SLOT(slotFindNext()), m_shortcuts, "find_next");
    m_findNext->setEnabled(b_histEnabled);

    m_findPrevious = new KAction(i18n("Find Pre&vious"), "previous", 0, this, SLOT(slotFindPrevious()), m_shortcuts, "find_previous");
    m_findPrevious->setEnabled(b_histEnabled);

    m_saveHistory = new KAction(i18n("S&ave History As..."), "filesaveas", 0, this, SLOT(slotSaveHistory()), m_shortcuts, "save_history");
    m_saveHistory->setEnabled(b_histEnabled);

    m_clearHistory = new KAction(i18n("Clear &History"), "history_clear", 0, this, SLOT(slotClearHistory()), m_shortcuts, "clear_history");
    m_clearHistory->setEnabled(b_histEnabled);

    m_clearAllSessionHistories =
        new KAction(i18n("Clear All H&istories"), "history_clear", 0, this, SLOT(slotClearAllSessionHistories()), m_shortcuts, "clear_all_histories");

    m_detachSession =
        new KAction(i18n("&Detach Session"), SmallIconSet("tab_breakoff"), 0, this, SLOT(slotDetachSession()), m_shortcuts, "detach_session");
    m_detachSession->setEnabled(false);

    m_renameSession =
        new KAction(i18n("&Rename Session..."), Qt::CTRL + Qt::ALT + Qt::Key_S, this, SLOT(slotRenameSession()), m_shortcuts, "rename_session");

    if(kapp->authorizeKAction("zmodem_upload"))
        m_zmodemUpload =
            new KAction(i18n("&ZModem Upload..."), Qt::CTRL + Qt::ALT + Qt::Key_U, this, SLOT(slotZModemUpload()), m_shortcuts, "zmodem_upload");

    monitorActivity = new KToggleAction(i18n("Monitor for &Activity"), SmallIconSet("activity"), 0, this, SLOT(slotToggleMonitor()), m_shortcuts,
                                        "monitor_activity");
    monitorActivity->setCheckedState(KGuiItem(i18n("Stop Monitoring for &Activity")));

    monitorSilence =
        new KToggleAction(i18n("Monitor for &Silence"), SmallIconSet("silence"), 0, this, SLOT(slotToggleMonitor()), m_shortcuts, "monitor_silence");
    monitorSilence->setCheckedState(KGuiItem(i18n("Stop Monitoring for &Silence")));

    masterMode = new KToggleAction(i18n("Send &Input to All Sessions"), "remote", 0, this, SLOT(slotToggleMasterMode()), m_shortcuts,
                                   "send_input_to_all_sessions");

    showMenubar = new KToggleAction(i18n("Show &Menubar"), "showmenu", 0, this, SLOT(slotToggleMenubar()), m_shortcuts, "show_menubar");
    ...
    m_saveProfile = new KAction(i18n("Save Sessions &Profile..."), SmallIconSet("filesaveas"), 0, this, SLOT(slotSaveSessionsProfile()), m_shortcuts,
    ...
    m_closeSession = new KAction(i18n("C&lose Session"), "fileclose", 0, this, SLOT(confirmCloseCurrentSession()), m_shortcuts, "close_session");
    ...
    new KAction(i18n("Go to Previous Session"), QApplication::reverseLayout() ? Qt::SHIFT + Qt::Key_Right : Qt::SHIFT + Qt::Key_Left, this,
                SLOT(prevSession()), m_shortcuts, "previous_session");
    new KAction(i18n("Go to Next Session"), QApplication::reverseLayout() ? Qt::SHIFT + Qt::Key_Left : Qt::SHIFT + Qt::Key_Right, this,
                SLOT(nextSession()), m_shortcuts, "next_session");

    for(int i = 1; i < 13; i++)
    { // Due to 12 function keys?
        new KAction(i18n("Switch to Session %1").arg(i), 0, this, SLOT(switchToSession()), m_shortcuts,
                    QString().sprintf("switch_to_session_%02d", i).latin1());
    }
...
bool Konsole::queryClose()
{
    ...
    if(b_warnQuit)
    {
        if(sessions.count() > 1)
        {
            switch(KMessageBox::warningYesNoCancel(this, i18n("You have open sessions (besides the current one). "
                                                              "These will be killed if you continue.\n"
                                                              "Are you sure you want to quit?"),
                                                   i18n("Really Quit?"), KStdGuiItem::quit(), KGuiItem(i18n("C&lose Session"), "fileclose")))
            {
...
{
    int result = KMessageBox::warningContinueCancel(this, i18n("The application running in Konsole does not respond to the close request. "
                                                               "Do you want Konsole to close anyway?"),
                                                    i18n("Application Does Not Respond"), KStdGuiItem::close());
...
void Konsole::slotSaveSessionsProfile()
{
    ...
    QString prof =
        KInputDialog::getText(i18n("Save Sessions Profile"), i18n("Enter name under which the profile should be saved:"), QString::null, &ok, this);
    if(ok)
    {
        QString path = locateLocal("data", QString::fromLatin1("konsole/profiles/") + prof, KGlobal::instance());
...
void Konsole::saveProperties(KConfig *config)
{
    ...
    { // This will not always work (ie 'winsami' saves as 'ws2')
        if(se)
...
void Konsole::readProperties(KConfig *config, const QString &schema, bool globalConfigOnly)
{
    ...
    if(!globalConfigOnly)
    {
        ...
        if(sch->hasSchemaFileChanged())
            sch->rereadSchemaFile();
...
void Konsole::pixmap_menu_activated(int item, TEWidget *tewidget)
{
    ...
    switch(item)
    {
        ...
        case 3: // center
        {
            ...
            bitBlt(&bgPixmap, (tewidget->size().width() - pm.width()) / 2, (tewidget->size().height() - pm.height()) / 2, &pm, 0, 0, pm.width(),
                   pm.height());
...
void Konsole::slotInstallBitmapFonts()
{
    ...
    if(!sl_installFonts.isEmpty())
    {
        if(KMessageBox::questionYesNoList(
               this, i18n("If you want to use the bitmap fonts distributed with Konsole, they must be installed.  After installation, you must "
                          "restart Konsole to use them.  Do you want to install the fonts listed below into fonts:/Personal?"),
               sl_installFonts, i18n("Install Bitmap Fonts?"), KGuiItem(i18n("&Install")), i18n("Do Not Install"))
           == KMessageBox::Yes)
        {
            for(QStringList::iterator it = sl_installFonts.begin(); it != sl_installFonts.end(); ++it)
            {
                QString sf = "fonts/" + *it;
                if(KIO::NetAccess::copy(locate("appdata", sf), "fonts:/Personal/", 0))
                {
                    b_installBitmapFonts = false;
...
void Konsole::createSessionTab(TEWidget *widget, const QIconSet &iconSet, const QString &text, int index)
{
...
void Konsole::slotConfigureKeys()
{
    ...
    for(uint i = 0; i < m_shortcuts->count(); i++)
    {
        ...
        if(!b_sessionShortcutsEnabled && m_shortcuts->action(i)->shortcut().count() && QString(m_shortcuts->action(i)->name()).startsWith("SSC_"))
        {
            b_sessionShortcutsEnabled = true;
    ...
    if(!ctrlKeys.isEmpty())
    {
        ...
        KMessageBox::informationList(this, i18n("You have chosen one or more Ctrl+<key> combinations to be used as shortcuts. "
                                     ...
                                     ctrlKeys, i18n("Choice of Shortcut Keys"), 0);
    }
}
...
void Konsole::reparseConfiguration()
{
    ...
    disconnect(sessionNumberMapper, SIGNAL(mapped(int)), this, SLOT(newSessionTabbar(int)));
...
void Konsole::slotSelectSize()
{
    ...
    switch(item)
    {
        ...
        case 4:
            ...
            break;
        case 6:
            SizeDialog dlg(te->Columns(), te->Lines(), this);
            if(dlg.exec())
    ...
    for(; from_it.current(); ++from_it)
    {
        ...
        if(from->isMasterMode())
        {
            ...
            for(; to_it.current(); ++to_it)
            {
                ...
                if(to != from)
                    disconnect(from->widget(), SIGNAL(keyPressedSignal(QKeyEvent *)), to->getEmulation(), SLOT(onKeyPress(QKeyEvent *)));
            }
        }
    }
}
    ...
    for(; from_it.current(); ++from_it)
    {
        ...
        if(from->isMasterMode())
        {
            ...
            for(; to_it.current(); ++to_it)
            {
                ...
                if(to != from)
                {
                    connect(from->widget(), SIGNAL(keyPressedSignal(QKeyEvent *)), to->getEmulation(), SLOT(onKeyPress(QKeyEvent *)));
                }
            }
        }
...
void Konsole::addSession(TESession *s)
{
    ...
    KRadioAction *ra = new KRadioAction(newTitle.replace('&', "&&"), s->IconName(), 0, this, SLOT(activateSession()), m_shortcuts);
...
void Konsole::setSessionEncoding(const QString &encoding, TESession *session)
{
    ...
    {
        if(QString::compare(KGlobal::charsets()->encodingForName(*it), t_encoding) == 0)
        {
            found_encoding = true;
        }
        i++;
...
void Konsole::newSession(int i)
{
    if(i == SESSION_NEW_WINDOW_ID)
    {
        ...
        Konsole *konsole =
            new Konsole(name(), b_histEnabled, !menubar->isHidden(), n_tabbar != TabNone, b_framevis, n_scroll != TEWidget::SCRNONE, 0, false, 0);
        konsole->newSession();
...
QString Konsole::newSession(KSimpleConfig *co, QString program, const QStrList &args, const QString &_term, const QString &_icon,
                            const QString &_title, const QString &_cwd)
{
    ...
    connect(te, SIGNAL(configureRequest(TEWidget *, int, int, int)), this, SLOT(configureRequest(TEWidget *, int, int, int)));
    if(te_old)
    {
    ...
    connect(s, SIGNAL(done(TESession *)), this, SLOT(doneSession(TESession *)));
    connect(s, SIGNAL(updateTitle(TESession *)), this, SLOT(updateTitle(TESession *)));
    connect(s, SIGNAL(notifySessionState(TESession *, int)), this, SLOT(notifySessionState(TESession *, int)));
    connect(s, SIGNAL(disableMasterModeConnections()), this, SLOT(disableMasterModeConnections()));
    connect(s, SIGNAL(enableMasterModeConnections()), this, SLOT(enableMasterModeConnections()));
    connect(s, SIGNAL(renameSession(TESession *, const QString &)), this, SLOT(slotRenameSession(TESession *, const QString &)));
    connect(s->getEmulation(), SIGNAL(changeColumns(int)), this, SLOT(changeColumns(int)));
    connect(s->getEmulation(), SIGNAL(changeColLin(int, int)), this, SLOT(changeColLin(int, int)));
    connect(s->getEmulation(), SIGNAL(ImageSizeChanged(int, int)), this, SLOT(notifySize(int, int)));
    connect(s, SIGNAL(zmodemDetected(TESession *)), this, SLOT(slotZModemDetected(TESession *)));
    connect(s, SIGNAL(updateSessionConfig(TESession *)), this, SLOT(slotUpdateSessionConfig(TESession *)));
    connect(s, SIGNAL(resizeSession(TESession *, QSize)), this, SLOT(slotResizeSession(TESession *, QSize)));
    connect(s, SIGNAL(setSessionEncoding(TESession *, const QString &)), this, SLOT(slotSetSessionEncoding(TESession *, const QString &)));
    connect(s, SIGNAL(getSessionSchema(TESession *, QString &)), this, SLOT(slotGetSessionSchema(TESession *, QString &)));
    connect(s, SIGNAL(setSessionSchema(TESession *, const QString &)), this, SLOT(slotSetSessionSchema(TESession *, const QString &)));
    connect(s, SIGNAL(changeTabTextColor(TESession *, int)), this, SLOT(changeTabTextColor(TESession *, int)));
...
void Konsole::newSession(const QString &sURL, const QString &title)
{
    ...
    if((url.protocol() == "file") && (url.hasPath()))
    {
        KSimpleConfig *co = defaultSession();
        path = url.path();
        newSession(co, QString::null, QStrList(), QString::null, QString::null, title.isEmpty() ? path : title, path);
        return;
    }
    else if((!url.protocol().isEmpty()) && (url.hasHost()))
    {
        ...
        if(url.port() && !isSSH)
            args.append(QCString().setNum(url.port()));
        newSession(NULL, protocol.latin1() /* protocol */, args /* arguments */, QString::null /*term*/, QString::null /*icon*/,
...
void Konsole::confirmCloseCurrentSession(TESession *_se)
{
    if(!_se)
        _se = se;
    if(KMessageBox::warningContinueCancel(this, i18n("Are you sure that you want to close the current session?"), i18n("Close Confirmation"),
                                          KGuiItem(i18n("C&lose Session"), "tab_remove"), "ConfirmCloseSession")
...
void Konsole::moveSessionLeft()
{
    ...
    createSessionTab(se->widget(), iconSetForSession(se), title.replace('&', "&&"), position - 1);
    tabwidget->showPage(se->widget());
...
void Konsole::notifySessionState(TESession *session, int state)
{
    ...
    switch(state)
    {
        ...
        case NOTIFYBELL:
            ...
            break;
        case NOTIFYACTIVITY:
            state_iconname = "activity";
            break;
        case NOTIFYSILENCE:
            state_iconname = "silence";
            break;
    }
    if(!state_iconname.isEmpty() && session->testAndSetStateIconName(state_iconname) && m_tabViewMode != ShowTextOnly)
    {

        QPixmap normal = KGlobal::instance()->iconLoader()->loadIcon(state_iconname, KIcon::Small, 0, KIcon::DefaultState, 0L, true);
        QPixmap active = KGlobal::instance()->iconLoader()->loadIcon(state_iconname, KIcon::Small, 0, KIcon::ActiveState, 0L, true);
...
void Konsole::addSessionCommand(const QString &path)
{
    ...
    if(typ.isEmpty() || txt.isEmpty() || typ != "KonsoleApplication" || (!exec.isEmpty() && pexec.isEmpty()))
    {
        if(!path.isEmpty())
            delete co;
    ...
    if((b_sessionShortcutsMapped == true) || (cmd_serial == SESSION_NEW_SHELL_ID))
        return;
...
void Konsole::createSessionMenus()
{
    ...
    { // All sessions have been deleted
        m_session->insertItem(SmallIconSet("window_new"), i18n("New &Window"), SESSION_NEW_WINDOW_ID);
        m_tabbarSessionsCommands->insertItem(SmallIconSet("window_new"), i18n("New &Window"), SESSION_NEW_WINDOW_ID);
        return;
    }
    ...
    insertItemSorted(m_tabbarSessionsCommands, SmallIconSet(icon), txt.replace('&', "&&"), SESSION_NEW_SHELL_ID);

    QString comment = cfg->readEntry("Comment");
    if(comment.isEmpty())
        comment = txt.prepend(i18n("New "));
    insertItemSorted(m_session, SmallIconSet(icon), comment.replace('&', "&&"), SESSION_NEW_SHELL_ID);
    m_session->insertItem(SmallIconSet("window_new"), i18n("New &Window"), SESSION_NEW_WINDOW_ID);
    m_tabbarSessionsCommands->insertItem(SmallIconSet("window_new"), i18n("New &Window"), SESSION_NEW_WINDOW_ID);
    m_session->insertSeparator();
    ...
    for(; it.current(); ++it)
    {
        ...
        insertItemSorted(m_tabbarSessionsCommands, SmallIconSet(icon), txt.replace('&', "&&"), it.currentKey());
        QString comment = (*it).readEntry("Comment");
        if(comment.isEmpty())
            comment = txt.prepend(i18n("New "));
        insertItemSorted(m_session, SmallIconSet(icon), comment.replace('&', "&&"), it.currentKey());
    }

    if(m_bookmarksSession)
    {
        m_session->insertSeparator();
        m_session->insertItem(SmallIconSet("keditbookmarks"), i18n("New Shell at Bookmark"), m_bookmarksSession);
        m_tabbarSessionsCommands->insertSeparator();
        m_tabbarSessionsCommands->insertItem(SmallIconSet("keditbookmarks"), i18n("Shell at Bookmark"), m_bookmarksSession);
    }
}
...
void Konsole::addScreenSession(const QString &path, const QString &socket)
{
    ...
    co->writePathEntry("Exec", QString::fromLatin1("SCREENDIR=%1 screen -r %2").arg(path).arg(socket));
    QString icon = "konsole";
    cmd_serial++;
...
void Konsole::detachSession(TESession *_se)
{
    ...
    else
    {
        ...
        for(; from_it.current(); ++from_it)
        {
            TESession *from = from_it.current();
            if(from->isMasterMode())
                disconnect(from->widget(), SIGNAL(keyPressedSignal(QKeyEvent *)), _se->getEmulation(), SLOT(onKeyPress(QKeyEvent *)));
        }
    }

    QColor se_tabtextcolor = tabwidget->tabColor(_se->widget());

    disconnect(_se, SIGNAL(done(TESession *)), this, SLOT(doneSession(TESession *)));
...
void Konsole::attachSession(TESession *session)
{
    ...
    KRadioAction *ra = new KRadioAction(title.replace('&', "&&"), session->IconName(), 0, this, SLOT(activateSession()), m_shortcuts);
    ...
    connect(session, SIGNAL(done(TESession *)), this, SLOT(doneSession(TESession *)));
...
void Konsole::renameSession(TESession *ses)
{
    ...
    title = KInputDialog::getText(i18n("Rename Session"), i18n("Session name:"), title, &ok, this);
...
HistoryTypeDialog::HistoryTypeDialog(const HistoryType &histType, unsigned int histSize, QWidget *parent)
    : KDialogBase(Plain, i18n("History Configuration"), Help | Default | Ok | Cancel, Ok, parent, 0, true, true)
{
...
void Konsole::slotFind()
{
    ...
    if(se->getEmulation()->findTextNext(m_find_pattern, forward, m_finddialog->case_sensitive(), m_finddialog->reg_exp()))
        m_find_found = true;
    else if(m_find_found)
    {
        if(forward)
        {
            if(KMessageBox::questionYesNo(m_finddialog, i18n("End of history reached.\n"
                                                             "Continue from the beginning?"),
                                          i18n("Find"), KStdGuiItem::cont(), KStdGuiItem::cancel())
               == KMessageBox::Yes)
            {
                m_find_first = true;
                slotFind();
        ...
        {
            if(KMessageBox::questionYesNo(m_finddialog, i18n("Beginning of history reached.\n"
        ...
        KMessageBox::information(m_finddialog, i18n("Search string '%1' not found.").arg(KStringHandler::csqueeze(m_find_pattern)), i18n("Find"));
}
...
void Konsole::slotSaveHistory()
{
    ...
    info.setFile(name);
    if(info.exists())
        query = KMessageBox::warningContinueCancel(this, i18n("A file with this name already exists.\nDo you want to overwrite it?"),
...
void Konsole::slotZModemUpload()
{
    if(se->zmodemIsBusy())
    {
        KMessageBox::sorry(this, i18n("<p>The current session already has a ZModem file transfer in progress."));
        return;
    }
    ...
    if(zmodem.isEmpty())
    {
        KMessageBox::sorry(this, i18n("<p>No suitable ZModem software was found on "
                                      "the system.\n"
                                      "<p>You may wish to install the 'rzsz' or 'lrzsz' package.\n"));
        return;
    }

    QStringList files = KFileDialog::getOpenFileNames(QString::null, QString::null, this, i18n("Select Files to Upload"));
    if(files.isEmpty())
        return;
...
{
    if(!kapp->authorize("zmodem_download"))
        return;
    ...
    if(zmodem.isEmpty())
    {
        KMessageBox::information(this, i18n("<p>A ZModem file transfer attempt has been detected, "
                                            "but no suitable ZModem software was found on "
                                            "the system.\n"
                                            "<p>You may wish to install the 'rzsz' or 'lrzsz' package.\n"));
        return;
    }
    KURLRequesterDlg dlg(KGlobalSettings::documentPath(), i18n("A ZModem file transfer attempt has been detected.\n"
                                                               "Please specify the folder you want to store the file(s):"),
                         this, "zmodem_dlg");
    dlg.setButtonOK(
        KGuiItem(i18n("&Download"), i18n("Start downloading file to specified folder."), i18n("Start downloading file to specified folder.")));
    if(!dlg.exec())
    {
...
void Konsole::slotPrint()
{
    ...
    if(printer.setup(this, i18n("Print %1").arg(se->Title())))
    {
        ...
        QPainter paint;
        paint.begin(&printer);
        se->print(paint, printer.option("app-konsole-printfriendly") == "true", printer.option("app-konsole-printexact") == "true");
        paint.end();
    }
}
...
SizeDialog::SizeDialog(const unsigned int columns, const unsigned int lines, QWidget *parent)
    : KDialogBase(Plain, i18n("Size Configuration"), Help | Default | Ok | Cancel, Ok, parent)
{
    QFrame *mainFrame = plainPage();
...
KonsoleFind::KonsoleFind(QWidget *parent, const char *name, bool /*modal*/) : KEdFind(parent, name, false), m_editorDialog(0), m_editRegExp(0)
{
