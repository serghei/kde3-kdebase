...
#else
inline cacol::cacol(UINT8 ty, int co) : t(ty), u(0), v(0), w(0)
{
    switch(t)
    {
        case CO_UND:
...
inline const QColor color256(UINT8 u, const ColorEntry *base)
{
    ...
    if(u < 216)
        return QColor(255 * ((u / 36) % 6) / 5, 255 * ((u / 6) % 6) / 5, 255 * ((u / 1) % 6) / 5);
    u -= 216;
...
inline QColor cacol::color(const ColorEntry *base) const
{
    ...
    {
        case CO_DFT:
            return base[u + 0 + (v ? BASE_COLORS : 0)].color;
        case CO_SYS:
            return base[u + 2 + (v ? BASE_COLORS : 0)].color;
        case CO_256:
            return color256(u, base);
        case CO_RGB:
            return QColor(u, v, w);
        default:
            return QColor(255, 0, 0); // diagnostic catch all
    }
}
...
class ca {
public:
    inline ca(UINT16 _c = ' ', cacol _f = cacol(CO_DFT, DEFAULT_FORE_COLOR), cacol _b = cacol(CO_DFT, DEFAULT_BACK_COLOR),
...
{
    return (f.t == CO_DFT) && base[f.u + 0 + (f.v ? BASE_COLORS : 0)].bold || (f.t == CO_SYS) && base[f.u + 2 + (f.v ? BASE_COLORS : 0)].bold;
}
