...
KParts::Part *konsoleFactory::createPartObject(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name,
...
void konsolePart::doneSession(TESession *)
{
    ...
    if(se && b_autoDestroy)
    {
        ...
        disconnect(se, SIGNAL(done(TESession *)), this, SLOT(doneSession(TESession *)));
        se->setConnect(false);
...
void konsolePart::makeGUI()
{
    ...
    if(kapp->authorizeKAction("settings"))
    {
        ...
        // Scrollbar
        selectScrollbar = new KSelectAction(i18n("Sc&rollbar"), 0, this, SLOT(slotSelectScrollbar()), settingsActions);
        ...
        selectBell = new KSelectAction(i18n("&Bell"), SmallIconSet("bell"), 0, this, SLOT(slotSelectBell()), settingsActions, "bell");
        ...
        m_fontsizes->insert(
            new KAction(i18n("&Enlarge Font"), SmallIconSet("viewmag+"), 0, this, SLOT(biggerFont()), settingsActions, "enlarge_font"));
        m_fontsizes->insert(
            new KAction(i18n("&Shrink Font"), SmallIconSet("viewmag-"), 0, this, SLOT(smallerFont()), settingsActions, "shrink_font"));
        ...
        selectSetEncoding =
            new KSelectAction(i18n("&Encoding"), SmallIconSet("charset"), 0, this, SLOT(slotSetEncoding()), settingsActions, "set_encoding");
        ...
        KAction *historyType = new KAction(i18n("&History..."), "history", 0, this, SLOT(slotHistoryType()), settingsActions, "history");
        historyType->plug(m_options);
        m_options->insertSeparator();

        // Select line spacing
        selectLineSpacing =
            new KSelectAction(i18n("Li&ne Spacing"), SmallIconSet("leftjust"), 0, this, SLOT(slotSelectLineSpacing()), settingsActions);

        QStringList lineSpacingList;
        lineSpacingList << i18n("&0") << i18n("&1") << i18n("&2") << i18n("&3") << i18n("&4") << i18n("&5") << i18n("&6") << i18n("&7") << i18n("&8");
        selectLineSpacing->setItems(lineSpacingList);
        selectLineSpacing->plug(m_options);

        // Blinking Cursor
        blinkingCursor = new KToggleAction(i18n("Blinking &Cursor"), 0, this, SLOT(slotBlinkingCursor()), settingsActions);
        blinkingCursor->plug(m_options);

        // Frame on/off
        showFrame = new KToggleAction(i18n("Show Fr&ame"), 0, this, SLOT(slotToggleFrame()), settingsActions);
        showFrame->setCheckedState(i18n("Hide Fr&ame"));
        showFrame->plug(m_options);

        // Word Connectors
        KAction *WordSeps = new KAction(i18n("Wor&d Connectors..."), 0, this, SLOT(slotWordSeps()), settingsActions);
        WordSeps->plug(m_options);

        // Use Konsole's Settings
        m_options->insertSeparator();
        m_useKonsoleSettings = new KToggleAction(i18n("&Use Konsole's Settings"), 0, this, SLOT(slotUseKonsoleSettings()), 0, "use_konsole_settings");
        m_useKonsoleSettings->plug(m_options);

        // Save Settings
        m_options->insertSeparator();
        KAction *saveSettings = new KAction(i18n("&Save as Default"), "filesave", 0, this, SLOT(saveProperties()), actions, "save_default");
    ...
    KAction *selectionEnd = new KAction(i18n("Set Selection End"), 0, te, SLOT(setSelectionEnd()), actions, "selection_end");
    selectionEnd->plug(m_popupMenu);

    KAction *copyClipboard = new KAction(i18n("&Copy"), "editcopy", 0, te, SLOT(copyClipboard()), actions, "edit_copy");
    copyClipboard->plug(m_popupMenu);

    KAction *pasteClipboard = new KAction(i18n("&Paste"), "editpaste", 0, te, SLOT(pasteClipboard()), actions, "edit_paste");
    ...
    KAction *closeSession =
        new KAction(i18n("&Close Terminal Emulator"), "fileclose", 0, this, SLOT(closeCurrentSession()), actions, "close_session");
...
void konsolePart::saveProperties()
{
    ...
    if(b_useKonsoleSettings)
    { // Don't save Settings if using konsolerc
...
void konsolePart::pixmap_menu_activated(int item)
{
    ...
    switch(item)
    {
        ...
        case 3: // center
        {
            ...
            bitBlt(&bgPixmap, (te->size().width() - pm.width()) / 2, (te->size().height() - pm.height()) / 2, &pm, 0, 0, pm.width(), pm.height());
...
void konsolePart::slotWordSeps()
{
    ...
    QString seps =
        KInputDialog::getText(i18n("Word Connectors"), i18n("Characters other than alphanumerics considered part of a word when double clicking:"),
                              s_word_seps, &ok, parentWidget);
    if(ok)
    {
        s_word_seps = seps;
        te->setWordCharacters(s_word_seps);
...
HistoryTypeDialog::HistoryTypeDialog(const HistoryType &histType, unsigned int histSize, QWidget *parent)
    : KDialogBase(Plain, i18n("History Configuration"), Help | Default | Ok | Cancel, Ok, parent)
{
    ...
    QObject::connect(m_btnEnable, SIGNAL(toggled(bool)), this, SLOT(slotHistEnable(bool)));
...
konsoleBrowserExtension::konsoleBrowserExtension(konsolePart *parent) : KParts::BrowserExtension(parent, "konsoleBrowserExtension")
{
}
...
void konsolePart::startProgram(const QString &program, const QStrList &args)
{
...
void konsolePart::newSession()
{
    ...
    connect(se, SIGNAL(done(TESession *)), this, SLOT(doneSession(TESession *)));
    connect(se, SIGNAL(openURLRequest(const QString &)), this, SLOT(emitOpenURLRequest(const QString &)));
    connect(se, SIGNAL(updateTitle(TESession *)), this, SLOT(updateTitle(TESession *)));
    connect(se, SIGNAL(enableMasterModeConnections()), this, SLOT(enableMasterModeConnections()));
    connect(se, SIGNAL(processExited(KProcess *)), this, SIGNAL(processExited(KProcess *)));
    connect(se, SIGNAL(receivedData(const QString &)), this, SIGNAL(receivedData(const QString &)));
    connect(se, SIGNAL(forkedChild()), this, SIGNAL(forkedChild()));
