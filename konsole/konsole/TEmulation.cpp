...
TEmulation::TEmulation(TEWidget *w) : gui(w), scr(0), connected(false), listenToKeyPress(false), m_codec(0), decoder(0), keytrans(0), m_findPos(-1)
{
...
void TEmulation::connectGUI()
{
    ...
    QObject::connect(gui, SIGNAL(beginSelectionSignal(const int, const int, const bool)), this,
                     SLOT(onSelectionBegin(const int, const int, const bool)));
    ...
    QObject::connect(gui, SIGNAL(copySelectionSignal()), this, SLOT(copySelection()));
...
void TEmulation::changeGUI(TEWidget *newgui)
{
    ...
    if(gui)
    {
        QObject::disconnect(gui, SIGNAL(changedHistoryCursor(int)), this, SLOT(onHistoryCursorChange(int)));
        QObject::disconnect(gui, SIGNAL(keyPressedSignal(QKeyEvent *)), this, SLOT(onKeyPress(QKeyEvent *)));
        QObject::disconnect(gui, SIGNAL(beginSelectionSignal(const int, const int, const bool)), this,
                            SLOT(onSelectionBegin(const int, const int, const bool)));
        QObject::disconnect(gui, SIGNAL(extendSelectionSignal(const int, const int)), this, SLOT(onSelectionExtend(const int, const int)));
        QObject::disconnect(gui, SIGNAL(endSelectionSignal(const bool)), this, SLOT(setSelection(const bool)));
        QObject::disconnect(gui, SIGNAL(copySelectionSignal()), this, SLOT(copySelection()));
        QObject::disconnect(gui, SIGNAL(clearSelectionSignal()), this, SLOT(clearSelection()));
        QObject::disconnect(gui, SIGNAL(isBusySelecting(bool)), this, SLOT(isBusySelecting(bool)));
        QObject::disconnect(gui, SIGNAL(testIsSelected(const int, const int, bool &)), this, SLOT(testIsSelected(const int, const int, bool &)));
    }
    gui = newgui;
    connectGUI();
}
...
{
    setCodec(c ? QTextCodec::codecForName("utf8") : QTextCodec::codecForLocale());
}
...
void TEmulation::showBulk()
{
    ...
    if(connected)
    {
        ...
        gui->setImage(image, scr->getLines(), scr->getColumns()); // actual refresh
