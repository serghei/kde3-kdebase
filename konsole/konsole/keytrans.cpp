...
{
    return ((mask & (1 << BITS_Alt)) && (bits & (1 << BITS_Alt))) || ((mask & (1 << BITS_AnyMod)) && (bits & (1 << BITS_AnyMod)));
}
...
KeyTrans::KeyTrans(const QString &path) : m_path(path), m_numb(0), m_fileRead(false)
{
    ...
    for(QPtrListIterator< KeyEntry > it(tableX); it.current(); ++it)
        if(it.current()->matches(key, bits, 0xffff))
        {
            ...
            *len = it.current()->txt.length();
            ...
            {
                static char buf[16];
                char *c, mask = '1' + BITS(0, bits & (1 << BITS_Shift)) + BITS(1, bits & (1 << BITS_Alt)) + BITS(2, bits & (1 << BITS_Control));
...
void KeytabReader::ReportToken() // diagnostic
{
    ...
    switch(sym)
    {
        ...
        case SYMEof:
            ...
            break;
        case SYMName:
            printf("Name: %s", res.latin1());
            break;
        case SYMOpr:
            printf("Opr : %s", res.latin1());
            break;
        case SYMString:
            printf("String len %d,%d ", res.length(), len);
...
#define assertSyntax(Cond, Message)                                                                                                                  \
    if(!(Cond))                                                                                                                                      \
    {                                                                                                                                                \
        ReportError(Message);                                                                                                                        \
        goto ERROR;                                                                                                                                  \
...
Loop:
    ...
    {
        getSymbol();
        assertSyntax(sym == SYMString, "Header expected") kt->m_hdr = i18n(res.latin1());
        getSymbol();
        assertSyntax(sym == SYMEol, "Text unexpected") getSymbol(); // eoln
        goto Loop;
    }
    if(sym == SYMName && !strcmp(res.latin1(), "key"))
    {
        ...
        getSymbol();
        assertSyntax(sym == SYMName, "Name expected") assertSyntax(syms->keysyms[res], "Unknown key name") ptrdiff_t key =
            (ptrdiff_t)(syms->keysyms[res]) - 1;
        ...
        while(sym == SYMOpr && (!strcmp(res.latin1(), "+") || !strcmp(res.latin1(), "-")))
        {
            ...
            getSymbol();
            // mode name
            assertSyntax(sym == SYMName, "Name expected") assertSyntax(syms->modsyms[res], "Unknown mode name") ptrdiff_t bits =
                (ptrdiff_t)(syms->modsyms[res]) - 1;
            if(mask & (1 << bits))
            {
        ...
        assertSyntax(sym == SYMOpr && !strcmp(res.latin1(), ":"), "':' expected") getSymbol();
        // string or command
        assertSyntax(sym == SYMName || sym == SYMString, "Command or string expected") ptrdiff_t cmd = 0;
        if(sym == SYMName)
        {
            assertSyntax(syms->oprsyms[res], "Unknown operator name") cmd = (ptrdiff_t)(syms->oprsyms[res]) - 1;
        ...
        getSymbol();
        assertSyntax(sym == SYMEol, "Unexpected text") goto Loop;
    }
    if(sym == SYMEol)
    {
        getSymbol();
