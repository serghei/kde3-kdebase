...
static const ColorEntry base_color_table[TABLE_COLORS] =
    ...
    // It contains the 8 ansiterm/xterm colors in 2 intensities.
    {
        ...
        // intensiv
        ColorEntry(QColor(0x00, 0x00, 0x00), 0, 1), ColorEntry(QColor(0xFF, 0xFF, 0xFF), 1, 0), ColorEntry(QColor(0x68, 0x68, 0x68), 0, 0),
        ColorEntry(QColor(0xFF, 0x54, 0x54), 0, 0), ColorEntry(QColor(0x54, 0xFF, 0x54), 0, 0), ColorEntry(QColor(0xFF, 0xFF, 0x54), 0, 0),
        ColorEntry(QColor(0x54, 0x54, 0xFF), 0, 0), ColorEntry(QColor(0xFF, 0x54, 0xFF), 0, 0), ColorEntry(QColor(0x54, 0xFF, 0xFF), 0, 0),
...
void TEWidget::setColorTable(const ColorEntry table[])
{
    ...
    if(!pm)
        ...
        else
        {
            ...
            int pixel = qAlpha(blend_color) << 24 | int(qRed(blend_color) * alpha) << 16 | int(qGreen(blend_color) * alpha) << 8
    ...
    , blend_color(qRgba(0, 0, 0, 0xff))
{
    ...
    QObject::connect((QObject *)cb, SIGNAL(selectionChanged()), this, SLOT(onClearSelection()));
...
void TEWidget::drawTextFixed(QPainter &paint, int x, int y, QString &str, const ca *attr)
{
    QString drawstr;
    unsigned int nc = 0;
    int w;
...
void TEWidget::drawAttrStr(QPainter &paint, QRect rect, QString &str, const ca *attr, bool pm, bool clear)
{
    ...
    if(!printerFriendly)
    {
        ...
        {
            if(pm || clear || (blinking && (attr->r & RE_BLINK)) || attr->b == cacol(CO_DFT, colorsSwapped ? DEFAULT_FORE_COLOR : DEFAULT_BACK_COLOR))
        ...
        if(m_isIMEdit && !tmpStr.isEmpty())
        { // imput method edit area background color
            QRect tmpRect = rect;
            if(str != m_imPreeditText)
            { // ugly hack
    ...
    if(!(blinking && (attr->r & RE_BLINK)))
    {
        ...
        if(attr->r & RE_UNDERLINE)
            paint.drawLine(rect.left(), rect.y() + a + 1, rect.right(), rect.y() + a + 1);
    }
}
...
void TEWidget::setImage(const ca *const newimg, int lines, int columns)
{
    ...
    for(y = 0; y < lins; y++)
    {
        ...
        for(x = 0; x < cols; x++)
        {
            if(((m_imPreeditLength > 0) && ((m_imStartLine == y) && ((m_imStart < m_imEnd) && ((x > m_imStart)) && (x < m_imEnd))
            ...
            for(x = 0; x < cols; x++)
            {
                ...
                if(dirtyMask[x])
                {
                    ...
                    for(len = 1; len < lln; len++)
                    {
                        ...
                        if(ext[x + len].f != cf || ext[x + len].b != cb || ext[x + len].r != cr || !dirtyMask[x + len] || isLineChar(c) != lineDraw
                           || (ext[x + len + 1].c == 0) != doubleWidth)
                            break;
                    ...
                    if(doubleWidth)
                        fixed_font = false;
                    drawAttrStr(paint, QRect(bX + tLx + font_w * x, bY + tLy + font_h * y, font_w * len, font_h), unistr, &ext[x], pm != NULL, true);
    ...
    if(resizing && terminalSizeHint)
    {
        ...
        mResizeWidget->move((width() - mResizeWidget->width()) / 2, (height() - mResizeWidget->height()) / 2 + 20);
        mResizeWidget->show();
...
void TEWidget::paintContents(QPainter &paint, const QRect &rect, bool pm)
{
    ...
    for(int y = luy; y <= rly; y++)
    {
        ...
        for(; x <= rlx; x++)
        {
            ...
            while(x + len <= rlx && image[loc(x + len, y)].f == cf && image[loc(x + len, y)].b == cb && image[loc(x + len, y)].r == cr
                  && (image[loc(x + len, y) + 1].c == 0) == doubleWidth && isLineChar(c = image[loc(x + len, y)].c) == lineDraw) // Assignment!
            {
                if(c)
                    disstrU[p++] = c; // fontMap(c);
            ...
            if(!isBlinkEvent || (cr & RE_BLINK))
            {
                ...
                if(doubleWidth)
                    fixed_font = false;
                QString unistr(disstrU, p);
                drawAttrStr(paint, QRect(bX + tLx + font_w * x, bY + tLy + font_h * y, font_w * len, font_h), unistr, &image[loc(x, y)], pm,
...
void TEWidget::mousePressEvent(QMouseEvent *ev)
{
    ...
    if(ev->button() == LeftButton)
    {
        ...
        else
        {
            ...
            {
                emit mouseSignal(0, (ev->x() - tLx - bX) / font_w + 1,
...
void TEWidget::extendSelection(QPoint pos)
{
    ...
    if(pos.x() < tLx + bX)
        pos.setX(tLx + bX);
    if(pos.x() > tLx + bX + columns * font_w - 1)
        pos.setX(tLx + bX + columns * font_w);
    if(pos.y() < tLy + bY)
        pos.setY(tLy + bY);
    if(pos.y() > tLy + bY + lines * font_h - 1)
        pos.setY(tLy + bY + lines * font_h - 1);
    ...
    if((here == pntSelCorr) && (scroll == scrollbar->value()))
        return; // not moved
        ...
        else
        {
            ...
            if(!mouse_marks && !(ev->state() & ShiftButton))
                emit mouseSignal(3, // release
                                 (ev->x() - tLx - bX) / font_w + 1, (ev->y() - tLy - bY) / font_h + 1 + scrollbar->value() - scrollbar->maxValue());
            releaseMouse();
        }
        dragInfo.state = diNone;
    }
    if(!mouse_marks && ((ev->button() == RightButton && !(ev->state() & ShiftButton)) || ev->button() == MidButton))
    {
        QPoint tL = contentsRect().topLeft();
        int tLx = tL.x();
        int tLy = tL.y();
...
void TEWidget::setSelection(const QString &t)
{
    ...
    QObject::disconnect(cb, SIGNAL(selectionChanged()), this, SLOT(onClearSelection()));

    cb->setSelectionMode(true);
    cb->setText(t);
    cb->setSelectionMode(false);

    QObject::connect(cb, SIGNAL(selectionChanged()), this, SLOT(onClearSelection()));
}
...
{
    if((e->type() == QEvent::Accel || e->type() == QEvent::AccelAvailable) && qApp->focusWidget() == this)
    {
    ...
    {
        QObject::disconnect((QObject *)cb, SIGNAL(dataChanged()), this, SLOT(onClearSelection()));
    }
...
void TEWidget::imEndEvent(QIMEvent *e)
{
    ...
    QRect repaintRect = QRect(bX + tLx, bY + tLy + font_h * m_imStartLine, contentsRect().width(), contentsRect().height());
    m_imStart = 0;
    m_imPreeditLength = 0;
...
{
    e->accept(QTextDrag::canDecode(e) || KURLDrag::canDecode(e));
}

enum dropPopupOptions
{
    paste,
    cd,
    cp,
