...
struct table
{
    int key;
    char *msg;
} paritytable[] = {{1, "NONE"}, {4, "ODD"}, {5, "EVEN"}, {-1, ""}}, nbitstable[] = {{1, "8"}, {2, "7"}, {-1, ""}},
  speedtable[] = {{0, "50"},    {8, "75"},    {16, "110"},  {24, "132.5"}, {32, "150"},   {40, "200"},   {48, "300"},    {56, "600"}, {64, "1200"},
                  {72, "1800"}, {80, "2000"}, {88, "2400"}, {96, "3600"},  {104, "4800"}, {112, "9600"}, {120, "19200"}, {-1, ""}};
...
main()
{
    ...
    do
    {
        ...
        ed(2);
        cup(5, 10);
        printf("VT100 test program, version %s", VERSION);
        cup(7, 10);
...
tst_screen()
{
    ...
    */
    ...
    for(background = 0; background <= 1; background++)
    {
        ...
        {
            cup(row, row);
            printf("This is 132 column mode, %s background.", background ? "dark" : "light");
        }
        holdit();
        rm("?3"); /* 80 cols */
        ed(2);    /* VT100 clears screen on SM3/RM3, but not obviously, so... */
        ...
        {
            cup(row, row);
            printf("This is 80 column mode, %s background.", background ? "dark" : "light");
        }
        holdit();
    }
    ed(2);
    ...
    for(soft = -1; soft <= 0; soft++)
    {
        ...
        for(row = 12; row >= 1; row -= 11)
        {
            ...
            for(down = 0; down >= -1; down--)
            {
                ...
                for(i = 1; i <= 30; i++)
                {
                    printf("%s scroll %s region %d Line %d\n", soft ? "Soft" : "Jump", down ? "down" : "up", 2 * (13 - row), i);
                    if(down)
                    {
                        ri();
                        ri();
    ...
    cup(1, 1);
    printf("%s", "This line should be the one above the bottom of the screeen. ");
    holdit();
    ed(2);
    rm("?6"); /* Origin mode (absolute) */
    cup(24, 1);
    printf("Origin mode test. This line should be at the bottom of the screen.");
    cup(1, 1);
...
tst_characters()
{
    ...
    */
    ...
    scs(1, 'B');
    cup(24, 1);
    printf("These are the installed character sets. ");
    holdit();
}
...
tst_doublesize()
{
    ...
    */
    ...
    cup(8, 1);
    decdhl(0);
    printf("lqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqk");
    cup(9, 1);
    decdhl(1);
    printf("lqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqk");
    cup(10, 1);
    decdhl(0);
    printf("x%c%c%c%c%cx", 9, 9, 9, 9, 9);
    cup(11, 1);
    decdhl(1);
    printf("x%c%c%c%c%cx", 9, 9, 9, 9, 9);
    cup(12, 1);
    ...
    cup(16, 1);
    decdhl(0);
    printf("mqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqj");
    cup(17, 1);
    decdhl(1);
    printf("mqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqj");
    scs(0, 'B');
...
tst_keyboard()
{
    ...
    */
    ...
    static struct natkey
    {
        ...
        int natcol;
        char *natsymbol;
    } natkeytab[][29] = {{{'"', 1, 12, "\""}, {'&', 1, 32, "&"}, {'/', 1, 37, "/"},  {'(', 1, 42, "("}, {')', 1, 47, ")"}, {'=', 1, 52, "="},
                          {'+', 1, 56, "+"},  {'?', 1, 57, "?"}, {'`', 1, 61, "`"},  {'@', 1, 62, "@"}, {'<', 1, 66, "<"}, {'>', 1, 67, ">"},
                          {'}', 2, 58, "}"},  {']', 2, 59, "]"}, {'^', 2, 63, "^"},  {'~', 2, 64, "~"}, {'|', 3, 55, "|"}, {'\\', 3, 56, "\\"},
                          {'{', 3, 60, "{"},  {'[', 3, 61, "["}, {'\'', 3, 71, "'"}, {'*', 3, 72, "*"}, {',', 4, 47, ","}, {';', 4, 48, ";"},
                          {'.', 4, 52, "."},  {':', 4, 53, ":"}, {'-', 4, 57, "-"},  {'_', 4, 58, "_"}, {'\0', 0, 0, ""}},
                         {{'"', 1, 12, "\""}, {'&', 1, 32, "&"}, {'/', 1, 37, "/"},  {'(', 1, 42, "("}, {')', 1, 47, ")"}, {'=', 1, 52, "="},
                          {'+', 1, 56, "+"},  {'?', 1, 57, "?"}, {'`', 1, 61, "`"},  {'@', 1, 62, "@"}, {'<', 1, 66, "<"}, {'>', 1, 67, ">"},
                          {'}', 2, 58, "}"},  {']', 2, 59, "]"}, {'~', 2, 63, "~"},  {'^', 2, 64, "^"}, {'|', 3, 55, "|"}, {'\\', 3, 56, "\\"},
                          {'{', 3, 60, "{"},  {'[', 3, 61, "["}, {'\'', 3, 71, "'"}, {'*', 3, 72, "*"}, {',', 4, 47, ","}, {';', 4, 48, ";"},
                          {'.', 4, 52, "."},  {':', 4, 53, ":"}, {'-', 4, 57, "-"},  {'_', 4, 58, "_"}, {'\0', 0, 0, ""}}};
    ...
    static struct ckey
    {
        int ccount;
        char *csymbol;
    } ckeytab[] = {{0, "NUL (CTRL-@ or CTRL-Space)"},
                   {0, "SOH (CTRL-A)"},
                   {0, "STX (CTRL-B)"},
                   {0, "ETX (CTRL-C)"},
                   {0, "EOT (CTRL-D)"},
    ...
    static char *keyboardmenu[] = {"Standard American ASCII layout", "Swedish national layout D47", "Swedish national layout E47",
                                   /* add new keyboard layouts here */
                                   ""};

    static char *curkeymodes[3] = {"ANSI / Cursor key mode RESET", "ANSI / Cursor key mode SET", "VT52 Mode"};

    static char *fnkeymodes[4] = {"ANSI Numeric mode", "ANSI Application mode", "VT52 Numeric mode", "VT52 Application mode"};
    ...
    if(kblayout)
    {
        ...
        for(j = 0; natkeytab[kblayout][j].natc != '\0'; j++)
        {
            ...
            {
                if(keytab[i].row == natkeytab[kblayout][j].natrow && keytab[i].col == natkeytab[kblayout][j].natcol)
                {
    ...
    println("CTRL-codes twice, including CTRL-S (X-Off) and CTRL-Q (X-Off)!");
...
tst_reports()
{
    ...
    */
    ...
    int i, reportpos;
    char *report, *report2;
    static char *attributes[][2] = {{"\033[?1;0c", "No options (vanilla VT100)"},
    ...
    printf("Report is: ");
    chrprint(report);
    if(!strcmp(report, "\033[0n"))
        printf(" -- means \"TERMINAL OK\"");
    else if(!strcmp(report, "\033[3n"))
        printf(" -- means \"TERMINAL OK\"");
    else
    ...
    {
        if(!strcmp(report, attributes[i][0]))
            break;
    }
    if(*attributes[i][0] == '\0')
    ...
    printf("Report is: ");
    chrprint(report);
    if(strlen(report) < 16 || report[0] != '\033' || report[1] != '[' || report[2] != '2' || report[3] != ';')
        println(" -- Bad format");
    else
    {
        reportpos = 4;
        ...
        if(parity == 0 || nbits == 0 || clkmul == 0)
            ...
            println(" -- OK");
        printf("This means: Parity %s, %s bits, xmitspeed %s, recvspeed %s.\n", lookup(paritytable, parity), lookup(nbitstable, nbits),
               lookup(speedtable, xspeed), lookup(speedtable, rspeed));
    ...
    printf("Report is: ");
    chrprint(report2);
    if(strlen(report2) < 3 || report2[2] != '3')
        println(" -- Bad format");
    else
    {
        report2[2] = '2';
...
tst_vt52()
{
    ...
    static struct rtabl
    {
        char *rcode;
        char *rmsg;
    } resptable[] = {{"\033/K", " -- OK (means Standard VT52)"}, {"\033/Z", " -- OK (means VT100 emulating VT52)"}, {"", " -- Unknown response"}};
...
tst_insdel()
{
    ...
    */
    ...
    for(scr132 = 0; scr132 <= 1; scr132++)
    {
        ...
        decstbm(0, 0);
        cup(2, 1);
        printf("Top line: A's, bottom line: X's, this line, nothing more. ");
        holdit();
        cup(2, 1);
        ed(0);
        cup(1, 2);
...
tst_bugs()
{
    ...
    static char *menutable[] = {"Exit to main menu", "Bug A: Smooth scroll to jump scroll", "Bug B: Scrolling region", "Bug C: Wide to narrow screen",
                                "Bug D: Narrow to wide screen", "Bug E: Cursor move from double- to single-wide line",
                                "Bug F: Column mode escape sequence", "Wrap around with cursor addressing", "Erase right half of double width lines",
                                "Funny scroll regions",
                                /* Add more here */
                                ""};

    static char *hmsg[] = {"Test of known bugs in the DEC VT100 series. The numbering of some of",
...
char *lookup(t, k) struct table t[];
int k;
{
