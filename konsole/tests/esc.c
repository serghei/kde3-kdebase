...
{ /* Select character Set */
    printf("%c%c%c%c%c%c%c", 27, g ? ')' : '(', c, 27, g ? '(' : ')', 'B', g ? 14 : 15);
}
sgr(ps) char *ps;
...
#ifdef SARG10
    if(trmop(01031, 0) < 5)
        zleep(500); /* wait longer if low speed */
    else
        zleep(100);
#else
    zleep(100);
...
#ifdef SARG20
    while(jsys(SIBE, 2, _PRIIN) != 0)
    { /* read input until buffer is empty */
        ejsys(BIN, _PRIIN);
        result[i++] = jsac[2];
        if(i == 79)
            break;
...
superbin(bin) int bin;
{
  ...
  if (v) {
        printf("STIW superbinary setting failed");
        exit(); }
  ttymode = bin;
}
