...
Tab_widget::Tab_widget(QWidget *parent_P, const char *name_P) : QTabWidget(parent_P, name_P)
{
    ...
    connect(general_tab, SIGNAL(action_type_changed(int)), SLOT(set_action_type_slot(int)));
    ...
    for(tab_pos_t i = TAB_FIRST; i < TAB_END; ++i)
...
void Tab_widget::save_current_action_changes()
{
    ...
    {
        Action_data_group *old = static_cast< Action_data_group * >(module->current_action_data());
        Action_data_group *item =
            static_cast< Action_group_tab * >(pages[TAB_GROUP_GENERAL])->get_data(module->current_action_data()->parent(), NULL);
        item->set_conditions(static_cast< Condition_list_tab * >(pages[TAB_CONDITIONS])->get_data(item));
        for(Action_data_group::Iterator it = old->first_child(); it;)
        {
    ...
    else if(current_type == DATA)
    {
        ...
        switch(current_data_type)
        {
            case TYPE_GENERIC:
            {
                Generic_action_data *tmp = new Generic_action_data(module->current_action_data()->parent(), name, comment, NULL, NULL, NULL, enabled);
                tmp->set_triggers(static_cast< Triggers_tab * >(pages[TAB_TRIGGERS])->get_data(tmp));
                tmp->set_conditions(static_cast< Condition_list_tab * >(pages[TAB_CONDITIONS])->get_data(tmp));
                tmp->set_actions(static_cast< Action_list_tab * >(pages[TAB_ACTIONS])->get_data(tmp));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_COMMAND_URL_SHORTCUT:
            {
                Command_url_shortcut_action_data *tmp =
                    new Command_url_shortcut_action_data(module->current_action_data()->parent(), name, comment, enabled);
                tmp->set_trigger(static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->get_data(tmp));
                tmp->set_action(static_cast< Command_url_tab * >(pages[TAB_COMMAND_URL])->get_data(tmp));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_MENUENTRY_SHORTCUT:
            {
                Menuentry_shortcut_action_data *tmp =
                    new Menuentry_shortcut_action_data(module->current_action_data()->parent(), name, comment, enabled);
                tmp->set_trigger(static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->get_data(tmp));
                tmp->set_action(static_cast< Menuentry_tab * >(pages[TAB_MENUENTRY])->get_data(tmp));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_DCOP_SHORTCUT:
            {
                Dcop_shortcut_action_data *tmp = new Dcop_shortcut_action_data(module->current_action_data()->parent(), name, comment, enabled);
                tmp->set_trigger(static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->get_data(tmp));
                tmp->set_action(static_cast< Dcop_tab * >(pages[TAB_DCOP])->get_data(tmp));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_KEYBOARD_INPUT_SHORTCUT:
            {
                Keyboard_input_shortcut_action_data *tmp =
                    new Keyboard_input_shortcut_action_data(module->current_action_data()->parent(), name, comment, enabled);
                tmp->set_trigger(static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->get_data(tmp));
                tmp->set_action(static_cast< Keyboard_input_tab * >(pages[TAB_KEYBOARD_INPUT])->get_data(tmp));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_KEYBOARD_INPUT_GESTURE:
            {
                Keyboard_input_gesture_action_data *tmp =
                    new Keyboard_input_gesture_action_data(module->current_action_data()->parent(), name, comment, enabled);
                tmp->set_triggers(static_cast< Gesture_triggers_tab * >(pages[TAB_GESTURE_TRIGGER])->get_data(tmp));
                tmp->set_action(static_cast< Keyboard_input_tab * >(pages[TAB_KEYBOARD_INPUT])->get_data(tmp));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_ACTIVATE_WINDOW_SHORTCUT:
            {
                Activate_window_shortcut_action_data *tmp =
                    new Activate_window_shortcut_action_data(module->current_action_data()->parent(), name, comment, enabled);
                tmp->set_trigger(static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->get_data(tmp));
                tmp->set_action(new Activate_window_action(tmp, static_cast< Windowdef_list_tab * >(pages[TAB_WINDOW])->get_data()));
                module->set_current_action_data(tmp);
                break;
            }
            case TYPE_END:
...
void Tab_widget::load_current_action()
{
    ...
    {
        static_cast< Action_group_tab * >(pages[TAB_GROUP_GENERAL])->set_data(static_cast< Action_data_group * >(module->current_action_data()));
        static_cast< Condition_list_tab * >(pages[TAB_CONDITIONS])->set_data(module->current_action_data()->conditions());
    }
    else if(current_type == DATA)
    {
        ...
        {
            case TYPE_GENERIC:
                kdDebug(1217) << "loading unknown" << endl;
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(tmp);
                static_cast< Condition_list_tab * >(pages[TAB_CONDITIONS])->set_data(tmp->conditions());
                static_cast< Triggers_tab * >(pages[TAB_TRIGGERS])->set_data(tmp->triggers());
                static_cast< Action_list_tab * >(pages[TAB_ACTIONS])->set_data(tmp->actions());
                break;
            case TYPE_COMMAND_URL_SHORTCUT:
            {
                kdDebug(1217) << "loading command_url_shortcut" << endl;
                Command_url_shortcut_action_data *item = static_cast< Command_url_shortcut_action_data * >(tmp);
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(item);
                static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->set_data(item->trigger());
                static_cast< Command_url_tab * >(pages[TAB_COMMAND_URL])->set_data(item->action());
                break;
            }
            case TYPE_MENUENTRY_SHORTCUT:
            {
                kdDebug(1217) << "loading menuentry_shortcut" << endl;
                Menuentry_shortcut_action_data *item = static_cast< Menuentry_shortcut_action_data * >(tmp);
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(item);
                static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->set_data(item->trigger());
                static_cast< Menuentry_tab * >(pages[TAB_MENUENTRY])->set_data(item->action());
                break;
            }
            case TYPE_DCOP_SHORTCUT:
            {
                kdDebug(1217) << "loading dcop_shortcut" << endl;
                Dcop_shortcut_action_data *item = static_cast< Dcop_shortcut_action_data * >(tmp);
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(item);
                static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->set_data(item->trigger());
                static_cast< Dcop_tab * >(pages[TAB_DCOP])->set_data(item->action());
                break;
            }
            case TYPE_KEYBOARD_INPUT_SHORTCUT:
            {
                kdDebug(1217) << "loading keyboard_input_shortcut" << endl;
                Keyboard_input_shortcut_action_data *item = static_cast< Keyboard_input_shortcut_action_data * >(tmp);
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(item);
                static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->set_data(item->trigger());
                static_cast< Keyboard_input_tab * >(pages[TAB_KEYBOARD_INPUT])->set_data(item->action());
                break;
            }
            case TYPE_KEYBOARD_INPUT_GESTURE:
            {
                kdDebug(1217) << "loading keyboard_input_gesture" << endl;
                Keyboard_input_gesture_action_data *item = static_cast< Keyboard_input_gesture_action_data * >(tmp);
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(item);
                static_cast< Gesture_triggers_tab * >(pages[TAB_GESTURE_TRIGGER])->set_data(item->triggers());
                static_cast< Keyboard_input_tab * >(pages[TAB_KEYBOARD_INPUT])->set_data(item->action());
                break;
            }
            case TYPE_ACTIVATE_WINDOW_SHORTCUT:
            {
                ...
                Activate_window_shortcut_action_data *item = static_cast< Activate_window_shortcut_action_data * >(tmp);
                static_cast< General_tab * >(pages[TAB_GENERAL])->set_data(item);
                static_cast< Shortcut_trigger_tab * >(pages[TAB_SHORTCUT_TRIGGER])->set_data(item->trigger());
                static_cast< Windowdef_list_tab * >(pages[TAB_WINDOW])->set_data(item->action()->window());
                break;
            }
            case TYPE_END:
                assert(false);
...
void Tab_widget::check_action_type()
{
    ...
    { // Action_data_group
        kdDebug(1217) << "setting group" << endl;
    ...
    {
        action_type_t is_type = type(static_cast< Action_data * >(module->current_action_data()));
        kdDebug(1217) << "setting data " << is_type << endl;
