...
Action_list_widget::Action_list_widget(QWidget *parent_P, const char *name_P) : Action_list_widget_ui(parent_P, name_P), selected_item(NULL)
{
    ...
    clear_data();
    connect(actions_listview, SIGNAL(doubleClicked(QListViewItem *, const QPoint &, int)), this, SLOT(modify_pressed()));
...
void Action_list_widget::set_data(const Action_list *data_P)
{
    ...
    actions_listview->clear();
    for(Action_list::Iterator it(*data_P); *it; ++it)
...
Action_list *Action_list_widget::get_data(Action_data *data_P) const
{
    ...
    for(QListViewItem *pos = actions_listview->firstChild(); pos != NULL; pos = pos->nextSibling())
...
void Action_list_widget::new_selected(int type_P)
{
    ...
    if(dlg != NULL)
    {
        ...
        if(action != NULL)
            actions_listview->setSelected(create_listview_item(action, actions_listview, NULL, selected_item, false), true);
...
void Action_list_widget::copy_pressed()
{
    if(!selected_item)
        return;
    actions_listview->setSelected(create_listview_item(selected_item->action(), selected_item->parent() ? NULL : actions_listview,
                                                       selected_item->parent(), selected_item, true),
                                  true);
}
...
Action_list_item *Action_list_widget::create_listview_item(Action *action_P, QListView *parent1_P, QListViewItem *parent2_P, QListViewItem *after_P,
                                                           bool copy_P)
{
...
void Action_list_widget::edit_listview_item(Action_list_item *item_P)
{
    ...
    else if(Keyboard_input_action *action = dynamic_cast< Keyboard_input_action * >(item_P->action()))
        dlg = new Keyboard_input_action_dialog(action);
    else if(Activate_window_action *action = dynamic_cast< Activate_window_action * >(item_P->action()))
...
Dcop_action_dialog::Dcop_action_dialog(Dcop_action *action_P) : KDialogBase(NULL, NULL, true, "", Ok | Cancel), action(NULL) // CHECKME caption
{
