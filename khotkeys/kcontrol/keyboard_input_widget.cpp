...
Keyboard_input_widget::Keyboard_input_widget(QWidget *parent_P, const char *name_P) : Keyboard_input_widget_ui(parent_P, name_P)
{
    clear_data();
    // KHotKeys::Module::changed()
    connect(action_window_radio, SIGNAL(clicked()), module, SLOT(changed()));
    connect(active_window_radio, SIGNAL(clicked()), module, SLOT(changed()));
    connect(specific_window_radio, SIGNAL(clicked()), module, SLOT(changed()));
    connect(keyboard_input_multilineedit, SIGNAL(textChanged()), module, SLOT(changed()));
...
Keyboard_input_action *Keyboard_input_widget::get_data(Action_data *data_P) const
{
    ...
    return new Keyboard_input_action(data_P, keyboard_input_multilineedit->text(), windows, active_window_radio->isChecked());
}
