...
Windowdef_list_widget::Windowdef_list_widget(QWidget *parent_P, const char *name_P)
    : Windowdef_list_widget_ui(parent_P, name_P), autodetect_object(NULL), autodetect_slot(NULL), selected_item(NULL)
{
    ...
    connect(windows_listview, SIGNAL(doubleClicked(QListViewItem *, const QPoint &, int)), this, SLOT(modify_pressed()));
...
void Windowdef_list_widget::set_data(const Windowdef_list *data_P)
{
    ...
    windows_listview->clear();
    for(Windowdef_list::Iterator it(*data_P); *it; ++it)
...
Windowdef_list *Windowdef_list_widget::get_data() const
{
    ...
    for(QListViewItem *pos = windows_listview->firstChild(); pos != NULL; pos = pos->nextSibling())
...
void Windowdef_list_widget::new_selected(int type_P)
{
    ...
    switch(type_P)
    {
        case TYPE_WINDOWDEF_SIMPLE: // Windowdef_simple
            dlg = new Windowdef_simple_dialog(new Windowdef_simple("", "", Windowdef_simple::NOT_IMPORTANT, "", Windowdef_simple::NOT_IMPORTANT, "",
    ...
    if(dlg != NULL)
    {
        ...
        if(window != NULL)
            windows_listview->setSelected(create_listview_item(window, windows_listview, NULL, selected_item, false), true);
...
{
    windows_listview->setSelected(create_listview_item(selected_item->window(), selected_item->parent() ? NULL : windows_listview,
...
Windowdef_list_item *Windowdef_list_widget::create_listview_item(Windowdef *window_P, QListView *parent1_P, QListViewItem *parent2_P,
...
Windowdef_simple_dialog::Windowdef_simple_dialog(Windowdef_simple *window_P, QObject *obj_P, const char *slot_P)
