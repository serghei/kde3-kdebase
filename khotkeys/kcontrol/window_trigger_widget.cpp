...
Window_trigger_widget::Window_trigger_widget(QWidget *parent_P, const char *name_P) : Window_trigger_widget_ui(parent_P, name_P)
{
    clear_data();
    // KHotKeys::Module::changed()
    connect(window_appears_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(window_disappears_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(window_activates_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(window_deactivates_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
}
...
void Window_trigger_widget::set_data(const Window_trigger *trigger_P)
{
    ...
    window_appears_checkbox->setChecked(trigger_P->triggers_on(Window_trigger::WINDOW_APPEARS));
    window_disappears_checkbox->setChecked(trigger_P->triggers_on(Window_trigger::WINDOW_DISAPPEARS));
    window_activates_checkbox->setChecked(trigger_P->triggers_on(Window_trigger::WINDOW_ACTIVATES));
    window_deactivates_checkbox->setChecked(trigger_P->triggers_on(Window_trigger::WINDOW_DEACTIVATES));
}
