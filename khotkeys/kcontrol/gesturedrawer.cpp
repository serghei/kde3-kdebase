...
GestureDrawer::GestureDrawer(QWidget *parent, const char *name) : QFrame(parent, name), _data(QString::null)
{
...
void GestureDrawer::paintEvent(QPaintEvent *ev)
{
    ...
    for(Q_UINT32 index = 1; index < _data.length(); ++index)
    {
        ...
        if(index == 1)
        {
            ...
            p.fillRect(startPoint.x() - 2, startPoint.y() - 2, 4, 4, QBrush(black));
        }
...
void GestureDrawer::drawArrowHead(QPoint &start, QPoint &end, QPainter &p)
{
