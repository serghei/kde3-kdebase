...
Action_data_group *khotkeys_get_menu_root(Action_data_group *data_P)
{
    ...
    return new Action_data_group(data_P, i18n(MENU_EDITOR_ENTRIES_GROUP_NAME), i18n("These entries were created using Menu Editor."),
                                 new Condition_list("", NULL), // CHECKME tenhle condition list
                                 Action_data_group::SYSTEM_MENUENTRIES, true);
}

Menuentry_shortcut_action_data *khotkeys_get_menu_entry_internal(Action_data_group *data_P, const QString &entry_P)
{
...
KService::Ptr khotkeys_find_menu_entry_internal(const Action_data_group *data_P, const QString &shortcut_P)
{
    ...
    for(Action_data_group::Iterator it = data_P->first_child(); it; ++it)
    {
        ...
        if(Menuentry_shortcut_action_data *entry = dynamic_cast< Menuentry_shortcut_action_data * >(*it))
        {
            if(entry->trigger() && entry->trigger()->shortcut().toString() == shortcut_P)
            {
                if(entry->action())
...
QString khotkeys_get_menu_entry_shortcut(const QString &entry_P)
{
    Settings settings;
    settings.read_settings(true);
    Menuentry_shortcut_action_data *entry = khotkeys_get_menu_entry_internal(settings.actions, entry_P);
    if(entry == NULL)
    {
...
bool khotkeys_menu_entry_moved(const QString &new_P, const QString &old_P)
{
    Settings settings;
    settings.read_settings(true);
    Menuentry_shortcut_action_data *entry = khotkeys_get_menu_entry_internal(settings.actions, old_P);
    if(entry == NULL)
    {
        delete settings.actions;
        return false;
    ...
    Menuentry_shortcut_action_data *new_entry = new Menuentry_shortcut_action_data(parent, new_name, entry->comment(), entry->enabled(true));
...
QString khotkeys_change_menu_entry_shortcut(const QString &entry_P, const QString &shortcut_P)
{
    Settings settings;
    settings.read_settings(true);
    Menuentry_shortcut_action_data *entry = khotkeys_get_menu_entry_internal(settings.actions, entry_P);
    ...
    else
    {
        ...
        Menuentry_shortcut_action_data *entry_tmp = new Menuentry_shortcut_action_data(NULL, entry->name(), entry->comment(), entry->enabled(false));
