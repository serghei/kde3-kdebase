...
Triggers_tab::Triggers_tab(QWidget *parent_P, const char *name_P) : Triggers_tab_ui(parent_P, name_P), selected_item(NULL)
{
    ...
    connect(triggers_listview, SIGNAL(doubleClicked(QListViewItem *, const QPoint &, int)), this, SLOT(modify_pressed()));
...
void Triggers_tab::set_data(const Trigger_list *data_P)
{
    ...
    triggers_listview->clear();
    for(Trigger_list::Iterator it(*data_P); *it; ++it)
...
Trigger_list *Triggers_tab::get_data(Action_data *data_P) const
{
    ...
    for(QListViewItem *pos = triggers_listview->firstChild(); pos != NULL; pos = pos->nextSibling())
...
void Triggers_tab::new_selected(int type_P)
{
    ...
    switch(type_P)
    {
        case TYPE_SHORTCUT_TRIGGER:                                                     // Shortcut_trigger
            dlg = new Shortcut_trigger_dialog(new Shortcut_trigger(NULL, KShortcut())); // CHECKME NULL ?
            break;
        case TYPE_GESTURE_TRIGGER:                                                      // Gesture trigger
            dlg = new Gesture_trigger_dialog(new Gesture_trigger(NULL, QString::null)); // CHECKME NULL ?
            break;
        case TYPE_WINDOW_TRIGGER:                                                                 // Window trigger
            dlg = new Window_trigger_dialog(new Window_trigger(NULL, new Windowdef_list(""), 0)); // CHECKME NULL ?
            break;
    ...
    if(dlg != NULL)
    {
        ...
        if(trg != NULL)
            triggers_listview->setSelected(create_listview_item(trg, triggers_listview, selected_item, false), true);
        delete dlg;
    }
}
...
Trigger_list_item *Triggers_tab::create_listview_item(Trigger *trigger_P, QListView *parent_P, QListViewItem *after_P, bool copy_P)
{
...
Shortcut_trigger_widget::Shortcut_trigger_widget(QWidget *parent_P, const char *) : QWidget(parent_P)
{
    ...
    lay->addStretch();
    clear_data();
    connect(bt, SIGNAL(capturedShortcut(const KShortcut &)), this, SLOT(capturedShortcut(const KShortcut &)));
}
...
{
    return !bt->shortcut().isNull() ? new Shortcut_trigger(data_P, bt->shortcut()) : NULL;
}

// Shortcut_trigger_dialog

Shortcut_trigger_dialog::Shortcut_trigger_dialog(Shortcut_trigger *trigger_P)
    : KDialogBase(NULL, NULL, true, "", Ok | Cancel)
    , // CHECKME caption
    trigger(NULL)
{
...
Gesture_trigger_dialog::Gesture_trigger_dialog(Gesture_trigger *trigger_P)
    ...
    , _page(NULL)
{
    _page = new GestureRecordPage(_trigger->gesturecode(), this, "GestureRecordPage");

    connect(_page, SIGNAL(gestureRecorded(bool)), this, SLOT(enableButtonOK(bool)));
