...
GestureRecordPage::GestureRecordPage(const QString &gesture, QWidget *parent, const char *name)
    : QVBox(parent, name), _recorder(NULL), _resetButton(NULL), _tryOne(NULL), _tryTwo(NULL), _tryThree(NULL), _gest(QString::null), _tryCount(1)
{
    QString message;
    ...
    label->setAlignment(QLabel::AlignLeft | QLabel::WordBreak | QLabel::AlignVCenter);

    _recorder = new GestureRecorder(this, "recorder");
    _recorder->setMinimumHeight(150);
    setStretchFactor(_recorder, 1);
    connect(_recorder, SIGNAL(recorded(const QString &)), this, SLOT(slotRecorded(const QString &)));
    ...
    connect(_resetButton, SIGNAL(clicked()), this, SLOT(slotResetClicked()));
    ...
    switch(_tryCount)
    {
        ...
        default:
            KMessageBox::information(
                this, i18n("You have already completed the three required drawings. Either press 'Ok' to save or 'Reset' to try again."));
    }
}
