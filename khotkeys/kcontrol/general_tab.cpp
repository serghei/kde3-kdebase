...
General_tab::General_tab(QWidget *parent_P, const char *name_P) : General_tab_ui(parent_P, name_P)
{
    for(Tab_widget::action_type_t i = Tab_widget::TYPE_FIRST; i < Tab_widget::TYPE_END; ++i)
        switch(i)
        {
            ...
            case Tab_widget::TYPE_COMMAND_URL_SHORTCUT:
                action_type_combo->insertItem(i18n("Keyboard Shortcut -> Command/URL (simple)"), i);
                break;
            case Tab_widget::TYPE_MENUENTRY_SHORTCUT:
                action_type_combo->insertItem(i18n("K-Menu Entry (simple)"), i);
                break;
            case Tab_widget::TYPE_DCOP_SHORTCUT:
                action_type_combo->insertItem(i18n("Keyboard Shortcut -> DCOP Call (simple)"), i);
                break;
            case Tab_widget::TYPE_KEYBOARD_INPUT_SHORTCUT:
                action_type_combo->insertItem(i18n("Keyboard Shortcut -> Keyboard Input (simple)"), i);
                break;
            case Tab_widget::TYPE_KEYBOARD_INPUT_GESTURE:
                action_type_combo->insertItem(i18n("Gesture -> Keyboard Input (simple)"), i);
                break;
            case Tab_widget::TYPE_ACTIVATE_WINDOW_SHORTCUT:
                action_type_combo->insertItem(i18n("Keyboard Shortcut -> Activate Window (simple)"), i);
                break;
    ...
    connect(action_name_lineedit, SIGNAL(textChanged(const QString &)), module, SLOT(changed()));
    connect(disable_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(comment_multilineedit, SIGNAL(textChanged()), module, SLOT(changed()));
    connect(action_type_combo, SIGNAL(activated(int)), module, SLOT(changed()));
}

void General_tab::clear_data()
{
    disconnect(action_name_lineedit, SIGNAL(textChanged(const QString &)), this, SLOT(action_name_changed(const QString &)));
    disconnect(action_type_combo, SIGNAL(activated(int)), this, SIGNAL(action_type_changed(int))); // CHECKME neodpoji to sloty od nej ?
...
void General_tab::set_data(const Action_data *data_P)
{
    ...
    connect(action_name_lineedit, SIGNAL(textChanged(const QString &)), SLOT(action_name_changed(const QString &)));
    connect(action_type_combo, SIGNAL(activated(int)), SIGNAL(action_type_changed(int)));
}
