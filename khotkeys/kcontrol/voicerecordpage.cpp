...
VoiceRecordPage::VoiceRecordPage(const QString &voiceid_P, QWidget *parent, const char *name) : QVBox(parent, name), _original_voiceId(voiceid_P)
{
    ...
    _label->setAlignment(QLabel::AlignLeft | QLabel::WordBreak | QLabel::AlignVCenter);
...
void VoiceRecordPage::slotChanged()
{
    ...
    emit voiceRecorded(voiceCodeOK && (((_recorder1->state() == VoiceRecorder::sModified || _recorder2->state() == VoiceRecorder::sModified
