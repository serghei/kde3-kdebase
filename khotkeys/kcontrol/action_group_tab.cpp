...
Action_group_tab::Action_group_tab(QWidget *parent_P, const char *name_P) : Action_group_tab_ui(parent_P, name_P)
{
    clear_data();
...
{
    disconnect(action_name_lineedit, SIGNAL(textChanged(const QString &)), this, SLOT(action_group_name_changed(const QString &)));
...
void Action_group_tab::set_data(const Action_data_group *data_P)
{
    ...
    connect(action_name_lineedit, SIGNAL(textChanged(const QString &)), this, SLOT(action_group_name_changed(const QString &)));
    system_group = data_P->system_group();
}

Action_data_group *Action_group_tab::get_data(Action_data_group *parent_P, Condition_list *conditions_P) const
{
    QString name = action_name_lineedit->text();
    return new Action_data_group(parent_P, name, comment_multilineedit->text(), conditions_P, system_group, !disable_checkbox->isChecked());
}
