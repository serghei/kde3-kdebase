...
Windowdef_simple_widget::Windowdef_simple_widget(QWidget *parent_P, const char *name_P) : Windowdef_simple_widget_ui(parent_P, name_P)
{
    ...
    connect(window_title_combo, SIGNAL(activated(int)), module, SLOT(changed()));
    connect(window_title_lineedit, SIGNAL(textChanged(const QString &)), module, SLOT(changed()));
    connect(window_class_combo, SIGNAL(activated(int)), module, SLOT(changed()));
    connect(window_class_lineedit, SIGNAL(textChanged(const QString &)), module, SLOT(changed()));
    connect(window_role_combo, SIGNAL(activated(int)), module, SLOT(changed()));
    connect(window_role_lineedit, SIGNAL(textChanged(const QString &)), module, SLOT(changed()));
    connect(type_normal_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(type_dialog_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(type_dock_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
    connect(type_desktop_checkbox, SIGNAL(clicked()), module, SLOT(changed()));
...
void Windowdef_simple_widget::set_data(const Windowdef_simple *data_P)
{
    ...
    type_desktop_checkbox->setChecked(data_P->type_match(Windowdef_simple::WINDOW_TYPE_DESKTOP));
}
...
Windowdef_simple *Windowdef_simple_widget::get_data() const
{
    return new Windowdef_simple(comment_lineedit->text(), window_title_lineedit->text(),
                                static_cast< Windowdef_simple::substr_type_t >(window_title_combo->currentItem()), window_class_lineedit->text(),
                                static_cast< Windowdef_simple::substr_type_t >(window_class_combo->currentItem()), window_role_lineedit->text(),
