...
Actions_listview_widget::Actions_listview_widget(QWidget *parent_P, const char *name_P)
    : Actions_listview_widget_ui(parent_P, name_P), recent_item(NULL), saved_current_item(NULL)
{
    ...
    connect(actions_listview, SIGNAL(current_changed(QListViewItem *)), SLOT(current_changed(QListViewItem *)));
...
void Actions_listview_widget::build_up_recursively(Action_data_group *parent_P, Action_listview_item *item_parent_P)
{
...
Action_listview_item *Actions_listview_widget::create_item(QListViewItem *parent_P, QListViewItem *after_P, Action_data_base *data_P)
{
    if(parent_P != NULL)
