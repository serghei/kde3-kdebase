...
Action_data_base::Action_data_base(Action_data_group *parent_P, const QString &name_P, const QString &comment_P, Condition_list *conditions_P,
                                   bool enabled_P)
    : _parent(parent_P), _conditions(conditions_P), _name(name_P), _comment(comment_P), _enabled(enabled_P)
{
    if(parent())
...
Action_data_base::Action_data_base(KConfig &cfg_P, Action_data_group *parent_P) : _parent(parent_P)
{
...
Action_data_base *Action_data_base::create_cfg_read(KConfig &cfg_P, Action_data_group *parent_P)
{
    ...
    if(type == "ACTION_DATA_GROUP")
    {
        ...
        {
            for(Action_data_group::Iterator it = parent_P->first_child(); it; ++it)
            {
...
Action_data_group::Action_data_group(KConfig &cfg_P, Action_data_group *parent_P) : Action_data_base(cfg_P, parent_P)
{
...
{
    for(Action_data_group::Iterator it = first_child(); it; ++it)
        (*it)->update_triggers();
}

// Action_data

Action_data::Action_data(KConfig &cfg_P, Action_data_group *parent_P) : Action_data_base(cfg_P, parent_P)
{
    QString save_cfg_group = cfg_P.group();
...
{
    for(Action_list::Iterator it(*_actions); it; ++it)
        it.current()->execute();
...
{
    for(Trigger_list::Iterator it = *triggers_P; it; ++it)
        _triggers->append(*it);
...
void Action_data::add_actions(Action_list *actions_P, Action *after_P)
{
    ...
    for(Action_list::Iterator it = *actions_P; it; ++it)
...
void Action_data::update_triggers()
{
    ...
    for(Trigger_list::Iterator it = (*triggers()); it; ++it)
...
Command_url_shortcut_action_data::Command_url_shortcut_action_data(Action_data_group *parent_P, const QString &name_P, const QString &comment_P,
...
template <> KDE_EXPORT void Simple_action_data< Shortcut_trigger, Command_url_action >::cfg_write(KConfig &cfg_P) const
{
    base::cfg_write(cfg_P);
...
Menuentry_shortcut_action_data::Menuentry_shortcut_action_data(Action_data_group *parent_P, const QString &name_P, const QString &comment_P,
...
template <> KDE_EXPORT void Simple_action_data< Shortcut_trigger, Menuentry_action >::cfg_write(KConfig &cfg_P) const
{
    base::cfg_write(cfg_P);
...
template <> KDE_EXPORT void Simple_action_data< Shortcut_trigger, Dcop_action >::cfg_write(KConfig &cfg_P) const
{
    base::cfg_write(cfg_P);
...
template <> KDE_EXPORT void Simple_action_data< Shortcut_trigger, Keyboard_input_action >::cfg_write(KConfig &cfg_P) const
{
    base::cfg_write(cfg_P);
...
template <> KDE_EXPORT void Simple_action_data< Shortcut_trigger, Activate_window_action >::cfg_write(KConfig &cfg_P) const
{
    base::cfg_write(cfg_P);
