...
Action_list::Action_list(KConfig &cfg_P, Action_data *data_P) : QPtrList< Action >()
{
    setAutoDelete(true);
...
Command_url_action::Command_url_action(KConfig &cfg_P, Action_data *data_P) : Action(cfg_P, data_P)
{
...
void Command_url_action::execute()
{
    ...
    switch(uri.uriType())
    {
        ...
        case KURIFilterData::SHELL:
        {
            if(!kapp->authorize("shell_access"))
                return;
            if(!KRun::runCommand(cmd + (uri.hasArgsAndOptions() ? uri.argsAndOptions() : ""), cmd, uri.iconName()))
            {
                // CHECKME ?
            }
            break;
...
Dcop_action::Dcop_action(KConfig &cfg_P, Action_data *data_P) : Action(cfg_P, data_P)
{
...
{
    return i18n("DCOP : ") + remote_application() + "::" + remote_object() + "::" + called_function();
}

Action *Dcop_action::copy(Action_data *data_P) const
{
    return new Dcop_action(data_P, remote_application(), remote_object(), called_function(), arguments());
}

// Keyboard_input_action

Keyboard_input_action::Keyboard_input_action(KConfig &cfg_P, Action_data *data_P) : Action(cfg_P, data_P)
{
...
{
    return new Keyboard_input_action(data_P, input(), dest_window() ? dest_window()->copy() : NULL, _active_window);
}

// Activate_window_action

Activate_window_action::Activate_window_action(KConfig &cfg_P, Action_data *data_P) : Action(cfg_P, data_P)
{
    QString save_cfg_group = cfg_P.group();
