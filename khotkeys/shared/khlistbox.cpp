...
KHListBox::KHListBox(QWidget *parent_P, const char *name_P)
    : QListBox(parent_P, name_P), saved_current_item(NULL), in_clear(false), force_select(false)
{
    connect(this, SIGNAL(selectionChanged(QListBoxItem *)), SLOT(slot_selection_changed(QListBoxItem *)));
    connect(this, SIGNAL(currentChanged(QListBoxItem *)), SLOT(slot_current_changed(QListBoxItem *)));
    // CHECKME grrr
