...
Gesture::Gesture(bool /*enabled_P*/, QObject *parent_P) : _enabled(false), recording(false), button(0), exclude(NULL)
{
...
{
    if(_enabled && handlers.count() > 0 && (exclude == NULL || !exclude->match(Window_data(windows_handler->active_window()))))
    {
...
void Gesture::register_handler(QObject *receiver_P, const char *slot_P)
{
    ...
    connect(this, SIGNAL(handle_gesture(const QString &, WId)), receiver_P, slot_P);
    if(handlers.count() == 1)
        update_grab();
}
...
void Gesture::unregister_handler(QObject *receiver_P, const char *slot_P)
{
    ...
    disconnect(this, SIGNAL(handle_gesture(const QString &, WId)), receiver_P, slot_P);
    if(handlers.count() == 0)
        update_grab();
}
...
bool Gesture::x11Event(XEvent *ev_P)
{
    ...
    else if(ev_P->type == ButtonRelease && ev_P->xbutton.button == button && recording)
    {
        recording = false;
        nostroke_timer.stop();
        stroke.record(ev_P->xbutton.x, ev_P->xbutton.y);
    ...
    { // ignore small initial movement
        if(nostroke_timer.isActive() && abs(start_x - ev_P->xmotion.x_root) < 10 && abs(start_y - ev_P->xmotion.y_root) < 10)
            return true;
        nostroke_timer.stop();
...
#undef XScrL
        for(int i = 0; i < 8; ++i)
            XGrabButton(qt_xdisplay(), button, mods[i], qt_xrootwin(), False, ButtonPressMask | ButtonReleaseMask | mask[button], GrabModeAsync,
...
bool Stroke::record(int x, int y)
{
    ...
    else
    {
        ...
        if(abs(delx) > abs(dely)) // step by the greatest delta direction
        {
            ...
            for(int ix = points[point_count].x; (delx > 0) ? (ix < x) : (ix > x); (delx > 0) ? ++ix : --ix)
            {
        ...
        { // same thing, but for dely larger than delx case...
            ...
            // go from the last point to the current, whatever direction it may be
            for(int iy = points[point_count].y; (dely > 0) ? (iy < y) : (iy > y); (dely > 0) ? ++iy : --iy)
            {
                // step the other axis by the correct increment
                if(delx < 0)
...
char *Stroke::translate(int min_bin_points_percentage_P, int scale_ratio_P, int min_points_P)
{
    ...
    for(int pos = 0; pos <= point_count; ++pos)
    {
        ...
        {   // we are moving to a new bin -- consider adding to the sequence
            ...
            if(bin_count >= (min_bin_points_percentage_P * point_count / 100) || sequence_count == 0)
            {
