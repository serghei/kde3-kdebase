...
Windows::Windows(bool enable_signal_P, QObject *parent_P)
    : QObject(parent_P), signals_enabled(enable_signal_P), kwin_module(new KWinModule(this)), _action_window(0)
{
    ...
    if(signals_enabled)
    {
        ...
        connect(kwin_module, SIGNAL(activeWindowChanged(WId)), SLOT(active_window_changed_slot(WId)));
    }
}
...
{
    for(QValueList< WId >::ConstIterator it = kwin_module->windows().begin(); it != kwin_module->windows().end(); ++it)
    {
        Window_data tmp(*it);
...
WId Windows::window_at_position(int x, int y)
{
    ...
    for(int i = 0; i < 10; ++i)
    {
        ...
        if(!XTranslateCoordinates(qt_xdisplay(), parent, parent, x, y, &destx, &desty, &child) || child == None)
            return 0;
...
Window_data::Window_data(WId id_P) : type(NET::Unknown)
{
...
Windowdef_list::Windowdef_list(KConfig &cfg_P) : QPtrList< Windowdef >()
{
    setAutoDelete(true);
...
Windowdef_simple::Windowdef_simple(const QString &comment_P, const QString &title_P, substr_type_t title_type_P, const QString &wclass_P,
...
Windowdef_simple::Windowdef_simple(KConfig &cfg_P) : Windowdef(cfg_P)
{
