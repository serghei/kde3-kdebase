...
KHListView::KHListView(QWidget *parent_P, const char *name_P)
    : KListView(parent_P, name_P), saved_current_item(NULL), in_clear(false), ignore(false), force_select(false)
{
    connect(this, SIGNAL(selectionChanged(QListViewItem *)), SLOT(slot_selection_changed(QListViewItem *)));
    connect(this, SIGNAL(currentChanged(QListViewItem *)), SLOT(slot_current_changed(QListViewItem *)));
    // CHECKME grrr
    connect(this, SIGNAL(selectionChanged()), SLOT(slot_selection_changed()));
    connect(&insert_select_timer, SIGNAL(timeout()), SLOT(slot_insert_select()));
}
