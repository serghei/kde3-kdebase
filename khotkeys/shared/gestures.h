...
public:
    ...
    char *translate(int min_bin_points_percentage_P = MIN_BIN_POINTS_PERCENTAGE, int scale_ratio_P = SCALE_RATIO,
...
class KDE_EXPORT Gesture : public QWidget // not QObject because of x11EventFilter()
{
    Q_OBJECT
public:
...
class DeleteObject : public QObject {
    Q_OBJECT
public:
    DeleteObject(QWidget *widget_P, QObject *parent_P) : QObject(parent_P), widget(widget_P)
    {
    }
