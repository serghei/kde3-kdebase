...
Condition::Condition(Condition_list_base *parent_P) : _parent(parent_P)
{
    if(_parent)
        _parent->append(this);
}

Condition::Condition(KConfig &, Condition_list_base *parent_P) : _parent(parent_P)
{
    if(_parent)
        _parent->append(this);
}
...
void Condition::debug_list(const QPtrList< Condition > &list_P, int depth_P)
{
    ...
    for(i = 0; i < depth_P; ++i)
        tmp[i] = ' ';
    tmp[i] = '\0';
    for(QPtrListIterator< Condition > it(list_P); it; ++it)
        (*it)->debug(depth_P + 1);
}
#endif
...
Condition_list_base::Condition_list_base(KConfig &cfg_P, Condition_list_base *parent_P) : Condition(parent_P)
{
...
Condition_list::Condition_list(KConfig &cfg_P, Action_data_base *data_P) : Condition_list_base(cfg_P, NULL), data(data_P)
{
...
Active_window_condition::Active_window_condition(KConfig &cfg_P, Condition_list_base *parent_P) : Condition(cfg_P, parent_P)
{
    QString save_cfg_group = cfg_P.group();
...
Existing_window_condition::Existing_window_condition(KConfig &cfg_P, Condition_list_base *parent_P) : Condition(cfg_P, parent_P)
{
    QString save_cfg_group = cfg_P.group();
    cfg_P.setGroup(save_cfg_group + "Window");
    _window = new Windowdef_list(cfg_P);
...
Not_condition::Not_condition(KConfig &cfg_P, Condition_list_base *parent_P) : Condition_list_base(cfg_P, parent_P)
{
...
And_condition::And_condition(KConfig &cfg_P, Condition_list_base *parent_P) : Condition_list_base(cfg_P, parent_P)
{
    // CHECKME kontrola poctu ?
}
...
Or_condition::Or_condition(KConfig &cfg_P, Condition_list_base *parent_P) : Condition_list_base(cfg_P, parent_P)
{
    // CHECKME kontrola poctu ?
}
