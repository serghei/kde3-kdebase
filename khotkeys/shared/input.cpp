...
Kbd::Kbd(bool grabbing_enabled_P, QObject *parent_P) : QObject(parent_P)
{
...
void Kbd::activate_receiver(Kbd_receiver *receiver_P)
{
    ...
    if(rcv.active)
        return;
    rcv.active = true;
    for(QValueList< KShortcut >::ConstIterator it(rcv.shortcuts.begin()); it != rcv.shortcuts.end(); ++it)
        grab_shortcut(*it);
}
...
void Kbd::grab_shortcut(const KShortcut &shortcut_P)
{
    ...
    else
    {
        ...
        kga->insert(name, name, QString::null, shortcut_P, shortcut_P, this, SLOT(key_slot(QString)));
...
void Kbd::key_slot(QString key_P)
{
    ...
    if(!grabs.contains(shortcut))
        return;
    for(QMap< Kbd_receiver *, Receiver_data >::ConstIterator it = receivers.begin(); it != receivers.end(); ++it)
        if((*it).shortcuts.contains(shortcut) && (*it).active && it.key()->handle_key(shortcut))
            return;
}
...
static bool xtest()
{
    ...
    xtest_available = (XTestQueryExtension(qt_xdisplay(), &dummy1, &dummy2, &dummy3, &dummy4) == True);
    return xtest_available;
}
#endif
