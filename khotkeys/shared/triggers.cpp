...
Shortcut_trigger::Shortcut_trigger(Action_data *data_P, const KShortcut &shortcut_P) : Trigger(data_P), _shortcut(shortcut_P)
{
    keyboard_handler->insert_item(shortcut(), this);
}

Shortcut_trigger::Shortcut_trigger(KConfig &cfg_P, Action_data *data_P) : Trigger(cfg_P, data_P), _shortcut(cfg_P.readEntry("Key", 0))
{
    keyboard_handler->insert_item(shortcut(), this);
}
...
Window_trigger::Window_trigger(KConfig &cfg_P, Action_data *data_P) : Trigger(cfg_P, data_P), active(false)
{
...
void Window_trigger::active_window_changed(WId window_P)
{
    ...
    /*    bool matches = windows()->match( Window_data( window_P ));
        existing_windows[ window_P ] = matches;*/
    bool matches = existing_windows.contains(window_P) ? existing_windows[window_P] : false;
...
Gesture_trigger::Gesture_trigger(Action_data *data_P, const QString &gesturecode_P) : Trigger(data_P), _gesturecode(gesturecode_P)
{
}

Gesture_trigger::Gesture_trigger(KConfig &cfg_P, Action_data *data_P) : Trigger(cfg_P, data_P)
{
...
Voice_trigger::Voice_trigger(KConfig &cfg_P, Action_data *data_P) : Trigger(cfg_P, data_P)
{
