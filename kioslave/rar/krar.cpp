...
KRar::KRar(const QString &filename) : KArchive(0L)
{
    m_filename = filename;
    m_dirs = 0;
    m_files = 0;
...
bool KRar::openArchive(int mode)
{
    ...
    if(a < 7)
        return false;
    if(!memcmp(marker_block, "\x52\x61\x72\x21\x1a\x07\x00", 8))
        return false;
        ...
        if(header[2] == '\x74') // File header
        {
            ...
            */
            ...
            else
            {
                ...
                // link
                // mode |= S_IFLNK;
                // file = new KArchiveFile( this, filename.right( filename.length() - filename.findRev( '/' ) - 1 ), mode, time->toTime_t(),
                // rootDir()->user(), rootDir()->group(), QString::null, dev->at(), fileSize );
                //}else{
                mode |= S_IFREG;
                file = new KArchiveFile(this, filename.right(filename.length() - filename.findRev('/') - 1), mode, time->toTime_t(),
                                        rootDir()->user(), rootDir()->group(), QString::null, dev->at(), fileSize);
