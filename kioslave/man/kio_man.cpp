 ...
 */
static void stripExtension(QString *name)
{
    int pos = name->length();
...
MANProtocol::MANProtocol(const QCString &pool_socket, const QCString &app_socket) : QObject(), SlaveBase("man", pool_socket, app_socket)
{
    assert(!_self);
    _self = this;
...
QMap< QString, QString > MANProtocol::buildIndexMap(const QString &section)
{
    ...
    for(QStringList::ConstIterator it_dir = man_dirs.begin(); it_dir != man_dirs.end(); ++it_dir)
    {
        if(QFile::exists(*it_dir))
        {
            ...
            if(it_name == names.end())
            {
                ...
                connect(&proc, SIGNAL(receivedStdout(KProcess *, char *, int)), SLOT(slotGetStdOutput(KProcess *, char *, int)));
...
QStringList MANProtocol::manDirectories()
{
    ...
    for(QStringList::ConstIterator it_dir = m_manpath.begin(); it_dir != m_manpath.end(); it_dir++)
    {
        // Translated pages in "<mandir>/<lang>" if the directory
        // exists
        QStringList languages = KGlobal::locale()->languageList();

        for(QStringList::ConstIterator it_lang = languages.begin(); it_lang != languages.end(); it_lang++)
        {
            if(!(*it_lang).isEmpty() && (*it_lang) != QString("C"))
            {
                ...
                if(::stat(QFile::encodeName(dir), &sbuf) == 0 && S_ISDIR(sbuf.st_mode))
                {
                    const QString p = QDir(dir).canonicalPath();
                    if(!man_dirs.contains(p))
                        man_dirs += p;
                }
            }
        }
...
QStringList MANProtocol::findPages(const QString &_section, const QString &title, bool full_path)
{
    ...
    //
    for(QStringList::ConstIterator it_sect = sect_list.begin(); it_sect != sect_list.end(); it_sect++)
    {
...
void MANProtocol::get(const KURL &url)
{
    ...
    else if(foundPages.count() > 1)
    {
        ...
        if((foundPages.count() == 2) && (((foundPages[0] + ".gz") == foundPages[1]) || (foundPages[0] == (foundPages[1] + ".gz"))))
            pageFound = true;
        else
...
char *MANProtocol::readManPage(const char *_filename)
{
    ...
    if(filename.contains("sman", false)) // file_mimetype == "text/html" || )
    {
        ...
        QApplication::connect(&proc, SIGNAL(receivedStdout(KProcess *, char *, int)), this, SLOT(slotGetStdOutput(KProcess *, char *, int)));
...
QStringList MANProtocol::buildSectionList(const QStringList &dirs) const
{
    ...
    {
        for(QStringList::ConstIterator dir = dirs.begin(); dir != dirs.end(); ++dir)
        {
            QDir d((*dir) + "/man" + (*it));
            if(d.exists())
            {
...
void MANProtocol::showMainIndex()
{
    ...
    for(it = sections.begin(); it != sections.end(); ++it)
        os << "<tr><td><a href=\"man:(" << *it << ")\" accesskey=\"" << (((*it).length() == 1) ? (*it) : (*it).right(1)) << "\">" << i18n("Section ")
...
void MANProtocol::constructPath(QStringList &constr_path, QStringList constr_catmanpath)
{
    ...
    if(mc.open(IO_ReadOnly))
    {
        ...
        while(!is.eof())
        {
            ...
            else if(manpath_map_regex.search(line, 0) == 0)
            {
                ...
                const QStringList mapping = QStringList::split(space_regex, line);
    ...
    // Default paths
    static const char *manpaths[] = {"/usr/X11/man",     "/usr/X11R6/man", "/usr/man",          "/usr/local/man",     "/usr/exp/man",
                                     "/usr/openwin/man", "/usr/dt/man",    "/opt/freetool/man", "/opt/local/man",     "/usr/tex/man",
                                     "/usr/www/man",     "/usr/lang/man",  "/usr/gnu/man",      "/usr/share/man",     "/usr/motif/man",
                                     "/usr/titools/man", "/usr/sunpc/man", "/usr/ncd/man",      "/usr/newsprint/man", NULL};
    ...
    if(::getenv("PATH"))
    {
        const QStringList path = QStringList::split(":", QString::fromLocal8Bit(::getenv("PATH")));
...
void MANProtocol::checkManPaths()
{
    ...
    if(manpath_env.isEmpty() || manpath_env[0] == ':' || manpath_env[manpath_env.length() - 1] == ':' || manpath_env.contains("::"))
    {
    ...
    for(QStringList::const_iterator it = path_list_env.begin(); it != path_list_env.end(); ++it)
    {
        struct stat sbuf;
        ...
        else
        {
            ...
            for(QStringList::Iterator it2 = constr_path.begin(); it2 != constr_path.end(); it2++)
            {
                dir = (*it2);
...
int compare_man_index(const void *s1, const void *s2)
{
    ...
    {
        i = qstrnicmp(m1->manpage_begin, m2->manpage_begin, m2->manpage_len);
        if(!i)
            return 1;
        return i;
    }
    ...
    if(m1->manpage_len < m2->manpage_len)
    {
        i = qstrnicmp(m1->manpage_begin, m2->manpage_begin, m1->manpage_len);
        if(!i)
            return -1;
        return i;
    }

    return qstrnicmp(m1->manpage_begin, m2->manpage_begin, m1->manpage_len);
}
...
void MANProtocol::showIndex(const QString &section)
{
    ...
    for(QMap< QString, QString >::ConstIterator it = pagemap.begin(); it != pagemap.end(); ++it)
    {
        os << "<tr><td><a href=\"man:" << it.data() << "\">\n"
           << it.key() << "</a></td><td>&nbsp;</td><td> " << (indexmap.contains(it.key()) ? indexmap[it.key()] : "") << "</td></tr>" << endl;
    }
    ...
    if(indexlist[0]->manpage_len > 0)
    {
        ...
        const QString appendixstr = QString(" [<a href=\"#%1\" accesskey=\"%2\">%3</a>]\n").arg(firstchar).arg(firstchar).arg(firstchar);
        indexLine.append(appendixstr);
    }
    os << "<tr><td class=\"secidxnextletter\""
    ...
    for(int i = 0; i < listlen; i++)
    {
        ...
        if(last_index->manpage_len == manindex->manpage_len && !qstrncmp(last_index->manpage_begin, manindex->manpage_begin, manindex->manpage_len))
        {
            continue;
        }
        ...
        os << "<tr><td><a href=\"man:" << manindex->manpath << "\">\n";

        ((char *)manindex->manpage_begin)[manindex->manpage_len] = '\0';
        os << manindex->manpage_begin << "</a></td><td>&nbsp;</td><td> "
           << (indexmap.contains(manindex->manpage_begin) ? indexmap[manindex->manpage_begin] : "") << "</td></tr>" << endl;
        last_index = manindex;
    }
    ...
    for(QManIndexListIterator mit(manpages); mit.current(); ++mit)
    {
...
void MANProtocol::getProgramPath()
{
    ...
    outputError(
        i18n("Could not find the sgml2roff program on your system. Please install it, if necessary, and extend the search path by adjusting the "
             "environment variable PATH before starting KDE."));
    finished();
    exit();
}
