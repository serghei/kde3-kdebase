...
NNTPProtocol::NNTPProtocol(const QCString &pool, const QCString &app, bool isSSL)
    : TCPSlaveBase((isSSL ? NNTPS_PORT : NNTP_PORT), (isSSL ? "nntps" : "nntp"), pool, app, isSSL)
{
...
void NNTPProtocol::setHost(const QString &host, int port, const QString &user, const QString &pass)
{
    DBG << "setHost: " << (!user.isEmpty() ? (user + "@") : QString("")) << host << ":" << ((port == 0) ? m_iDefaultPort : port) << endl;

    if(isConnectionValid() && (mHost != host || m_iPort != port || mUser != user || mPass != pass))
        nntp_close();
...
void NNTPProtocol::fillUDSEntry(UDSEntry &entry, const QString &name, long size, bool posting_allowed, bool is_article)
{
    ...
    atom.m_long = (is_article) ? (S_IRUSR | S_IRGRP | S_IROTH) : (S_IRUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH | posting);
    atom.m_str = QString::null;
    entry.append(atom);
...
{
    ERR << "Unexpected response to " << command << " command: (" << res_code << ") " << readBuffer << endl;
    error(ERR_INTERNAL, i18n("Unexpected server response to %1 command:\n%2").arg(command).arg(readBuffer));
