...
sftpProtocol::sftpProtocol(const QCString &pool_socket, const QCString &app_socket)
    : SlaveBase("kio_sftp", pool_socket, app_socket), mConnected(false), mPort(-1), mMsgId(0)
{
...
bool sftpProtocol::isSupportedOperation(int type)
{
    switch(type)
    {
        ...
        default:
            kdDebug(KIO_SFTP_DB) << "isSupportedOperation(type:" << type << "): unrecognized operation type" << endl;
            break;
    }
    ...
    while(code == SSH2_FX_OK)
    {
        if((code = sftpRead(handle, offset, len, buff)) == SSH2_FX_OK)
        {
            ...
            if(!foundMimetype)
            {
                ...
                {
                    // determine mimetype
                    KMimeMagicResult *result = KMimeMagic::self()->findBufferFileType(mimeBuffer, src.fileName());
                    kdDebug(KIO_SFTP_DB) << "sftpGet(): mimetype is " << result->mimeType() << endl;
...
void sftpProtocol::openConnection()
{
    ...
    kdDebug(KIO_SFTP_DB) << "openConnection(): " << mUsername << "@" << mHost << ":" << mPort << endl;
    ...
    if(mUsername.isEmpty() && mPassword.isEmpty())
    {
        kdDebug(KIO_SFTP_DB) << "openConnection(): checking cache "
                             << "info.username = " << info.username << ", info.url = " << info.url.prettyURL() << endl;
    ...
    while(!(mConnected = ssh.connect()))
    {
        ...
        kdDebug(KIO_SFTP_DB) << "openConnection(): "
                                "Got "
                             << err << " from KSshProcess::connect()" << endl;
        ...
        switch(err)
        {
            ...
            case KSshProcess::ERR_NEED_PASSPHRASE:
                ...
                kdDebug(KIO_SFTP_DB) << "openConnection(): info.username = " << info.username << ", info.url = " << info.url.prettyURL() << endl;
                ...
                if(dlgResult)
                {
                    ...
                    {
                        error(ERR_COULD_NOT_AUTHENTICATE, i18n("Please enter a username and password"));
                        continue;
                    }
                }
                else
                ...
                {
                    kdDebug(KIO_SFTP_DB) << "openConnection(): Username changed from " << mUsername << " to " << info.username << endl;
                    ...
                    {
                        kdDebug(KIO_SFTP_DB) << "openConnection(): "
                                                "Adding username to options list"
                                             << endl;
                        opt.opt = KSshProcess::SSH_USERNAME;
                        usernameIt = opts.append(opt);
                    }
                    ...
                    {
                        kdDebug(KIO_SFTP_DB) << "openConnection(): "
                                                "Adding password to options list"
                                             << endl;
                        opt.opt = KSshProcess::SSH_PASSWD;
                        passwdIt = opts.append(opt);
                    }
    ...
    if(type == SSH2_FXP_VERSION)
    {
        ...
        {
            error(ERR_UNSUPPORTED_PROTOCOL, i18n("SFTP version %1").arg(version));
            closeConnection();
            return;
        }
    }
    ...
    kdDebug(KIO_SFTP_DB) << "sftpProtocol(): caching info.username = " << info.username << ", info.url = " << info.url.prettyURL() << endl;
...
void sftpProtocol::sftpPut(const KURL &dest, int permissions, bool resume, bool overwrite, int fd)
{
    ...
    kdDebug(KIO_SFTP_DB) << "sftpPut(): " << dest << ", resume=" << resume << ", overwrite=" << overwrite << endl;
    ...
    if(markPartial)
    {
        ...
        if(code == SSH2_FX_OK)
        {
            ...
            else if(!overwrite && !resume)
            {
                ...
                kdDebug(KIO_SFTP_DB) << "sftpPut(): can resume = " << resume << ", offset = " << offset;
...
{
    kdDebug(KIO_SFTP_DB) << "put(): " << url << ", overwrite = " << overwrite << ", resume = " << resume << endl;
...
void sftpProtocol::rename(const KURL &src, const KURL &dest, bool overwrite)
{
    ...
    {
        error(ERR_UNSUPPORTED_ACTION, i18n("The remote host does not support renaming files."));
        return;
    }
...
void sftpProtocol::symlink(const QString &target, const KURL &dest, bool overwrite)
{
    ...
    {
        error(ERR_UNSUPPORTED_ACTION, i18n("The remote host does not support creating symbolic links."));
        return;
    }
...
{
    kdDebug(KIO_SFTP_DB) << "slave_status(): connected to " << mHost << "? " << mConnected << endl;
...
bool sftpProtocol::getPacket(QByteArray &msg)
{
    ...
    if(len == 0 || len == -1)
    {
        kdDebug(KIO_SFTP_DB) << "getPacket(): read of packet length failed, ret = " << len << ", error =" << strerror(errno) << endl;
        closeConnection();
    ...
    while(msgLen)
    {
        ...
        if(len == 0 || len == -1)
        {
            ...
            kdDebug(KIO_SFTP_DB) << "getPacket(): nothing to read, ret = " << len << ", error =" << strerror(errno) << endl;
            closeConnection();
...
bool sftpProtocol::putPacket(QByteArray &p)
{
    ...
    if(ret <= 0)
    {
        kdDebug(KIO_SFTP_DB) << "putPacket(): write failed, ret =" << ret << ", error = " << strerror(errno) << endl;
        return false;
    }
...
int sftpProtocol::sftpRename(const KURL &src, const KURL &dest)
{
    ...
    QByteArray p;
    QDataStream s(p, IO_WriteOnly);
    s << (Q_UINT32)(1 /*type*/ + 4 /*id*/ + 4 /*str length*/ + slen + 4 /*str length*/ + dlen);
...
int sftpProtocol::sftpSymLink(const QString &_target, const KURL &dest)
{
    ...
    QByteArray p;
    QDataStream s(p, IO_WriteOnly);
    s << (Q_UINT32)(1 /*type*/ + 4 /*id*/ + 4 /*str length*/ + tlen + 4 /*str length*/ + dlen);
...
int sftpProtocol::sftpOpen(const KURL &url, const Q_UINT32 pflags, const sftpFileAttr &attr, QByteArray &handle)
{
    ...
    QByteArray p;
    QDataStream s(p, IO_WriteOnly);
    s << (Q_UINT32)(1 /*type*/ + 4 /*id*/ + 4 /*str length*/ + len + 4 /*pflags*/ + attr.size());
...
int sftpProtocol::sftpRead(const QByteArray &handle, KIO::filesize_t offset, Q_UINT32 len, QByteArray &data)
{
    ...
    Q_UINT32 id, expectedId;
    id = expectedId = mMsgId++;
    s << (Q_UINT32)(1 /*type*/ + 4 /*id*/ + 4 /*str length*/ + handle.size() + 8 /*offset*/ + 4 /*length*/);
...
int sftpProtocol::sftpWrite(const QByteArray &handle, KIO::filesize_t offset, const QByteArray &data)
{
    ...
    Q_UINT32 id, expectedId;
    id = expectedId = mMsgId++;
    s << (Q_UINT32)(1 /*type*/ + 4 /*id*/ + 4 /*str length*/ + handle.size() + 8 /*offset*/ + 4 /* data size */ + data.size());
    ...
    if(id != expectedId)
    {
        kdError(KIO_SFTP_DB) << "sftpWrite(): sftp packet id mismatch, got " << id << ", expected " << expectedId << endl;
        return -1;
    }
