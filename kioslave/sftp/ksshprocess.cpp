 ...
 */
QRegExp KSshProcess::versionStrs[] = {QRegExp("OpenSSH_3\\.[6-9]|OpenSSH_[1-9]*[4-9]\\.[0-9]"), QRegExp("OpenSSH"), QRegExp("SSH Secure Shell")};
...
const char *const KSshProcess::passphrasePrompt[] = {"Enter passphrase for key", "Enter passphrase for key", "Passphrase for key"};

const char *const KSshProcess::authSuccessMsg[] = {"Authentication succeeded", "ssh-userauth2 successful", "Received SSH_CROSS_AUTHENTICATED packet"};

const char *const KSshProcess::authFailedMsg[] = {"Permission denied (", "Permission denied (", "Authentication failed."};

const char *const KSshProcess::tryAgainMsg[] = {"please try again", "please try again", "adjfhjsdhfdsjfsjdfhuefeufeuefe"};

QRegExp KSshProcess::hostKeyMissingMsg[] = {QRegExp("The authenticity of host|No (DSA|RSA) host key is known for"),
                                            QRegExp("The authenticity of host|No (DSA|RSA) host key is known for"),
                                            QRegExp("Host key not found from database")};

const char *const KSshProcess::continuePrompt[] = {"Are you sure you want to continue connecting (yes/no)?",
                                                   "Are you sure you want to continue connecting (yes/no)?",
                                                   "Are you sure you want to continue connecting (yes/no)?"};

const char *const KSshProcess::hostKeyChangedMsg[] = {"WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!",
                                                      "WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!",
                                                      "WARNING: HOST IDENTIFICATION HAS CHANGED!"};

QRegExp KSshProcess::keyFingerprintMsg[] = {QRegExp("..(:..){15}"), QRegExp("..(:..){15}"), QRegExp(".....(-.....){10}")};

QRegExp KSshProcess::knownHostsFileMsg[] = {QRegExp("Add correct host key in (.*) to get rid of this message."),
...
const char *const KSshProcess::changeHostKeyOnDiskPrompt[] = {"as;jf;sajkfdslkfjas;dfjdsa;fj;dsajfdsajf", "as;jf;sajkfdslkfjas;dfjdsa;fj;dsajfdsajf",
...
QRegExp KSshProcess::hostKeyVerifyFailedMsg[] = {
    QRegExp("Host key verification failed\\."), QRegExp("Host key verification failed\\."),
    QRegExp("Disconnected; key exchange or algorithm? negotiation failed \\(Key exchange failed\\.\\)\\.")};

const char *const KSshProcess::connectionClosedMsg[] = {"Connection closed by remote host", "Connection closed by remote host",
...
KSshProcess::SshVersion KSshProcess::version()
{
    ...
    kdDebug(KSSHPROC) << "KSshPRocess::version(): version number = " << mVersion << endl;

    if(mVersion == UNKNOWN_VER)
    {
        kdDebug(KSSHPROC) << "KSshProcess::version(): "
                             "Sorry, I don't know about this version of ssh"
                          << endl;
...
void KSshProcess::kill(int signal)
{
    ...
    kdDebug(KSSHPROC) << "KSshProcess::kill(signal:" << signal << "): ssh pid is " << pid << endl;
    kdDebug(KSSHPROC) << "KSshPRocess::kill(): we are " << (mConnected ? "" : "not ") << "connected" << endl;
    kdDebug(KSSHPROC) << "KSshProcess::kill(): we are " << (mRunning ? "" : "not ") << "running a ssh process" << endl;
...
{
    kdDebug(KSSHPROC) << "KSshProcess::acceptHostKey(accept:" << accept << ")" << endl;
...
QString KSshProcess::getLine()
{
    ...
    if(buffer.empty())
    {
        ...
        while((buffer.size() == 0) && (milliseconds < (60 * 1000)))
        {
            ...
            tv.tv_sec = 60;
            tv.tv_usec = 0; // 60 second timeout
            ...
            {
                kdDebug(KSSHPROC) << "KSshProcess::getLine(): "
                                     "Exception on pty file descriptor."
                                  << endl;
            }
            ...
            {
                kdDebug(KSSHPROC) << "KSshProcess::getLine(): "
                                     "Exception on std err file descriptor."
                                  << endl;
            }
