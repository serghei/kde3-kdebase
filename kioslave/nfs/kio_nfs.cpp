...
NFSFileHandle::NFSFileHandle() : m_isInvalid(FALSE)
{
    memset(m_handle, '\0', NFS_FHSIZE + 1);
    //   m_detectTime=time(0);
}

NFSFileHandle::NFSFileHandle(const NFSFileHandle &handle) : m_isInvalid(FALSE)
{
...
NFSProtocol::NFSProtocol(const QCString &pool, const QCString &app) : SlaveBase("nfs", pool, app), m_client(0), m_sock(-1), m_lastCheck(time(0))
{
...
NFSFileHandle NFSProtocol::getFileHandle(QString path)
{
    ...
    int clnt_stat =
        clnt_call(m_client, NFSPROC_LOOKUP, (xdrproc_t)xdr_diropargs, (char *)&dirargs, (xdrproc_t)xdr_diropres, (char *)&dirres, total_timeout);
...
void NFSProtocol::openConnection()
{
    ...
    int clnt_stat = clnt_call(m_client, MOUNTPROC_EXPORT, (xdrproc_t)xdr_void, NULL, (xdrproc_t)xdr_exports, (char *)&exportlist, total_timeout);
    if(!checkForError(clnt_stat, 0, m_currentHost.latin1()))
        return;
    ...
    for(; exportlist != 0; exportlist = exportlist->ex_next)
    {
        ...
        clnt_stat = clnt_call(m_client, MOUNTPROC_MNT, (xdrproc_t)xdr_dirpath, (char *)(&(exportlist->ex_dir)), (xdrproc_t)xdr_fhstatus,
                              (char *)&fhStatus, total_timeout);
...
void NFSProtocol::listDir(const KURL &_url)
{
    ...
    do
    {
        ...
        int clnt_stat = clnt_call(m_client, NFSPROC_READDIR, (xdrproc_t)xdr_readdirargs, (char *)&listargs, (xdrproc_t)xdr_readdirres,
                                  (char *)&listres, total_timeout);
    ...
    for(QStringList::Iterator it = filesToList.begin(); it != filesToList.end(); it++)
    {
        ...
        int clnt_stat =
            clnt_call(m_client, NFSPROC_LOOKUP, (xdrproc_t)xdr_diropargs, (char *)&dirargs, (xdrproc_t)xdr_diropres, (char *)&dirres, total_timeout);
        if(!checkForError(clnt_stat, dirres.status, (*it)))
            return;
        ...
        if(S_ISLNK(dirres.diropres_u.diropres.attributes.mode))
        {
            ...
            int clnt_stat = clnt_call(m_client, NFSPROC_READLINK, (xdrproc_t)xdr_nfs_fh, (char *)&nfsFH, (xdrproc_t)xdr_readlinkres,
                                      (char *)&readLinkRes, total_timeout);
            if(!checkForError(clnt_stat, readLinkRes.status, (*it)))
                return;
            ...
            else
            {
                ...
                else
                {
                    ...
                    clnt_stat = clnt_call(m_client, NFSPROC_GETATTR, (xdrproc_t)xdr_diropargs, (char *)&dirargs, (xdrproc_t)xdr_attrstat,
                                          (char *)&attrAndStat, total_timeout);
                    if(!checkForError(clnt_stat, attrAndStat.status, tmpStr))
                        return;
...
void NFSProtocol::stat(const KURL &url)
{
    ...
    int clnt_stat = clnt_call(m_client, NFSPROC_GETATTR, (xdrproc_t)xdr_diropargs, (char *)&dirargs, (xdrproc_t)xdr_attrstat, (char *)&attrAndStat,
                              total_timeout);
    if(!checkForError(clnt_stat, attrAndStat.status, path))
        return;
    UDSEntry entry;
    entry.clear();
    ...
    if(S_ISLNK(attrAndStat.attrstat_u.attributes.mode))
    {
        ...
        int clnt_stat = clnt_call(m_client, NFSPROC_READLINK, (xdrproc_t)xdr_nfs_fh, (char *)&nfsFH, (xdrproc_t)xdr_readlinkres, (char *)&readLinkRes,
                                  total_timeout);
        if(!checkForError(clnt_stat, readLinkRes.status, path))
            return;
        kdDebug(7121) << "link dest is -" << readLinkRes.readlinkres_u.data << "-" << endl;
        QCString linkDest(readLinkRes.readlinkres_u.data);
        atom.m_uds = KIO::UDS_LINK_DEST;
...
void NFSProtocol::mkdir(const KURL &url, int permissions)
{
    ...
    int clnt_stat =
        clnt_call(m_client, NFSPROC_MKDIR, (xdrproc_t)xdr_createargs, (char *)&createArgs, (xdrproc_t)xdr_diropres, (char *)&dirres, total_timeout);
    if(!checkForError(clnt_stat, dirres.status, thePath))
        return;
    finished();
}
...
void NFSProtocol::del(const KURL &url, bool isfile)
{
    ...
    if(isfile)
    {
        ...
        int clnt_stat = clnt_call(m_client, NFSPROC_REMOVE, (xdrproc_t)xdr_diropargs, (char *)&dirOpArgs, (xdrproc_t)xdr_nfsstat, (char *)&nfsStat,
                                  total_timeout);
        if(!checkForError(clnt_stat, nfsStat, thePath))
            return;
    ...
    else
    {
        ...
        int clnt_stat =
            clnt_call(m_client, NFSPROC_RMDIR, (xdrproc_t)xdr_diropargs, (char *)&dirOpArgs, (xdrproc_t)xdr_nfsstat, (char *)&nfsStat, total_timeout);
        if(!checkForError(clnt_stat, nfsStat, thePath))
            return;
        kdDebug(7121) << "removing " << thePath << " from cache" << endl;
        m_handleCache.remove(m_handleCache.find(thePath));
...
void NFSProtocol::chmod(const KURL &url, int permissions)
{
    ...
    int clnt_stat =
        clnt_call(m_client, NFSPROC_SETATTR, (xdrproc_t)xdr_sattrargs, (char *)&sAttrArgs, (xdrproc_t)xdr_nfsstat, (char *)&nfsStat, total_timeout);
    if(!checkForError(clnt_stat, nfsStat, thePath))
        return;
...
void NFSProtocol::get(const KURL &url)
{
    ...
    {
        int clnt_stat =
            clnt_call(m_client, NFSPROC_READ, (xdrproc_t)xdr_readargs, (char *)&readArgs, (xdrproc_t)xdr_readres, (char *)&readRes, total_timeout);
        if(!checkForError(clnt_stat, readRes.status, thePath))
            return;
...
void NFSProtocol::put(const KURL &url, int _mode, bool _overwrite, bool /*_resume*/)
{
    ...
    if(_mode == -1)
        createArgs.attributes.mode = 0644;
    else
        createArgs.attributes.mode = _mode;
    ...
    diropres dirOpRes;
    int clnt_stat = clnt_call(m_client, NFSPROC_CREATE, (xdrproc_t)xdr_createargs, (char *)&createArgs, (xdrproc_t)xdr_diropres, (char *)&dirOpRes,
                              total_timeout);
    if(!checkForError(clnt_stat, dirOpRes.status, fileName))
        return;
    ...
    do
    {
        ...
        if(result > 0)
        {
            do
            {
                ...
                int clnt_stat = clnt_call(m_client, NFSPROC_WRITE, (xdrproc_t)xdr_writeargs, (char *)&writeArgs, (xdrproc_t)xdr_attrstat,
                                          (char *)&attrStat, total_timeout);
                // kdDebug(7121)<<"written"<<endl;
                if(!checkForError(clnt_stat, attrStat.status, fileName))
                    return;
...
void NFSProtocol::rename(const KURL &src, const KURL &dest, bool _overwrite)
{
    ...
    int clnt_stat =
        clnt_call(m_client, NFSPROC_RENAME, (xdrproc_t)xdr_renameargs, (char *)&renameArgs, (xdrproc_t)xdr_nfsstat, (char *)&nfsStat, total_timeout);
    if(!checkForError(clnt_stat, nfsStat, destPath))
        return;
    finished();
}
...
void NFSProtocol::copy(const KURL &src, const KURL &dest, int _mode, bool _overwrite)
{
    ...
    int clnt_stat = clnt_call(m_client, NFSPROC_CREATE, (xdrproc_t)xdr_createargs, (char *)&createArgs, (xdrproc_t)xdr_diropres, (char *)&dirOpRes,
                              total_timeout);
    if(!checkForError(clnt_stat, dirOpRes.status, destPath))
        return;
    // we created the file successfully
    destFH = dirOpRes.diropres_u.diropres.file.data;
    kdDebug(7121) << "file -" << fileName << "- in dir -" << parentDir << "- created successfully" << endl;
    ...
    do
    {
        ...
        if(bytesRead > 0)
        {
            ...
            clnt_stat = clnt_call(m_client, NFSPROC_WRITE, (xdrproc_t)xdr_writeargs, (char *)&writeArgs, (xdrproc_t)xdr_attrstat, (char *)&attrStat,
                                  total_timeout);
            // kdDebug(7121)<<"written"<<endl;
...
void NFSProtocol::symlink(const QString &target, const KURL &dest, bool)
{
    ...
    nfsstat nfsStat;
    int clnt_stat = clnt_call(m_client, NFSPROC_SYMLINK, (xdrproc_t)xdr_symlinkargs, (char *)&symLinkArgs, (xdrproc_t)xdr_nfsstat, (char *)&nfsStat,
                              total_timeout);
    if(!checkForError(clnt_stat, nfsStat, destPath))
        return;
