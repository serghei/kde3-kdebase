...
FloppyProtocol::FloppyProtocol(const QCString &pool, const QCString &app)
    : SlaveBase("floppy", pool, app), m_mtool(0), m_stdoutBuffer(0), m_stderrBuffer(0), m_stdoutSize(0), m_stderrSize(0)
{
...
bool FloppyProtocol::stopAfterError(const KURL &url, const QString &drive)
{
    ...
    else if(line.find("Permission denied") > -1)
    {
        error(KIO::ERR_SLAVE_DEFINED, i18n("Could not access %1.\nMake sure the floppy in drive %2 is a DOS-formatted floppy disk \nand that the "
                                           "permissions of the device file (e.g. /dev/fd0) are set correctly (e.g. rwxrwxrwx).")
                                          .arg(url.prettyURL(), drive));
    }
...
void FloppyProtocol::listDir(const KURL &_url)
{
    ...
    do
    {
        ...
        if(stderrEvent)
        {
            if(readStderr() == 0)
                loopFinished = true;
            else if(stopAfterError(url, drive))
            {
                loopFinished = true;
                errorOccured = true;
            }
...
void FloppyProtocol::rename(const KURL &src, const KURL &dest, bool _overwrite)
{
    ...
    if(_overwrite)
        args << "mren"
             << "-o" << (srcDrive + srcFloppyPath) << (destDrive + destFloppyPath);
    else
        args << "mren"
             << "-D"
             << "s" << (srcDrive + srcFloppyPath) << (destDrive + destFloppyPath);
    ...
    do
    {
        ...
        if(stderrEvent)
        {
            if(readStderr() == 0)
                loopFinished = true;
            else if(stopAfterError(src, srcDrive))
            {
                loopFinished = true;
                errorOccured = true;
            }
