...
FstabBackend::FstabBackend(MediaList &list, bool networkSharesOnly) : QObject(), BackendBase(list), m_networkSharesOnly(networkSharesOnly)
{
...
#ifdef Q_OS_FREEBSD
    connect(&m_mtabTimer, SIGNAL(timeout()), this, SLOT(handleMtabChange()));
...
{
    if(mount->mountType() == "swap" || mount->mountType() == "tmpfs" || mount->mountType() == "sysfs" || mount->mountType() == "fdescfs"
       || mount->mountType() == "kernfs" || mount->mountType() == "usbfs" || mount->mountType().contains("proc") || mount->mountType() == "unknown"
       || mount->mountType() == "none" || mount->mountType() == "sunrpc" || mount->mountedFrom() == "none" || mount->mountedFrom() == "tmpfs"
       || mount->mountedFrom().find("shm") != -1 || mount->mountPoint() == "/dev/swap" || mount->mountPoint() == "/dev/pts"
       || mount->mountPoint().find("/proc") == 0 || mount->mountPoint().find("/sys") == 0

       // We might want to display only network shares
       // since HAL doesn't handle them
       || (networkSharesOnly && mount->mountType().find("smb") == -1 && mount->mountType().find("cifs") == -1
...
void FstabBackend::handleMtabChange(bool allowNotification)
{
    ...
    for(; it != end; ++it)
    {
        ...
        if(::inExclusionPattern(*it, m_networkSharesOnly))
            continue;
        ...
        if(!m_mtabIds.contains(id) && m_fstabIds.contains(id))
        {
            ...
            m_mediaList.changeMediumState(id, true, false, mime, icon, label);
        }
#if 0
    ...
    for(; it2 != end2; ++it2)
    {
        if(!new_mtabIds.contains(*it2) && m_fstabIds.contains(*it2))
        {
            ...
            m_mediaList.changeMediumState(*it2, false, false, mime, icon, label);
        }
#if 0
...
QString FstabBackend::generateId(const QString &devNode, const QString &mountPoint)
{
    QString d = KStandardDirs::realFilePath(devNode);
    QString m = KStandardDirs::realPath(mountPoint);

    return "/org/kde/mediamanager/fstab/" + d.replace("/", "") + m.replace("/", "");
}
...
void FstabBackend::guess(const QString &devNode, const QString &mountPoint, const QString &fsType, bool mounted, QString &mimeType, QString &iconName,
                         QString &label)
{
    enum
    {
...
#endif
    if(devType == CDWRITER || devNode.find("cdwriter") != -1 || mountPoint.find("cdwriter") != -1 || devNode.find("cdrecorder") != -1
       || mountPoint.find("cdrecorder") != -1 || devNode.find("cdburner") != -1 || mountPoint.find("cdburner") != -1 || devNode.find("cdrw") != -1
       || mountPoint.find("cdrw") != -1 || devNode.find("graveur") != -1)
    {
        mimeType = "media/cdwriter";
        label = i18n("CD Recorder");
    }
    else if(devType == DVD || devType == DVDWRITER || devNode.find("dvd") != -1 || mountPoint.find("dvd") != -1)
    {
        mimeType = "media/dvd";
        label = i18n("DVD");
    }
    else if(devType == CD || devNode.find("cdrom") != -1 || mountPoint.find("cdrom") != -1
            // LINUX SPECIFIC
    ...
    else if(devNode.find("fd") != -1 || mountPoint.find("fd") != -1 || devNode.find("floppy") != -1 || mountPoint.find("floppy") != -1)
    {
    ...
    else if(mountPoint.find("removable") != -1 || mountPoint.find("hotplug") != -1 || mountPoint.find("usb") != -1
            || mountPoint.find("firewire") != -1 || mountPoint.find("ieee1394") != -1 || devNode.find("/usb/") != -1)
    {
    ...
    else if(fsType.find("smb") != -1 || fsType.find("cifs") != -1 || devNode.find("//") != -1)
    {
        mimeType = "media/smb";
        label = i18n("Remote Share");
    }
