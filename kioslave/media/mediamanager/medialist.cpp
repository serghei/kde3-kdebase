...
bool MediaList::changeMediumState(const Medium &medium, bool allowNotification)
{
    ...
    if(medium.isMountable())
    {
        ...
        m->mountableState(device_node, mount_point, fs_type, mounted);
    }
    else
    {
...
bool MediaList::changeMediumState(const QString &id, const QString &baseURL, bool allowNotification, const QString &mimeType, const QString &iconName,
                                  const QString &label)
{
    kdDebug(1219) << "MediaList::changeMediumState(" << id << ", " << baseURL << ", " << mimeType << ", " << iconName << ")" << endl;
    ...
    emit mediumStateChanged(id, medium->name(), !medium->needMounting(), allowNotification);
    return true;
}

bool MediaList::changeMediumState(const QString &id, const QString &deviceNode, const QString &mountPoint, const QString &fsType, bool mounted,
                                  bool allowNotification, const QString &mimeType, const QString &iconName, const QString &label)
{
    kdDebug(1219) << "MediaList::changeMediumState(" << id << ", " << deviceNode << ", " << mountPoint << ", " << fsType << ", " << mounted << ", "
                  << mimeType << ", " << iconName << ")" << endl;
...
bool MediaList::changeMediumState(const QString &id, bool mounted, bool allowNotification, const QString &mimeType, const QString &iconName,
                                  const QString &label)
{
    kdDebug(1219) << "MediaList::changeMediumState(" << id << ", " << mounted << ", " << mimeType << ", " << iconName << ")" << endl;
    ...
    if(!medium->mountableState(mounted))
        return false;
...
{
    kdDebug(1219) << "MediaList::setUserLabel(" << name << ", " << label << ")" << endl;
    ...
    emit mediumStateChanged(medium->id(), name, !medium->needMounting(), false);
    return true;
}
