...
DiscType::DiscType(Type type) : m_type(type)
{
}

bool DiscType::isKnownDisc() const
{
    return m_type != None && m_type != Unknown && m_type != UnknownType && m_type != Broken;
}

bool DiscType::isDisc() const
{
    return m_type != None && m_type != Unknown && m_type != Broken;
}
...
public:
    ...
    {
        kdDebug(1219) << "PollingThread::PollingThread(" << devNode << ")" << endl;
        m_stop = false;
...
LinuxCDPolling::LinuxCDPolling(MediaList &list) : QObject(), BackendBase(list)
{
    connect(&m_mediaList, SIGNAL(mediumAdded(const QString &, const QString &, bool)), this, SLOT(slotMediumAdded(const QString &)));

    connect(&m_mediaList, SIGNAL(mediumRemoved(const QString &, const QString &, bool)), this, SLOT(slotMediumRemoved(const QString &)));

    connect(&m_mediaList, SIGNAL(mediumStateChanged(const QString &, const QString &, bool, bool)), this,
            SLOT(slotMediumStateChanged(const QString &)));
...
void LinuxCDPolling::slotMediumAdded(const QString &id)
{
    ...
    if(mime.find("dvd") == -1 && mime.find("cd") == -1)
        return;
...
{
    kdDebug(1219) << "LinuxCDPolling::slotMediumStateChanged(" << id << ")" << endl;
...
static QString baseType(const Medium *medium)
{
    ...
    FstabBackend::guess(devNode, mountPoint, fsType, mounted, mimeType, iconName, label);
...
static void restoreEmptyState(MediaList &list, const Medium *medium, bool allowNotification)
{
    ...
    list.changeMediumState(id, devNode, mountPoint, fsType, mounted, allowNotification, mimeType, iconName, label);
}


void LinuxCDPolling::applyType(DiscType type, const Medium *medium)
{
    kdDebug(1219) << "LinuxCDPolling::applyType(" << type << ", " << medium->id() << ")" << endl;
    ...
    switch(type)
    {
        ...
        case DiscType::Audio:
        case DiscType::Mixed:
            m_mediaList.changeMediumState(id, "audiocd:/?device=" + dev, notify, "media/audiocd");
            break;
        case DiscType::VCD:
...
DiscType LinuxCDPolling::identifyDiscType(const QCString &devNode, const DiscType &current)
{
...
bool LinuxCDPolling::hasDirectory(const QCString &devNode, const QCString &dir)
{
    ...
    if(Q_BYTE_ORDER != Q_LITTLE_ENDIAN)
        tl = ((tl << 24) & 0xFF000000) | ((tl << 8) & 0xFF0000) | ((tl >> 8) & 0xFF00) | ((tl >> 24) & 0xFF);
