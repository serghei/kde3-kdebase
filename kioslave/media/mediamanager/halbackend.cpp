...
HALBackend::HALBackend(MediaList &list, QObject *parent)
    : QObject(), BackendBase(list), m_halContext(NULL), m_halStoragePolicy(NULL), m_parent(parent)
{
    s_HALBackend = this;
}
...
void HALBackend::setVolumeProperties(Medium *medium)
{
    ...
    medium->setName(generateName(libhal_volume_get_device_file(halVolume)));

    medium->mountableState(libhal_volume_get_device_file(halVolume), /* Device node */
    ...
    if(libhal_volume_is_disc(halVolume))
    {
        ...
        if((discType == LIBHAL_VOLUME_DISC_TYPE_CDROM) || (discType == LIBHAL_VOLUME_DISC_TYPE_CDR) || (discType == LIBHAL_VOLUME_DISC_TYPE_CDRW))
        ...
        if((discType == LIBHAL_VOLUME_DISC_TYPE_DVDROM) || (discType == LIBHAL_VOLUME_DISC_TYPE_DVDRAM) || (discType == LIBHAL_VOLUME_DISC_TYPE_DVDR)
           || (discType == LIBHAL_VOLUME_DISC_TYPE_DVDRW) || (discType == LIBHAL_VOLUME_DISC_TYPE_DVDPLUSR)
...
bool HALBackend::setFstabProperties(Medium *medium)
{
    ...
    if(!mp.isNull() && !medium->id().startsWith("/org/kde"))
    {
        ...
        medium->mountableState(medium->deviceNode(), mp, /* Mount point */
...
void HALBackend::setCameraProperties(Medium *medium)
{
    ...
    DBusError error;
    dbus_error_init(&error);
    if(cam && libhal_device_property_exists(m_halContext, udi, "usb.linux.device_number", NULL)
       && libhal_device_property_exists(m_halContext, udi, "usb.bus_number", NULL))
        device.sprintf("camera://%s@[usb:%03d,%03d]/", cam, libhal_device_get_property_int(m_halContext, udi, "usb.bus_number", &error),
...
void HALBackend::hal_device_property_modified(LibHalContext *ctx, const char *udi, const char *key, dbus_bool_t is_removed, dbus_bool_t is_added)
{
...
void HALBackend::hal_device_condition(LibHalContext *ctx, const char *udi, const char *condition_name, const char *message)
{
...
static QString mount_priv(const char *udi, const char *mount_point, const char **poptions, int noptions, DBusConnection *dbus_connection)
{
    DBusMessage *dmesg, *reply;
    DBusError error;

    const char *fstype = "";
    if(!(dmesg = dbus_message_new_method_call("org.freedesktop.Hal", udi, "org.freedesktop.Hal.Device.Volume", "Mount")))
    {
        kdDebug() << "mount failed for " << udi << ": could not create dbus message\n";
        return i18n("Internal Error");
    }

    if(!dbus_message_append_args(dmesg, DBUS_TYPE_STRING, &mount_point, DBUS_TYPE_STRING, &fstype, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING, &poptions,
                                 noptions, DBUS_TYPE_INVALID))
    {
...
QString HALBackend::listUsingProcesses(const Medium *medium)
{
    ...
    {
        fullmsg = i18n(
            "Moreover, programs still using the device "
...
void HALBackend::slotResult(KIO::Job *job)
{
    ...
    if(job->error() == KIO::ERR_COULD_NOT_UNMOUNT)
    {
        ...
        qerror += "<p>"
                  + i18n(
                        "Unfortunately, the device <b>%1</b> (%2) named <b>'%3'</b> and "
                        "currently mounted at <b>%4</b> could not be unmounted. ")
                        .arg("system:/media/" + medium->name(), medium->deviceNode(), medium->prettyLabel(), medium->prettyBaseURL().pathOrURL())
                  + "</p>";
...
QString HALBackend::unmount(const QString &_udi)
{
    ...
    if(!(dmesg = dbus_message_new_method_call("org.freedesktop.Hal", udi, "org.freedesktop.Hal.Device.Volume", "Unmount")))
    {
    ...
    if(!dbus_message_append_args(dmesg, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING, &options, 0, DBUS_TYPE_INVALID))
    {
