...
MediaModule::MediaModule(QWidget *parent, const char *name, const QStringList &) : KCModule(MediaFactory::instance(), parent, name)
{
    ...
    connect(m_notifierModule, SIGNAL(changed(bool)), this, SLOT(moduleChanged(bool)));

    m_managerModule = new ManagerModule(this, "manager");
    tab->addTab(m_managerModule, i18n("&Advanced"));
    connect(m_managerModule, SIGNAL(changed(bool)), this, SLOT(moduleChanged(bool)));


    KAboutData *about = new KAboutData("kcmmedia", I18N_NOOP("Storage Media"), "0.6", I18N_NOOP("Storage Media Control Panel Module"),
                                       KAboutData::License_GPL_V2, I18N_NOOP("(c) 2005 Jean-Remy Falleri"));
