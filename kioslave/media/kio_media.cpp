...
MediaProtocol::MediaProtocol(const QCString &protocol, const QCString &pool, const QCString &app) : ForwardingSlaveBase(protocol, pool, app)
{
    connect(&m_impl, SIGNAL(warning(const QString &)), this, SLOT(slotWarning(const QString &)));
}
...
void MediaProtocol::put(const KURL &url, int permissions, bool overwrite, bool resume)
{
...
{
    kdDebug(1219) << "MediaProtocol::rename: " << src << ", " << dest << ", " << overwrite << endl;
    ...
    if(ok && src_path.isEmpty() && dest_path.isEmpty() && src.protocol() == "media" && dest.protocol() == "media")
    {
...
void MediaProtocol::stat(const KURL &url)
{
    ...
    if(path.isEmpty())
    {
        ...
        if(m_impl.statMedium(name, entry) || m_impl.statMediumByLabel(name, entry))
        {
            statEntry(entry);
            finished();
        }
