...
PropertiesPage::PropertiesPage(QWidget *parent, const QString &_id) : PropertiesPageGUI(parent), id(_id)
{
    ...
    if(list.size())
    {
        ...
        if(!options.contains("journaling") && !options.contains("shortname") && !options.contains("uid") && !options.contains("utf8")
...
bool PropertiesPage::save()
{
    ...
    {
        KMessageBox::sorry(this, i18n("Saving the changes failed"));
