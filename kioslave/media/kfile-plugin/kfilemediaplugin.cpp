...
KFileMediaPlugin::KFileMediaPlugin(QObject *parent, const char *name, const QStringList &args) : KFilePlugin(parent, name, args)
{
...
bool KFileMediaPlugin::readInfo(KFileMetaInfo &info, uint /*what*/)
{
    ...
    if(!mount_point.isEmpty() && medium.isMounted())
    {
        ...
        if(::statvfs(QFile::encodeName(mount_point), &vfs) != -1)
        {
            ...
            QApplication::style().drawPrimitive(QStyle::PE_Panel, &p, QRect(0, 0, 150, 20), cg, QStyle::Style_Sunken);
...
void KFileMediaPlugin::addMimeType(const char *mimeType)
{
    ...
    KFileMimeTypeInfo::GroupInfo *group = addGroupInfo(info, "mediumInfo", i18n("Medium Information"));

    KFileMimeTypeInfo::ItemInfo *item = addItemInfo(group, "free", i18n("Free"), QVariant::ULongLong);
