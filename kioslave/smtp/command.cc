...
#ifdef HAVE_LIBSASL2
static sasl_callback_t callbacks[] = {{SASL_CB_ECHOPROMPT, NULL, NULL}, {SASL_CB_NOECHOPROMPT, NULL, NULL}, {SASL_CB_GETREALM, NULL, NULL},
                                      {SASL_CB_USER, NULL, NULL},       {SASL_CB_AUTHNAME, NULL, NULL},     {SASL_CB_PASS, NULL, NULL},
                                      {SASL_CB_CANON_USER, NULL, NULL}, {SASL_CB_LIST_END, NULL, NULL}};
#endif
...
Command::Command(SMTPProtocol *smtp, int flags) : mSMTP(smtp), mComplete(false), mNeedResponse(false), mFlags(flags)
{
    assert(smtp);
}
...
bool EHLOCommand::processResponse(const Response &r, TransactionState *)
{
    ...
    {
        if(mEHLONotSupported)
        { // HELO failed...
            mSMTP->error(KIO::ERR_INTERNAL_SERVER, i18n("The server rejected both EHLO and HELO commands "
    ...
    mSMTP->error(KIO::ERR_UNKNOWN,
                 i18n("Unexpected server response to %1 command.\n%2").arg(mEHLONotSupported ? "HELO" : "EHLO").arg(r.errorMessage()));
    return false;
}
...
bool StartTLSCommand::processResponse(const Response &r, TransactionState *)
{
    ...
    if(r.code() != 220)
    {
        mSMTP->error(r.errorCode(), i18n("Your SMTP server does not support TLS. "
    ...
    if(tlsrc != -3)
        ...
        mSMTP->messageBox(KIO::SlaveBase::Information, i18n("Your SMTP server claims to "
...
#define SASLERROR                                                                                                                                    \
    mSMTP->error(KIO::ERR_COULD_NOT_AUTHENTICATE, i18n("An error occured during authentication: %1").arg(QString::fromUtf8(sasl_errdetail(conn))));

//
// AUTH - rfc 2554
//
AuthCommand::AuthCommand(SMTPProtocol *smtp, const char *mechanisms, const QString &aFQDN, KIO::AuthInfo &ai)
    : Command(smtp, CloseConnectionOnError | OnlyLastInPipeline), mAi(&ai), mFirstTime(true)
{
#ifdef HAVE_LIBSASL2
    int result;
    mMechusing = 0;
    ...
    result = sasl_client_new("smtp", aFQDN.latin1(), 0, 0, callbacks, 0, &conn);
    if(result != SASL_OK)
    {
        SASLERROR
        return;
    ...
    do
    {
        result = sasl_client_start(conn, mechanisms, &client_interact, &mOut, &mOutlen, &mMechusing);
...
#else
    mSMTP->error(KIO::ERR_COULD_NOT_AUTHENTICATE, i18n("Authentication support is not compiled into kio_smtp."));
#endif
}
...
QCString AuthCommand::nextCommandLine(TransactionState *)
{
    ...
    else
    {
        ...
        do
        {
            result = sasl_client_step(conn, challenge.isEmpty() ? 0 : challenge.data(), challenge.size(), &client_interact, &mOut, &mOutlen);
            if(result == SASL_INTERACT)
                if(!saslInteract(client_interact))
                {
                    return "";
...
bool AuthCommand::processResponse(const Response &r, TransactionState *)
{
    if(!r.isOk())
    {
        if(mFirstTime)
            if(haveCapability("AUTH"))
                mSMTP->error(KIO::ERR_COULD_NOT_LOGIN, i18n("Your SMTP server does not support %1.\nChoose a different authentication method.\n%2")
                                                           .arg(mMechusing)
                                                           .arg(r.errorMessage()));
            else
                mSMTP->error(KIO::ERR_COULD_NOT_LOGIN, i18n("Your SMTP server does not support authentication.\n"
                                                            "  %2")
                                                           .arg(r.errorMessage()));
        else
            mSMTP->error(KIO::ERR_COULD_NOT_LOGIN, i18n("Authentication failed.\n"
    ...
    mFirstTime = false;
    mLastChallenge = r.lines().first(); // ### better join all lines with \n?
    mNeedResponse = false;
    return true;
}
...
QCString TransferCommand::nextCommandLine(TransactionState *ts)
{
    ...
    {
        ts->setFailedFatally(KIO::ERR_INTERNAL, i18n("Could not read data from application."));
        mComplete = true;
        mNeedResponse = true;
        return 0;
    }
...
bool TransferCommand::processResponse(const Response &r, TransactionState *ts)
{
    ...
    {
        ts->setFailed();
        mSMTP->error(r.errorCode(), i18n("The message content was not accepted.\n"
                                         "%1")
                                        .arg(r.errorMessage()));
        return false;
    }
