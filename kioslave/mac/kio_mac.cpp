...
MacProtocol::MacProtocol(const QCString &pool, const QCString &app) : QObject(), SlaveBase("mac", pool, app)
{
...
void MacProtocol::listDir(const KURL &url)
{
    ...
    else
    {
        ...
        *myKProcess << "hpls"
        ...
        connect(myKProcess, SIGNAL(receivedStdout(KProcess *, char *, int)), this, SLOT(slotGetStdOutput(KProcess *, char *, int)));

        myKProcess->start(KProcess::Block, KProcess::All);

        if((!myKProcess->normalExit()) || (!myKProcess->exitStatus() == 0))
        {
            error(ERR_SLAVE_DEFINED, i18n("There was an error with hpls - please ensure it is installed"));
        }

        // clean up
        delete myKProcess;
        myKProcess = 0;
        disconnect(myKProcess, SIGNAL(receivedStdout(KProcess *, char *, int)), this, SLOT(slotGetStdOutput(KProcess *, char *, int)));
...
QValueList< KIO::UDSAtom > MacProtocol::doStat(const KURL &url)
{
    ...
    else if(!filename.isEmpty())
    {
        ...
        *myKProcess << "hpls"
        ...
        if((!myKProcess->normalExit()) || (!myKProcess->exitStatus() == 0))
        {
            error(ERR_SLAVE_DEFINED, i18n("hpls did not exit normally - please ensure you have installed the hfsplus tools"));
        }
    ...
    { // filename is empty means we're looking at root dir
...
QString MacProtocol::prepareHP(const KURL &url)
{
    ...
    *myKProcess << "hpmount";
    standardOutputStream = QString::null;
    connect(myKProcess, SIGNAL(receivedStderr(KProcess *, char *, int)), this, SLOT(slotGetStdOutput(KProcess *, char *, int)));
    ...
    delete myKProcess;
    myKProcess = 0;
    disconnect(myKProcess, SIGNAL(receivedStderr(KProcess *, char *, int)), this, SLOT(slotGetStdOutput(KProcess *, char *, int)));
    ...
    if((!myKProcess->normalExit()) || (!myKProcess->exitStatus() == 0))
    {
        ...
        error(ERR_SLAVE_DEFINED, i18n("hpmount did not exit normally - please ensure that hfsplus utils are installed,\n"
    ...
    while(s != -1)
    {
        ...
        *myKProcess << "hpcd" << dir;
        ...
        if((!myKProcess->normalExit()) || (!myKProcess->exitStatus() == 0))
        {
            error(ERR_SLAVE_DEFINED, i18n("hpcd did not exit normally - please ensure it is installed"));
            return NULL;
        }
...
QValueList< KIO::UDSAtom > MacProtocol::makeUDS(const QString &_line)
{
    ...
    else if(fileRE.exactMatch(line))
    {
        ...
        atom.m_uds = KIO::UDS_ACCESS;
        if(QString(fileRE.cap(1)) == QString("F"))
        { // if locked then read only
            atom.m_long = 0444;
        }
        else
        {
        ...
        if(QString(fileRE.cap(2)) == QString("adrp") || QString(fileRE.cap(2)) == QString("fdrp"))
        {
            atom.m_uds = KIO::UDS_FILE_TYPE;
            atom.m_long = S_IFREG;
            entry.append(atom);
