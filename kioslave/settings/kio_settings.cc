...
void SettingsProtocol::stat(const KURL &url)
{
    ...
    if(grp && grp->isValid())
    {
        createDirEntry(entry,
                       (m_runMode == SettingsMode) ? i18n("Settings") : ((m_runMode == ApplicationsMode) ? i18n("Applications") : i18n("Programs")),
    ...
    else
    {
        ...
        if(service && service->isValid())
        {
            ...
            createFileEntry(entry, service->name(), url.url(1) + service->desktopEntryName(), "application/x-desktop", service->icon(),
    ...
    for(it = list.begin(); it != list.end(); ++it)
    {
        ...
        else
        {
            ...
            createFileEntry(entry, s->name(), url.url(1) + s->desktopEntryName(), "application/x-desktop", s->icon(),
                            locate("apps", s->desktopEntryPath()));
        }
