...
void TestTrash::setup()
{
    ...
    for(QValueVector< int >::const_iterator it = writableTopDirs.begin(); it != writableTopDirs.end(); ++it)
    {
           ...
           (writableTopDirs.count() > 1)) // but well, if we have no choice, take it
        {
            ...
            kdDebug() << "OK, found another writable partition: topDir=" << m_otherPartitionTopDir << " trashDir=" << m_otherPartitionTrashDir
                      << " id=" << m_otherPartitionId << endl;
            break;
        }
    }
...
void TestTrash::trashFile(const QString &origFilePath, const QString &fileId)
{
    ...
    if(!ok)
        kdError() << "moving " << u << " to trash failed with error " << KIO::NetAccess::lastError() << " " << KIO::NetAccess::lastErrorString()
                  << endl;
    assert(ok);
...
void TestTrash::trashDirectoryOwnedByRoot()
{
    ...
    assert(err == KIO::ERR_ACCESS_DENIED || err == KIO::ERR_CANNOT_OPEN_FOR_READING);
...
void TestTrash::listRootDir()
{
    ...
    connect(job, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), SLOT(slotEntries(KIO::Job *, const KIO::UDSEntryList &)));
    bool ok = KIO::NetAccess::synchronousRun(job, 0);
    assert(ok);
