 ...
 */
LDAPProtocol::LDAPProtocol(const QCString &protocol, const QCString &pool, const QCString &app) : SlaveBase(protocol, pool, app)
{
    mLDAP = 0;
    mTLS = 0;
    mVer = 3;
...
void LDAPProtocol::LDAPErr(const KURL &url, int err)
{
    ...
    {
        if(err == LDAP_SUCCESS)
            ldap_get_option(mLDAP, LDAP_OPT_ERROR_NUMBER, &err);
        if(err != LDAP_SUCCESS)
            ldap_get_option(mLDAP, LDAP_OPT_ERROR_STRING, &errmsg);
    }
    if(err == LDAP_SUCCESS)
        return;
    kdDebug(7125) << "error code: " << err << " msg: " << ldap_err2string(err) << " Additonal error message: '" << errmsg << "'" << endl;
    QString msg;
    QString extraMsg;
    if(errmsg)
    {
        ...
        */
        ...
        default:
            error(ERR_SLAVE_DEFINED,
                  i18n("LDAP server returned the error: %1 %2\nThe LDAP URL was: %3").arg(ldap_err2string(err)).arg(extraMsg).arg(url.prettyURL()));
    }
}

void LDAPProtocol::controlsFromMetaData(LDAPControl ***serverctrls, LDAPControl ***clientctrls)
{
    QString oid;
    bool critical;
    QByteArray value;
    ...
    while(hasMetaData(QString::fromLatin1("SERVER_CTRL%1").arg(i)))
    {
        ...
        kdDebug(7125) << "server ctrl #" << i << " value: " << val << " oid: " << oid << " critical: " << critical
    ...
    while(hasMetaData(QString::fromLatin1("CLIENT_CTRL%1").arg(i)))
    {
        ...
        LDIF::splitControl(val, oid, critical, value);
        kdDebug(7125) << "client ctrl #" << i << " value: " << val << " oid: " << oid << " critical: " << critical
                      << " value: " << QString::fromUtf8(value, value.size()) << endl;
...
int LDAPProtocol::asyncSearch(LDAPUrl &usrc)
{
    ...
    kdDebug(7125) << "asyncSearch() dn=\"" << usrc.dn() << "\" scope=" << usrc.scope() << " filter=\"" << usrc.filter()
                  << "\" attrs=" << usrc.attributes() << endl;
    retval = ldap_search_ext(mLDAP, usrc.dn().utf8(), scope, usrc.filter().isEmpty() ? QCString() : usrc.filter().utf8(), attrs, 0, serverctrls,
                             clientctrls, 0, mSizeLimit, &msgid);
...
void LDAPProtocol::addControlOp(LDAPControl ***pctrls, const QString &oid, const QByteArray &value, bool critical)
{
    LDAPControl **ctrls;
    ...
    kdDebug(7125) << "addControlOp: oid:'" << oid << "' val: '" << QString::fromUtf8(value, value.size()) << "'" << endl;
...
void LDAPProtocol::addModOp(LDAPMod ***pmods, int mod_type, const QString &attr, const QByteArray &value)
{
    ...
    {
        while(mods[i] != 0 && (strcmp(attr.utf8(), mods[i]->mod_type) != 0 || (mods[i]->mod_op & ~LDAP_MOD_BVALUES) != mod_type))
            i++;
...
void LDAPProtocol::LDAPEntry2UDSEntry(const QString &dn, UDSEntry &entry, const LDAPUrl &usrc, bool dir)
{
    UDSAtom atom;
...
void LDAPProtocol::changeCheck(LDAPUrl &url)
{
    ...
    if(!authSASL && bindname.isEmpty())
        bindname = mUser;

    if(tls != mTLS || ver != mVer || authSASL != mAuthSASL || mech != mMech || mRealm != realm || mBindName != bindname || mTimeLimit != timelimit
       || mSizeLimit != sizelimit)
    {
        ...
        mSizeLimit = sizelimit;
        kdDebug(7125) << "parameters changed: tls = " << mTLS << " version: " << mVer << "SASLauth: " << mAuthSASL << endl;
        openConnection();
        if(mAuthSASL)
        {
            url.setUser(mUser);
...
void LDAPProtocol::setHost(const QString &host, int port, const QString &user, const QString &password)
{
    ...
    kdDebug(7125) << "setHost: " << host << " port: " << port << " user: " << mUser << " pass: [protected]" << endl;
}
...
void LDAPProtocol::fillAuthInfo(AuthInfo &info)
{
    ...
    info.comment = QString::fromLatin1(mProtocol) + "://" + mHost + ":" + QString::number(mPort);
    ...
    for(; interact->id != SASL_CB_LIST_END; interact++)
    {
        if(interact->id == SASL_CB_AUTHNAME || interact->id == SASL_CB_PASS)
        {
            ...
            if(info.username.isEmpty() || info.password.isEmpty())
            {
                ...
                if(!((mFirstAuth && cached) || (mFirstAuth ? openPassDlg(info) : openPassDlg(info, i18n("Invalid authorization information.")))))
                {
...
void LDAPProtocol::openConnection()
{
    ...
    if((ldap_set_option(mLDAP, LDAP_OPT_PROTOCOL_VERSION, &version)) != LDAP_OPT_SUCCESS)
    {

        closeConnection();
        error(ERR_UNSUPPORTED_ACTION, i18n("Cannot set LDAP protocol version %1").arg(version));
        return;
    }
    ...
    if(mSizeLimit)
    {
        ...
        {
            closeConnection();
            error(ERR_UNSUPPORTED_ACTION, i18n("Cannot set size limit."));
            return;
        }
    }
    ...
    if(mTimeLimit)
    {
        ...
        {
            closeConnection();
            error(ERR_UNSUPPORTED_ACTION, i18n("Cannot set time limit."));
            return;
        }
    }
    ...
    {
        closeConnection();
        error(ERR_SLAVE_DEFINED, i18n("SASL authentication not compiled into the ldap ioslave."));
        return;
    }
#endif
    ...
    while(!auth)
    {
        if(!mAuthSASL && ((mFirstAuth && !(mBindName.isEmpty() && mPassword.isEmpty()) && // For anonymous bind
                           (mBindName.isEmpty() || mPassword.isEmpty()))
                          || !mFirstAuth))
        {
            if((mFirstAuth && cached) || (mFirstAuth ? openPassDlg(info) : openPassDlg(info, i18n("Invalid authorization information."))))
            {
        ...
        ret =
#if defined HAVE_SASL_H || defined HAVE_SASL_SASL_H
            mAuthSASL ? ldap_sasl_interactive_bind_s(mLDAP, NULL, mechanism.utf8(), NULL, NULL, LDAP_SASL_INTERACTIVE, &kldap_sasl_interact, this) :
#endif
                      ldap_simple_bind_s(mLDAP, mBindName.utf8(), mPassword.utf8());

        mFirstAuth = false;
        if(ret != LDAP_INVALID_CREDENTIALS && ret != LDAP_INSUFFICIENT_ACCESS && ret != LDAP_INAPPROPRIATE_AUTH)
        {
        ...
        do
        {
            ...
            switch(ret)
            {
                ...
                case LDIF::EndEntry:
                    ...
                    switch(ldif.entryType())
                    {
                        ...
                        case LDIF::Entry_Del:
                            ...
                            controlsFromMetaData(&serverctrls, &clientctrls);
                            ldaperr = ldap_delete_ext_s(mLDAP, ldif.dn().utf8(), serverctrls, clientctrls);
                            FREELDAPMEM;
                            break;
                        case LDIF::Entry_Modrdn:
                            kdDebug(7125) << "kio_ldap_modrdn olddn:" << ldif.dn() << " newRdn: " << ldif.newRdn()
                                          << " newSuperior: " << ldif.newSuperior() << " deloldrdn: " << ldif.delOldRdn() << endl;
                            controlsFromMetaData(&serverctrls, &clientctrls);
                            ldaperr = ldap_rename_s(mLDAP, ldif.dn().utf8(), ldif.newRdn().utf8(),
                                                    ldif.newSuperior().isEmpty() ? QCString() : ldif.newSuperior().utf8(), ldif.delOldRdn(),
                        ...
                        case LDIF::Entry_Mod:
                            ...
                            {
                                controlsFromMetaData(&serverctrls, &clientctrls);
                                ldaperr = ldap_modify_ext_s(mLDAP, ldif.dn().utf8(), lmod, serverctrls, clientctrls);
                                FREELDAPMEM;
                            }
                            break;
                        case LDIF::Entry_Add:
                            ...
                            {
                                controlsFromMetaData(&serverctrls, &clientctrls);
                                ldaperr = ldap_add_ext_s(mLDAP, ldif.dn().utf8(), lmod, serverctrls, clientctrls);
                ...
                case LDIF::Control:
                    ...
                    break;
                case LDIF::Err:
                    error(ERR_SLAVE_DEFINED, i18n("Invalid LDIF file in line %1.").arg(ldif.lineNo()));
                    FREELDAPMEM;
                    return;
            }
